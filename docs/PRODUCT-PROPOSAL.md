...
The repeatable production chain is part of that product, not internal convenience: define a machine-readable capability descriptor, project retained typed evidence inputs through the public protocol, verify that decoding reconstructs the same checked input, evaluate through the existing reference route, and generate the fixtures, suite, and per-case claim-source classification before asking an independent implementer to write code. A hand-curated guide may explain the result, but it must not be the only connection between evidence and the normalized handover.
...
Each release should ship a machine-readable manifest beside its binary and evidence bundle. The implemented flat and one-group correlation slices now have separate exact Lean-generated operation records and finite external-evidence boundaries in schema-2 [`supported-fragment-v1.json`](../reference/supported-fragment-v1.json), documented by [`PROTOCOL.md`](PROTOCOL.md). The development [`flat-validation-empty-logic-v1` descriptor](../reference/flat-validation-empty-logic-v1.capability.json), [`IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md`](IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md), and generated [`conformance suite`](../reference/flat-validation-empty-logic-v1.conformance.json) form the first cold knowledge-transfer exercise: its eight cases are mechanically projected, with four exact external firing/polarity claims and four externally silent cases whose exact verdict is Lean-account only. [`IMPLEMENTER-KIT-CORRELATION.md`](IMPLEMENTER-KIT-CORRELATION.md) and [`single-group-correlation-v1.conformance.json`](../reference/single-group-correlation-v1.conformance.json) remain the higher-complexity captured-outer handover, but their normalized fixture/evidence associations are still manual. Neither development handover is a released capability merely because it appears in the development manifest. The broader release shape below remains illustrative because theorem roots and whole-bundle evidence identity are not yet packaged into that process manifest.
...
The first condition is now more concretely realized: the non-repeatable flat fragment and one-group correlation fragment are exposed through the coherent normalized process contract in [`PROTOCOL.md`](PROTOCOL.md), with deterministic black-box tests, generated per-operation support/evidence metadata, and a candidate runner exercised against the Lean reference. The flat cold-handover descriptor, fixtures, suite, and evidence classifications were mechanically produced from typed retained replay inputs before an isolated Rust implementation; under its recorded isolation boundary, that implementation passed all eight indexed cases without reported renewed kernel research and detected one exact source-declared divergence, the observable `Or` half of another, and one additional inverse row-gate variant. This validates the finite knowledge-transport path rather than one curated document, but generated Rust-versus-Lean differentials, six exact source-declared exercises, broader replication, and exact negative-protocol closure remain pending. The correlation suite remains manually evidence-cross-referenced and has no completed cold implementation. Adopt the full proposal as product strategy when a real non-Lean consumer completes the entire selected capability gate, is willing to run the reference or suite in CI, and retained evidence plus proof metadata can be packaged reproducibly. Until then, it guides product-shaped architecture without treating these development capsules as a release commitment or forcing process infrastructure into every semantic capsule.
