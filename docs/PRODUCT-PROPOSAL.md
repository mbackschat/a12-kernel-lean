...
Each release should ship a machine-readable manifest beside its binary and evidence bundle. Current [`supported-fragment-v2.json`](../reference/supported-fragment-v2.json) demonstrates separate operation records and finite evidence boundaries for reference semantics 0.3.0, while [`reference-semantics-lineage-v1.json`](../reference/reference-semantics-lineage-v1.json) preserves the digest-locked 0.2.0 history; appearance in either is not release approval. Protocol 1 has no per-request reference-semantics selector, so a product integration must inspect `--manifest` from the exact binary and pin the binary or release digest. [`IMPLEMENTATION-MAP.md`](IMPLEMENTATION-MAP.md) owns live semantic and evidence readiness; the frozen 0.2.0 [flat](IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md) and [correlation](IMPLEMENTER-KIT-CORRELATION.md) kits own their exact projection integrity and cold-consumer gaps. The broader release shape below remains illustrative because proof-root and whole-bundle evidence identity are not yet packaged into the process manifest.
...
The generated flat shipment and its first isolated non-Lean implementation provide initial evidence that the proposed knowledge-transport shape is viable for one finite evaluator capability; they do not establish release readiness or broader language coverage. The exact cold implementation, mutation qualification, and later 52-case generated-differential outcome are owned by [`IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md`](IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md#generated-differential-outcome-2026-07-15), while current semantic and external-evidence readiness is owned by [`IMPLEMENTATION-MAP.md`](IMPLEMENTATION-MAP.md). Adopt the full proposal as product strategy when a real non-Lean consumer completes the entire selected capability gate, is willing to run the reference or suite in CI, and retained evidence plus proof metadata can be packaged reproducibly. Until then, it guides product-shaped architecture without treating development shipments as a release commitment or forcing process infrastructure into every semantic capsule.
