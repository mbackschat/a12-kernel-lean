...
For an adopted public product, its repeatable production chain is part of the product rather than internal convenience. An evaluator should reuse an existing machine-readable descriptor and checked public projection where they fit; a missing bridge is designed only from the adopted capability's concrete need and requires separate approval. Importer and refactoring products analogously justify any checked source-to-target projection or generated validity/preservation material from their exact claim. These production mechanisms are not prerequisites for bounded artifact-only consumer probes. A hand-curated guide may explain a released result, but it must not be the only connection between maintained semantics and a normalized public shipment.
