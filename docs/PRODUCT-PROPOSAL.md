...
The first condition is now more concretely realized: the non-repeatable flat fragment and one-group correlation fragment are exposed through the coherent normalized process contract in [`PROTOCOL.md`](PROTOCOL.md), with deterministic black-box tests, generated per-operation support/evidence metadata, and a candidate runner exercised against the Lean reference. The flat cold-handover descriptor, fixtures, suite, and evidence classifications are mechanically produced from typed retained replay inputs before the independent Rust spike; this tests the handover-production process rather than one curated document. The correlation suite remains manually evidence-cross-referenced. Both kits still carry release-blocking research/evidence limits, negative-protocol and process-hardening gaps, and incomplete cold-implementation records. Adopt the full proposal as product strategy when a real non-Lean consumer completes the cold gate without renewed kernel research, is willing to run the reference or suite in CI, and retained evidence plus proof metadata can be packaged reproducibly. Until then, it guides product-shaped architecture without treating these development capsules as a release commitment or forcing process infrastructure into every semantic capsule.
