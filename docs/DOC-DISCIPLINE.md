...
- A small concrete documentation, implementation, corpus, capture, or portability improvement for a12-dmkits updates [`A12-DMKITS-FEEDBACK.md`](A12-DMKITS-FEEDBACK.md) with the audited upstream revision, exact source links, recommendation, and acceptance check. A coherent upstream capability with its own architecture and acceptance lifecycle may instead use a separately registered cross-project proposal with copy-ready prompts. The user transfers either artifact to the a12-dmkits agent; this project never implements it in a sibling or copied worktree. Once upstream work is reported in progress, leave that handoff stable, do not send a parallel request, and reconcile only after the user supplies the committed revision and disposition. Neither surface owns a12-dmkits' live status.
