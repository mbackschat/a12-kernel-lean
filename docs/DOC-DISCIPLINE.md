...
### `limit` bounds a positive claim; `denominator` names the surface searched for a negative one

The two keys answer different questions and neither replaces the other. A **`limit`** row bounds what a positive claim covers — the kinds, arms, locales, or topologies a measurement reached. A **`denominator`** row names the surface actually *searched* when a record asserts an **absence**: which subcommands, input modes, formats, or document shapes were enumerated, and which were not. `limit` keeps its existing meaning on all 283 rows that carry it and is never rekeyed.

A record whose `claim` row asserts an absence carries a `denominator` row, and the hygiene guard requires one on records a commit range adds or modifies. The trigger is the phrasing the root [`CLAUDE.md`](../CLAUDE.md) **mandates** — `no witness known as of <rev>`, `unmeasured`, `not measured`, `untested`, `never` — and deliberately not the spellings it forbids: no existing `limit` row contains a forbidden spelling, because a compliant author does not write one, so a guard keyed to `unauthorable` or `no legal model` would fire on nothing. Scoping to changed records is not laxity: a denominator can only be written by someone who knows which surface was searched, so backfilling one for an older record would invent the very thing the row exists to record.

**Known limit of the guard, and it is the important one.** The trigger reaches an absence phrased as a *negative existential*. It does **not** reach an absence stated without such a phrase. A claim of the form "an over-limit row receives no outcome at all" contains no trigger word, so the guard passes it silently, and that shape is exactly the one that once nearly removed a proved clause — the discriminator there was a document whose target cell was left empty, which no phrasing rule can detect. Read this guard as covering the *stated-negative* class only. The guard for the other class is a load-bearing citation: a clause cites the checkpoint that settles it, so removing the claim forces reading its basis.
