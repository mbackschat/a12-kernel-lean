...
Open work does not belong in the findings ledger. [`SEMANTICS-GAPS.md`](SEMANTICS-GAPS.md) is the open-only index of missing kernel behavior, required static legality, checked construction, and semantic conformance obligations. A completed gap is deleted in the completing commit; Git records its history, while implemented/proved/evidence status remains in [`IMPLEMENTATION-MAP.md`](IMPLEMENTATION-MAP.md). Immediate sequencing lives in [`PLAN.md`](PLAN.md), durable milestone and reevaluation criteria live in [`PROJECT-DESIGN.md`](PROJECT-DESIGN.md), and proposed release progression lives in [`PRODUCT-PROPOSAL.md`](PRODUCT-PROPOSAL.md). Do not create separate architecture, harness, consumer, or protocol gap ledgers unless one later demonstrates a distinct audience and lifecycle.

The mandatory same-context capsule assessment in [`TESTING.md`](TESTING.md#same-context-capsule-closure-assessment) is a review procedure, not a new report series. It writes only concrete outcomes: fix a same-capsule defect, add or amend one live semantic gap, record an exceptional durable finding, or update the existing capability/evidence/plan owner. A clean assessment leaves no documentation artifact.

## Same-change update triggers

- A changed semantic ownership boundary, public representation contract, dependency direction, composition invariant, or adopted or rejected encoding alternative updates [`ARCHITECTURE.md`](ARCHITECTURE.md). A routine widening inside an already-described owner does not, and neither does a new module that introduces no such change; that fact belongs in [`IMPLEMENTATION-MAP.md`](IMPLEMENTATION-MAP.md). Correcting an architecture sentence the change made false is always in scope.
...
- A newly discovered semantic obligation, changed prerequisite/blocker/reopening trigger, or completed gap updates [`SEMANTICS-GAPS.md`](SEMANTICS-GAPS.md); delete a completed entry and remove its live references rather than retaining a shipped-status list. A completed widening updates [`IMPLEMENTATION-MAP.md`](IMPLEMENTATION-MAP.md) and leaves in the owning gap only residual or newly discovered open obligations; it never appends a completed-implementation bullet.
...
- A changed current checkpoint, preserved observation, immediate step order, or resume instruction updates [`PLAN.md`](PLAN.md). Completed-unit narratives and the full open backlog do not belong there.
