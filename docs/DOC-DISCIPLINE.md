...
- A concrete a12-dmkits improvement first records the local semantic, evidence, or process gap in its existing owner, with the audited upstream revision, exact source links, broken or missing invariant, recommendation, compatibility consequence, and separating acceptance check. Prepare the upstream request using [`SEMANTIC-CAPSULE-PIPELINE-PROPOSAL.md`](SEMANTIC-CAPSULE-PIPELINE-PROPOSAL.md#future-a12-dmkits-engagements); create a temporary registered cross-project proposal only for a coherent capability with its own architecture and acceptance lifecycle. The user transfers the request to the a12-dmkits agent; this project never implements it in a sibling or copied worktree. Once upstream work is reported in progress, leave that handoff stable, do not send a parallel request, and reconcile only after the user supplies the committed revision and disposition. Move durable results into their semantic, evidence, testing, source, architecture, or plan owner, then delete the completed handoff and rely on Git history. No local handoff owns a12-dmkits' live status.
