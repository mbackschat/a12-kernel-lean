...
|---|---|---|---|
| [`evidence/`](../evidence/) | Retained external observations, historical raw audit units, exact probe inputs, and compact accepted semantic bundles | Raw units preserve what a pinned kernel run observed and how its historical producer qualified it; accepted compact bundles are the ordinary Lean replay interface, not a second oracle | Append, version, or explicitly correct raw observations; add a compact bundle under a versioned producer identity only after its purpose-specific contract is accepted; never rewrite an accepted raw unit |
...
The direct-cascade migration demonstrates how the complete raw audit unit and compact Lean replay bundle coexist without duplicating responsibility. The producing a12-dmkits revisions verified and certified the raw packet, runner, input, capability, qualification, closure, receipt, recapture relations, and compact export. This repository retains those bytes but ordinary replay treats the raw unit as opaque: one small generic reader checks the compact bundle contract, and a typed family decoder checks only the pinned semantic input/observation projection. The migration ran the former complete binder and compact lane together once before deleting the packet-specific Lean machinery; upstream later retired the producer implementation from current `main`.
...
[`evidence/scenarios/`](../evidence/scenarios/) currently contains the frozen `string-direct-cascade-v1` scenario root authored for the now-retired a12-dmkits portable capture command. It contains the complete input model, explicit document placements, operation, requested runners, probes, required channels, qualification policy, and projection identities, but no expected value, verdict, diagnostic, agreement flag, mutation outcome, or Lean result. Its exact bytes remain the historical canonical request copied into the retained packet. It is not a required shape for a future producer capability.
...
The historical scenario identity and bytes are immutable. Its official answer is a separate immutable unit: the complete packet and packet receipt, packet-local capabilities declaration, out-of-band packet-receipt digest, separate qualification profile/report/receipt sidecars and their out-of-band receipt digest, and the producer-certified compact bundle consumed by the project-owned typed projection. Its scenario-specific mutation receipt and two-clean-capture no-drift report remain process-qualification companions, never reclassified as kernel observations; every runner observation and declared fidelity gap is retained. A future accepted capability owns its own reviewed input, output, assurance, and retention contract and must not inherit this V1 packet shape by default.
...
- Historical evidence layouts and disposable-harness receipts remain immutable. A future accepted handback retains the complete observation and assurance material required by its own reviewed contract plus the smallest compact semantic account needed for ordinary Lean replay; it does not automatically reproduce the retired V1 packet, capabilities, qualification, or receipt graph. Do not retain only selected agreeing observations when the accepted contract claims a richer raw boundary. Kernel binaries, kernel-linked types or implementation code, volatile execution logs, and generated machine-specific state remain outside `evidence/`.
