...
|---|---|---|---|
| [`evidence/`](../evidence/) | Retained external observations, complete certified raw audit units, exact probe inputs, and—after the accepted migration—compact producer-certified semantic bundles | Raw units preserve what a pinned kernel run observed and how the producer qualified it; successor compact bundles will be the ordinary Lean replay interface, not a second oracle | Append, version, or explicitly correct raw observations; add compact bundles under versioned producer identities only after their producer contract is accepted; never rewrite an accepted raw unit |
...
The accepted target makes the complete raw audit unit and compact Lean replay bundle coexist without duplicating responsibility. a12-dmkits will verify and certify the raw packet, runner, input, capability, qualification, closure, receipt, recapture relations, and compact export. This repository will retain those bytes but ordinary replay will treat the raw unit as opaque; one small generic reader will check the compact bundle contract and a typed family decoder will check only the semantic input/observation projection. Until that target is implemented, the transitional direct-cascade binder still performs both jobs and no compact bundle is a current replay input.
...
- During the current transition, each legacy project-owned projection remains bound by its existing `lake test` lane. After compact migration, the producer-certified bundle names the raw audit identity and ordinary Lean replay binds only the typed family projection to that compact bundle; it does not rebind every raw artifact.
...
Some runtime projections still repeat cells, rows, paths, and structured conditions that were manually read from retained placements. Those transcriptions are reviewed but not all mechanically derived from the full case and model. The current narrow binders protect their relationships, but repeating that packet-specific pattern is now the principal scalability gap rather than the recommended remedy. Direct cascade must prove the compact producer-certified bundle boundary and delete its old binder first. Only if that measured migration succeeds should the older String lanes move; no new placement-to-packet binder or repository-wide raw inventory should be added merely for uniformity.
