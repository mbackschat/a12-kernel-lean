...
The compact green receipt deliberately omits 52 duplicate per-case request/response transcripts. Its non-profile artifact digests, platform target, elapsed time, and output usage are runner-recorded historical assertions; the checker validates their schema, syntax, platform admission, and bounds but cannot prove that an external historical process produced them. Its expected reference distribution is recomputed through the current evaluator, not replayed from a checkout of the profile-pinned source revision. Git review, the runner's preflight/postflight checks, immutable profile/result identities, and the retained receipt therefore form the audit trail; the later checker is a consistency guard, not an execution authenticator.
