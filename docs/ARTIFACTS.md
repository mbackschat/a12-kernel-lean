...
The first member is [`string-direct-cascade-v1`](../evidence/scenarios/string-direct-cascade-v1/README.md). Its existence and successful structural validation do not increment the retained-observation count or close the cascade's external-evidence gap. A disposable smoke run may prove that the transport can consume the question, but its results remain non-evidence when deleted rather than retained through the official lifecycle.
