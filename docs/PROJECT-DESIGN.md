...
The primary goal is eventual **100% semantic conformance with A12 kernel 30.8.1** for the complete observable validation-and-computation language and every static legality rule required to evaluate it faithfully. `a12-kernel-lean` must become the versioned, self-contained mechanized semantics-of-record, executable reference semantics, and navigable body of knowledge for that whole target, not permanently stop at a convenient supported fragment. Current fragment boundaries remain mandatory honest staging labels; they are coverage checkpoints and fail-closed interfaces, never a terminal scope reduction. The theory should capture conditions and computations—including formal checking, operator- and context-sensitive absence, iteration and correlation, ordered state change, partial validation, and message polarity—with explicit provenance; empirically anchor every primitive semantic choice to kernel 30.8.1 through retained differential evidence; prove checked surface-to-core elaboration, evaluator correctness against independently meaningful judgments or traces, and semantics-preserving transformations or optimized implementations; and act as a semantics factory that turns this knowledge into purpose-specific, versioned, research-closed shipments. Concrete consumers include independent evaluators in any implementation language, importers from explicitly versioned external subsets such as JSON Schema, A12 rule-analysis and rule-refactoring tools, checked explanations and certificates, kernel-version delta and migration tools, and proofs that real rule models establish business invariants. Every downstream capability must declare its source and target fragments, observable projection, unsupported cases, and exact claimed relation; consumer probes are a mandatory adequacy check on the semantics, not permission to leave the kernel account incomplete.

This document is the project constitution: it owns the durable decision, users, semantic boundary, artifact and evidence doctrine, delivery model, coverage horizons, and success criteria. The canonical documentation registry is [`README.md`](README.md). In particular, proposed product scope belongs in [`PRODUCT-PROPOSAL.md`](PRODUCT-PROPOSAL.md), live coverage and sequencing in [`IMPLEMENTATION-MAP.md`](IMPLEMENTATION-MAP.md) and [`PLAN.md`](PLAN.md), and concrete proof and encoding practice in [`LEAN-FORMALIZATION.md`](LEAN-FORMALIZATION.md) and [`ARCHITECTURE.md`](ARCHITECTURE.md). Project-owned [`LEAN-ENCODING-GUIDE.md`](LEAN-ENCODING-GUIDE.md) remains consulted staged guidance rather than a plan or implementation-status surface.

## Decision

> Completely preserve the observable A12 30.8.1 validation and computation semantics as a versioned, self-contained mechanized theory: prose explains behavior and provenance; Lean definitions state and execute it; named theorems and counterexamples capture universal consequences and limits; corpus and differential evidence connect that theory empirically to the real kernel. Partial supported fragments are delivery stages toward complete coverage, not the final objective.

### Why derive — the animating principle and its assurance classes

Capturing the semantics is worthless if the knowledge cannot be **pulled back out and used**. That is why this project has consumers at all, and it is the principle that settles conflicts between the goals above: the theory exists to be extracted from, not merely to be correct.

The intended economics are explicit. Invest **once** in extracting the kernel's behaviour precisely and expressing it in a form more precise than prose, then derive each consumer from that body of knowledge at low marginal cost — in deliberate contrast to building each consumer the way a peer clean-room interpreter must, through its own campaign of kernel probes and reviews. Lean is chosen because it is more precise than prose and because a machine can consume it; prose retains its separate job of explaining behaviour and provenance, but it is not the artifact a consumer derives from. The project began as an experiment in whether Lean could serve that role at all; that question is now settled in the affirmative, and the derivation economics are what remain to be proved.

At least three consumers must eventually ship: an independent evaluator (Rust), SMT-backed analysis, and rule refactoring. Every other consumer profile in [`USE-CASES.md`](USE-CASES.md) is checked as coverage grows, because a consumer check is the standing test of whether knowledge transfer actually works.

**Derivation is not one mechanism, and the assurance classes are not interchangeable.** Each shipped consumer must declare which of these it uses and claim exactly the corresponding strength:

| Mechanism | What it establishes | Fits |
|---|---|---|
| Certificate checking — untrusted producer, small trusted Lean checker | genuine inherited proof, per result | Analyze, Transform, Verify, Synthesize |
| Differential agreement against the Lean reference | finite empirical agreement over executed inputs | Execute |
| Knowledge transport through shipment material | fast reimplementation; conformance still empirical | Execute |
| Lean-to-target extraction or verified compilation | not available: no mature path, and verified compilation needs a formal target semantics | none currently |

The consequence is that an **interpreter cannot inherit proof** — it evaluates continuously, so it reaches only empirical agreement — while an analyzer or refactoring tool produces artifacts a checker can validate and therefore can. "Derived from Lean" is never by itself an assurance claim.
...
[`A12-DMKITS-SPEC-SYNC-LEDGER.md`](A12-DMKITS-SPEC-SYNC-LEDGER.md) records current reconciliation of spec changes into a12-dmkits, while the [historical ledger](archived/A12-DMKITS-SPEC-SYNC-LEDGER-THROUGH-2026-08-28.md) retains the frozen earlier revision receipts. Neither is evidence that keeps a semantic claim honest.
...
These tiers induce five independently reported completion levels: **internally executable and tested**, **important laws and non-laws proved**, **kernel correspondence retained**, **public protocol exposed**, and **consumer-probe qualified**. A clause may advance through the first two without waiting for the last three. Kernel calibration is batched by coherent semantic family; protocol exposure happens only for an adopted capability; consumer qualification happens only at a capability milestone. Never make an internally useful semantic definition wait for unrelated transport or product work, and never promote an internal result to a correspondence or shipment claim by implication.
...
The unit of semantic closure is a **semantic capsule**, not an unchecked batch of evaluator branches. The planning unit may be a **mechanism-first family batch**: audit several related clauses together, identify the source-confirmed shared mechanism and actual deviations, implement that mechanism once, and close its members as individually reviewable capsules. Every completed capsule contains:
...
The project optimizes for semantic throughput rather than capsule count. A family batch should amortize source archaeology, representation review, and generic proof work across several clauses while preserving exact per-clause claims. Keystone mechanisms with several downstream uses—checked expression evaluation, resolved operand streams, repeatable addressing, computation state transition, temporal profiles, and message construction—take priority over isolated low-connectivity operators when their prerequisites and discriminators are ready. This is not permission to invent a universal framework: audit may identify a candidate seam, but extraction waits until a second completed semantic user demonstrates the same meaning, result domain, and law. That second user may close within the same family batch.

Executable Lean is a means, not an independently valuable completion target. A semantic representation is adequate only when it preserves the distinctions needed by at least one named consumer task and can support that task's decision procedure, mapping, preservation relation, counterexample, or explanation without making the consumer rediscover kernel behavior. Findings therefore flow back into representation choice, proof obligations, separating cases, exclusions, and sequencing; a downstream adapter must never conceal an inadequate semantic root. Capability milestones test this criterion through bounded consumer probes, while product adoption and shipment qualification remain separate decisions.
...
[`IMPLEMENTER-GUIDE.md`](IMPLEMENTER-GUIDE.md) owns the detailed shipment contents, implementation/qualification split, research-closure checklist, cold-consumer gate, downstream playbooks, and disagreement protocol. [`IMPLEMENTER-KIT-CORRELATION.md`](IMPLEMENTER-KIT-CORRELATION.md) is the first current high-complexity evaluator-shipment design spike and owns the fresh V2 runtime probe. The first executed isolated flat evaluator experiment used the retired 0.2.0/V1 capability; its exact ownership is the [historical experiment record](archived/REFERENCE-SEMANTICS-0.2.0-AND-RUST-EXPERIMENT.md), while [`IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md`](IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md) now describes only the current V2 development capability. Importer and refactoring profiles remain future capabilities until a concrete selected fragment, consumer, evidence plan, and preservation obligation are adopted.
...
This method deliberately avoids a speculative universal operator framework. A family audit may identify a candidate shared mechanism when it establishes the same branch structure, result domain, and law across admitted members, but extraction occurs only after two semantic users are completed against that account; the second may close in the same batch. The first unsupported instance stays direct and named. An optimized table, generic fold, or compiled evaluator becomes a separate refinement target only when a real consumer task and preservation theorem justify it. Cedar's executable-spec/validation/theorem separation is the default project shape, while Radix's evaluator/relation and transformation-preservation patterns are used only where their narrower proof form fits.

Audit a ready family in one read-only pass before implementing its members: map all operator branches, shared state, deviations, and evidence status once, then reuse that account. Prove parametric scan, verdict, observation, or transition laws at the shared mechanism and derive operator-specific results by specialization; do not accumulate renamed copies of the same proof. Build a separating conformance matrix whose cases each kill a realistic competing account—wrong order, substitution, polarity, scope, poison, or short-circuit—and omit examples that add no independent distinction. Ordinary capsules update only their actual document owners: coverage and the active checkpoint always, a durable finding only for a non-obvious reusable conclusion, the sync ledger only for a behavioral spec change, and architecture, protocol, evidence, README, or consumer kits only when their owned boundary genuinely changes.

The live coverage and adequacy artifact is [`IMPLEMENTATION-MAP.md`](IMPLEMENTATION-MAP.md): it records the supported cells and evidence gaps, while [`PLAN.md`](PLAN.md) owns the persisted cross-session implementation checkpoint. Language-neutral kernel behavior belongs in the project-owned [`../spec/`](../spec/) body and remains grounded in the sources indexed by [`SOURCES.md`](SOURCES.md); every new outbound semantic change is tracked in [`A12-DMKITS-SPEC-SYNC-LEDGER.md`](A12-DMKITS-SPEC-SYNC-LEDGER.md), and frozen earlier dispositions remain in its [historical archive](archived/A12-DMKITS-SPEC-SYNC-LEDGER-THROUGH-2026-08-28.md). The durable reason for a Lean distinction belongs in [`LEAN-FINDINGS.md`](LEAN-FINDINGS.md), and the current representation in [`ARCHITECTURE.md`](ARCHITECTURE.md). This prevents the implementation matrix from becoming a second semantics specification or an unreviewed backlog.
...
8. Run bounded consumer probes at capability milestones: after a reusable family closes, after a major addressing or computation boundary changes, or before declaring a public compatibility capability. Select a named task profile from [`IMPLEMENTER-GUIDE.md`](IMPLEMENTER-GUIDE.md), map its required clauses and observable relation to existing specifications, executable cases, theorem/non-law links, support boundaries, and evidence status, then classify semantic dependency closure, shipment research closure, and cold-consumer qualification separately. An evaluator profile is language-neutral—Rust and Python need different implementation profiles, not different A12 semantics—while importers and refactoring tools require their own mapping or preservation relations. Use artifact-only readback by default; an external prototype, protocol, runner, dependency, or qualification path needs its own approval.
...
These horizons express prerequisite and coverage direction, not a total phase-completion schedule. Project-owned [`LEAN-ENCODING-GUIDE.md`](LEAN-ENCODING-GUIDE.md) supplies the bottom-up dependency spine and traps inside each capsule; working context owns the active semantic unit, while [`PLAN.md`](PLAN.md) persists it only for cross-session handoff. A capsule may cross horizons only when its exact prerequisites are internally closed for the claim being reused. Breadth does not count as progress while an active capsule lacks its theorem/counterexample obligation, coverage entry, trust gate, or honest evidence status. A relation or trace semantics is added only when it exposes meaningful steps or supplies an independent refinement target, not as ceremonial duplication.
...
- Keep the executable core dependency-free while that remains economical. If mature theory would be cheaper and safer through Mathlib, propose a separate version-pinned proof target with its licensing, build, and maintenance consequences; add it only after explicit dependency approval.
...
- **Cost of the next consumer:** the derivation principle above predicts that each additional consumer costs materially less than building it through its own kernel-probe campaign. That prediction is the project's distinguishing claim and must be measured rather than assumed: record, per completed consumer, what it needed beyond the shipped material and every question that forced renewed A12 research. A consumer that required fresh kernel archaeology falsifies the economics for its capability, whatever else it achieved. Gap-discovery into an already-converged peer is the complementary measure: corrections accepted by an independent implementation that believed itself complete are direct evidence that the analytical arm finds what an empirical one cannot.
- **Semantic throughput:** over a sustained implementation interval, approximately 70–80% of effort should go to semantic definitions, focused conformance, and payoff-bearing proofs; approximately 10–20% may go to source audit and independent review; documentation and settled gates should normally consume the remainder. These are steering ranges rather than time accounting, but a sustained deviation toward process, evidence, or documentation requires stopping and explaining the concrete semantic blocker.
- **Infrastructure cadence:** at least three substantial semantic work units land between infrastructure or product-boundary work units. A reproduced defect blocking current semantics may justify an exception only after its need, smaller alternatives, and intended scope are surfaced and approved.

The first proof-bearing capsule passed the project's initial value reevaluation: subsequent capsules have produced reusable laws, clarified false generalizations, consumer readbacks, and representation corrections beyond the Kotlin interpreter plus tests. Reevaluate at family milestones and consumer probes. If sustained work produces only another evaluator and ceremonial `rfl` facts, or if a consumer still needs renewed kernel archaeology, redesign or stop before reproducing more operator surface.
...
Current evidence counts, supported fragments, and remaining adequacy obligations live in [`IMPLEMENTATION-MAP.md`](IMPLEMENTATION-MAP.md). A verified cross-session baseline and any handed-off semantic unit live in [`PLAN.md`](PLAN.md); uninterrupted selection and immediate work stay in working context. A durable conclusion from completing or failing a capsule belongs in [`LEAN-FINDINGS.md`](LEAN-FINDINGS.md) or this constitution only when it changes the project method, authority model, success criteria, or long-term decision.
