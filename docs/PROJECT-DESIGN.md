...
The first external-evidence bridge is operational: 11 retained kernel 30.8.1 observations replay through a closed typed projection with `lake test`, covering focused empty, malformed-branch, absolute-required, and positive-path witnesses. The immediate gate remains evidence completion rather than evaluator breadth: add the required filled/malformed companions and path-separating/static-legality cases recorded in [`EVIDENCE.md`](EVIDENCE.md), then reassess whether the implemented capsule is externally adequate. Once that gate closes, proceed to iteration capsule 5a (single-level star and uncorrelated `Having`) and only then 5b (`$` as an explicit captured-outer binding). The legal kernel precision witness also remains open because it reaches scale 20 through arithmetic; the direct rescaling examples are internal helper laws, not external whole-rule evidence.
