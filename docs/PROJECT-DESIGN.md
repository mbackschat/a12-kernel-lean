...
A cold implementation is also a learning experiment, not a binary certification ritual. Retain which artifacts actually guided the implementer, which distinctions or exclusions caused hesitation, which repetition helped, which seeded defects the suite detected, where lifecycle instructions drifted, and what remained attested rather than reproducible. Feed capsule-specific observations back into the owning implementation kit and generalized process changes into [`IMPLEMENTER-GUIDE.md`](IMPLEMENTER-GUIDE.md); keep immediate follow-up experiments in [`PLAN.md`](PLAN.md). A successful downstream run validates knowledge transport only for its pinned capability and executed inputs and creates no new kernel evidence by itself.

[`IMPLEMENTER-GUIDE.md`](IMPLEMENTER-GUIDE.md) owns the exact implementation-capsule contents, research-closure checklist, cold-implementer gate, downstream playbook, and disagreement protocol. A semantic capsule closes the internal theory and evidence obligations above; its implementation capsule is the consumer-facing projection required before release support. [`IMPLEMENTER-KIT-CORRELATION.md`](IMPLEMENTER-KIT-CORRELATION.md) is the first high-complexity handover design spike, while [`IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md`](IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md) owns the first executed isolated implementation experiment. Both keep their remaining evidence, differential, and cold-qualification gaps explicit rather than treating runnable tooling as automatic release closure.
