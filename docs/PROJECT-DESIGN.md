...
Capsule 5a and the externally observed slice of 5b demonstrate this distinction. Capsule 5a has closed both sides for its narrow one-group validation truth fragment: ordered candidates, uncorrelated row-local `Having`, selected numeric sum, an exact relation bridge, filter-before-consumer theorem, checked current-row-collapse non-law, and seven retained kernel observations. Capsule 5b contributes explicit captured outer and candidate environments, filter-only inner/outer origins, origin-presence checking, naive correlated selection, a guarded any-filled observer, origin/judgment/selection/self-match proofs, and four retained kernel observations for clean filled Number direction, equality self-match, and explicit repetition self-exclusion. The observations preserve OMISSION but replay only firing and ordered outer-row pointers because filtered-result polarity remains a later capsule. Empty/malformed correlation behavior and the unused operator/origin combinations remain internal executable locks or definitions with focused external evidence pending.
...
- the continuing `$` correlation program: single-level star/uncorrelated `Having` and captured-outer correlation have landed; kept-row observation ordering, repeatable elaboration, and later optimized-refinement work remain separately staged; [`LEAN-FINDINGS.md`](LEAN-FINDINGS.md#lf3--correlation-requires-two-explicit-environments) records the prerequisites, differentials, proof obligations, checked non-laws, and deliberate deferrals;
...
The retained external-evidence gate now replays 30 focused kernel 30.8.1 observations through three closed typed projections with `lake test`: 19 flat/path/required runtime or diagnostic cases, seven uncorrelated-iteration cases, and four captured-outer correlation cases. The path probes falsified the earlier ancestor-tier assumption and drove the root correction recorded in [`LF6`](LEAN-FINDINGS.md#lf6--bare-name-resolution-is-local-or-global-not-an-ancestor-walk). The 5b cases preserve their actual executed stored `$` conditions and separate direction (`[2,3]`), self-match (`[1,2,3]`), excluded distinct (`[]`), and excluded duplicates (`[1,2]`); both kernel strategies and the a12-dmkits interpreter agree on all four.
...
Correlation capsule 5b is internally implemented at its deliberately narrow boundary. `CapturedSingleGroupContext` holds the explicit outer row; each candidate supplies the inner row; `HavingOrigin` exists only inside `CorrelatedHaving`; and `OriginCheckedCorrelatedHaving` proves only that both origins occur. Number equality, inequality, and less-than plus structural repetition comparison execute as a naive ordered nested loop. Numeric empty comparison uses zero, invalid operands are unknown and drop, `CurrentRepetition` reads the 1-based structural row, and the narrow guarded any-filled observer returns only `K`. Proofs establish numeric-reference origin stability/locality, evaluator-to-judgment and selector-to-relation bridges, selection-level explicit self-exclusion, and selection-level usable self-match. Four retained clean-filled cases close the external gate only for numeric inner-less-than-outer, numeric equality self-match, and repetition-not-equal self-exclusion with a numeric equality conjunct. Correlated empty/malformed inputs, numeric `notEqual`, and repetition `equal`/`lessThan` remain `external evidence pending`; the first two input states do have internal separating locks.

The next correlation capsule is kept-row observation ordering: connect the correlated selector to phase-aware document reads and prove the filtered-out consumer footprint boundary with malformed-kept/malformed-dropped witnesses. It must not silently broaden 5b into full repeatable syntax. Filtered-result polarity, repeatable elaboration and diagnostics (including field/path/equality-scale legality), `Document` adaptation, nested/multi-star and cross-group behavior, hash-join optimization and refinement, computation, partial validation, and general consumers remain deliberately deferred. The legal kernel precision witness also remains open because it reaches scale 20 through arithmetic; the direct rescaling examples are internal helper laws, not external whole-rule evidence.
