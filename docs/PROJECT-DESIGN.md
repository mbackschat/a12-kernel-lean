...
Capsule 5a and the externally observed slices of 5b/5c demonstrate this distinction. Capsule 5a has closed both sides for its narrow one-group validation truth fragment: ordered candidates, uncorrelated row-local `Having`, selected numeric sum, an exact relation bridge, filter-before-consumer theorem, checked current-row-collapse non-law, and seven retained kernel observations. Capsule 5b contributes explicit captured outer and candidate environments, filter-only inner/outer origins, origin-presence checking, naive correlated selection, and four retained clean-filled observations for Number direction, equality self-match, and explicit repetition self-exclusion. Capsule 5c separates the outer guard from the starred consumer with `evalSelectedAnyFilled`, `evalGuardedAnyFilledOn`, and `firingRowsOn`; proves the validation-observation footprint of the guard plus selected consumers; and retains the all-valid plus symmetric malformed dropped-versus-kept cases. The observations preserve OMISSION but replay only firing membership, restored to the projection's document-row order, because filtered-result polarity and external kernel emission ordering remain outside the claim. Correlated filter-operand empty/malformed behavior and the unused operator/origin combinations remain internal executable locks or definitions with focused external evidence pending.
...
- the continuing `$` correlation program: single-level star/uncorrelated `Having`, captured-outer correlation, and kept-row observation ordering have landed; remaining admitted external gaps, repeatable elaboration, and later optimized-refinement work stay separately staged; [`LEAN-FINDINGS.md`](LEAN-FINDINGS.md#lf3--correlation-requires-two-explicit-environments) records the prerequisites, differentials, proof obligations, checked non-laws, and deliberate deferrals;
...
The retained external-evidence gate now replays 33 focused kernel 30.8.1 observations through three closed typed projections with `lake test`: 19 flat/path/required runtime or diagnostic cases, seven uncorrelated-iteration cases, and seven captured-outer correlation cases. The path probes falsified the earlier ancestor-tier assumption and drove the root correction recorded in [`LF6`](LEAN-FINDINGS.md#lf6--bare-name-resolution-is-local-or-global-not-an-ancestor-walk). Across four models, the correlation cases preserve their actual executed stored `$` conditions and separate direction (`[2,3]`), self-match (`[1,2,3]`), excluded distinct (`[]`), excluded duplicates (`[1,2]`), all-valid consumers (`[1,2]`), first malformed consumer (`[1]`), and second malformed consumer (`[2]`). Groovy-dynamic, static-Java, and the a12-dmkits interpreter agree on all seven. The driver also resolves each authored absolute-or-relative condition path and the error field against the retained model before requiring exact condition equality; this strengthens evidence binding without claiming general repeatable elaboration.
