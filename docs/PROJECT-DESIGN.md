...
Capsule 5a demonstrates this distinction and has now closed both sides for its narrow truth fragment: its single-group ordered selector, uncorrelated row-local `Having`, selected numeric sum, exact relation bridge, filter-before-consumer theorem, checked non-law, trust/build coverage, and seven retained kernel observations are present. The observations preserve OMISSION but replay only fired/silent because filtered-result polarity remains a later semantic capsule.
...
The first external-evidence gate is closed for its deliberately narrow supported clauses: 19 retained kernel 30.8.1 observations replay through a closed typed projection with `lake test`, covering empty/branch behavior, all three absolute-required cell states, explicit path match/mismatch, and separating local/model-wide/rejection cases for bare names. The separating probes falsified the earlier ancestor-tier assumption and drove the root correction recorded in [`LF6`](LEAN-FINDINGS.md#lf6--bare-name-resolution-is-local-or-global-not-an-ancestor-walk), demonstrating why evidence precedes breadth.

Iteration capsule 5a is closed for its narrow truth fragment: it models ordered explicit instantiated rows for one repeatable group, optional uncorrelated row-local `Having`, keep-iff-fired filtering, and validation-only numeric `Sum` as a truth-`K` selection observer. Its exact ordered `SelectRows` bridge and filter-before-consumer theorem apply Cedar's executable-slice/reachability pattern without introducing a speculative general framework; the checked neighboring non-law rejects current-row collapse. Seven retained kernel 30.8.1 observations cover unfiltered and filtered sums, firing and silent boundaries, empty selection, invalid-filter drop, and invalid-consumer drop/keep. The next capsule may now add `$` as an explicit captured-outer binding. Polarity, nested/multi-star, joins, elaboration/static legality, `Document` adaptation, computation, partial validation, and general aggregates remain deliberately deferred. The legal kernel precision witness also remains open because it reaches scale 20 through arithmetic; the direct rescaling examples are internal helper laws, not external whole-rule evidence.
