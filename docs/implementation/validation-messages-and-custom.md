...
- `remains`: The remaining non-field parameter forms, a keyed parameter's **admission** and rendering (which need a keyed condition spine this flat fragment has not got), the grammar's accented name letters that the bounded-ASCII template gate refuses first, what an admitted group parameter renders, the group position's three lexical refusal codes, locale, providers, and repeatable pointers: [SG10](../SEMANTICS-GAPS.md#sg10--message-construction-and-formal-output-integration)
