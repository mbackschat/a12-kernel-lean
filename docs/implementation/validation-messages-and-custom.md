...
- `assurance`: Four token-filter mutations also fail: return the selected star alone, route `$` through the reopened pointer, force the unmarked candidate exact, or omit the selected star; the last is independently excluded by the source-retention law. Group-expansion and unstarred-coordinate separators are owned by [the group record](../IMPLEMENTATION-MAP.md#cap-group-operand-reference-projection). L is exact for token filtered-star coordinates at the [message-address checkpoint](../SOURCES.md#src-message-address-dialects); Number and Boolean/Confirm correspondence remains pending. C/X/Q none.
