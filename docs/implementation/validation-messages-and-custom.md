...
- `boundary`: A second **position** is admitted beside the name one: `$#…$` takes a group, spelled as an absolute path or as one of two keyword shorthands for the endpoints of the rule's own ancestor chain. Its single gate is containment running **opposite** to the computation declaring-group gate — the *named* group must contain the rule's group — and it needs no separate existence test, because every ancestor of a declared group is itself declared. Two historic terminals keep the Kernel's own distinct refusal class rather than collapsing into the unknown-group one. What an admitted group parameter renders stays caller-supplied, because the Kernel's own rendering for the position is unmeasured.
...
- `assurance`: The group position is **L-exact for static admission** at the [group-parameter checkpoint](../SOURCES.md#src-message-group-parameter): the rule's own group and the root admitted, a **declared** descendant, a declared sibling, an unknown path, and every relative spelling refused alike, both keyword shorthands admitted, the two retired terminals on their own code, and three same-vintage words plus an arbitrary name on the unknown-group code — against plain-text and name-position controls in the same batch, so the admissions are not an unreachable-checker artifact. The declared descendant and sibling are the separators that make containment rather than existence the operative gate. Both shorthands are **proved** to be shorthands inside that one gate rather than admissions beside it: each resolves to one endpoint of the rule's own chain whenever that chain is representable. Outbound as [SPEC-2026-08-29-04](../A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-29-04); C/X/Q none
- `remains`: The remaining non-field parameter forms, a keyed parameter's **admission** and rendering (which need a keyed condition spine this flat fragment has not got), the grammar's accented name letters that the bounded-ASCII template gate refuses first, what an admitted group parameter renders, segment quoting inside a group argument, whether the root shorthand names the rule chain's root or a model-wide one, locale, providers, and repeatable pointers: [SG10](../SEMANTICS-GAPS.md#sg10--message-construction-and-formal-output-integration)
