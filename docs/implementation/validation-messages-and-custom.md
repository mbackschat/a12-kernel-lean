...
- `boundary`: A second **position** is admitted beside the name one: `$#…$` takes a group, spelled as an absolute path or as one of two keyword shorthands for the endpoints of the rule's own ancestor chain, `RootGroup` naming that chain's root rather than a model-wide one. An absolute path resolves in the two gates the Kernel reports apart: its first segment must name a declared root group, and only then does containment decide, running **opposite** to the computation declaring-group gate in that the *named* group must contain the rule's group. Below the root gate containment needs no separate existence test, because every ancestor of a declared group is itself declared.
- `boundary`: Group segments take the name grammar's quote escape, but the terminal match is on the raw argument, so quoting a terminal turns it back into an ordinary group name. A malformed spelling is a parse failure on the Kernel's own **three** lexical classes rather than one: a quote fault, a trailing separator, and a doubled separator are separate diagnostics, decided by the malformed shape. The character position the Kernel's lexer code additionally carries is a payload this fragment does not reproduce. Two historic terminals keep the Kernel's own distinct refusal class. What an admitted group parameter **renders** is the named group's 1-based repetition index in the firing row, `1` for a nonrepeatable group; the caller-supplied bytes are gone and `ValidationMessageInputs` no longer carries a group provider at all.
- `boundary`: Only the **nonrepeatable** arm of that rendering is reachable here, and by construction rather than by omission: the containment gate makes the named group a prefix of the rule's row group, and this route's condition spine refuses a repeatable reference, so the fragment has no repeatable rule host to name a repeatable group. The per-row arm is stated by `spec/11` and left unreachable rather than modelled and untested.
...
- `assurance`: The group position is **L-exact for static admission** across two batches. The [group-parameter checkpoint](../SOURCES.md#src-message-group-parameter) fixes the admitted set: the rule's own group and its ancestors admitted, a **declared** descendant, a declared sibling, and every relative spelling refused alike, both keyword shorthands admitted, the two retired terminals on their own code, and three same-vintage words plus an arbitrary name on the unknown-group code.
- `assurance`: The [resolution checkpoint](../SOURCES.md#src-message-group-parameter-resolution) then corrected this project's first account on three points before they could ship as claims: quoted segments are admitted, a quoted terminal is not a terminal, and an undeclared root has its own code ahead of containment. It also supplies the two-root model that separates the root shorthand's meaning. Every batch carries plain-text and name-position controls, so no admission is an unreachable-checker artifact. Both entry gates are **proved** applied: an admitted absolute argument provably names a declared root and contains the rule's group, and each shorthand provably resolves to one endpoint of the rule's own chain whenever that chain is representable. Outbound as [SPEC-2026-08-29-04](../A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-29-04); C/X/Q none
- `assurance`: The three lexical classes are **L-exact for the measured shapes** at that same resolution checkpoint and needed no new observation, because `spec/11` already carried them. The separator is the trailing/doubled pair: neither is distinguishable from the other by its characters alone, so a retained case on both refutes the account that the class follows the argument's characters rather than the empty segment's position. No law is stated, because the classification's helpers are private construction internals and exporting them would buy a weaker claim than the pair already carries.
- `assurance`: The group position's **rendering** is measured too, at the [rendered-index checkpoint](../SOURCES.md#src-message-group-parameter-rendered-index), and it moved that value out of the caller exactly as the category access was moved earlier: a per-row rule firing on rows 2 and 3 renders `2` and `3`, which separates the row's own coordinate from a firing ordinal, and a nonrepeatable group renders `1`. A second model differing only in declared labels renders identically while labelling the *field* changes what the name position renders in the same message, so a label cannot reach this position and "labels are inert here" cannot explain it. Outbound as [SPEC-2026-09-05-02](../A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-09-05-02); C/X/Q none
- `remains`: The remaining non-field parameter forms, a keyed parameter's **admission** and rendering (which need a keyed condition spine this flat fragment has not got), the grammar's accented name letters that the bounded-ASCII template gate refuses first, the character position the lexer class carries, locale, providers, and repeatable pointers: [SG10](../SEMANTICS-GAPS.md#sg10--message-construction-and-formal-output-integration)
...
- `boundary`: Token value count projects each declaring field instance. Its measured filtered-star carrier adds the `Having`'s Number fields: an unmarked candidate at or below the reopened level stays wildcarded, `$` stays concrete at the captured firing row, and `CurrentRepetition` contributes no field pointer. The selected value pointer is retained alongside those dependencies, proved for every successful token-star projection. Stored/category projection remains invisible. Number and Boolean/Confirm filtered stars remain refused rather than inheriting the token observation.
- `boundary`: Group-bearing entity-list, Boolean/Confirm count, group-list, and group-presence leaves delegate recursive membership and coordinates to [group-operand reference projection](../IMPLEMENTATION-MAP.md#cap-group-operand-reference-projection). Canonical-token projection remains invisible; a row-paired `SumOfProducts` projects both value fields under one shared-path certificate; the ordinary non-starred `repeatableFieldPresence` leaf lets a standard iteration-guard rule project end to end with the guard's reference merging into the guarded leaf's by deduplication; and the `flat` and non-model-indexed `numeric` leaves are classified.
...
- `assurance`: Eleven established non-group mutations still fail retained cases: wildcard the bound prefix, never wildcard, off-by-one on `firstStar`, use a fixed single wildcard, sieve a numeric leaf whose fixed group count would have answered, classify only one comparison side, let a context-free atom contribute or refuse, treat a delegated scalar atom as coordinate-free, project one operand of a two-field atom, report projection-specific rather than declaring identity, or project one side of a row-paired product.
- `assurance`: Four token-filter mutations also fail: return the selected star alone, route `$` through the reopened pointer, force the unmarked candidate exact, or omit the selected star; the last is independently excluded by the source-retention law. Group-expansion and unstarred-coordinate separators are owned by [the group record](../IMPLEMENTATION-MAP.md#cap-group-operand-reference-projection). L is exact for token filtered-star coordinates at the [message-address checkpoint](../SOURCES.md#src-message-address-dialects); Number and Boolean/Confirm correspondence remains pending.
- `assurance`: Q closes the bounded [filtered-token Explain probe](../consumer-probes/EXPLAIN.md#filtered-token-having-pointer-probe): an isolated reader recovered both exact normalized sets, rejected five wrong accounts, and returned `insufficient information` for Number and Boolean/Confirm without a guessed branch. C/X none.
- `remains`: Number and Boolean/Confirm filtered-star operand coordinates, the `RepetitionNotUnique` leaf, `fillToFix`, the message-record channel, and the raw per-path-part pointer domain: [SG10](../SEMANTICS-GAPS.md#sg10--message-construction-and-formal-output-integration)

<a id="cap-shared-message-pointer"></a>
#### Shared message pointer

- `boundary`: One resolved field identity plus concrete/wildcard/unknown coordinates, distinct from exact `CellAddr`, reused by every implemented formal-message and callback error channel. Bound coordinates recover an exact cell; an unbound reopened level contributes a wildcard and makes exact recovery impossible without retaining the authored operand form.
...
- `assurance`: E/P closed for the normalized field-instance domain, including lossless exact embedding and the proved non-exactness and full coordinate arity of every reopened pointer. Typed-vector L for the unbound-level selector is at the [source registry](../SOURCES.md); C/X/Q none
...
#### Custom-condition authoring and callback
...
- `owner`: [`CustomFieldValidity.lean`](../../A12Kernel/Semantics/CustomFieldValidity.lean), [proofs](../../A12Kernel/Proofs/CustomFieldValidity.lean), [cases](../../A12Kernel/Conformance/CustomFieldValidity.lean); the operand slot is [`Elaboration/CustomFieldValidity.lean`](../../A12Kernel/Elaboration/CustomFieldValidity.lean) with [its cases](../../A12Kernel/Conformance/CustomFieldValidityOperand.lean)
...
- `boundary`: the operand slot admits an evaluated String or an Enumeration declaration and refuses every other kind. Number, Boolean, and Confirm carry the measured `MVK_NO_STRING_OR_ENUM_OR_EXT_ENUM`, and so does a **Custom**-typed declaration, which is a String declaration here and therefore invisible to a kind-keyed gate ([checkpoint](../sources/evaluation-and-application-routes.md#src-validtype-operand-gates)). A temporal or DateRange operand is refused with **no class claimed**, because none was observed. Extensible Enumeration is admitted with ordinary Enumeration and not separated: this flat model carries no extensible variant
- `remains`: which value an admitted Enumeration operand feeds the validator, whether the clause's *value-validation* qualifier is this theory's raw-String mode, the refusal classes of the temporal and DateRange operands, and validation-condition-leaf integration: [SG7](../SEMANTICS-GAPS.md#sg7--string-pattern-and-custom-field-completion)
