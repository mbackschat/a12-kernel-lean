...
- `assurance`: the String operand's **no-value** rule is Kernel-retained under both operators at the [inequality checkpoint](../sources/having-filter-probes.md#src-string-inequality-empty-operand). Inequality is the operator that could have refuted it, since a participating empty would differ from any nonempty literal and fire; it stays silent while both live controls fire. A formally invalid String operand remains unmeasured and inequality cannot settle it, because an unknown leaf and a suppressed one both fail to fire.
...
- `boundary`: The same rule-owned projection checks the exact sole text-indexed group under `NumberOfFilledGroups(...) > 0` independently through the checked group/index/token selector, then returns `MVK_SEMANTIC_INDEX_NOT_ALLOWED` only at the measured outside error-field locus. An unindexed group and the unmeasured inside locus stay unmapped on this surface, and no runtime count form is added.
...
- `assurance`: E/P close the indexed carrier's exact refusal, group/index/token retention, outside-versus-inside locus distinction, and invalid-selection fail-closed control. L locks the exact indexed `NumberOfFilledGroups` row at the [reviewed PR2 carrier checkpoint](../SOURCES.md#src-pr2-semantic-index-carrier-matrix); C/X/Q none.
- `remains`: Nested/different repeatable scopes, other indexed carriers, comparisons, operators, arities, authored orders, executable newly admitted group-list/count conditions, and other assembly refusals: [SG9](../SEMANTICS-GAPS.md#sg9--paths-indices-and-static-legality-completion)
...
- `boundary`: The computation arm's `NumberOfFilledGroups` folds a **mixed** operand list whose forms contribute unlike quantities: a fixed operand's zero-or-one by descendant content, and a starred repeatable operand's instantiated row count, which no presence predicate expresses. `GroupCountOperandReading` owns the two forms and their contributions add **once per authored position**, so the same group named twice contributes twice. A fixed operand carries both measured content constituents — its descendant cells and whether a repeatable descendant has an instantiated row — because row instantiation is not expressible as a cell read at any width. The starred constructor takes the in-capacity count; excluding an over-limit row stays with the caller.
- `boundary`: The fold merges nothing on containment either, but that branch is unreachable through elaboration: both arms refuse a containment pair statically, so the fold's total-function behavior there carries no kernel claim ([static-gate checkpoint](../SOURCES.md#src-group-count-static-gates-both-arms)).
...
- `case`: [starred group-count cases](../../A12Kernel/Conformance/StarredGroupCountComputation.lean) replay the seven measured documents at both targets, with three instantiated empty rows counting three as the discriminator and the emptied fixed operand as the mixed separator. They also replay the peer-measured two-star rows, where the same group named twice counting six is what separates a per-position sum from a per-group one. The [static-gate cases](../../A12Kernel/Conformance/NumericComputation/StarredGroupCount.lean) add the containment and root refusals with their admitted controls; the ancestor/descendant counting cases that stood there were deleted, because the Kernel refuses that pair at authoring time and the answer they locked was this project's own invention.
- `assurance`: E/P closed for resolved product; source-grounded. The disjunction is now **externally measured** rather than only source-grounded: the row constituent alone at the [repeatable-descendant checkpoint](../SOURCES.md#src-repeatable-descendant-group-count), and both constituents separated against one operand in the peer fixture that checkpoint reviews, with a neither-constituent control ruling out an unconditional count. The measurement is on the count carrier; this arm is shown to agree rather than measured itself. **Repetition depth is measured too** at the [deep checkpoint](../sources/group-and-iteration-probes.md#src-deep-repeatable-descendant-group-count): a descendant two repetition levels below the operand counts as one level does, so the constituent follows subtree containment rather than an axis count and the compute arm's depth gate is lifted. The half-instantiated shape — an outer row with no inner row — stays this project's own reading.
- `assurance`: E/P closed for the mixed fold, externally calibrated at the [starred group-count checkpoint](../SOURCES.md#src-starred-group-count-computation) on both codegen strategies. `numberOfFilledGroupsForComputationOperands_fixed` constrains the new fold by the established plain multi-group count in that direction only; `_append` states the decomposition a consumer needs. The addressed evaluator now consumes this fold, while the scalar route keeps refusing fail-closed; [`numeric-and-temporal.md`](numeric-and-temporal.md#cap-scalar-number-computation) owns that elaboration coverage.
