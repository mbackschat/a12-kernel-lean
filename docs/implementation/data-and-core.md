...
- `boundary`: The computation arm's `NumberOfFilledGroups` folds a **mixed** operand list whose forms contribute unlike quantities: a fixed operand's zero-or-one by descendant content, and a starred repeatable operand's instantiated row count, which no presence predicate expresses. `GroupCountOperandReading` owns the two forms and their contributions add **once per authored position**, so the same group named twice contributes twice. The starred constructor takes the in-capacity count; excluding an over-limit row stays with the caller.
