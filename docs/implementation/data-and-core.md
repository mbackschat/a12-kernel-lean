...
- `case`: [starred group-count cases](../../A12Kernel/Conformance/StarredGroupCountComputation.lean) replay the seven measured documents at both targets, with three instantiated empty rows counting three as the discriminator and the emptied fixed operand as the mixed separator.
...
- `assurance`: E/P closed for the mixed fold, externally calibrated at the [starred group-count checkpoint](../SOURCES.md#src-starred-group-count-computation) on both codegen strategies. `numberOfFilledGroupsForComputationOperands_fixed` constrains the new fold by the established plain multi-group count in that direction only; `_append` states the decomposition a consumer needs. Elaboration coverage is **none**: the checked scalar computation still refuses both admitted shapes fail-closed, so this clause has its meaning and its measurement without a consumer yet.
