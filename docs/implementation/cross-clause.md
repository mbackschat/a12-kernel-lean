...
- `assurance`: E closes the count ladder one through four against a `max 3` star, with the capacity boundary as the polarity separator; instantiated-versus-filled rows, where two rows count two whether both, one, or neither carries a value; the fixed member's contribution, which drops the total by exactly one when emptied; and the refusals — fixed-only, single-operand, and root-group.
...
- `assurance`: L is the [list-extent checkpoint](../SOURCES.md#src-group-count-list-extent) on both codegen strategies — the count ladder, the capacity boundary's `VALUE_ERROR` against three `OMISSION_ERROR` rows below it, and the instantiated-but-empty separator. Not measured: two starred members in one list, a starred member owning a further repeatable descendant, an unretained repeatability, an unavailable member's interaction with the sum, and the mixed list's own overlap rule — so no overlap gate is invented for a mixed pair, and overlap is enforced over the fixed members alone where it is measured. C/X/Q none.
