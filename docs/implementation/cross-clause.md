...
- `boundary`: within the validation arm every consumer reads one decided-presence projection, `asGroupListPresence`. **A formal error blocks only the negative answer**, so a group with admitted content is filled whatever else its descendants carry, and only an undecided group makes a count unavailable. `numberOfFilledGroups` once re-derived that condition beside the projection and disagreed with it; the count is now defined through it, which is what keeps the count and `GroupFilled` from answering differently about one state. `filledGroupCount_indifferentToErrorOnDecidedGroup` states the asymmetry and `undecidedHead_makesFilledGroupCountUnknown` carries the hypothesis that replaced the falsified `erroneous` one.
- `assurance`: E for the error dimension at the [unavailability checkpoint](../SOURCES.md#src-group-count-unavailability) on both codegen strategies — the fires-with-a-filled-sibling row against the silent malformed-only row, repeated on the second operand and controlled by an unreferenced-group error. P for the two laws above. The **coverage** dimension is a consequence of the shared projection and not a measured row: `validateFull` cannot observe partial coverage, so visible content deciding a partly covered group is stated as mechanism. The structural leg is measured too: a group carrying an over-limit error still counts when its content is admitted, so `definitelyFilled` ignoring `erroneous` is right for both error kinds rather than only the cell one. Not measured: a call-local partial-coverage error, and `noneRelevant` coverage.
...
- `boundary`: **the distinct count is the one member whose expansion-kind class is not keyed by the operator at all, so it claims none.** `NumberOfDifferentValues` reports the class of its operand list's *first* member, and two groups differing only in declaration order flip the code, so a projection keyed by operator alone is wrong for one of them — as the shipped one was, having inherited a String class measured on a String-first expansion and applied it to a Number-first fixture ([checkpoint](../sources/group-and-iteration-probes.md#src-distinct-count-first-operand-class), outbound [`SPEC-2026-08-31-13`](../A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-31-13)). Projecting the right code needs the first operand's kind carried in the refusal, which no consumer has yet asked for.
...
- `boundary`: the four aggregate operators carry their own measured row; the Number value count shares the resolver and has none of its own, while its Token and Boolean sibling kinds are measured — the record states that rather than implying a third row exists. `FieldValuesNotUnique` uses a different resolver by design and is unmeasured on this dimension, so it keeps the complete formal-cell view.
...
- `case`: [group extent and admission cases](../../A12Kernel/Conformance/FieldEntityGroupOperand.lean), [checked computation cases](../../A12Kernel/Conformance/NumberEntityGroupComputation.lean), and [declared-capacity extent cases](../../A12Kernel/Conformance/GroupOperandCapacity.lean).
