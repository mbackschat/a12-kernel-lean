...
- `boundary`: under **full validation** a **fixed** group operand narrows to its declared-capacity extent for `Sum`, both extrema, `NumberOfDifferentValues`, and the Number value count; a **starred** group operand keeps the complete formal-cell view, unmeasured on this dimension and pinned by its own case so the split stays visible. A row beyond its group's declared repeatability is not in the domain the operand denotes, so its cell is neither aggregated nor able to make the aggregate unavailable ([checkpoint](../sources/inbound-group-operand-batches.md#src-group-operand-capacity-consumer-sweep)). The four aggregate operators carry their own measured row; the Number value count shares the resolver and has none of its own, while its Token and Boolean sibling kinds are measured — the record states that rather than implying a third row exists. `FieldValuesNotUnique` uses a different resolver by design and is unmeasured on this dimension, so it keeps the complete formal-cell view.
...
- `boundary`: that fixed-group surface narrows to the operand's declared-capacity extent under full validation, so a matching cell beyond its group's declared repeatability neither counts nor makes the count unavailable; kernel 30.8.1 answers `0` there and `1` on the control one index lower ([checkpoint](../sources/inbound-group-operand-batches.md#src-group-operand-capacity-consumer-sweep)). The **starred** whole-group carrier still counts the complete extent under full validation, which is unmeasured on this dimension rather than a second measured rule; [SG13](../SEMANTICS-GAPS.md#sg13--group-list-and-group-count-completion) carries the probe.
...
- `boundary`: under full validation a **fixed** group operand narrows to its declared-capacity extent, so a `true` cell beyond its group's declared repeatability neither counts nor makes the count unavailable; kernel 30.8.1 answers `0` there and `1` on the control one index lower ([checkpoint](../sources/inbound-group-operand-batches.md#src-group-operand-capacity-consumer-sweep)). A **starred** group operand keeps the complete formal-cell view, unmeasured on this dimension and pinned by a case so the split is visible; [SG13](../SEMANTICS-GAPS.md#sg13--group-list-and-group-count-completion) carries the probe that would settle it.
