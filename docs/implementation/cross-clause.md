...
- `boundary`: under **full validation** a group operand narrows to its declared-capacity extent for `Sum`, both extrema, `NumberOfDifferentValues`, and the Number value count, **starred or not**. The two forms were briefly split here on the fixed measurement alone; a [starred-operand probe](../sources/group-and-iteration-probes.md#src-starred-group-operand-extent) answered every arm identically and the split is gone. A row beyond its group's declared repeatability is not in the domain the operand denotes, so its cell is neither aggregated nor able to make the aggregate unavailable ([checkpoint](../sources/inbound-group-operand-batches.md#src-group-operand-capacity-consumer-sweep)).
