...
- Build small semantic capsules around an executable **reference evaluator** — `#eval`-able, internally guarded, checked by replaying the a12-dmkits [`corpus/`](../../a12-rulekit/corpus), and compared with focused kernel observations exported through its external [`adapter/`](../../a12-rulekit/adapter) harness in the local `a12-rulekit/` checkout.
...
The differential path assigns distinct roles rather than treating all executable sources as interchangeable. The kernel is the authoritative behavior. The a12-dmkits [`adapter/`](../../a12-rulekit/adapter) is the external harness and result-export boundary for focused kernel probes, never a dependency of the Lean theory. Its [`corpus/`](../../a12-rulekit/corpus) is the portable replay boundary consumed by this repository. Its [`interpreter/`](../../a12-rulekit/interpreter) is a secondary clean-room implementation useful for triangulation, fuzzing ideas, and detecting disagreements; it is not an oracle, and kernel evidence resolves any conflict. This preserves the clean-room boundary: kernel execution and result normalization happen externally, while committed Lean inputs contain only portable own-domain evidence.
...
`OriginCheckedCorrelatedHaving` proves only that both `usesInner` and `usesOuter` hold: an all-`$` filter fails with `missingInner`, while an uncorrelated condition remains in the earlier iteration fragment rather than masquerading as a correlation test. It does not establish field scope, path legality, or equality-scale legality; those remain obligations of the later repeatable elaborator. The admitted comparison subset is Number `equal`, `notEqual`, and `lessThan`, plus the same operators over structural repetition indices and conjunction. Numeric empty is resolved as zero locally to this comparison clause; a formally invalid operand yields `K.unknown`, and selection keeps only `K.tru`, so invalid/unknown filter rows drop. `CurrentRepetition` reads the structural `RowIndex` directly and does not observe a cell.

`SingleCorrelatedStar.select` is deliberately the straightforward ordered nested loop: for each captured outer row it scans every same-group candidate in document order, including the outer row itself. Self-match is therefore real, and only an authored `CurrentRepetition(inner) != CurrentRepetition(outer)` excludes it. `evalGuardedAnyFilled` is a narrow truth observer for the retained rule shape `FieldFilled(outer F) And AtLeastOneFieldFilled(G*/F Having filter)`; it does not claim general field-quantifier or filtered-result polarity semantics.

[`A12Kernel/Proofs/Correlation.lean`](../A12Kernel/Proofs/Correlation.lean) proves outer numeric-reference stability, inner numeric-reference locality, executable truth equivalence with the independently stated `CorrelatedHaving.Holds`, exact ordered selector equivalence with `SelectCorrelatedRows`, selection-level explicit self-exclusion, and selection-level usable same-field self-match. [`A12Kernel/Conformance/Correlation.lean`](../A12Kernel/Conformance/Correlation.lean) locks the neighboring non-laws: self-exclusion is not implicit, reversing inner/outer direction changes the selection, and missing either origin fails the checked boundary. Four retained observations cover directional less-than firing on rows 2 and 3, self-match firing on every row, self-excluded distinct values firing nowhere, and self-excluded duplicate values firing on rows 1 and 2. Both kernel strategies and the a12-dmkits interpreter agree on all four.

This capsule still starts after repeatable path resolution and static authoring legality and does not adapt the general `Document`. Deliberate deferrals are filtered-result polarity; repeatable elaboration and authoring diagnostics; phase-aware kept-row consumer footprints; nested/multi-star and cross-group binding; hash-join optimization and a refinement proof back to the nested loop; computation, partial validation, and general filtered consumers.
...
- `A12Kernel/Semantics/Correlation.lean` — explicit captured outer row, inner/outer filter origins, checked correlated `Having`, naive ordered selection, and a guarded any-filled truth observer.
...
- `A12Kernel/Proofs.lean` — trusted theorem root; algebra, information order, checked-cell invariants, phase laws, required-staging preservation, elaboration/context coherence, ordered-selection/filter-before-consumer proofs, and the correlated origin/judgment/selection/self-match laws.
- `A12Kernel/Conformance.lean` — executable semantic and elaboration locks for the supported fragment, including both single-level iteration capsules and their checked non-laws.
- `A12Kernel/Basic.lean` — smoke + `kernelVersion`.

The external evidence lane is deliberately outside those roots: `A12Kernel/Evidence/Schema.lean` and `Replay.lean` own the flat/path/required projection; `A12Kernel/Evidence/IterationSchema.lean` and `IterationReplay.lean` own the uncorrelated single-level selection projection; `A12Kernel/Evidence/CorrelationSchema.lean` and `CorrelationReplay.lean` own the captured-outer projection; and `A12Kernel/EvidenceMain.lean` performs IO over the retained cases. The `checkKernelEvidence` executable is the `lake test` driver; it is not imported by the library, conformance root, or trusted theorem root. [`EVIDENCE.md`](EVIDENCE.md) owns the transport and claim boundary.
