...
### PE4–PE6 final decisions

The cascade pilot and focused conformance target are green without changing a theorem statement or executable definition. The independent flat checked-context success/error lookup pair also elaborates with a narrow `simp only` set; this validates the scoped simplification practice, not a claim that flat lookup has the cascade's nested `Except.map` mechanism. Correlation elaboration remains the positive control for explicit case analysis. The trust audit covers the new theorem closure and reports only the project's permitted logical axioms.

| Technique | Decision and exact applicability | Theorem/result domain tested | Benefit | Failure mode or counterexample | Cedar/Radix basis | Cascade outcome | Independent A12 outcome |
|---|---|---|---|---|---|---|---|
| A — definitional reduction / `rfl` | accept for intentionally transparent constructor equations only | required dependency rejection, attempted-payload non-exposure, prior-independent checked outcome | shortest proof accurately exposes a deliberate equation | cannot skip `Except.map`; an unconditional single-finding poison equation is false for `.required` | both projects use reduction for closed constructor equations | accepted for three transparent laws; rejected for successful nested reads | retained for carried certificates and similarly direct equations |
| B — named equation plus `rw` | accept in the observation/overlay scope when the equation names a semantic phase | clean-value and non-required-poison observation followed by target shadowing | separates phase semantics from the transport proof and gives one proof owner | a lemma merely restating `Except.map` would be tactic scaffolding; poison needs `cause ≠ .required` | Cedar owns effect bridges; Radix owns evaluator/state bridge ladders | accepted through `Proofs/Observation.lean` and existing shadow laws | existing `checkContext_lookup_coherent` remains the named lookup-to-read bridge; no shared generic effect helper was justified |
| C — reviewed local `simp only` | accept in trusted proofs to expose one known effect/lookup constructor before domain reasoning; plain `simp` may still close a local goal | exact nested dependency/read equality and flat checked-context success/error equality | makes the outer constructor reduction and rewrite dependencies reviewable | broad unfolding could hide an error or make recursive evaluators part of global normal form | Cedar guide and `simp_do_let`; Radix uses local `simp only` around `andThen` and interpreter cases | accepted for `withDependencyOutcome`, `ofOutcome`, `pure`, `Except.map`, and constructor injection | accepted for both `checkContext_lookup_coherent` branches without changing statements |
| D — explicit enum/`Except` cases | accept when constructors communicate the complete semantic domain | target error, `FormalCause`, dependency outcome, and correlation/raw-context validation branches | keeps accepted, no-value, target error, inherited poison, and rejected required visible | success-only projection would erase outer dependency faults or residual read faults | Cedar typechecker inversion and residual errors; Radix relation/interpreter case proofs | accepted for target-error and inherited-poison families | existing correlation and elaboration case proofs remain clearer than a generic tactic |
| E — global `[simp]` equation | reject for this study | proposed outcome/overlay and recursive evaluator equations | no demonstrated benefit beyond local sets | rich bridge has a rejected branch; no cross-instance need, no safety/termination evidence, and unrelated-proof stability would be harder to review | Cedar keeps domain simplification mainly in theorem utilities; Radix's `[simp]` equations are small closed reducers | no rule added | no rule needed by flat or correlation proofs |

Final scope: local effect exposure plus named domain equations is reusable; the nested dependency/read result mechanism is cascade-specific; no universal contributor rule or new `CLAUDE.md` requirement is justified.
