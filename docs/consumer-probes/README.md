...
| [Explain](EXPLAIN.md) | green; filtered Number aggregate movement handover amber | Structured source-first traces, exact whole-call inventories on success and post-plan failure, generated-preliminary parent-local DateRange explanation, implicit computation message polarity plus `fillToFix`, origin-sensitive filtered-token pointers, plain/filtered String/Boolean/Confirm capacity and movement, and filtered Number capacity amounts with value-count movement checked against Lean |
