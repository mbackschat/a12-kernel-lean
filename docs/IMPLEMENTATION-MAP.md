...
- One authored checked pattern leaf resolves a direct ordinary nonrepeatable evaluated-String path against the validated flat model, admits its constant source through that compiler-associated boundary, and evaluates through the declaration-owned checked context plus the existing normalized resolved consumer. Its surface type cannot encode an expression-valued left side or nonconstant right side. Raw, registered-custom/predefined, arbitrary declared-pattern, and repeatable fields fail with explicit boundary errors; connective-tree and whole-rule embedding remain separate.
...
- Fifteen pattern-admission declarations distinguish Java-syntax precedence, all four possessive suffixes, lookbehind/named/atomic group prefixes, every documented forbidden escape, uppercase `\P`, nested versus escaped/sequential class opens, the admitted empty source, admitted lookahead/flags/Java escapes, exact source-to-matcher association, and successful versus rejected runtime delegation. Seven authored-condition groups separate both operators, semantic and row emptiness, formal unavailability, CRLF normalization, wrong kind, both admission failures, and every intentionally unsupported checked-context shape. Eight laws characterize compiler rejection, successful two-stage classification/delegation, exact checked evaluation, the row gate, sole-field reference traversal, and VALUE-only firing.
