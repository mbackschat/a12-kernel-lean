...
- Flat, numeric-expression, and scalar group-presence validation leaves now share one resolved connective tree, Boolean empty-row fold, model-aware field-reference traversal, and exact left-to-right short-circuit evaluator. Existing flat conditions remain a type alias over their atomic leaves, universal preservation laws prevent their embedding from changing verdicts or references, and group leaves delegate to the established product-state evaluator through an explicit resolver.
...
- Resolved group presence independently folds admission, error, and three-level relevance, then gives scalar predicates, fixed-list predicates, strict numeric count, and parent-requiredness their consumer-specific projections. Checked scalar predicates now reach that same state through the shared condition and whole-rule boundary; `RuleGroup` provides subtree error-field membership without introducing another traversal.
...
- Direct ordering, arithmetic fillability, tolerance, partial relevance, mixed flat/numeric/group-presence condition integration, generated-rule details, and the label/value display refinements remain source/test anchored and project-locally `external evidence pending`.
...
- Boundary: IF193's product state and consumer projections after descendant scope and relevance are resolved, plus checked scalar `GroupFilled`/`GroupNotFilled` integration for nonrepeatable group paths and the already-selected `RuleGroup` instance.
...
- Cases cover malformed-only, admitted-plus-malformed, duplicate index, created and over-limit rows, partial positive/empty, full empty, list availability, strict count availability, requiredness activation, checked `RuleGroup`/ordinary-path evaluation, wildcard and unaddressed-repeatable rejection, subtree error-field admission, sibling rejection, and fired-message projection.

- This is internally complete at levels 1–2 for resolved inputs and the checked scalar-condition/whole-rule seam.
- Descendant/document/group-instance construction, ordinary repeatable-path addressing, repeatable negative-condition legality, and broader required-rule orchestration are indexed by [`SG1`](SEMANTICS-GAPS.md#sg1--general-checked-document-construction) and [`SG2`](SEMANTICS-GAPS.md#sg2--general-repeatable-addressing-and-operand-construction).
