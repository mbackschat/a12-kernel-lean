...
- Combined/zero target bounds, permitted CRLF target measurement with exact-payload retention, lone-CR non-normalization, forbidden-break target error, integrated target lowering, no-value/poison bypass, and CRLF/LF/lone-CR ingestion are internal Lean laws/cases not separately exercised by retained local evidence. The checked String-computation matrix also locks accepted/rejected `[0-9]+` and registered-custom sources through the prepared context before the unchanged copy/concatenation/range and target evaluators.
...
- **Implemented narrowly; wider target correspondence remains pending.** Coverage includes declaration-owned ordinary String line-break/min/max policy, model-complete ordinary declared-pattern preparation, direct equality/inequality, complete nonrepeatable evaluated-String `Length` as a scale-0 numeric source plus its reduced four-ordering projection, presence, absolute requiredness, present-empty placement, CRLF normalization, checked nonrepeatable literal and field-valued `AtLeastOne`/`No`/`NotAll`, scalar literal and field-valued Included/NotIncluded, resolved pattern consumption, one checked authored ordinary nonrepeatable pattern leaf with compiler-associated execution through the prepared context, checked scalar String expressions including ordinary nonrepeatable `RangeAsString`, the raw-String value-read gate and eliminated maximum-length declaration, and checked ordinary String target construction with combined/zero bounds plus forbidden/permitted line-break behavior.
...
- Generated numeric validation narrows checked String `Length`, `RangeAsNumber`, and ordinary Enumeration/category `FieldValueAsNumber` atoms directly into the validation atom algebra without reconstructing or re-resolving their surface paths. Model-wide nonrepeatable source admission, normalized length/range or selected-token evaluation, missing-source polarity, derived scale, and the computed target mismatch therefore remain the same checked expressions rather than generated-rule exceptions.
...
- Flat, numeric-expression, scalar group-presence, and fixed field/group-list validation leaves now share one resolved connective tree, Boolean empty-row fold, model-aware field-reference traversal, and exact left-to-right short-circuit evaluator. Existing flat conditions remain a type alias over their atomic leaves, universal preservation laws prevent their embedding from changing verdicts or references, scalar group leaves delegate to the established product-state evaluator through an explicit resolver, and fixed group-list leaves reuse the same checked field observations and resolved group states.
