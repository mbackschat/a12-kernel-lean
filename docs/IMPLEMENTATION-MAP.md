...
- One shared prepared String context validates the model once, prepares every ordinary declared pattern and registered custom validator against that model, and dispatches the mutually exclusive declaration profiles before the same flat condition tree. The canonical full-evaluation entry point requires explicit compiler, locale, `World`, and model inputs and retains distinct preparation and condition errors; the former flat route is explicitly unprepared and retained only for its fail-closed separator. The built-in compiler exposes only the locally implemented `[0-9]+` matcher rather than treating other sources as pattern-free. A patterned field can no longer bypass its matcher through the custom-prepared presence route, and a mixed row checks its patterned and custom fields through their distinct owners.
...
- one exact nonrepeatable structured-plan rule message rendered only after firing, shared by checked flat and mixed whole rules; caller-supplied resolved group state remains separate from field observations
