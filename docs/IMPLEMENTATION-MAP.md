...
- One prepared declared-String capability validates the model and exact field lookup before reusing the same compiler-associated admission for any nonempty ordinary declaration source. The preparation proof retains declaration/source/matcher coherence, while absent and explicitly empty declaration sources carry no matcher. Its checked-cell and one-field context routes delegate through the shared ordered formal checker; a two-source separator prevents matcher substitution, and the general route agrees with the retained `[0-9]+` specialization.
- One authored checked pattern leaf resolves a direct ordinary nonrepeatable evaluated-String path against the validated flat model, admits its constant source through that compiler-associated boundary, and evaluates through the declaration-owned checked context plus the existing normalized resolved consumer. Its surface type cannot encode an expression-valued left side or nonconstant right side. Raw, registered-custom/predefined, unprepared arbitrary declared-pattern, and repeatable fields fail with explicit boundary errors; prepared-context integration with the connective tree and whole-rule boundary remains separate.
