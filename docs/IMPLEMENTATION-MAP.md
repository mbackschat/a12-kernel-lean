...
- plain authoring-region checks plus recursively staged rounding/`Abs` admission at ordinary arithmetic operand positions, with immediate/grouped literal rejection at every wrapper boundary, direct/grouped numeric-`BaseYear` admission, wrapper-over-wrapper composition, wrapper consumption of the separately checked direct-extremum form, and canonical numeric operand-list `Min`/`Max` admission over direct sources with at most one direct constant
...
- Admitted expressions are unary arithmetic including power and direct aggregates, where each rounding/`Abs` node may occur at an ordinary operand position and recursively consumes another admitted unary expression or the separately checked direct-extremum form. Direct/grouped numeric `BaseYear` and every separately closed Number source are admitted; an immediate/grouped literal remains illegal at each wrapper boundary. Canonical Min/Max over direct sources with at most one direct constant remains a separate direct form; arbitrary expression-valued extremum operands remain outside this checked subset.
...
- Numeric and date-component `BaseYear` sources retain a non-expandable scale-0 summary and fixed runtime fillability. The latter selects direct January 1 or a range endpoint before applying Day/Month/Quarter/Year extraction. Both participate in the existing arithmetic, tolerance, and wrapper evaluator. Direct/grouped numeric `BaseYear` is admitted because it is a number-like reference operation rather than literal syntax. Wrapper-over-wrapper and wrapper-over-direct-extremum shapes are closed; wider Base-Year-bearing or arbitrary expression-valued Min/Max authoring remains fail-closed.
...
- a12-dmkits `FieldValueCoercionDiffTest` retains dual-kernel-route evidence for direct numeric-Enumeration conversion. Accepted [`SPEC-2026-07-22-13`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-07-22-13--fieldvalueasnumber-checks-the-complete-selected-source-domain) and reviewed revision `27afb555` add selected-category, lexical rejection, digit-budget, derived-scale, symmetric missing-polarity, String-contract, and non-ASCII host-decimal-digit evidence. Lean still implements only the checked ordinary ASCII Enumeration/category subset, and project-local retained correspondence remains pending under SG12.
...
- The checked computation operation resolves one nonrepeatable Number target and every scalar or direct-aggregate Number operand through the shared authored-tree traversal, accepts constant-only or field-bearing unary arithmetic and power after the existing authoring/summary checks, and admits operation-form `Round`/`Abs` at ordinary operand positions, recursively around another wrapper, or around the separately checked direct-extremum form. The same route therefore covers direct/grouped numeric `BaseYear`, Number fields, `RangeAsNumber`, `FieldValueAsNumber`, aggregates, Base-Year-derived date components, stored temporal components, and Date differences without source-specific wrapper evaluators; canonical `Min`/`Max` remains a distinct direct fold over sources with at most one constant. Wrapper admission rejects an immediate or grouped literal at each wrapper boundary, preserves nested body and enclosing division/power errors, and evaluates staged transforms in authored order. The operation rejects its target at every nested operand position and applies the exact result-scale gate unless the one explicit warning flag is present. A dedicated attachment boundary accepts the complete externally resolved target policy once, rejects scale/signedness drift from the resolved target, and returns a wrapper whose evaluation has no policy argument. The checked core retains the warning flag, while the wrapper retains every target constraint and delegates expression evaluation to the established numeric computation result consumer before choosing ordinary or suppressed classification. Wider arbitrary expression-valued extremum operands remain fail-closed pending expansion of the extremum owner itself.
