...
- Multi-field String and Enumeration literal-token and field-valued `AtLeastOne`/`No`/`NotAll`, multi-field Number field-valued quantifiers, scalar String/Enumeration/Number literal Included/NotIncluded, and scalar String/Enumeration/Number field-valued membership are checked through the same flat model with per-cell partial relevance. Nested starred Number-fields/direct-Number-values, String-fields/literal-values, direct-String-fields/starred-String-values, and Enumeration-fields/literal-values routes now own checked expansion, optional `Having`, per-cell partial relevance, and wildcard/ancestor extent before that evaluator; multiple/mixed starred operands, starred Enumeration values sides, and whole-rule orchestration remain open.
