...
- plain authoring-region checks plus operation-specific direct-source rounding/`Abs` admission for Number fields, `RangeAsNumber`, and `FieldValueAsNumber`, direct aggregate rounding, and canonical numeric operand-list `Min`/`Max` admission over direct fields with at most one direct constant
...
- Admitted expressions are plain arithmetic including power and direct aggregates, direct Number-field, `RangeAsNumber`, or `FieldValueAsNumber` root rounding/`Abs`, direct aggregate rounding, or canonical Min/Max over direct fields with at most one direct constant.
