...
- One-field literal-token `AtLeastOne`/`No`/`NotAll` is checked through the same flat model; the general value-list evaluator still begins after expansion and any `Having` filtering.
- Table/open/dynamic/partial declarations, repeatable category access, scalar value-list predicates, multi-field/value-side field lists, actual path/star expansion and filtering, checked RNU scope/topology/`@From`/pointer messages, protocol exposure, and project-local portable evidence remain open
