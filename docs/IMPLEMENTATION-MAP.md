...
- `FlatFieldDecl.requiredness` retains the optional model-owned requiredness mode, and `requirednessScopeFor` resolves it to absolute, nearest-repeatable, or direct-parent scope without erasing the gate. `applyAbsoluteRequiredAt` is the third checked-document consumer: it admits only a declaration resolved as genuinely absolute, evaluates the existing generated mandatory rule against the immutable base context, returns its separate authored-validation context without changing computation input, and rejects optional or group-gated fields explicitly. Partial generated staging delegates to that same certificate rather than treating its caller-supplied field list as proof of requiredness.
...
- `applyPartialGeneratedPreliminary` is the bounded call-local counterpart. It validates and normalizes the caller's wildcardable relevant entities, ignores concrete selectors beyond model capacity, builds uniqueness from relevant physical candidates rather than filtering full findings afterward, composes the existing absolute nonrepeatable required evaluator, and retains the generated channels separately over one immutable base. The same default-eligibility projection becomes `silentlyUnavailable` rather than an injected cell: those addresses are removed before index-finding construction, relevant reads return the distinct cause-free outcome, out-of-set addresses still fail explicitly, and containing groups receive the separate silent error bit.
...
- [`SG1`](SEMANTICS-GAPS.md#sg1--general-checked-document-construction) remains open. Flat/custom, over-repetition, resolved group-presence, declaration-certified absolute-required, and full/partial all-scalar index consumers—including transient default injection and silent suppression—now share the checked input. Computation's eager operand-index behavior is source-closed but cannot be projected without the future checked whole-model plan; a caller-supplied field list is forbidden because it would weaken plan certification. Ordinary partial-condition masking and wider repeatable consumers remain SG1 work, while computation activation, result projection, and application remain later SG4 phases.
...
- The low-level RNU evaluator remains total over caller-supplied rows outside the checked typed route without a kernel-correspondence claim. The authored route accepts Number, ordinary String, and direct ordinary Enumeration keys along one branch. Temporal and Boolean/Confirm raw-spelling identity remain open because the current checked semantic payload does not retain the kernel's exact internally stored text for those cases; prepared custom String keys remain explicitly rejected until their authoring and document-cache contract is source-closed. Every checked key projects the deepest row environment to its declaration-owned repeatable prefix before invoking its existing classifier, so a future SG1 document view need not rediscover ancestor addressing.
...
- Descendant/document/group-instance construction, ordinary repeatable-path addressing, starred or mixed fixed/repeatable group lists, nonrepeatable terminal groups beneath a star, repeatable negative-condition legality, and broader required-rule orchestration are indexed by [`SG1`](SEMANTICS-GAPS.md#sg1--general-checked-document-construction) and [`SG2`](SEMANTICS-GAPS.md#sg2--general-repeatable-addressing-and-operand-construction).
