...
- One checked unfiltered nested String-star route admits a nonempty literal token side, validates the exact String declaration, and reuses the same topology, declaration-owned checking, normalized token cell, hierarchical omitted-tail state, per-cell partial relevance, and separate wildcard/ancestor extent fact before the common quantifier dispatcher. The generic over-limit reader and full/partial topology-to-side constructors moved to the checked star-path owner at their second typed consumer; Number now delegates through them.
...
- Six checked nested-star String cases separate normalized matching, malformed-cell asymmetry, hierarchical omitted-tail polarity, all three partial-relevance outcomes with masked-read independence, concrete-all-rows versus wildcard extent, empty literal admission, and exact kind rejection. Six trusted laws retain nonempty literals, literal-side shape, canonical partial order/tail state, extent classification, read locality, and exact full/partial delegation. Kernel source and maintained a12-dmkits `ValueListQuantifierStarDiffTest` triangulate the full-validation route; partial extent correspondence is pending under [`SPEC-2026-07-22-09`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-07-22-09--partial-starred-value-list-extent-requires-wildcard-coverage) and project-local evidence remains pending under [`SG12`](SEMANTICS-GAPS.md#sg12--retained-kernel-correspondence-coverage).
...
- Multi-field String and Enumeration literal-token and field-valued `AtLeastOne`/`No`/`NotAll`, multi-field Number field-valued quantifiers, scalar String/Enumeration/Number literal Included/NotIncluded, and scalar String/Enumeration/Number field-valued membership are checked through the same flat model with per-cell partial relevance. Nested starred Number-fields/direct-Number-values and unfiltered String-fields/literal-values routes now own checked expansion, per-cell partial relevance, and wildcard/ancestor extent before that evaluator; String `Having`, starred value sides, Enumeration stars, mixed starred/plain operand lists, and whole-rule orchestration remain open.
