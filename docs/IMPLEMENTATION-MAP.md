...
- One nonrepeatable ordinary Enumeration field now reaches the checked flat `AtLeastOne`/`No`/`NotAll` value-list quantifiers with an exact stored/category projection and a nonempty literal-token side. Every literal is admitted against the selected domain, the core re-derives the model-owned projection, runtime delegates directly to the existing `ValueListQuantifier.eval`, and the Enumeration projection state is shared with literal and field comparison rather than copied. Relevance masks the field before either side is consumed; the full-validation row gate delegates to `ValueListQuantifier.canFireOnEmpty`, so only `No` evaluates a blank row and fires OMISSION for its clean empty field.
...
- Ten checked flat value-list cases separate exact stored lowering, stored and many-to-one category matches, `No` versus `NotAll` on a blank row, relevance masking, stored/category literal-domain failures, empty-list rejection, and non-Enumeration rejection. Trusted laws retain the quantifier-owned empty-row capability, relevance masking, and the universal clean-empty `No` OMISSION result.
...
- One-field literal-token `AtLeastOne`/`No`/`NotAll` and scalar Included/NotIncluded are checked through the same flat model; the general value-list evaluator still begins after expansion and any `Having` filtering.
- Table/open/dynamic/partial declarations, repeatable category access, multi-field/value-side field lists, scalar field-valued list entries, actual path/star expansion and filtering, checked RNU scope/topology/`@From`/pointer messages, protocol exposure, and project-local portable evidence remain open
