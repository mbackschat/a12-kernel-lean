...
- Pending [`SPEC-2026-07-22-05`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-07-22-05--enumeration-field-list-literals-use-the-union-of-selected-domains) records the newly separated union-admission rule and the current a12-dmkits intersection/category-erasure mismatch. Lean's checked multi-field lowering follows the pinned kernel checker while the peer correction is pending.
...
- Multi-field literal-token `AtLeastOne`/`No`/`NotAll` and scalar Included/NotIncluded are checked through the same flat model; the general value-list evaluator still begins after expansion and any `Having` filtering.
- Table/open/dynamic/partial declarations, repeatable category access, field-valued value sides, scalar field-valued list entries, actual path/star expansion and filtering, checked RNU scope/topology/`@From`/pointer messages, protocol exposure, and project-local portable evidence remain open
