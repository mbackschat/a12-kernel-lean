...
- Concrete declaration/path lowering is blocked at one shared representation boundary: all current field-ID-indexed model and evaluation contexts store scalar `Value`, while `FullDate`, `TimeOfDay`, and `Instant` remain separate proof-bearing types. A future architecture capsule must choose one common storage/dependency account; per-kind contexts and a second temporal declaration tree are excluded.
