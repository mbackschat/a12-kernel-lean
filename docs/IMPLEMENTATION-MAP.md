...
- One shared prepared String context validates the model once, prepares every ordinary declared pattern and registered custom validator against that model, and dispatches the mutually exclusive declaration profiles before the same flat condition tree. Its full-evaluation entry point retains distinct preparation and condition errors, explicit `World`/locale/compiler inputs, and one existing evaluator. A patterned field can no longer bypass its matcher through the custom-prepared presence route, and a mixed row checks its patterned and custom fields through their distinct owners.
