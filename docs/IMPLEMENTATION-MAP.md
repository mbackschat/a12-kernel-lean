...
- Multi-field String and Enumeration literal-token and field-valued `AtLeastOne`/`No`/`NotAll`, multi-field Number field-valued quantifiers, scalar String/Enumeration/Number literal Included/NotIncluded, and scalar String/Enumeration/Number field-valued membership are checked through the same flat model with per-cell partial relevance. One nested starred Number-fields/direct-Number-values route now owns expansion, optional `Having`, and unfiltered per-cell partial relevance before that evaluator; other starred side shapes, kinds, and whole-rule orchestration remain open.
