...
- Multi-field literal-token and field-valued `AtLeastOne`/`No`/`NotAll` plus scalar Included/NotIncluded are checked through the same flat model; the general value-list evaluator still begins after expansion and any `Having` filtering.
