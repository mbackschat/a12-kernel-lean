...
- plain authoring-region checks plus operation-specific direct-source rounding/`Abs` admission for Number fields, `RangeAsNumber`, `FieldValueAsNumber`, numeric aggregates, Base-Year-derived date components, stored temporal components, and Date-only month/year differences, and canonical numeric operand-list `Min`/`Max` admission over direct fields with at most one direct constant
...
- a checked closed validation dispatch over all six direct ordinary comparison operators and four fixed tolerance ranges, with numeric `BaseYear`, its direct/range-selected date-component extractions, admitted Date/Time/DateTime field components, checked ordinary Enumeration/category `FieldValueAsNumber`, Date-only month/year differences, direct Number field-list aggregates, and ordinary nonrepeatable `RangeAsNumber` supported as shared sources
- one checked nonrepeatable computation-operation consumer with model-resolved target, Number-field, numeric-`BaseYear`, Base-Year date-component, admitted temporal-field component, checked ordinary Enumeration/category `FieldValueAsNumber`, Date-only month/year-difference, direct Number field-list aggregate, and ordinary String-range sources, shared support for plain arithmetic plus direct Number-field, `RangeAsNumber`, `FieldValueAsNumber`, aggregate, Base-Year-derived date-component, stored temporal-component, and Date-difference `Round`/`Abs`, and direct Number-field/one-constant `Min`/`Max`, default-unsuppressed result-scale admission, explicit warning bypass, nested target-reference rejection, distinct numeric value/domain-failure/inherited-poison evaluation, one-time complete target-policy attachment, and retained proof-coherent target dispatch
