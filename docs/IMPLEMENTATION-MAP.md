...
- [`SG1`](SEMANTICS-GAPS.md#sg1--general-checked-document-construction) remains open. Flat/custom, over-repetition, resolved group-presence, declaration-certified absolute-required, and full/partial all-scalar index consumers—including transient default injection and silent suppression—now share the checked input. Computation's eager operand-index subset, ordinary partial-condition masking, wider repeatable consumers, computation activation, result projection, and application remain later phases.
