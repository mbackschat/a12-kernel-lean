...
- plain authoring-region checks plus operation-specific direct-source rounding/`Abs` admission for Number fields, `RangeAsNumber`, `FieldValueAsNumber`, and numeric aggregates, and canonical numeric operand-list `Min`/`Max` admission over direct fields with at most one direct constant
...
- Admitted expressions are plain arithmetic including power and direct aggregates, direct Number-field, `RangeAsNumber`, `FieldValueAsNumber`, or aggregate root rounding/`Abs`, or canonical Min/Max over direct fields with at most one direct constant.
