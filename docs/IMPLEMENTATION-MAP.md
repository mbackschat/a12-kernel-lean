...
- Resolved full-Date equality, inequality, strict order, and inclusive order share the existing calendar chronology. Their classified validation projection reuses the symmetric scalar operand state: formal unavailability dominates, no value does not fire, and a true comparison fires OMISSION exactly when either present value retains missing provenance. Laws characterize equality, operand/operator exchange across truth and verdict, strict-direction exclusion, unknown/no-value handling, and both firing polarities.
...
- Resolved Time equality, inequality, strict order, and inclusive order compare decoded `TimeOfDay.secondsSinceMidnight` coordinates after comparable-component checking and the runtime's shared time-only parse anchor. Date, Time, and DateTime now share one operation-neutral comparison enum and the same classified scalar verdict path; Time-specific laws preserve equality, strict-direction exclusion, truth, and polarity under operand/operator exchange.
...
- The Date, Time, and DateTime comparison owners start after static component admission and after each operand has been classified as a present full Date, decoded time of day, or exact instant with symmetric missing provenance, no value, or formal unavailability. Literal typing/parsing, AM/PM decoding, partial-Date resolution, model-zone selection, raw-cell classification, and checked model integration remain outside.
