...
- [`SG1`](SEMANTICS-GAPS.md#sg1--general-checked-document-construction) remains open. Flat/custom, over-repetition, resolved group-presence, absolute-required, and full/no-default-partial all-scalar index consumers now share the checked input. Model-owned index defaults, ordinary partial-condition masking, wider repeatable consumers, computation activation, result projection, and application remain later phases.
