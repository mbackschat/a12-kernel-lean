...
- Resolved full-Date equality, inequality, strict order, and inclusive order share the existing calendar chronology; laws characterize equality, operand/operator exchange, and strict-direction exclusion.
...
- Maintained a12-dmkits `DateLiteralDiffTest` establishes that a typed Date literal is canonicalized before equality and ordering, so comparison follows date identity and chronology rather than stored text.
...
- Date coverage includes the unbounded positive-era account, resolved six-way full-Date comparison, admitted full-Date day/month/year shifts and signed completed-period differences, resolved three-part construction classification, and the proved calendar-coordinate successor/strict-monotonicity bridge.
...
- The resolved Date comparison truth function starts after both operands are present full Dates. Literal typing/parsing, partial-Date resolution, empty/formal classification, validation polarity, DateTime instant comparison, and checked model integration remain outside.
...
- Open:
  ...
  - Date comparison operand classification/polarity, DateTime instant comparison, and checked integration
