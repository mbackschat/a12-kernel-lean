...
- **Implemented internally, partial; direct ordering external evidence pending:** all six nonrepeatable Number comparisons, Boolean/Confirm/String equality/inequality, six-way temporal field-to-field, field-to-typed-Date-literal, and field/`Now` comparison in either operand order, plus Number/Boolean/Confirm/String/Date/Time/DateTime presence, are executable.
...
- Static temporal format admission retains one six-component set and two deliberate gates. Direct comparison checks year/date classes plus equality-only time presence without exact component equality; extrema require exact component equality after optional Base Year supplementation. Laws establish symmetry, exact-to-coarse implication, and equality-to-ordering implication; cases lock unequal date precision, Date/DateTime, unequal Time seconds display, Base Year, date/time class, and full-DateTime separators.
...
- Kernel `CheckVergleichsBedingungImpl`, `DateFormat`, and `CheckEntityListenUtils` establish the coarse direct-comparison and exact aggregate component gates. `SPEC-2026-07-22-01` requests the missing peer prose/admission matrix without duplicating the pending aggregate-execution request.
- Kernel temporal aggregate checkers and `DateCombiner`/`VkDate` establish comparable-component admission plus resolved Time-coordinate and DateTime-instant selection through the same empty, non-relevance, and missingness fold as Date. The reviewed a12-dmkits interpreter already shares that temporal fold, but its adapter conversion boundary remains Date-only and no focused Time/DateTime aggregate differential was found; `SPEC-2026-07-21-07` requests that bounded reconciliation.
...
- The resolved Date, Time, and DateTime comparison owners start after static component admission and after each operand has been classified as a present full Date, decoded time of day, or exact instant with symmetric missing provenance, no value, or formal unavailability. All three accept their matching typed validation observation directly. The checked field-to-field route performs declaration/path lowering and compares already-parsed exact runtime instants; the field-to-Date-literal route begins after lexical classification plus model-zone/Base-Year decoding and preserves the literal's component shape; the checked `Now` route injects the explicit evaluation world after its extra time-component gate. Concrete token typing, AM/PM decoding, partial-Date text resolution, `Today`, and model-zone parsing remain outside.
- Concrete declaration/path lowering has one shared storage representation and checked presence/comparison path. The existing heterogeneous field-ID-indexed `Value` context carries `temporal TemporalKind Instant`; `FieldKind.temporal` retains declaration-owned components, formal checking admits only the matching tag, and `FlatTemporalField` reuses ordinary presence, requiredness, generated guards, model-context construction, multi-field relevance, and exact-instant comparison. `FlatTemporalOperand` owns field, fixed typed literal, and dynamic `Now` shapes without a parallel evaluator. Proof-bearing wall-label types remain resolved parser inputs rather than storage alternatives.
...
- Open:
  ...
  - `Today`, plus `Now` outside checked direct comparison
