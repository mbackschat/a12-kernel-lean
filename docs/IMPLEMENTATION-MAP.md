...
- Construction classification and its direct numeric component projection remain reason-bearing but do not yet retain calendar identity, compose temporal no-value reasons beyond extraction, implement date differences, or implement legacy-hybrid month/year operations. Typed stored/full-Date, Time, and DateTime observations plus the heterogeneous flat context now close their direct numeric projection boundaries; checked authored-expression lowering still remains separate.
