...
- `applyPartialGeneratedPreliminary` is the bounded no-default partial counterpart. It validates and normalizes the caller's wildcardable relevant entities, ignores concrete selectors beyond model capacity, builds uniqueness from relevant physical candidates rather than filtering full findings afterward, composes the existing absolute nonrepeatable required evaluator, and retains the two finding channels separately over one immutable base. Its relevant-read query rejects out-of-set addresses explicitly so this preliminary API cannot masquerade as the later ordinary-condition UNKNOWN mask.
- `groupPresenceInputFromSlice` is the single group-scope fold parameterized by a selected row/cell slice; `groupPresenceInputFromCells` preserves its full/source-complete specialization. The full and partial preliminary views reuse that owner to retain admitted duplicate content, include absent mandatory cells as group errors, exclude nonrelevant formal errors, and avoid turning a relevant phantom into source topology.
...
- [`SG1`](SEMANTICS-GAPS.md#sg1--general-checked-document-construction) remains open. Flat/custom, over-repetition, resolved group-presence, declaration-certified absolute-required, and full/no-default-partial all-scalar index consumers now share the checked input. The requiredness and Enumeration S-value declaration prerequisites are closed; transient full-default injection, partial silent unavailability, ordinary partial-condition masking, wider repeatable consumers, computation activation, result projection, and application remain later phases.
