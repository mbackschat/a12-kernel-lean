...
- Four formal-message cases and three laws lock exact address/code metadata, fixed ERROR/VALUE classification, supplied-empty priority, fallback selection, and fallback opacity.
...
- Registered custom validation is checked through relevance-first sampling and the ordinary cell root, supplied message bytes render at an already-resolved label boundary, one formal-message projector owns project code, supplied-versus-fallback text, address, and fixed ERROR/VALUE metadata, and the explicit named validity pair reuses the same registry/context interface. Label resolution/provider invocation, integration into a full validation result or general checked condition AST, repetition-key projection, checked field-declaration integration, partial row construction, predefined declarative registry precedence, and public protocol remain open.
