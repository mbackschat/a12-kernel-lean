...
- The checked guard traversal rejects the exact computed target ID in common and every alternative condition before phase lowering, retaining common versus one-based alternative position. The separate checked numeric-operation boundary rejects the same target throughout a plain-arithmetic or admitted direct root value-function tree, retains the exact-scale warning choice, and routes its evaluated result through the matching target branch. One unconditional expression operation now produces its generated target-filled mismatch by preserving that checked tree under model-wide nonrepeatable admission; guarded expression-table integration remains indexed by [`SG3`](SEMANTICS-GAPS.md#sg3--shared-checked-condition-and-expression-tree).
...
- one checked unconditional numeric-expression generated rule over model-wide nonrepeatable operands, separated from ordinary same-group rule admission
...
- Generated-computation validation and mixed ordinary whole rules reuse the same post-fire boundary and outcome. The generated fragment's optional common condition sits once outside the literal mismatch disjunction, guards reject direct computed-target self-reference, ERROR severity does not fix message polarity, and tolerance metadata cannot affect computation selection. One unconditional checked numeric expression now uses the same mixed tree under explicit model-wide nonrepeatable admission; guarded expression branches remain open.
...
- one checked unconditional expression-valued generated rule whose cross-group Number and temporal-component operands preserve computation scope
