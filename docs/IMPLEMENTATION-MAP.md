...
- Kernel `CheckKonstanteReferenzjahrImpl`, `CheckVergleichsBedingungImpl`, `CodeGenCreator.endVisit(IDateRangeExtractionOperation)`, numeric fractional-digit checking, and `RuntimeController.getReferenzjahr` establish declared-model admission, Number-versus-Date dispatch, exact scale-0 equality, immutable date/time class, and model-zone January 1/December 31 resolution. Maintained a12-dmkits `BaseYearDiffTest` triangulates the numeric and direct-date projections across both kernel strategies; `PointInTimeSourceReadTest` locks authored range-source acceptance and roundtrip, while peer range execution remains the recorded upstream gap. This repository retains no portable Base Year observation.
...
- Exact formal causes, raw/checked date-component cells, two- and four-part forms, missing-Base-Year authoring rejection outside direct comparison, constant-only authoring rejection, exact parser/component bounds, full-row gating, nested extraction/difference/addition consumers, stored/computed target admission, and protocol exposure remain open.
