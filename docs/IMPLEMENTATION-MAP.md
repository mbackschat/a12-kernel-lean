...
- Owners: [`Semantics/GroupPresence.lean`](../A12Kernel/Semantics/GroupPresence.lean), [`Elaboration/ValidationCondition.lean`](../A12Kernel/Elaboration/ValidationCondition.lean), [`Elaboration/ValidationRule.lean`](../A12Kernel/Elaboration/ValidationRule.lean), [`Proofs/GroupPresence.lean`](../A12Kernel/Proofs/GroupPresence.lean), [`Proofs/ValidationCondition.lean`](../A12Kernel/Proofs/ValidationCondition.lean), [`Conformance/GroupPresence.lean`](../A12Kernel/Conformance/GroupPresence.lean), and [`Conformance/ValidationCondition.lean`](../A12Kernel/Conformance/ValidationCondition.lean).
- Boundary: IF193's product state and consumer projections after descendant scope and relevance are resolved, plus checked scalar `GroupFilled`/`GroupNotFilled` and fixed multi-entity group-list integration for nonrepeatable paths and the already-selected `RuleGroup` instance.
...
- The state drives exact scalar `GroupFilled`/`GroupNotFilled` verdicts, all five fixed-list group predicates, plain multi-group `NumberOfFilledGroups`, and the parent-filled requiredness gate. Scalar predicates are one shared-condition leaf family; the runtime resolver returns an optional state by checked group path, making absence `UNKNOWN` instead of conflating it with empty.
...
- Static assembly rejects empty lists, direct duplicates, group/descendant overlap, repeatable ordinary paths, illegal roots, and insufficient arity. A legal fixed singleton canonicalizes to the existing scalar field/group-presence leaf instead of creating a second semantic representation.
- The numeric count is stricter: any erroneous or not-fully-relevant operand makes the whole count unavailable.
- Trusted laws characterize the two scalar firing regions, row and duplicate admission, rejected scalar exclusion, exact generic and group-only tally partitions, conservative-verdict firing preservation, checked list delegation, error absorption into the numeric count, and the requiredness bridge.
- Cases cover malformed-only, admitted-plus-malformed, duplicate index, created and over-limit rows, partial positive/empty, full empty, all five fixed-list operators, mixed field/group evaluation, field and group unavailability, decisive known witnesses, singleton canonicalization, overlap/root/arity/repeatable diagnostics, strict count availability, requiredness activation, checked `RuleGroup`/ordinary-path evaluation, subtree error-field admission, sibling rejection, and whole-rule message projection.
