...
- **Implemented narrowly; external evidence pending outside the cited range controls.** Checked validation accepts two same-group expressions with at least one field, the admitted expression classes above including ordinary nonrepeatable `RangeAsNumber`, six ordinary operators, four tolerance ranges, and the exact-scale-warning bypass.
...
- Generated numeric validation narrows a checked `RangeAsNumber` atom directly into the validation atom algebra without reconstructing or re-resolving its surface path. Model-wide nonrepeatable String-source admission, normalized range evaluation, missing-source polarity, and the computed target mismatch therefore remain the same checked expression rather than a generated-rule exception.
