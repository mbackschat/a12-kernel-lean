...
- Model-owned construction of a general authored filter condition, partial relevance, mixed field declarations, RNU/group-presence adaptation, and every wider repeated consumer remain indexed by [`SG1`](SEMANTICS-GAPS.md#sg1--general-checked-document-construction) and [`SG2`](SEMANTICS-GAPS.md#sg2--general-repeatable-addressing-and-operand-construction). The shared checked condition/expression tree is closed; the existing model-to-operator consumers include the one-level Number aggregate/`FirstFilledValue` routes and the resolved filtered nested Number side.
...
- Boundary: one ordered resolved Number operand, checked construction of one unfiltered finite one-level Number star, and order-aware full/partial validation over one checked nested Number star, shared by §3, §8, §10, and §11.
...
- The scan's shared step/finalize mechanism now has two real consumers: ordinary resolved evaluation and checked partial validation. The partial adapter matches each topology environment against exact or ancestor relevant-entity patterns immediately before target classification, returns a distinct nonrelevant result for a reached excluded cell, and stops relevance and value reads after a present or unavailable cell.
- Trusted laws prove prefix/suffix termination, empty-prefix idempotence, empty identities, both projections, the central non-law that computation forgets the empty-prefix distinction while validation preserves it, nonrelevance before either target reader, and present-head suffix/read independence.
...
- Boundary: IF194's structural decision, checked model/path lowering, general document-topology construction, one unfiltered or resolved-filtered nested Number cell-stream consumer, the all-rows full/partial relevance gate, and order-aware per-cell partial `FirstFilledValue` relevance for that Number source.
...
- `CorrelatedHaving.selectEnvironments` applies an already-resolved numeric/repetition validation filter to those canonical leaves with distinct complete candidate and captured environments. `CheckedStarNumberSource.resolvedValidationHavingValueSide` maps the declaration-owned target reader only over the retained environments and records filter presence even when none survive.
