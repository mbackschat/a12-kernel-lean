...
- a checked closed validation dispatch over all six direct ordinary comparison operators and four fixed tolerance ranges, with numeric `BaseYear` and its direct/range-selected date-component extractions supported as fixed scale-0 sources in plain arithmetic
- one checked nonrepeatable computation-operation consumer with model-resolved target, Number-field, numeric-`BaseYear`, and Base-Year date-component sources, shared support for plain arithmetic plus direct-field `Round`/`Abs` and direct-field/one-constant `Min`/`Max`, default-unsuppressed result-scale admission, explicit warning bypass, nested target-reference rejection, distinct numeric value/domain-failure/inherited-poison evaluation, one-time complete target-policy attachment, and retained proof-coherent target dispatch
...
- Checked validation resolves two same-group expressions over Number fields, numeric `BaseYear`, and direct numeric component extraction from direct or range-selected Base-Year date sources, lowers each once, and gates empty rows before reads. At least one actual field remains mandatory, so every source-only or literal-only condition retains the constant-expression rejection.
...
- Typed full-Date and DateTime validation observations share one direct Day/Month/Quarter/Year numeric projection. Present values expose fixed decoded date components, DateTime ignores its clock, empty sources become symmetric fillable zero, and exact formal causes survive until the numeric verdict projection.
...
- Maintained a12-dmkits `DateExtractDiffTest` and `DateFunctionsOverDateTimeDiffTest` separate all four direct date components, DateTime's ignored clock, and the empty-to-zero branch across both kernel strategies and the peer interpreter.
...
- Construction classification and its direct numeric component projection remain reason-bearing but do not yet retain calendar identity, compose temporal no-value reasons beyond extraction, implement date differences, or implement legacy-hybrid month/year operations. Separately, typed stored/full-Date and DateTime observations now close direct numeric extraction without claiming heterogeneous flat-context lowering.
