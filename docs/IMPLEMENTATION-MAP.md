...
- Concrete declaration/path lowering is ready for the next shared representation capsule. The ownership audit selected the existing heterogeneous field-ID-indexed `Value` context, moved exact `Instant` to `Core`, and excluded per-kind contexts and a second temporal declaration tree. The next step is to add distinct Date/Time/DateTime value tags over the exact runtime coordinate, then connect declaration-owned component/format policy; proof-bearing wall-label types remain resolved parser inputs rather than storage alternatives.
