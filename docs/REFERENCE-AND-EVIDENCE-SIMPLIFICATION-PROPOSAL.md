...
The compact family must retain enough information to replay the same empirical claim: kernel behavior version, stable case identity and order, normalized semantic input, externally observed output at the claimed fidelity, provenance revision, and digest references to the archived raw source. If a current upstream producer can certify those bytes without resurrecting the retired capture V1, use that certification. Otherwise label the result honestly as a project-reviewed compact projection, bind the exact pre-compaction Git revision and raw digests, and keep the full raw material recoverable there.
