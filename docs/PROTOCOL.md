...
Run `lake exe checkReferenceProcess` to verify the frozen descriptor, suite, [fixtures](../examples/reference-cli/flat-evidence/), support-manifest evidence boundary, adjacent source-maintainer [mutation qualification plan](../reference/flat-validation-empty-logic-v1.mutation-plan.json), generated profile, and retained result through their lineage-pinned SHA-256 identities. The one-off projection bridge that originally produced these artifacts is retired from the current tree and remains inspectable at Git revision `03186c1`; a changed capability must receive a new identity and current shipment path rather than rewriting this historical record. The mutation plan targets candidate behavior exposed through protocol 1 but is not part of the wire contract or the immutable first cold bundle.
...
[`A12Kernel/ReferenceProcessTestMain.lean`](../A12Kernel/ReferenceProcessTestMain.lean) invokes the compiled executable, checks the process and JSON contract, compares the current shipped manifest and lineage mirror, digest-checks frozen v1 artifacts, exercises hostile and near-miss inputs, runs current v2 suite integrity self-tests, and uses the compiled current Lean reference as their control candidate. Frozen v1 suites remain runnable on demand and their accepted bytes remain covered by the historical lock. [`TESTING.md`](TESTING.md#reference-process-harness-and-sample-data) owns these harness mechanics; this document owns the observable wire contract.
...
The runner also has an in-memory integrity mode that validates canonical case metadata and evidence links, then mutates independent suite, manifest, fixture, and candidate-process boundaries and requires rejection. `checkReferenceProcess` runs these structural self-tests and the compiled Lean reference controls for current v2 suites; frozen v1 suites remain available for explicit historical reproduction:
