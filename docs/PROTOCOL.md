...
This document owns the exact process and JSON contract of the product-shaped Lean reference interpreter. Wire protocol version 1 currently exposes two disjoint normalized operations over existing checked evaluators: flat validation and one direct-child captured-outer correlation shape. The current executable and all current kits and suites use reference semantics 0.3.0/V2. This is not the concrete English/German A12 DSL, general DM-JSON, or raw user-input parsing.
...
The six recognized comparison operator tags are `equal`, `notEqual`, `less`, `lessEqual`, `greater`, and `greaterEqual`. Only `equal` and `notEqual` are supported by this public flat reference operation; the four ordering tags deliberately reach a stable unsupported-operator diagnostic rather than being guessed or treated as malformed JSON. The internal checked flat theory also admits direct Number `<` and `>=`, but that Lean-only semantic capsule does not widen this wire contract.
...
### Development flat implementer slice

The current language-neutral semantics and exclusions are in [`IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md`](IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md), its nine-case runnable index is [`flat-validation-empty-logic-v2.conformance.json`](../reference/flat-validation-empty-logic-v2.conformance.json), and its requests and responses live under [`examples/reference-cli/`](../examples/reference-cli/). The first eight cases cover the original evidence-derived empty-logic boundary; the ninth is the corrected directional-Number witness. [`supported-fragment-v2.json`](../reference/supported-fragment-v2.json) is the machine-readable capability boundary rather than a separate descriptor.

Run `lake exe checkReferenceProcess` to check the current manifest, process contract, fixtures, V2 suite integrity, and compiled Lean reference controls. This is a development handover, not full flat-operation release closure: passing its indexed cases supports only the named finite slice. Direct String comparison and `Length(String)` exist only in the internal checked semantics/evidence lane today; wire protocol version 1 still admits only Number, Boolean, and Confirm field kinds and has no String or `Length` request form.

Reference semantics 0.2.0/V1 and the completed Rust experiment are retired history, not current runnable artifacts or qualification. Their exact identities, digests, outcomes, and Git recovery points are preserved in the [archived experiment record](archived/REFERENCE-SEMANTICS-0.2.0-AND-RUST-EXPERIMENT.md); they must never be relabeled as V2.
...
The response envelope intentionally identifies protocol 1 and kernel behavior 30.8.1, not the executable semantics line. Use the exact binary's `--manifest` result and a binary or release digest as the 0.3.0/V2 compatibility pin.
...
The current support identity is generated directly from [`Support.lean`](../A12Kernel/Reference/Support.lean): reference semantics 0.3.0, protocol 1, manifest schema 2, kernel behavior 30.8.1, and the two V2 suite identities. The retired 0.2.0/V1 lineage and Rust qualification are not part of current source or process gates; use the [archived experiment record](archived/REFERENCE-SEMANTICS-0.2.0-AND-RUST-EXPERIMENT.md) and its pinned Git revisions for historical inspection.
...
Each adjacent `.response.json` file is the expected response and can be run with the same redirection pattern shown above. The generated flat set under [`flat-evidence/`](../examples/reference-cli/flat-evidence/) and the curated correlation set are completely indexed for the current account by their [flat V2](../reference/flat-validation-empty-logic-v2.conformance.json) and [correlation V2](../reference/single-group-correlation-v2.conformance.json) suites; those machine-readable suites, not a duplicated Markdown list, own their complete finite case inventories. [`ARTIFACTS.md`](ARTIFACTS.md#examples-human-examples-and-regression-fixtures) explains which examples are generated or curated and how they evolve.

[`A12Kernel/ReferenceProcessTestMain.lean`](../A12Kernel/ReferenceProcessTestMain.lean) invokes the compiled executable, checks the process and JSON contract, compares the current shipped manifest, exercises hostile and near-miss inputs, runs current V2 suite integrity self-tests, and uses the compiled current Lean reference as their control candidate. [`TESTING.md`](TESTING.md#reference-process-harness-and-sample-data) owns these harness mechanics; this document owns the observable wire contract.
...
The `checkCandidateConformance` suite runner validates the selected suite against the committed support manifest named by that suite, then requires deterministic candidate responses and compares expected and actual JSON structurally, so object-key order is not cross-language normative. It does not call the candidate's `--manifest` or prove that the candidate advertises that identity independently; the suite path and any surrounding qualification record supply the claim being tested. Each candidate invocation uses the shared bounded relay with a 10-second deadline, 1-second cleanup deadline, 1 MiB stdin and stdout caps, and a 64 KiB stderr cap. The relay provides cooperative macOS/Linux process-group cleanup and streamed resource bounds, not security isolation, and the suite has no aggregate campaign budget. These runner limits are process policy rather than part of the normalized wire contract.

The runner also has an in-memory integrity mode that validates canonical case metadata and evidence links, then mutates independent suite, manifest, fixture, and candidate-process boundaries and requires rejection. `checkReferenceProcess` runs these structural self-tests and the compiled Lean reference controls for the current V2 suites:
...
The current [flat](IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md) and [correlation](IMPLEMENTER-KIT-CORRELATION.md) kits use 0.3.0/V2 identities and commands. Historical 0.2.0/V1 results are not current qualification even when the current binary happens to agree on an old indexed case.
