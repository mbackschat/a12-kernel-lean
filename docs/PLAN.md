...
- Semantic baseline: checked temporal field-to-field, field-to-typed-Date-literal, field/`Today`, and field/`Now` comparison over one shared exact-instant operand core. `Today` carries the checked model zone id and resolves midnight through an explicit world capability; UTC and the versioned Berlin profile are concrete instances without becoming the canonical zone ceiling.
- Its focused red/green checks, full 240-job `lake build`, 51/51 retained-observation replay, and trust audit over 760 theorem roots / 12,403 declarations / 140 logical modules passed.
...
Audit `BaseYear` as the next checked point-in-time consumer across its Number and Date dispatch. Confirm the exact source-owned static gates and model-zone January 1 resolution, then reuse the existing numeric and temporal operand roots only where their current result domains fit. A second zone consumer may justify widening the `Today` capability into a shared profile boundary; do not prebuild that generalization before the source audit identifies the common contract.
