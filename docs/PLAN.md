...
- **Ready — heterogeneous temporal model context:** the ownership audit selected the existing field-ID-indexed `Value` context and excluded per-kind contexts. Add distinct Date/Time/DateTime storage tags over exact `Instant`, then connect declaration-owned component/format policy; do not store proof-bearing wall labels as a parallel runtime domain.
