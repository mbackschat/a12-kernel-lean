...
1. **Adopt the static-legality route for diagnostic identity.** The kernel consistency oracle is reachable locally, so the active unit's `MVK_` coverage no longer needs upstream requests. First use should be a calibration probe whose answer is already known — the `SPEC-2026-08-05-04` reordered-operand pair — so a method error shows up as disagreement with a measured result rather than as a plausible new fact. Its first new use after calibration is the compute-arm static admission rule for a fixed group count, which [`SG13`](SEMANTICS-GAPS.md#sg13--group-list-and-group-count-completion) now names as its only remaining measurable axis.
...
**None.** Both former blockers closed on 2026-08-06; the SG13 one is now implemented and needs no further record here.
