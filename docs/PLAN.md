...
1. **Close computed Date target diagnostic parity next.** Project the already represented partial-precision target refusal to measured `MVK_INVALID_DATE_TYPE`, inventory the separate unsupported-suppression route before changing any surface, and leave formats, zones, stored partial-Date input, and runtime target checking untouched.
