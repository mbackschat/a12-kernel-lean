...
1. **Static-legality route: calibrated, blocked on provenance.** The kernel consistency oracle is reachable locally and the method is now validated against the already-measured `SPEC-2026-08-05-04` reordered-operand pair, recorded in [`SOURCES.md`](SOURCES.md). It cannot yet produce a citable observation; see the blocker below. Once a clean launcher exists, the first new use is the compute-arm static admission rule for a fixed group count, which [`SG13`](SEMANTICS-GAPS.md#sg13--group-list-and-group-count-completion) names as its only remaining measurable axis.
...
**One, and it needs an owner decision.** The static-legality route's *method* is calibrated but its *provenance* is not, so the route cannot yet produce a citable retained observation.

- **No clean `dmtool` launcher is buildable here.** The installed launcher at `../a12-rulekit/cli/build/install/dmtool/bin/dmtool` self-reports source `568e6a1b` **(dirty)** and predates the sibling's current HEAD. [`TESTING.md`](TESTING.md#structured-dmtool-probes-and-feedback) forbids anchoring a retained observation to a dirty launcher, and rebuilding it fails: `:cli:installDist` cannot resolve the Kotlin Gradle plugin from the configured corporate Artifactory, and the sibling is now dirty under active peer edits, which bars sibling builds regardless. The calibration itself passed and is recorded in [`SOURCES.md`](SOURCES.md) as method validation. **Recommendation: get a clean launcher built out-of-band** — either on a network with Artifactory access, or by asking the a12-dmkits owner for a clean-revision build — because every remaining `MVK_` family in the active semantic unit depends on this one route, and the alternative is marking that whole axis external-evidence pending. The narrower alternative, downloading a release binary through the skill's installer, is deliberately not taken: [`TESTING.md`](TESTING.md#structured-dmtool-probes-and-feedback) forbids installing another copy, and a new binary is a dependency decision.
