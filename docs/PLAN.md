...
1. **Project ordinary String computation self-reference next.** The clean exact-source matrix already establishes direct-copy and `RangeAsString` target references plus a persisted accepted other-field control. Project only those measured local self-reference routes to the existing `MVK_ERROR_REFERENCE_TO_CALCULATED_FIELD` class. Leave other String authoring failures unmapped and do not widen the capsule into precondition or scheduling semantics.
2. **Run one bounded Translate/Explain consumer probe after the String capsule closes.** Exercise the shared Number and String self-reference capability through the existing Lean decision surface: translate measured refusals to the exact Kernel code, explain uncovered refusals as locally rejected but externally unmapped, and keep scale suppression distinct from successful admission. Feed any representation defect back into the semantic owner; add no harness or protocol.
