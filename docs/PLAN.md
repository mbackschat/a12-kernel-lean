...
- `state`: all eight admitted declaration pairs classify stored text and execute the direct single-level starred `FirstFilledValue` route, and source/target admission compares declared component sets so every crossing within a set works in both directions with the target's spelling deciding the output. Direct bound and singular overlap still refuse the two lexical variants.
...
- `next`: select a unit from SG6's `remains` list or another gap; no temporal branch has a pre-verified red locus waiting. Route discovery for a kernel-behaviour question starts at the maintained probe route, per LF95.
