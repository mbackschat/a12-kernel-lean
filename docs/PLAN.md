...
**Last full gate, 2026-07-30:** `lake build` 661 jobs · trust audit **1737 theorem roots; 40007 declarations in 379 modules** · `lake test` 51/51. Re-run the applicable tier commands in [`TESTING.md`](TESTING.md#tier-gates) before relying on these changing counts.
...
Compose one checked scalar Number producer with one checked String `FieldValueAsString` consumer in a heterogeneous nonrepeatable run. Preserve the Number family outcome and String family outcome separately while exposing the producer's completed stored decimal to the reached String read; a cleared or poisoned producer must remain distinguishable from an unchanged input Number. Reuse both family plans, the immutable checked document, and reached-read dependency semantics without introducing a generic graph or family-erasing result.

## Immediate sequence

1. Inventory the scalar Number and String run plans, completed overlays, dependency reads, rich result types, and existing `FieldValueAsString` conversion. Identify the smallest family-tagged activation and result boundary that preserves both owners rather than replacing them.
2. Red-first, compute a Number producer and read its completed stored decimal from a later String target. Separate changed success, clean no-value, reached producer poison, and an ordinary source Number control so a document-only shortcut cannot pass.
3. Add the minimum heterogeneous supplied-order execution over those two admitted families. Reuse the family evaluators and result projections; keep dependency order static and invalidity propagation read-driven.
4. Prove that the combined boundary preserves each family outcome and that an unreached cross-family read cannot propagate producer poison. Exercise one Execute/Analyze query and keep repeatable targets, broader family sums, whole-document application, and public protocol exposure excluded.
