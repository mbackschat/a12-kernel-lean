...
- `next`: SG6's `open` overlap item needs the operator's runtime account for a yearless operand before its admission can widen; `interpretationOfYear` waits on a dmtool authoring key. Select either another SG6 unit with a reachable oracle or a different gap.
