...
- `next`: finish the yearless-overlap consuming half named in SG6's `route-state-overlap`, which needs the occurrence scan generic over the interval type before admission widens. `interpretationOfYear` waits on a dmtool authoring key that a dated feedback note requests.
