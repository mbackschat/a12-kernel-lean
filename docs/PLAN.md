...
Close the next checked temporal-lowering prerequisite after static component admission. Audit whether the existing raw/checked cell boundary can be parameterized over the already resolved Date, Time, and DateTime value types without changing current scalar callers or creating another value tree. First lock typed present/empty/formally-unavailable observation; leave parsing, model-zone resolution, and authored declaration construction outside unless a second completed consumer establishes their shared shape.
...
- **Active — typed checked temporal observation:** determine whether parameterizing the current cell/observation owner can retain temporal values and existing placement/formal-cause invariants without a parallel checked-cell hierarchy.
