...
- `state`: all eight admitted declaration pairs classify stored text, execute the direct single-level starred `FirstFilledValue` route with component-set crossing, and gain bound extraction once Base Year completes them. Singular overlap remains narrower than the measured Kernel gate.
...
- `next`: no temporal branch has a pre-verified red locus. The refused yearless overlap shapes (starred, filtered, plural, repetition lists) each need their own probe row first; `interpretationOfYear` waits on the dmtool authoring key requested in the 2026-08-22 feedback note.
