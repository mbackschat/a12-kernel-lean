...
- `state`: the direct year-bearing `yyyy`/`yyyy-MM` bound family is internally closed through extraction, fixed-Date comparison, numeric components, and its consumer probe. Its bound compositions and direct `yyyy` overlap remain external evidence pending.
...
- `next`: admit direct nonrepeatable stored `MM` DateRange operands only when the checked model supplies Base Year, retain first/latest-day identity for leap and ordinary configured years through both bounds plus the existing full-Date comparison and component consumers, and keep unconfigured `MM` plus every `MM-dd` profile refused.
- `blocked-on`: none.
- `consumer-probe-trigger`: passed for the direct year-bearing fragment-bound family; active again if the Base-Year-completed fragment-bound family closes.
- `resume`: `rg -n 'supportsDirectBound|sourceSupportsBound|elaborateDateRangeBound|yearlessMonth|baseYear' A12Kernel/Elaboration/DateRangeBound.lean A12Kernel/Conformance/DateRangeBound.lean A12Kernel/Elaboration/DateRangeInput.lean`
