...
- `next`: the unconfigured yearless plural side, whose admission and three runtime rows are already measured, so it begins with expectations rather than route discovery. The starred, filtered, and repetition-list shapes still need their own probe rows first. `interpretationOfYear` is no longer blocked: the [instrument handback](SOURCES.md#src-dmtool-2026-08-22-instrument-handback) shipped its authoring key, so a wrapping-range capsule is now an ordinary unstarted route with a verified oracle.
