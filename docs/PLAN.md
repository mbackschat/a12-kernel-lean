...
- `state`: DateRange stored input and checked consumers support six operation-specific policies, while flat declaration validation still checks only source presence. It therefore differs from the Kernel-measured exact eight-pair declaration allowlist and wrongly treats an empty separator as intrinsically invalid.
