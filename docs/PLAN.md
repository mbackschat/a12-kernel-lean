...
- Semantic baseline: checked temporal field-to-field, field-to-typed-Date-literal, field/`Today`, field/`Now`, and field/temporal-`BaseYear` comparison over one shared exact-instant operand core, plus checked field/numeric-`BaseYear` comparison. `Today` and temporal `BaseYear` share an explicit model-zone capability; UTC and the versioned Berlin profile are concrete instances without becoming the canonical zone ceiling.
- Its focused red/green checks, full 241-job `lake build`, 51/51 retained-observation replay, and trust audit over 763 theorem roots / 12,410 declarations / 140 logical modules passed.
...
Audit numeric `BaseYear` in the checked expression and tolerance route. Determine whether the polymorphic surface source can lower through the existing numeric expression elaborator while preserving the exact scale-0 equality gate and tolerance's deliberate scale bypass; do not add a second numeric AST or comparison evaluator.
