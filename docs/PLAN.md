...
- `next`: no temporal branch has a pre-verified red locus. The remaining refused overlap shapes (starred, filtered, plural, repetition lists) each need their own probe row first, and the plural route additionally needs its authored list syntax established before any yearless plural row can be measured; `interpretationOfYear` waits on the dmtool authoring key requested in the 2026-08-22 feedback notes.
