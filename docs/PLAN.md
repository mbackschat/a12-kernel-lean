...
- `next`: `interpretationOfYear`'s `FROM`/`TO` half, selected and measured at the [interpretation checkpoint](SOURCES.md#src-date-range-year-interpretation). Lean already refuses a wrapping range with the counterpart cause, so the unit is the optional policy field plus the two-year completion; the measured per-row scalar-plus-star shape stays blocked on rule iteration context rather than on evidence. `interpretationOfYear` is no longer blocked: the [instrument handback](SOURCES.md#src-dmtool-2026-08-22-instrument-handback) shipped its authoring key, so a wrapping-range capsule is now an ordinary unstarted route with a verified oracle.
