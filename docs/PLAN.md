...
1. **Close computed Number target-scale diagnostic parity next.** Inventory the existing computation and suppression owners, then project only the measured `MVK_INVALID_COMPARE_DEC_PLACES` target-mismatch route while keeping self-reference, unsupported suppression, and every other numeric refusal distinct or unmapped. Reuse the existing scale predicate and focused admission matrices; add no new representation or evidence route.
