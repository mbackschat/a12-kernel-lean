...
- Semantic baseline: checked temporal field-to-field, field-to-typed-Date-literal, field/`Today`, field/`Now`, and field/temporal-`BaseYear` comparison over one shared exact-instant operand core, plus numeric `BaseYear` as a fixed scale-0 source in checked direct comparison, plain arithmetic, and tolerance. `Today` and temporal `BaseYear` share an explicit model-zone capability; UTC and the versioned Berlin profile are concrete instances without becoming the canonical zone ceiling.
- Its focused red/green checks, full 241-job `lake build`, 51/51 retained-observation replay, and trust audit over 764 theorem roots / 12,509 declarations / 140 logical modules passed.
...
Audit numeric `BaseYear` in checked computation operations. Determine whether the computation surface can reuse the validation atom/resolution seam while retaining target-reference rejection and result-scale admission; extract a shared source atom only at this second consumer, and do not add another expression AST or evaluator.
