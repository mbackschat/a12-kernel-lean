...
- `next`: correct `DirectDateRangeOverlapFragmentProfile.accepts` so a configured model admits both lexical spellings, which SG6 records as a one-predicate change with its measurement already in hand. Then choose among the refused yearless overlap shapes or another gap.
