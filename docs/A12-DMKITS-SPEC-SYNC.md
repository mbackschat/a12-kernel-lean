...
- **Compatibility:** This is a behavioral correction to the a12-dmkits interpreter for legal tolerance rules and implicit computation validation. The four public syntax tokens and kernel behavior version do not change. The local Lean capsule covers pure two-operand runtime truth, polarity, unknown propagation, and fixed thresholds only; checked authoring, row gating, and implicit computation lowering remain explicit later work.
