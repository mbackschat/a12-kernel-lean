...
- **Compatibility:** This corrects the downstream semantic classification without changing the already observed fresh/stale value delta. An interpreter that currently clears cleanly requires a root fix at the arithmetic-domain-failure-to-computation-target boundary.
