...
This rule governs the *form* an upstream request takes; which channel carries it is decided by [the ledger's kinds of entry](A12-DMKITS-SPEC-SYNC-LEDGER.md#kinds-of-entry). Simple, isolated upstream changes receive a paste-ready prompt. A cross-project capability or retirement with architecture and lifecycle consequences receives a temporary proposal document under this repository's `docs/` until accepted. It must name the audited a12-dmkits revision, exact source mechanism, required versioned bytes, producer/consumer responsibility, compatibility and retirement effects, separating tests where relevant, acceptance gates, and handback format.
