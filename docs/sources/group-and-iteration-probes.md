...
- `survey`: the other group-resolving routes were checked rather than assumed. `RepetitionNotUnique` refuses an empty reference group for its own reason, measured, since no key field can lie inside a group with no fields. The fixed-group operand and the scalar computation-descendant query both refuse repeatable groups already, and for a nonrepeatable group presence implies a subtree field, so neither can reach one. The index-relevance predicate is a document-side pattern rather than an operand admission.
