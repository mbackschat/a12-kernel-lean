...
#### The extrema's admitted kind set is Number **or Date**, wider than `Sum`'s, read locally 2026-09-05

- `revision`: a12-dmkits `655e80c3fb579f339cd84017f04b208d277858af`. The cited file is **unmodified** at that revision — blob `ab2b22f5747bbfcb89da377c3951f807fab694f1`, working-copy SHA-256 `2c776529e31403e3e654ec3bc4c692d138b28594453f8fadcc6a0d4766b59517` — while the sibling tree carried unrelated modifications in six other files. Recorded rather than waved past, because a dirty peer tree is the normal state here and the guarantee that matters is per file.
- `route`: real-kernel `KernelLaws#checkModel`, the same [carrier sweep](#src-group-carrier-admission-sweep)'s whole-table assertion at a later revision. Read-only source reading; nothing was run.
- `test`: [`GroupOperandCarrierAdmissionLawsTest`](../../../a12-rulekit/src/test/java/io/github/mbackschat/a12/dm/rulekit/validate/laws/GroupOperandCarrierAdmissionLawsTest.java), rows `MinValue|MaxValue/date-group/*`, `*/date-field-control`, `MinValue/date-group-plus-date-field`, `Sum/date-group`, `MinValue/mixed-kind-group`, `MinValue|MaxValue/string-group/*`, and `MinValue/string-field-list-control`. Operand kinds are the file's own documented fixture, which states that every group used *as* a group operand is homogeneous and names `Lines` (NUMBER beside STRING) as the deliberate exception.
- `claim`: **`MaxValue`/`MinValue` admit a Date operand and `Sum` does not, so the two operators' admitted sets differ and neither can be read off the other.** A Date group is accepted non-repeatable and starred-repeatable, a starred Date field path is accepted, and a Date group *beside* a Date field is accepted; the same Date group under `Sum` is refused `MVK_NO_NUMBER`.
- `claim`: the extrema's `MVK_NOT_SORTABLE` is measured for a homogeneous **String** group in both repetition shapes, for the starred String field path `MinValue(Adjusters*/Name)`, and for the mixed NUMBER/STRING expansion `MinValue(Lines*)`. The starred field path is the whole of the "explicit field list draws the same code" evidence behind [`spec/07`](../../spec/07-repetition-and-iteration.md); a written-out multi-field list is that reading extended rather than a measured row.
- `local-consequence`: **this refuted a Kernel class this project shipped, at two loci of one mechanism.** `NumberEntityElabError.aggregateDiagnostic?` projected `MVK_NOT_SORTABLE` under the extrema from any error meaning "this Number family cannot hold the operand", so two Date fields and an all-Date group each reported a refusal for a model the Kernel accepts. The error values now carry the offending kind and the projection consults it; see [`LF140`](../LEAN-FINDINGS.md). The `Sum` arm was already correct and is the control that separates the two.
- `limit`: static admission only, and the sortable set is bounded below rather than closed — Time, DateTime, DateFragment, DateRange, and Enumeration operands are unmeasured at this gate, so they project no class instead of an inferred one. No runtime comparison of a Date extremum follows.
- `sync`: none. [`spec/07`](../../spec/07-repetition-and-iteration.md) already records the extrema admitting temporal operands and already carries the unsortable refusal, so nothing in the canonical account changes; the correction is Lean-only.
