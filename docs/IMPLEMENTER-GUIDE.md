...
The flat and correlation operations are narrow development references, not a complete A12 interpreter kit. Their exact readiness and integrity gaps belong in the owning [flat](IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md) and [correlation](IMPLEMENTER-KIT-CORRELATION.md) kits. Generically, the candidate runner validates the selected suite's closed compatibility identity, finite evidence scope, case classifications, referenced evidence, deterministic process behavior, and structural JSON results; agreement establishes only that suite's observable cases. The reference-process gate runs each suite's integrity checks and Lean-reference control. The qualification checker caps stored JSON, individual logs, tree size, and tree depth, but the current process runners do not yet impose a wall-clock timeout, cap stdout or stderr while the child is running, or enforce an aggregate run budget. The pinned development replay is therefore not an untrusted-execution sandbox. High-volume generated differentials or untrusted candidates require those explicit resource limits before scale-up; batch or long-running process architecture still requires measurement and a separate protocol decision. [`PLAN.md`](PLAN.md) owns that next bounded differential step.
