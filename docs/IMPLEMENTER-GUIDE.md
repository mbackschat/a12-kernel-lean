...
A **consumer probe** is a bounded pre-product exercise against a draft task profile and the semantic material that currently exists. It asks whether a prospective evaluator, importer, refactoring tool, analyzer, synthesizer, explainer, or other consumer can reconstruct the task's decision procedure or relation, identify its exclusions and assurance limits, and expose missing semantics without repeating kernel archaeology. Artifact-only isolated readback is the default; an external prototype, dependency, protocol, runner, or qualification campaign requires separate approval. When the exercised material has a canonical [`spec/`](../spec/) clause, the isolated reader receives that clause rather than its Lean owner, because the clause is what an independent consumer reimplements from and the Lean tree is not part of the shipment. A distinction the reader cannot recover from the clause is therefore a `spec/` defect rather than a probe failure, and it returns here as a clause correction.
...
For an explicitly adopted public evaluator shipment produced from retained evidence, reuse an existing machine-readable capability descriptor and checked projection-to-protocol bridge when one fits. If the adopted task cannot be closed through an existing route, propose the smallest missing bridge and obtain approval before building it. A bounded artifact-only consumer probe does not require protocol, generated qualification, or a machine-readable descriptor before its first readback or implementation. Importer and refactoring shipments likewise receive checked source-to-target projection or generated qualification material only when their adopted task and assurance claim require it; those mechanisms are not automatic prerequisites for pre-product probes.
...
Under its recorded isolation boundary, the historical 0.2.0/V1 Rust experiment recovered an idiomatic typed evaluator from the language-neutral model, staged decision procedure, complete verdict tables, worked traces, separating fixtures, exclusions, and evidence classifications without consulting Lean source, the kernel, or a12-dmkits. This validates the basic evaluator-shipment shape for one finite historical capability while keeping isolation an attested experimental condition; it is not current 0.3.0/V2 qualification. The exact identities, results, and Git recovery points are in the [archived experiment record](archived/REFERENCE-SEMANTICS-0.2.0-AND-RUST-EXPERIMENT.md). Repetition between the model, algorithm, traces, laws, and non-laws helped the cold reader, while exact normative rules repeated verbatim increased navigation cost; retain the distinct explanatory views but generate cross-references from one canonical rule statement.

The fresh current experiment did start from a new repository and used only the current correlation kit, suite, and twelve runtime fixtures; the historical Rust source, V1 fixtures, Lean implementation, kernel, and a12-dmkits were not implementation inputs. Its dependency-free generic Rust runtime, finite laws/non-laws, and staged mutation exercise were accepted at the bounded Phase 1 claim recorded in the [`correlation kit`](IMPLEMENTER-KIT-CORRELATION.md#fresh-rust-runtime-consumer-probe). This establishes that the maintained language-neutral handover can transport the named resolved runtime without renewed A12 research. It does not qualify the current JSON operation, checked lowering, static authoring cases, general decimals, or a general interpreter.

The experiment confirms three process choices. Deliver cold readback, implementation, and seeded mutations as separate prompts so later answers cannot contaminate the natural result. Inspect the exact candidate source and tests before releasing mutations; reports alone cannot detect fixture branching or hidden lowering. For a bounded pre-product probe, ordinary Git identities, explicit source inventories, source review, and restored-state tests were sufficient—no packet format, generated qualification campaign, or new harness was justified. A later protocol/candidate-runner phase remains a separate owner and dependency decision.

Owner participation is required at three explicit gates. First, the user selects the exact current capability and whether the experiment begins immediately or after the preferred semantic milestone. Second, after this repository prepares the isolated allowlist, prompt, expected artifacts, and stop conditions, the user starts or authorizes the separate Rust repository/session; this project does not create a downstream implementation silently. Third, the user ferries the frozen natural report and any later qualification result back for assessment. Any new dependency, protocol exposure, qualification mechanism, or permanent shipment artifact remains a separate approval.

An artifact-only numeric evaluator probe on 2026-07-19 exercised the lighter, pre-shipment form of this gate. An isolated reader reconstructed the pure lowering, preflight, ordered evaluation, target, delta, application, and dependency decision procedure and its clean-empty-versus-poison separator without additional A12 research. The same reader correctly found that end-to-end support was not research-closed because checked target authoring, assignment-scale admission, downstream context injection and scheduling, portable numeric evidence, and a task interface were still absent. This is the intended useful failure: semantic dependency closure, shipment research closure, and cold-consumer qualification must be reported separately. The probe justified no new runner, protocol, qualification campaign, or framework.
...
The historical post-cold packet/checker campaign tested a stronger qualification design, but its machinery is retired and is not the default for a new probe or shipment. Its exact assurance classes, digests, outcomes, and recovery instructions live in the [archived experiment record](archived/REFERENCE-SEMANTICS-0.2.0-AND-RUST-EXPERIMENT.md). A future adopted qualification mechanism must start from its concrete consumer claim, reuse the settled current runner where sufficient, remain smaller than the semantic value it protects, and receive explicit approval; do not restore the historical packet pattern by default.
...
## Public evaluator shipment playbook
...
The current executable tools and shipped development materials are evaluator-shaped. The 0.3.0 reference executable and V2 manifest are documented in [`PROTOCOL.md`](PROTOCOL.md); runnable inputs live under [`../examples/reference-cli/`](../examples/reference-cli/); retained observations and their limits are in [`EVIDENCE.md`](EVIDENCE.md); theorem, counterexample, and trust methodology is in [`TESTING.md`](TESTING.md). The current [flat kit](IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md) and [flat V2 suite](../reference/flat-validation-empty-logic-v2.conformance.json) own the finite empty-logic handover; the current [correlation kit](IMPLEMENTER-KIT-CORRELATION.md) and [correlation V2 suite](../reference/single-group-correlation-v2.conformance.json) own the finite captured-outer handover. Historical 0.2.0/V1 claims and Rust results remain only in the [archived experiment record](archived/REFERENCE-SEMANTICS-0.2.0-AND-RUST-EXPERIMENT.md). No JSON Schema importer or rule-refactoring shipment is currently implemented; their task profiles are forward design constraints, not advertised support. The current flat interaction is:
...
`checkReferenceProcess` is the non-writing current-process, manifest, fixture, and V2 suite gate. The current [`flat kit`](IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md) owns the case-level external-versus-Lean response classification and capability boundary. Generated testing metadata never expands the retained evidence boundary. The generic candidate runner validates the selected suite against its committed support manifest but does not query the candidate's `--manifest`; pin candidate bytes and the claimed compatibility tuple in the qualification record.

The completed 0.2.0/V1 packet/checker and generated-differential workflows are historical rather than current tools. Their exact revisions, digests, finite outcomes, and recovery instructions are centralized in the [archived experiment record](archived/REFERENCE-SEMANTICS-0.2.0-AND-RUST-EXPERIMENT.md). Do not restore the machinery to the current tree for anticipated reuse or relabel its results as V2.

The flat and correlation operations are narrow development references, not a complete A12 interpreter kit. Their exact readiness and integrity gaps belong in the owning [flat](IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md) and [correlation](IMPLEMENTER-KIT-CORRELATION.md) kits. Generically, the candidate runner validates the selected suite's closed compatibility identity, finite evidence scope, case classifications, referenced evidence, deterministic process behavior, and structural JSON results; agreement establishes only that suite's observable cases. The reference-process gate runs integrity self-tests and Lean-reference controls for current V2 suites. `checkCandidateConformance` uses the shared bounded relay with a 10-second invocation deadline, 1-second cleanup deadline, 1 MiB stdin and stdout caps, and a 64 KiB stderr cap; it has no aggregate campaign budget and is cooperative same-credential process control rather than an untrusted-code sandbox. The archived 52-case Rust result remains evidence that the historical transported decision procedure generalized beyond the eight handed-over fixtures without downstream kernel research, but it transfers neither Lean proofs nor kernel correspondence and does not qualify V2.
