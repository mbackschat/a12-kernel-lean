...
The next Rust experiment starts in a fresh repository and receives only current capability material. The historical Rust source, V1 fixtures, expected outcomes, capability identities, and qualification artifacts are not implementation inputs: reusing them would preserve retired 0.2.0 assumptions and invalidate the test of current knowledge transport. The old repository remains an immutable historical control whose reports and architecture may be compared only after the new implementation's natural result is frozen. Generic lessons such as typed closed sums, cold-readback structure, mutation prediction, and qualification reporting may inform this guide, but no old semantic code is copied forward. The preferred timing is after the cross-level-correlation capsule and its second consumer-probe pass expose whether a self-contained current shipment can carry the harder addressing boundary; an immediate current V2 flat/correlation exercise remains possible only by a separate owner decision.
