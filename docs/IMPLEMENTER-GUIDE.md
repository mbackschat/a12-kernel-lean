...
The current executable tools and shipped development materials are evaluator-shaped. The 0.3.0 reference executable and v2 manifest are documented in [`PROTOCOL.md`](PROTOCOL.md); runnable inputs live under [`../examples/reference-cli/`](../examples/reference-cli/); retained observations and their limits are in [`EVIDENCE.md`](EVIDENCE.md); theorem, counterexample, and trust methodology is in [`TESTING.md`](TESTING.md). [`IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md`](IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md) is the first, now frozen 0.2.0 cold-consumer exercise, backed by its generated [`capability descriptor`](../reference/flat-validation-empty-logic-v1.capability.json), [`conformance suite`](../reference/flat-validation-empty-logic-v1.conformance.json), and [evidence-derived fixtures](../examples/reference-cli/flat-evidence/). [`IMPLEMENTER-KIT-CORRELATION.md`](IMPLEMENTER-KIT-CORRELATION.md) is the frozen higher-complexity 0.2.0 captured-outer shipment, backed by [`single-group-correlation-v1.conformance.json`](../reference/single-group-correlation-v1.conformance.json). Current candidate controls use the corresponding [flat v2](../reference/flat-validation-empty-logic-v2.conformance.json) and [correlation v2](../reference/single-group-correlation-v2.conformance.json) suites; historical claims remain pinned to v1. No JSON Schema importer or rule-refactoring shipment is currently implemented; their task profiles are forward design constraints, not advertised support. The current flat interaction is:
...
`checkReferenceProcess` is the non-writing digest gate for the frozen v1 descriptor, suite, fixtures, mutation plan, generated profile, and retained result as well as the live current-process gate. The [`flat kit`](IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md) owns the exact 0.2.0 case-level external-versus-Lean response classification, cold outcome, and qualification state. Generated testing metadata never expands the retained evidence boundary. The generic candidate runner validates the selected suite against its committed support manifest but does not query the candidate's `--manifest`; pin candidate bytes and the claimed compatibility tuple in the qualification record.

The completed packet/checker and generated-differential workflows are historical rather than current tools. The mutation packet exported at source revision `e408c9bd87ab8de576c900f2e42e0f13e868da76` and the 52-case campaign pinned to source revision `2cdc37746737d83241f91cd89fa0b56c99c2d47a` both qualified Rust candidate revision `d213005b3972c2acd8f67e87f523a923d69f6a54` within their finite boundaries. Their source remains inspectable at Git revision `03186c1`; reproducing an old command requires a clean checkout of that revision and the exact pinned candidate inputs. Do not restore the machinery to the current tree for anticipated reuse.

The flat and correlation operations are narrow development references, not a complete A12 interpreter kit. Their exact readiness and integrity gaps belong in the owning [flat](IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md) and [correlation](IMPLEMENTER-KIT-CORRELATION.md) kits. Generically, the candidate runner validates the selected suite's closed compatibility identity, finite evidence scope, case classifications, referenced evidence, deterministic process behavior, and structural JSON results; agreement establishes only that suite's observable cases. The reference-process gate runs integrity self-tests and Lean-reference controls for current v2 suites, while frozen v1 suites remain runnable on demand and digest-locked. `checkCandidateConformance` uses the shared bounded relay with a 10-second invocation deadline, 1-second cleanup deadline, 1 MiB stdin and stdout caps, and a 64 KiB stderr cap; it has no aggregate campaign budget and is cooperative same-credential process control rather than an untrusted-code sandbox. The historical 52-case Rust result remains evidence that the transported decision procedure generalized beyond the eight handed-over fixtures without downstream kernel research, but it transfers neither Lean proofs nor kernel correspondence. [`IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md`](IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md#generated-differential-outcome-2026-07-15) owns the exact outcome.
