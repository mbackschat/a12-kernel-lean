...
At the clean a12-dmkits baseline `5c1f04925c4f6d88641eb95bedd411fbbf79b416`, the unchanged source-shipped `dmtool` 0.10.0 route extends that focused placement measurement to stored Enumeration and named category conversion. A four-row `/Order/Lines` model with `StoredAmount := FieldValueAsNumber(Band)` and `CategoryAmount := FieldValueAsNumber(Band -> Numeric)` passed the real-kernel `model check` gate as `KERNEL_CONFIRMED`. The public interpreter separated stored token `1` from category token `.5`, produced `-1.25` and `12.50` from the category projection, retained `123` as a target error against maximum `99`, and cleared the row with out-of-domain stored token `BOGUS`. The route still omits repetition coordinates and was not a focused dynamic-Groovy/generated-Java run, so it calibrates selected projection and outcome classes while exact row identity remains internally locked.
