...
`DateUtil` owns strict fresh local-label construction under the model zone. [`DateAddOperation.st`](../../a12-kernel/kernel-tool/kernel-core-codegen-condition/src/main/resources/internal/templates/validation/java/operationsDir/DateAddOperation.st) and its backing bean preserve signed amount order into `RuntimeController`; `BedingungsOperatorHelper` then owns `GregorianCalendar` addition and completed-period/day differences. Fresh construction and calendar addition must not be merged: at Berlin’s repeated 1916 midnight, a forward day landing selects the earlier CEST instant while reverse landing from the other side selects the later CET instant chosen by fresh construction. Across a gap, forward addition keeps the pre-gap wall clock and reverse addition keeps the post-gap wall clock. These are direction-specific landing rules, not one selector applied to a negated amount.
