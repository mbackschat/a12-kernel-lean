...
**The CLI's own fail-fast gates can pre-empt the kernel's, and then the route measures the tool.** Confirmed 2026-08-29: `field add` refuses a declared `maxFractionalDigits` above 14 in its own spec validation, with exit 2, no envelope, and stderr *"invalid field spec: maxFractionalDigits must be ≤ 14 (the kernel's cap)"*, so the kernel's `fieldScaleCap` gate is no longer reachable through this route. The message names the kernel while the refusal is local, which is how such a row becomes a false attribution. The tell is the envelope: a kernel verdict carries `verification: KERNEL_CONFIRMED` and a `MVK_*` diagnostic, and a front-end refusal carries neither. Read it before attributing any refusal, and treat a claim whose gate the CLI duplicates as not re-dischargeable here.
