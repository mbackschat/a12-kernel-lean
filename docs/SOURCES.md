...
`DateUtil` owns strict fresh local-label construction under the model zone. [`DateAddOperation.st`](../../a12-kernel/kernel-tool/kernel-core-codegen-condition/src/main/resources/internal/templates/validation/java/operationsDir/DateAddOperation.st), its backing bean, and `RuntimeMethodNameFactory` preserve source-then-signed-amount order into `RuntimeController`; `BedingungsOperatorHelper` then owns the unit-specific `GregorianCalendar` mutation and completed-period/day differences. Fresh construction and calendar addition must not be merged, and calendar units must not share an inferred overlap selector. At Berlin’s repeated 1916 midnight, forward `DAY_OF_MONTH` addition selects the earlier CEST instant while reverse day addition selects the later CET instant chosen by fresh construction; across a gap, forward day addition keeps the pre-gap wall clock and reverse day addition keeps the post-gap wall clock. By contrast, `addiereMonate` delegates to `Calendar.add(MONTH, amount)` after its end-of-month correction, and both signed month routes into the repeated midnight select the later CET/fresh instant. Audit each field mutation independently rather than applying the day rule to a negated amount or another unit.
...
| `381211833647b8495dde44de19a5952ac87743e5` | Boolean/Confirm stored-token and decimal-ingestion review, with source revisions `a1e39e18`, `43f8e5f4`, and `4b144f89` |
...
| `a1e39e18e4a74276da73b5a665d30521dcd99563`, `43f8e5f460a2fddd8fbcfa58bdcb3f3f0f87e203`, `4b144f89be51a14f8facdff5ee81b590e5294fb5` | exact Boolean/Confirm stored-token, Number minimum-scale, and decimal transport child revisions consolidated by `381211833647b8495dde44de19a5952ac87743e5` |
