...
Source inspection identifies a distinct candidate route for field-owned type-format and requiredness messages. [`FieldErrortextChecker`](../../a12-kernel/kernel-tool/kernel-core-service/src/main/java/com/mgmtp/a12/kernel/core/service/internal/errortext/FieldErrortextChecker.java) runs the common encoding, line/control, and even-dollar checks before comparing delimited parameters with the locale-fixed words supplied by [`CommonUtils.CONST_FIELD`](../../a12-kernel/kernel-tool/kernel-core-base-utils/src/main/java/com/mgmtp/a12/kernel/core/utils/internal/CommonUtils.java): source candidates `field`/`field.value` for US English and `Feld`/`Feld.Wert` for German. The Java runtime helper [`FehlerHandler.transformFehlerText`](../../a12-kernel/kernel-rt/kernel-core-runtime/src/main/java/com/mgmtp/a12/kernel/core/rt/_30_8/internal/formatdef/error/FehlerHandler.java) performs fixed name replacement and display-value replacement directly, with a null display falling back to the raw supplied value, and contains no visible call to the rule path's empty-parameter decoder. This source-derived composition establishes neither public `$$` output nor authorability: [`EXP-2026-07-30-01`](A12-DMKITS-SPEC-SYNC-LEDGER.md#exp-2026-07-30-01--field-owned-error-text-dollar-decoding-and-fixed-token-staging) requests the exact static and dual-kernel observation before Lean chooses a shared or distinct template representation.
