...
`DateUtil` owns strict fresh local-label construction under the model zone. `BedingungsOperatorHelper` owns `GregorianCalendar` addition and completed-period/day differences. Fresh construction and calendar addition must not be merged: at Berlin’s repeated 1916 midnight, fresh construction and a forward day landing select different exact instants. Reverse addition must be audited independently rather than inferred by negating the forward rule.
