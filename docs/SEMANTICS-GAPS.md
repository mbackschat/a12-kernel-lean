...
- `remaining`: [Boolean and Confirm constant computation target admission](SOURCES.md#src-boolean-confirm-constant-computation-targets) remains unimplemented: Boolean accepts `True` and `False`, while Confirm accepts only `True` and refuses `False` with `MVK_INVALID_COMPARE_TO_YES`.
