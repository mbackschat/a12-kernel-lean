...
- Open: the direct Boolean/Confirm value-count source, tally, scalar numeric computation, and generated mismatch validation are checked; repeatable Boolean/Confirm entity lists remain open and must reuse the checked scalar projection without re-reading text or acquiring model token metadata from ignored `@NotInD` declarations. Number formal-read checking remains unimplemented and must preserve the stored value regime so decimal-valued input receives the strip-and-minimum-scale projection while String-valued input remains verbatim, without replacing exact decimal source identity. Remaining group/addressed expression placements must reuse their existing family owners.
