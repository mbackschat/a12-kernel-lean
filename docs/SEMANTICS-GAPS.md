...
- `remaining`: **three untested retentions are inventoried and one probe request covers them.** A *starred field* operand under `MinValue`, `MaxValue`, `NumberOfDifferentValues`, and the value count answers `unknown .overRepetition` in [`FilledFieldStarCount.lean`](../A12Kernel/Conformance/FilledFieldStarCount.lean); a fixed group `Sum` does the same on the **computation** arm in [`NumberEntityGroupComputation.lean`](../A12Kernel/Conformance/NumberEntityGroupComputation.lean); and the extrema and `NumberOfDifferentValues` over a *starred* group have no row either.
- `remaining`: none of those three carries a checkpoint, and all are the [`LF124`](LEAN-FINDINGS.md#lf124--preserving-an-untested-lock-to-stay-evidence-exact-turns-it-into-the-claim-you-are-defending) shape — a stated reason pinned by a case written from the implementation, on a carrier adjacent to one now measured. The settled mechanism predicts every one projects. The request is authored and waits on a clean producer rather than on a decision; partial validation and nesting below the star stay outside it.
