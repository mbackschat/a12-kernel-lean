...
- `remaining`: the count's **unavailability rule was corrected against the Kernel** at that checkpoint and is outbound as [`SPEC-2026-08-31-01`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-31-01). A formal error blocks only the negative answer, so the shared clause now reads each operand through the decided-presence projection its own group-list predicates use. Still open on that dimension: a structural over-limit error on a group with admitted content, a call-local partial-coverage error, and `noneRelevant` coverage — the coverage leg follows the shared mechanism rather than a row, because `validateFull` cannot observe partial coverage.
...
- `remaining`: **ENUM was already implemented** and the audit found it so — [`cap-enumeration-result-and-application`](implementation/strings-and-enumerations.md#cap-enumeration-result-and-application) owns it, membership gate included. The rows added only its refusal's Kernel identity. **DATE_FRAGMENT** is the one kind left to write, and its blocker is a **renderer rather than a rule**: `FullDateTargetFormat` admits only the two complete formats and always emits three components. The [component-omitting checkpoint](sources/cross-layer-routes.md#src-component-omitting-date-formats) supplies the complete table — `yyyy` stores `2024`, `yyyy-MM` stores `2024-03`, `yyyyMM` stores `202403`, identically on DATE and DATE_FRAGMENT — so no kernel round is needed.
- `remaining`: extending that renderer is a **shared-type change reaching five families** (`DateRangeInput`, `DateTimeInput`, `DateRangeConstructionComparison`, `PartialDateInput`, `TemporalTargetPolicy`), so it is its own sequenced unit rather than folded into a carrier. It also closes both landed carriers' component-omitting exclusion, which is what makes the widening justified rather than anticipatory: two consumers are blocked on the same missing mechanism.
- `remaining`: the **yearless** formats `MM` and `MM-dd` need the model's Base Year in scope as well — without one the Kernel refuses a date constant outright, with one it admits. This project already carries `FlatModel.baseYear`. What that target *stores* under a Base Year is unmeasured, and `dmtool` has no route to declare one, so measuring it needs the instrument gap closed first.
- `remaining`: **DATETIME and DATERANGE have no known literal witness** at the revision the [family-gate checkpoint](sources/cross-layer-routes.md#src-constant-literal-family-gate) records, and stay unrepresentable for want of one rather than by decision. The range literal drew its own distinct `MVK_INVALID_COMPARE_TO_DATE_RANGE`, so something classified there. [`SPEC-2026-08-31-03`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-31-03) asks the peer for either witness.
