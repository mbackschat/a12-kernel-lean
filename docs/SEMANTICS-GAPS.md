...
- `remaining`: repeatable target families beyond bounded addressed ordinary String execution and the exact-address Boolean, Custom, DateFragment, Enumeration, sibling-star DateRange, DateTime, FULL Date, and Time `FirstFilledValue`, `DateFromDateTime`, and `TimeFromDateTime` result/application carriers. Exact-address ordinary String result/application and unsupported target families remain open.
- `remaining`: addressed DateFragment Kernel calibration and copy-versus-render discrimination; wider-profile and cross-spelling calibration for addressed DateRange; wider format, zone, and source-shape calibration for addressed DateTime; runtime calibration for addressed Time; runtime, dotted-format, and source-policy calibration for addressed FULL Date.
...
- `remaining`: sibling-parallel `DateFromDateTime`, materialized topology, later validation, valueless-source reason propagation, and maintained result/application correspondence.
