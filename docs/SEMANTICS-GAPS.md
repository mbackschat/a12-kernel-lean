...
- Unresolved source fact: compose one field-backed exact DateTime `AddDays` result into one checked `AddHours`/`AddMinutes`/`AddSeconds`, preserving calendar-then-elapsed operation order, exact instant, milliseconds, omission, and cause order. The peer's adjacent day probes still do not calibrate the source-offset/refit mechanism, which remains pending under [`SPEC-2026-07-29-01`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-07-29-01). Dynamic-`Now` reverse composition, wider formats, legal zones/pre-floor identities, detailed basic-check errors, repeatable placement, broader recursive lowering, and date-like distinct-count identity remain open.
