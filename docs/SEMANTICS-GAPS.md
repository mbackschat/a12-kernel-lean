...
- `remaining`: widen the checked multi-operation direct-field union beyond the successful three-stage Enumeration, Number-to-DateTime, four-stage `CurrentRepetition` filtered-Enumeration, and checked scalar numeric whole-call results plus the checked scalar numeric operation, table, run, and admitted generated-table plans to additional whole-call families, group containment beyond the fixed checked numeric operation and already resolved filter fields, generated index and preliminary findings, and rendered operand messages.
