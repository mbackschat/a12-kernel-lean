...
- `remaining`: the over-limit exclusion is measured on the **bare-constant** carrier and applied to the first-filled, conversion, construction, shift, enumeration, and indexed-parallel carriers as a stated assumption, since all of them ask only which rows their target has and no divergence between them is known. A calibration row on any second carrier would discharge it; until then the crossing is an assumption, not an inheritance.
