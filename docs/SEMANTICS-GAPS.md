...
- `remaining`: what is **not** implemented is the refusal that should still fire. An unstarred per-row operand of a repeatable declaring group must refuse a target outside it, and no checked elaborator derives iteration from an operand. Until it does, the estate is uniformly permissive where the Kernel is conditional, which is the opposite error from the one just fixed and is the next thing to close.
