...
- Unresolved source fact: connect the checked DateTime day result to the existing declaration-owned DateTime target/result/application path without widening the `Now`-only computation carrier into a speculative temporal AST. The peer's adjacent day probes still do not calibrate the source-offset/refit mechanism, which remains pending under [`SPEC-2026-07-29-01`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-07-29-01). Wider formats, legal zones/pre-floor identities, detailed basic-check errors, repeatable placement, broader recursive lowering, target/destination integration beyond this next direct route, and date-like distinct-count identity remain open.
