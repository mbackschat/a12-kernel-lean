...
- `remaining`: the **count's over-limit row domain is designed but unrun**, and it is reachable after all — an earlier reading closed it as unreachable on the ground that no document can put every row over capacity, but `NumberOfFilledFields` over a subtree whose in-capacity rows are instantiated and empty separates the accounts on the one filled cell. The rule is `FieldFilled(W) AND NumberOfFilledFields(Shell) > 0` over the `nover-inner` and `nover-inner-ctrl` documents already retained at that checkpoint. It was statically confirmed and never run because the producer worktree went dirty mid-round; this project's clause counts such a cell, so the run is a falsification attempt, not a formality.
