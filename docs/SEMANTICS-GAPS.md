...
- `remaining`: the count's over-limit row domain is **measured and corrected**. A fixed group operand's extent excludes an over-limit row: with the in-capacity rows instantiated and empty and the only filled cell beyond capacity, kernel 30.8.1 answers exactly `0` and fires `NoFieldFilled` ([checkpoint](sources/group-and-iteration-probes.md#src-group-operand-over-limit-extent)). The discriminator an earlier reading closed as unreachable was reachable — it needs a count of **cells**, not of rows, and a negative rule beside the positive one so `unknown` is separable from zero.
- `remaining`: which **further** group consumers project is now **measured and answered**, reconciled as [`EXP-2026-08-31-02`](A12-DMKITS-SPEC-SYNC-LEDGER.md#exp-2026-08-31-02). All six carriers project over a **fixed** group operand at the [inbound extent checkpoint](sources/inbound-group-operand-batches.md#src-group-operand-capacity-consumer-sweep) — both value counts, `Sum`, both extrema, and `NumberOfDifferentValues` — and a malformed cell in capacity makes `Sum` non-evaluable where the identical cell over limit does not, which settles the exclusion as a property of the **extent** rather than of any operator. `AtLeastOneFieldFilled` and `NoFieldFilled` are measured and agree with the count but have no checked group carrier here to correct.
- `remaining`: the four consumers still on the **complete formal-cell view** over a group operand — `MinValue`, `MaxValue`, `NumberOfDifferentValues`, and the value counts — are the correction that measurement leaves open here. Each now has its own row and should opt into `inCapacityAddressedCells` exactly as the count does. The **starred** carrier's malformed-in-capacity versus over-limit split is unmeasured and does not transfer across that boundary.
