...
- `remaining`: the group-count extent is **closed for repeatable descendants too**, on the route that carries row topology. This entry had deferred it to a repeatable group-runtime batch and warned against a widening of the fixed operand; the stated reason was that the operand lacked the repeatable mechanism the starred form had, and that reason expired when the addressed route's group-count arm acquired it — both arms now sit in one function over one document, so the widening is the batch. The scalar route still refuses, correctly. What stays open on the shape: a repeatable descendant more than one repetition level below the operand, which the gate refuses rather than composing, and whether an all-over-limit row set counts.
