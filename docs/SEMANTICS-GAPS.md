...
- `remaining`: **the count's over-limit row domain is measured and this project is wrong on it.** A fixed group operand's extent excludes an over-limit row: with the in-capacity rows instantiated and empty and the only filled cell beyond capacity, kernel 30.8.1 answers exactly `0` and fires `NoFieldFilled`, where this project answers `unknown` from a cell it retains with cause `overRepetition` ([checkpoint](sources/group-and-iteration-probes.md#src-group-operand-over-limit-extent)). The discriminator an earlier reading closed as unreachable was reachable — it needs a count of **cells**, not of rows.
- `blocked-on`: **the locus of that fix, an owner decision informed by one more observation.** Filtering at the shared group extent makes the measured family right and contradicts four established cases on the value-count, token-group, and Boolean-group carriers; filtering at the fill quantifiers alone leaves neighbouring group operands disagreeing about one cell in one document. Recommended account and the three observations that settle it are in [`EXP-2026-08-31-02`](A12-DMKITS-SPEC-SYNC-LEDGER.md#exp-2026-08-31-02); no Lean was changed.
