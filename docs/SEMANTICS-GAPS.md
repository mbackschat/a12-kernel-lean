...
- `remaining`: the repeatable **constant** target is implemented for Boolean and Confirm over the shared repeatable-target certificate. Result and application correspondence for that carrier is external evidence pending.
- `remaining`: the same shape for **String and Number** is measured on the Kernel but only partly reachable here, and the earlier claim that neither kind had any constant route was wrong — it was read off a filename rather than off representability. Both families carry a `literal` atom, and [`AddressedNumberExtremum`](../A12Kernel/Elaboration/AddressedNumberExtremum.lean) already admits a constant-only child into a repeatable target with that target's certificate owning iteration, so the *value* is reachable as `Max(-1)`. What no addressed family accepts is the **bare** authored constant, which the Kernel does admit and compute per row. The gap is therefore an authored surface, not a semantic mechanism; closing it makes String and Number the second and third consumers of the carrier Boolean now proves.
- `remaining`: one shape does differ between those carriers and is measured rather than assumed. A String target **validates** what is written, so unlike Boolean it can refuse a constant. The Kernel admits the over-long constant statically, then per row reports the exact attempted payload with `errored` set and no clear — the shape `StringTargetOutcome.errored` already carries, so the carrier reuses the existing target check rather than inventing a rejection branch ([checkpoint](sources/cross-layer-routes.md#src-repeatable-string-constant-target-check)).
