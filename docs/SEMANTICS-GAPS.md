...
- `evidence-needed`: one compute-arm row for an **over-limit** repeatable row under a starred count. The implementation excludes it, inheriting the validation carrier's measured rule through `inCapacityRowCount`, and that inheritance is an assumption rather than an observation — the two arms diverge on a formally invalid operand, so agreement here cannot be assumed from their agreeing elsewhere. a12-dmkits' existing fixture is rule-arm and it flagged the gap itself.
- `remaining`: also unmeasured on this carrier and therefore refused or unrepresented — a starred group owning its own repeatable descendant, two starred operands in one list, a nested or filtered star, a starred path navigating by parent count, and a duplicate rule for a mixed pair.
