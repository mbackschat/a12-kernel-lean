...
- `remaining`: still unmeasured on this carrier and therefore refused or unrepresented — a starred group owning its own repeatable descendant, a nested or filtered star, a starred path navigating by parent count, and a duplicate rule for a mixed fixed/starred pair.
