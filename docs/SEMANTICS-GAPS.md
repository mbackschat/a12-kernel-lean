...
- Unresolved source fact: both document-only and explicit-world shifted-`Now` Time execution are internally closed at the bounded nonrepeatable operation. The peer now calibrates the source-offset/refit day mechanism through the separating 369-day composite at `5716a964`; sign-independent month/year landings, millisecond and February-clear behavior, and constructed-Date exact-instant retention remain pending under [`SPEC-2026-07-29-01`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-07-29-01). Wider formats, legal zones/pre-floor identities, detailed basic-check errors, repeatable placement, broader recursive lowering, and date-like distinct-count identity remain open.
