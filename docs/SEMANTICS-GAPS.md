...
- `remaining`: bind generated numeric-table target completion to the target declaration's own certified policy before result projection or application; the current phase carrier retains the selected operation's target-check mode but deliberately accepts an explicit certified policy.
- `remaining`: widen the checked multi-operation direct-field union beyond the successful three-stage Enumeration, Number-to-DateTime, four-stage `CurrentRepetition` filtered-Enumeration, and checked scalar numeric whole-call results plus the checked scalar numeric operation, table, run, and admitted generated-table plans to findings retained with failed execution results, additional whole-call families, group containment beyond the fixed checked numeric operation and already resolved filter fields, generated index and preliminary findings, and rendered operand messages.
