...
- `remaining`: open on that dimension: filtered Number and Enumeration/category value-count stars, filters beyond the exact structural self-comparison, a second repetition level, nesting below the star, the Boolean and temporal uniqueness carriers, and partial validation, which the [sweep checkpoint](sources/group-list-and-capacity-probes.md#src-capacity-consumer-sweep) records as unmeasured rather than settled.
