...
- Unresolved source fact: widen beyond the deliberately insufficient ASCII/bare-field fragment only through the actual quoted/bilingual path grammar and diagnostics; audit field-owned templates, locale/display invocation, field format errors, category/index/`BaseYear` lookup, custom output, repeated pointers, `NumericTargetError` formal codes, and rendering orchestration.
- Upstream reconciliation needed: read-only inspection of a12-dmkits at `351bf9f28138783ef7ddc7d8c76db2ab4891a5d8` finds that its common renderer recognizes only nonempty name/value regex matches and therefore leaves `$$` doubled. Kernel source at `cb66e51fa7ab90b650698f861bf670754e2e1e66` and the already-canonical §13 account establish that an empty delimited parameter decodes to one literal `$`. This is an implementation/test gap, not a new `spec/` delta or an `EXP-` request; the required upstream discriminator is `renderMessage("Cost $$ [$Amount.value$]", ...) = "Cost $ [<opaque display>]")`, with token-looking display bytes retained unchanged and JVM/Node coverage through the existing common renderer tests.
