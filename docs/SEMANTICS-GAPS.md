...
- `remaining`: the component-dropping **renderer** is the family's whole open implementation item. The Kernel's rule is now fully specified: the target's declared format names the components kept, and a declared Base Year lifts the yearless admission refusal without contributing any component to the store ([Base Year store checkpoint](sources/cross-layer-routes.md#src-base-year-yearless-store)). `FlatModel.baseYear` already exists here, so no new model input is needed.
