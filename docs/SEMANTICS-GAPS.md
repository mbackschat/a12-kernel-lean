...
- `baseline`: the checked [flat mandatory-information carrier](implementation/cross-clause.md#cap-flat-mandatory-information) retains measured authored rule identities, derives all three result sets independently, and closes finite field/root dependency chains. The [direct-cycle](sources/rule-set-meta-information.md#src-mandatory-information-field-cycles) and [field-list-cycle](sources/rule-set-meta-information.md#src-mandatory-information-field-list-cycles) checkpoints add exact seeded, under-seeded, fully seeded, and unseeded two-node outcomes across direct, existential, and universal identities; arbitrary finite admitted field-guard cycles use that monotone closure internally. Empty multi-field forms, multiple-root rule sets, and cross-root guards remain rejected. The carrier also covers the retained flat presence and composition matrices plus one whole-rule filtered rejection; no `spec/` clause represents the wider service.
- `baseline`: the same carrier now implements the [threshold checkpoint](sources/rule-set-meta-information.md#src-mandatory-information-narrowing), its [reversed-strict successor](sources/rule-set-meta-information.md#src-mandatory-information-reversed-strict-count), the [paired false-comparison matrix](sources/rule-set-meta-information.md#src-mandatory-information-false-count-guards), and the [zero/one/two-seed separator](sources/rule-set-meta-information.md#src-mandatory-information-count-seed-cardinality): standalone count-root contribution, all four comparison spellings, signed-32-bit threshold narrowing, authored-literal identity beside the `-1` sentinel, accepted false guards with no target contribution, and accepted isolated operands independently of direct-seed cardinality. Dependency-derived or reused count operands still fail closed.
...
- `baseline`: the [distinct-count checkpoint](sources/rule-set-meta-information.md#src-mandatory-information-distinct-count) adds a second field-count operator identity: its exact standalone `< 1` rule shares the filled-count root contribution, but its exact direct-seed `>= 2` guard contributes no target where the filled-count guard closes. Other distinct-count bounds and comparisons remain uncalibrated.
...
- `baseline`: the [semantic-index checkpoint](sources/rule-set-meta-information.md#src-mandatory-information-semantic-index) retains one exact whole-rule exclusion: an ordinary negative-field leaf does not leak from a disjunction that also contains a literal-key semantic-indexed leaf, while an independent direct control still contributes. Concrete indices and wider semantic-index shapes remain fail-closed.
- `remaining`: inventory and measure the still-uncovered recognized rule families, wider count collector and dependency shapes, longer and multi-list-edge cycle correspondence, repetition, concrete indices and wider semantic-index behavior, filter internals, declaration interaction beyond unconditional/direct field/root seeds, generated index and computation-validation rules, and wider closure shapes before widening the checked carrier.
