...
- Current boundary: `ConditionTree` owns connective shape, empty-row eligibility, reference traversal support, static all-leaf admission, and exact verdict-aware short-circuit evaluation. `FlatCondition` is that tree over flat leaves, while `ValidationCondition` admits flat and resolved numeric-comparison leaves in one tree with shared evaluation, field-reference traversal, and per-leaf relevance. `CheckedValidationCondition` lifts only checked flat/numeric inputs, preserves the exact common row group, rejects mismatches, and carries model/core coherence. `ResolvedRule` and `CheckedResolvedRule` now parameterize the existing metadata/error-field certificate and sole post-verdict message emitter, so flat and mixed whole rules share the same outcome and rendering path. Laws prove flat/mixed verdict recovery, exact fired-message attachment, flat embedding, and checked coherence; cases separate mixed firing, short-circuit, relevance, references, checked group mismatch, and numeric-only error-field discovery. Generated validation still accepts only a nonempty literal fragment, and resolved aggregates are not expression nodes. See [§5](IMPLEMENTATION-MAP.md#5--numbers-and-decimals), [§11](IMPLEMENTATION-MAP.md#11--computations), and [§12](IMPLEMENTATION-MAP.md#12--validation-and-polarity).
- Prerequisite: audit whether `CheckedNumericComputationOperation` already entails the same-group and static-scale facts required to construct each generated `CheckedNumericComparison`; computation currently admits a different operand context than checked validation. Preserve target self-reference through the shared traversal, and do not introduce a generated-only comparison wrapper or re-elaborate resolved operations through guessed surface syntax. After that seam closes, add aggregate expression nodes rather than bypassing the tree.
