...
- `remaining`: widen the checked multi-operation direct-field union beyond the successful three-stage Enumeration, Number-to-DateTime, four-stage `CurrentRepetition` filtered-Enumeration, checked scalar numeric whole-call results, admitted generated scalar numeric whole-call results, and addressed DateTime sub-day whole-call results plus the checked scalar numeric operation, table, run, and admitted generated-table plans to additional whole-call families, group containment beyond the fixed checked numeric operation and already resolved filter fields, generated-preliminary integration into one concrete whole-call execution family, and rendered operand messages.
