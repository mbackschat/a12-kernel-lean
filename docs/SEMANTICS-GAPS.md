...
- `remaining`: the group-count extent is closed for nonrepeatable descendants and **deliberately excluded** for repeatable ones ([checkpoint](SOURCES.md#src-repeatable-descendant-group-count)). That shape counts structurally — an instantiated row with no filled cell counts — so it needs the document's row topology rather than the scalar cell-list projection, and lifting it means giving the group-count operand the repeatable mechanism the starred form already has. Select it with a repeatable group-runtime batch, not as a widening of the fixed operand.
...
- `remaining`: the type's **direction** is inherited from the rule arm, not measured on this carrier. Every retained row has its computed value below its seed, so they do not separate the directional reading from a coarser one; the discriminator is one document with a grow-only target whose computed value exceeds its seed. Offered to a12-dmkits under [`SPEC-2026-08-30-06`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-30-06) and reachable through the runtime probe route.
