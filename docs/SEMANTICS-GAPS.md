...
- `remaining`: two group-count shapes stay refused after the subtree extent closed ([checkpoint](SOURCES.md#src-nested-descendant-group-count-runtime)). A **repeatable descendant** below a fixed operand is Kernel-admitted statically and refused here, because a scalar context has no instantiated row and the count is unmeasured on both sides. A shell whose only descendant content is **formally invalid** is counted by the widened clause through the established direct-child rule, by uniformity rather than by observation; a12-dmkits has offered that runtime row, so request it with the next batch instead of opening an `EXP-` entry.
