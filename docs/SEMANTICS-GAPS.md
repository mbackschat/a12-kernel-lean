...
- `remaining`: **repetition depth is measured and the refusal is lifted.** A descendant two repetition levels below the operand counts exactly as one level does, at the [deep checkpoint](sources/group-and-iteration-probes.md#src-deep-repeatable-descendant-group-count), so `repeatableDescendantShape?` no longer gates on the axis count and the clause is stated for any depth — the mechanism is subtree containment, which the validation arm already used unbounded. What that leaves open is the **half-instantiated** shape, an outer row with no inner row beneath it: every measured document carries both levels or neither, and this project counts such a shell as present as its own reading. Outbound as [`SPEC-2026-08-30-08`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-30-08).
