...
- `remaining`: the group operand's declared-capacity extent is **landed and reconciled** — [`EXP-2026-08-31-02`](A12-DMKITS-SPEC-SYNC-LEDGER.md#exp-2026-08-31-02) and [`SPEC-2026-08-31-10`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-31-10) are terminal and [`cross-clause.md`](implementation/cross-clause.md#cap-number-group-aggregates) owns the coverage. Open on that dimension: `FieldValuesNotUnique`, which reaches the extent through a different resolver, has no row of its own, and is now a specified ready observation as [`EXP-2026-08-31-03`](A12-DMKITS-SPEC-SYNC-LEDGER.md#exp-2026-08-31-03) — all three carriers read the complete cell view today and nothing locks that answer, so the branch is unexercised rather than wrong; the three fill quantifiers that read declared-but-uninstantiated slots, which the instantiated-only group core cannot answer at all; and the two threshold quantifiers, the same mechanism at an unmeasured threshold.
