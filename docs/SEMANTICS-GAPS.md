...
- `remaining`: widen the four remaining equality-based placement gates to containment, matching the shared repeatable-target certificate and the Enumeration copy. They are [`AddressedNumericLeaf.lean`](../A12Kernel/Elaboration/AddressedNumericLeaf.lean), [`AddressedFieldValueAsString.lean`](../A12Kernel/Elaboration/AddressedFieldValueAsString.lean), [`AddressedDateFromDateTime.lean`](../A12Kernel/Elaboration/AddressedDateFromDateTime.lean), and [`AddressedTimeFromDateTime.lean`](../A12Kernel/Elaboration/AddressedTimeFromDateTime.lean).
- `remaining`: each of those refuses an ancestor declaration the Kernel admits. `AddressedNumberFirstFilledComputation` inherits the numeric-leaf gate although it is the same sibling-star operation as the already widened Boolean, String, and Custom families. The Time conformance module annotates its own narrower row so the divergence is not silent. The measurement is closed at the [declaring-group gate checkpoint](SOURCES.md#src-computation-declaring-group-gate), so this is implementation plus its cold review, not new evidence.
- `remaining`: the Lean side of the containment gate is unimplemented. `spec/` now states containment under derived iteration, reconciled with a12-dmkits as [`SPEC-2026-08-29-01`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-29-01), but no checked elaborator derives iteration from a computed field's own repeatable scope or enforces at-or-below containment against the declaring group.
