...
- `remaining`: unmeasured on that carrier and therefore unimplemented: a starred member whose group owns a further repeatable descendant, an unretained repeatability, and an unavailable member's interaction with the sum. Two starred members and the duplicate gate are now measured and implemented; the gate is **containment**, not distinctness, since two stars naming the same group are admitted and double-count.
