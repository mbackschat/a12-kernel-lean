...
- `remaining`: starred computation runtime is measured at the [starred group-count checkpoint](SOURCES.md#src-starred-group-count-computation) and its **meaning is implemented**: `GroupCountOperandReading` carries the two operand forms, whose unlike contributions — an indicator and a cardinality — add over one result domain, constrained by the fixed-only specialization law. What stays open is the elaboration and evaluation half — the checked operand list is homogeneous and fixed-only, and the scalar computation context has no row-topology channel, so both admitted shapes are still refused fail-closed.
