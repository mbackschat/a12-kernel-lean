...
- Current boundary: [§6](IMPLEMENTATION-MAP.md#6--dates-and-time) owns the implemented temporal surface. The immediate open calendar seam is reverse Berlin `AddDays`; Berlin month/year shifts and constructed-Date differences remain excluded.
...
- Unresolved source fact: establish reverse Berlin day landing, month/year correction, and constructed-Date difference offset policy independently. Wider formats, legal zones/pre-floor identities, detailed basic-check errors, recursive Date expressions, repeatable placement, target/destination integration, and date-like distinct-count identity remain open. Pending peer reconciliation for constructor, precision, and amount-source clauses is tracked in the [outbound ledger](A12-DMKITS-SPEC-SYNC-LEDGER.md).
