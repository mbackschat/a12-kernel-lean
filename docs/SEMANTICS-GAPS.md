...
- Unresolved source fact: both document-only and explicit-world shifted-`Now` Time execution are internally closed at the bounded nonrepeatable operation. The peer calibrates the source-offset/refit day mechanism through the separating 369-day composite at `5716a964` and sign-independent year landing through the dual-kernel 7670 versus pre-fix interpreter 7671 long spring-gap separator at `17f95bcc`. Revision `38b5046` closes combined kernel authorability for the quoted repeated-midnight comparison shapes and rejects their bare numeric spelling with `MVK_INVALID_ENTITY`. The local fresh-comparison matrix supplies direct semantic discriminators for constructed-Date exact-instant retention plus month and year overlap landing, but peer runtime implementation and measurement remain open; millisecond and February-clear behavior remain independent open arms under [`SPEC-2026-07-29-01`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-07-29-01). Wider formats, legal zones/pre-floor identities, detailed basic-check errors, repeatable placement, broader recursive lowering, and date-like distinct-count identity remain open.
