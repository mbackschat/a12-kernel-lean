...
- `remaining`: three smaller residues. The DateRange direct list keeps a source same-group narrowing that is this project's bounded subset rather than a Kernel gate, reachable with an absolute operand and refused with no diagnostic code; its certificate retains a `declaringGroup` it does not certify — all five `resolveFieldEntityOperandIn` operand kinds do guard `GroupPath.isValid`, so the lemma is true, but two of those guards sit behind `private` definitions in `SingleGroup.lean` and `StarGroup.lean`, so stating it means adding exported lemmas to three modules for a fact one conformance cell already exercises; and addressed Custom and addressed ordinary String separate placement only against a wholly unrelated group. Close each opportunistically when its owner is next touched.
...
- `remaining`: **owner decision.** An empty declared group is kernel-valid ([checkpoint](SOURCES.md#src-empty-declared-group)) and `FlatModel` cannot express one, because it represents a group only through its fields or its own repeatable declaration. Sixteen gates ask `hasGroupPath` and inherit that limit; none is wrong on the representable domain, and the message group position's root gate is the one that would diverge if the input could be built. Lifting it means giving groups their own declaration list, which changes a core type and every one of those gates, so it needs an approval rather than a capsule. Blocked on: owner decision.
...
- `remaining`: the `$#…$` group position carries its measured **static admission**, both resolution gates, and the quoting rule ([§13](IMPLEMENTATION-MAP.md#13--message-interpolation)). What stays open in it: what an admitted group parameter renders, which needs a runtime observation rather than a static one; the Kernel's three distinct lexical refusal codes, which this fragment reports as its one parse class because it tracks no character position; and whether a quoted segment may carry characters a bare one may not.
