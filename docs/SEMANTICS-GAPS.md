...
- Unresolved source fact: the exact computed `Time` target policy, `HH:mm:ss` rendering, zone-free clock identity, no-value reasons, and formal poison are source-closed; source-relative result classification and exact application are the immediate continuation. The peer's adjacent day probes still do not calibrate the source-offset/refit mechanism, which remains pending under [`SPEC-2026-07-29-01`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-07-29-01). Wider formats, legal zones/pre-floor identities, detailed basic-check errors, repeatable placement, broader recursive lowering, and date-like distinct-count identity remain open.
