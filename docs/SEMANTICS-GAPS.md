...
- `remaining`: the polarity of the implicit self-validation message on a starred count is an open discriminator. Every row of the starred capture reports `OMISSION_ERROR`, including rows whose computed value is three or four against a stored seed, where the [fixed-operand capture](SOURCES.md#src-repeatable-descendant-group-count) measured the ordinary omission/value split. The two captures differ in the operand list alone; no mechanism is claimed and none is implemented.
