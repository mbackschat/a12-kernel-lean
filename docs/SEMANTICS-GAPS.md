...
- `remaining`: starred computation runtime is measured at the [starred group-count checkpoint](SOURCES.md#src-starred-group-count-computation) and its **result domain is implemented**: `GroupCountOperandReading` carries the two unlike operand forms and their contributions add, constrained by the fixed-only specialization law. What stays open is the elaboration and evaluation half — the checked operand list is homogeneous and fixed-only, and the scalar computation context has no row-topology channel, so both admitted shapes are still refused fail-closed.
- `remaining`: that channel is the one real cost, and it is the same wall the fixed operand hit for a different reason. `ScalarComputationContext` is deliberately `FieldId → CheckedCell`; supplying an in-capacity row count per starred operand needs either a resolved count fixed at elaboration or an optional topology probe following the existing `world`/`worldRequired` precedent in that structure. Choose between them with a second consumer in hand, not for this operand alone.
...
- `remaining`: the polarity of the implicit self-validation message on a computation carrying a **starred operand** is an open discriminator. Every row of the starred capture reports `OMISSION_ERROR`, including rows whose computed value is three or four against a stored seed, where the [fixed-operand capture](SOURCES.md#src-repeatable-descendant-group-count) measured the ordinary omission/value split. The two captures differ in the operand list alone; no mechanism is claimed and none is implemented.
- `discriminator`: settle it with a **single-run witness** rather than a second whole capture, because a cross-capture comparison confounds the operand list with the model. One model carrying three targets computed at the same group — `NumberOfFilledGroups(Flat, Other)` with no wildcard anywhere, `NumberOfFilledGroups(Rows*)`, and `NumberOfFilledGroups(Flat, Rows*)` — over one document where every concrete referenced cell is filled and all three targets hold a mismatching seed. A wildcard-free `VALUE_ERROR` beside two starred `OMISSION_ERROR`s isolates the starred reference from every model- and document-level confound. Pair it with a row emptying the fixed operands, so the wildcard-free target's own polarity is shown to track its cells rather than being constant, and with a starred `Sum` target to separate the operand form from the group-count operator.
- `blocked-on`: a clean `../a12-rulekit/` worktree. The witness above was authored and run, but the probe stamps `producer.source.state` and a concurrent peer edit to a tracked test source made the tree `DIRTY` mid-session, which makes that artifact exploration-only by the [probe route's contract](TESTING.md#the-kernel-runtime-probe-route). The observation is therefore **not recorded here as a fact**: an answer whose bytes cannot be pinned is not evidence, and re-running is cheaper than a citation that resolves to nothing.
