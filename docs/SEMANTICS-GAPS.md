...
- `baseline`: the [declared-required checkpoint](sources/rule-set-meta-information.md#src-mandatory-information-declared-required) adds two isolated declaration-derived modes without asserting an internal generation mechanism: unconditional requiredness contributes globally and root-relatively plus its root, while parent-present requiredness contributes only root-relatively. Composition with authored or sibling declaration rules remains fail-closed.
- `remaining`: inventory and measure the still-uncovered recognized rule families, wider count collector and dependency shapes, longer and multi-list-edge cycle correspondence, repetition and index behavior, filter internals, coexisting or composed declaration-derived requirements, generated index and computation-validation rules, and wider closure shapes before widening the checked carrier.
