...
- Unresolved source fact: compose one field-backed exact sub-day shift into one checked DateTime `AddDays` without reconstructing its wall label, retaining elapsed-then-calendar operation order, milliseconds, omission, and inner-before-outer cause order. The peer's adjacent day probes still do not calibrate the source-offset/refit mechanism, which remains pending under [`SPEC-2026-07-29-01`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-07-29-01). Dynamic-`Now` cross-mechanism composition, wider formats, legal zones/pre-floor identities, detailed basic-check errors, repeatable placement, broader recursive lowering, and date-like distinct-count identity remain open.
