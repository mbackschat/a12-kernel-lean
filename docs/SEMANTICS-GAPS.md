...
- `remaining`: **two starred operands in one list, and a repeated starred group**, are internally closed and admitted — cardinalities add per authored occurrence, so a duplicate counts twice and nothing deduplicates it. External evidence is **pending a clean-tree window**: rows were observed but the sibling was clean before the run and dirty after, with no check in between, so none is cited. Re-run the three admission and three runtime rows together when a window opens.
