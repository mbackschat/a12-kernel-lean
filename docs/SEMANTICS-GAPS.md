...
- `baseline`: no Lean owner or `spec/` clause represents the full derivation. The [threshold checkpoint](sources/rule-set-meta-information.md#src-mandatory-information-narrowing) establishes the presence control, standalone count-root contribution, `EvaluatorVisitor` count-guard conjunction, signed-32-bit threshold narrowing, and authored-`-1` sentinel collision. The [presence decision matrix](sources/rule-set-meta-information.md#src-mandatory-information-presence-matrix) separates standalone field-set, root-only, and no-contribution results across negative and positive field/group forms, `And`/`Or`, direct presence quantifiers, ERROR/WARNING, and one repeatable `Having` pair. The [composition matrix](sources/rule-set-meta-information.md#src-mandatory-information-composition-matrix) separates global from root-relative field sets and retains forward, reverse, two-hop, and unmet-guard dependency outcomes.
- `remaining`: inventory and measure the still-uncovered recognized rule families, repetition and index behavior, and closure shapes beyond the flat acyclic conjunction matrix before choosing the full carrier. The measured slice now requires distinct global and root-relative field sets plus finite dependency closure, but does not justify extrapolating to cycles or unmeasured rule shapes.
...
- `consumer`: Analyze and Explain need the authored rule shape, derived-set identity, and the distinction between no numeric bound and a bound narrowing to the `-1` sentinel. The resolved experiment establishes the last distinction but not the collector that consumes it.
...
- `completion-gate`: the checked representation retains the authored literal separately from its host-converted threshold, the evaluator reproduces each caller's measured sentinel behavior, and the full collector matches all three derived sets without consumer-side reconstruction.
- `blocked-on`: none
- `route-state`: discovery-required
