...
- `remaining`: the over-limit exclusion is measured on **three** carriers — a bare constant, a sibling-star `FirstFilledValue`, and a `DateFromDateTime` conversion — across one, two, and three repetition levels, and it is **level-independent**: an excess row at the outer, middle, or inner axis excludes its own subtree alike, so the decision is the whole environment rather than the target level's coordinate. The indexed-parallel carrier is measured on its own and no longer assumed ([`SPEC-2026-08-31-08`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-31-08)), and it was measured with a *different* mechanism than the assumption predicted, so construction, shift, and enumeration keep theirs unlifted. Two levels overflowing at once behave as either alone.
...
- `remaining`: whether an emptied in-capacity domain leaves the *computed* extrema unavailable, as full validation's do, or answers a definite `0`. The computation probe read `0` but cannot settle it: no document in that run makes a computation known-unavailable, so a real zero and an unavailability rendered as zero are not separated. The discriminator is one computed field over an in-capacity **malformed** cell, whose validation answer is already settled as unavailable — a `cleared` or `errored` outcome there makes the zeros real. Cheap on the accepted route; not yet run.
