...
- `remaining`: determine cross-group fixed-target admission and execution for operation families beyond the measured Number constant and the current Boolean/Confirm constant specialization. The checked fixed String literal now retains declaration-group identity independently through table assembly, but its cross-group Kernel admission and execution remain unmeasured. Do not extrapolate the target-scope mechanism to list, construction, repeatable, or generated-validation routes.
- `external-gap`: that cross-group calibration has no available route. `dmtool computation add` derives placement from the computed field's parent and exposes no group override, so the shape is unauthorable through the structured verbs the kernel-probe contract routes authoring through, and `computation read` reports no declaring group either. The [authoring-block checkpoint](SOURCES.md#src-cross-group-computation-authoring-block) owns the exact measurement and its claim limit. The String, Boolean, and Confirm clauses stay `external evidence pending` until that capability lands.
