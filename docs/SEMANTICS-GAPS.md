...
- `remaining`: the component-dropping renderer covers **all seven** formats through the one shared table [`cap-omitting-date-input`](IMPLEMENTATION-MAP.md#cap-omitting-date-input) owns, with the yearless four gated on a declared Base Year and storing no year regardless ([Base Year store checkpoint](sources/cross-layer-routes.md#src-base-year-yearless-store)). What stays open is one external row: what a constant stores into the compact `MMdd` and `ddMM`. Their order is currently assumed from the reading direction and held there by an exhaustive round-trip case; the dashed siblings cannot settle it, because the compact pair differs from each other in exactly that order.
