...
- Current boundary: one exact one-level finite Number star, one checked correlation shape, checked model-owned general group-star path lowering, general `Document`-to-nested-tree/ordered-`Env` resolution, unfiltered and resolved-validation-`Having` nested Number cell streams with contextual over-repetition classification, caller-supplied RNU rows, and resolved group presence are closed. The resolved validation filter keeps candidate order, separates complete candidate and captured environments, and reads only retained targets; computation-filter poison/dependency behavior remains open. See [§9](IMPLEMENTATION-MAP.md#9--repetition-and-iteration), [reopened-star completeness and addressing](IMPLEMENTATION-MAP.md#reopened-star-structural-completeness-and-addressing), and [resolved Number aggregates](IMPLEMENTATION-MAP.md#resolved-number-aggregates).
- Prerequisite: next source-audit and attach wildcardable relevance, then mixed declarations and general authored filter-condition lowering after SG3 supplies the shared checked tree. Reuse `SingleGroup`, `ReopenedStarDomain`, `ResolvedStarTopology`, the general environment selector, `ValueListCell`, and the nested Number route; do not introduce a second row tree, cell parser, condition evaluator, or aggregate scan.
...
- Consumer and evidence: Execute, Transform, and Explain consumers require stable row identity and encounter order. IF193/IF194 and one-level aggregate differentials provide triangulation; project-local general repeatable evidence remains pending.
...
- Current boundary: `CheckedResolvedFlatRule` consumes `FlatCondition`; numeric validation/computation use a separate checked expression tree; generated validation accepts only a nonempty literal fragment; resolved aggregates are not expression nodes. See [§5](IMPLEMENTATION-MAP.md#5--numbers-and-decimals), [§11](IMPLEMENTATION-MAP.md#11--computations), and [§12](IMPLEMENTATION-MAP.md#12--validation-and-polarity).
- Prerequisite: inventory both completed consumers' source ASTs, evaluation result domains, reference traversal, relevance, scale admission, message boundary, and common-condition use. Preserve the at-least-one-field gate and reject target self-reference through the shared traversal.
