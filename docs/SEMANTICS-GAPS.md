...
- `baseline`: no Lean owner or `spec/` clause represents the full derivation. The [threshold checkpoint](sources/rule-set-meta-information.md#src-mandatory-information-narrowing) establishes the presence control, standalone count-root contribution, `EvaluatorVisitor` count-guard conjunction, signed-32-bit threshold narrowing, and authored-`-1` sentinel collision. The [presence decision matrix](sources/rule-set-meta-information.md#src-mandatory-information-presence-matrix) now separates standalone field-set, root-only, and no-contribution results across negative and positive field/group forms, `And`/`Or`, direct presence quantifiers, ERROR/WARNING, and one repeatable `Having` pair.
- `remaining`: inventory and measure the still-uncovered recognized rule families, then separate multi-rule collection and closure for all three returned sets before choosing a carrier. Equality of `mandatory` and `mandatoryForRootGroup` in the standalone matrix does not justify a general equivalence or a partial collector.
- `evidence-needed`: one coherent batch over the remaining recognized rule families and a bounded multi-rule composition matrix, including any additional repetition or filter branch whose answer changes the returned sets. Each retained case must identify which returned set changes.
