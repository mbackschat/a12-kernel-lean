...
- Current boundary: [§6](IMPLEMENTATION-MAP.md#6--dates-and-time) owns the implemented temporal surface. Signed Berlin constructed-Date `AddDays`/`AddMonths`/`AddYears` are closed after the checked floor with operation-specific exact landings; constructed-Date differences remain excluded.
...
- Unresolved source fact: establish the constructed-Date difference offset policy independently from the now-closed year-shift mutation. Wider formats, legal zones/pre-floor identities, detailed basic-check errors, recursive Date expressions, repeatable placement, target/destination integration, and date-like distinct-count identity remain open. Pending peer reconciliation for constructor, precision, amount-source, and operation-specific landing clauses is tracked in the [outbound ledger](A12-DMKITS-SPEC-SYNC-LEDGER.md).
