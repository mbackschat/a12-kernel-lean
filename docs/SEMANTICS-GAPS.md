...
- `remaining`: materialized document topology, later-rule execution beyond the exact one- and two-level ordinary direct-Number comparisons and one-level direct-String-length comparison over normalized applied topology, transport reconstruction, and pointer-dialect rendering.
