...
- `remaining`: **the computation's own over-repetition error channel is unmodeled, and its difference from validation's is already visible in retained bytes.** `CheckedDocument.overRepetitionFindings` owns the full-validation set and now says so; a computation reports through `formalErrorsInOperands`, scoped to what its operands reach, and the multiplicity capture's artifact holds ten validation findings against zero operand errors on one document and eight against four on another. A row-level `zuGrosseZeile` does reach that channel, so it is not field-scoped. What is unmeasured locally is its exact scope rule, because no retained document varies which operands a computation reads; the discriminator is an over-limit row in a group no computation names.
