...
- `remaining`: **the computation's own over-repetition error channel is unmodeled.** `CheckedDocument.overRepetitionFindings` owns the full-validation set and now says so; a computation reports through a narrower operand-scoped channel, and this project's retained bytes already show the two disagreeing — ten validation findings against zero operand errors on one document. The scope rule is measured upstream, so the obligation is to model it rather than to observe it: operand-scoped, reaching the operand's whole ancestry ([inbound checkpoint](sources/inbound-group-operand-batches.md#src-starred-field-operand-peer-reproduction)). Take the site assignment from those rows and the address spelling from the kernel-channel record, since the peer's pointers are its own projection.
...
- `remaining`: the group operand's declared-capacity extent is **landed and reconciled** — [`EXP-2026-08-31-02`](A12-DMKITS-SPEC-SYNC-LEDGER.md#exp-2026-08-31-02) and [`SPEC-2026-08-31-10`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-31-10) are terminal and [`cross-clause.md`](implementation/cross-clause.md#cap-number-group-aggregates) owns the coverage. Open on that dimension: `FieldValuesNotUnique`, which reaches the extent through a different resolver and has no row of its own; the three fill quantifiers that read declared-but-uninstantiated slots, which the instantiated-only group core cannot answer at all; and the two threshold quantifiers, the same mechanism at an unmeasured threshold.
