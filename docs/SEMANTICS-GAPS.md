...
- Current boundary: one exact one-level finite unfiltered Number star, one checked correlation shape, caller-supplied reopened trees, caller-supplied RNU rows, and resolved group presence are closed; see [§9](IMPLEMENTATION-MAP.md#9--repetition-and-iteration), [reopened-star completeness](IMPLEMENTATION-MAP.md#reopened-star-structural-completeness), and [resolved Number aggregates](IMPLEMENTATION-MAP.md#resolved-number-aggregates).
- Prerequisite: source-audit the model/document owner of group-instance enumeration, nested parent capture, `Having` discovery and filter order, wildcardable relevance, mixed declarations, and over-limit rows. Reuse `SingleGroup`, `ReopenedStarDomain`, existing correlation environments, and `ValueListCell`; do not introduce a second row tree or aggregate scan.
