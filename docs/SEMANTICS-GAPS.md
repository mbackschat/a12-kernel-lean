...
- Current boundary: one exact one-level finite Number star, one checked whole-rule correlation shape, checked model-owned general group-star path lowering, general `Document`-to-nested-tree/ordered-`Env` resolution, checked authored and resolved-validation-`Having` nested Number cell streams with contextual over-repetition classification, both all-rows wildcard relevance and order-aware per-cell partial `FirstFilledValue` relevance for that source, caller-supplied RNU rows, and resolved group presence are closed. The authored filter fragment reuses the common condition tree and checked star plan, admits Number/`CurrentRepetition` comparisons and conjunction, requires an ordinary reference to reach a reopened level, checks every ordinary reference against the complete candidate environment and every `$` reference against the captured rule environment, and reads only retained targets. All-rows and per-cell gates share path/index patterns but preserve their distinct operator decisions; wider filter leaves and authored `Or`, computation-filter poison/dependency, mixed declarations, and multi-operand per-slot encounter remain open. See [§9](IMPLEMENTATION-MAP.md#9--repetition-and-iteration), [reopened-star completeness and addressing](IMPLEMENTATION-MAP.md#reopened-star-structural-completeness-and-addressing), and [resolved Number `FirstFilledValue`](IMPLEMENTATION-MAP.md#resolved-number-firstfilledvalue).
