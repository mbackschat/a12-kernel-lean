...
- `remaining`: concrete reachability and Kernel scheduling correspondence for structural-failure arms that currently have only conditional internal laws. The one route found so far was a defect and is closed; the [over-limit target checkpoint](sources/group-and-iteration-probes.md#src-over-limit-computation-target) owns why every repeatable-target carrier now shares one excluding row domain.
...
- `remaining`: **the clearing half is measured and implemented on every carrier, the indexed-parallel one included.** A target cell holding a stored value in an over-limit row is `cleared`, so the row leaves the value-producing domain and stays in the clearing one. Every addressed and constant repeatable-target carrier emits its own no-value outcome at that row through `CheckedDocument.computationRowOutcomes`, and the parallel route now enumerates every instantiated target and takes the two dispositions rather than dropping the excess row. What is still open on that carrier is the applied document state after the clear, which the compute observation does not report.
...
- `remaining`: the over-repetition finding set's **structure** is implemented, absorption included ([capability](IMPLEMENTATION-MAP.md#cap-over-repetition-findings)). What stays open here is the rendered **text** of `zuGrosseZeile` and `zuGrosseKontextnummer`, and one unseparated discriminator behind the node inventory: whether a `zuGrosseKontextnummer` counts a cell the document **places** or a field the model **declares**. Every retained document writes every declared key of the rows it instantiates, so no artifact separates them; a document omitting one key under an over-limit row would.
- `remaining`: the address **spelling** question is reframed rather than open as stated: retained artifacts carry **two dialects in one channel**, an instance one that elides index one and a declaration one that leads with a slash, elides nothing, and marks each iterated level `[0]` ([dialect checkpoint](sources/temporal-and-message-probes.md#src-message-address-dialects)). So the overload matrix and the runtime probe need not disagree at all. A **third** surface is settled too: the rendered text of an over-repetition finding carries a bracket-free declaration path and states the offending index and the cap as separate numbers, so a consumer needing the offending row reads `field` rather than parsing the text. What stays open is which operand shapes select which of the two address dialects — a correlation over 22 captures, not a measured cause — and a `MessagePointer` carrying `fillToFix` must represent two address kinds rather than one shape with an optional index.
...
- `remaining`: the group operand's declared-capacity extent is **landed and reconciled** — [`EXP-2026-08-31-02`](A12-DMKITS-SPEC-SYNC-LEDGER.md#exp-2026-08-31-02) and [`SPEC-2026-08-31-10`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-31-10) are terminal and [`cross-clause.md`](implementation/cross-clause.md#cap-number-group-aggregates) owns the coverage. Open on that dimension: the three fill quantifiers that read declared-but-uninstantiated slots, which the instantiated-only group core cannot answer at all, and the two threshold quantifiers, the same mechanism at an unmeasured threshold.
- `remaining`: the consumers of the resolved operand stream still on the **complete** cell view are enumerated by call site and specified as one ready observation in [`EXP-2026-08-31-03`](A12-DMKITS-SPEC-SYNC-LEDGER.md#exp-2026-08-31-03) — uniqueness on both carriers, the token `NumberOfDifferentValues` and token `FirstFilledValue` for every operand form, and the token value count's starred-field branch. Nothing locks any of those answers, so each branch is unexercised rather than wrong.
...
- `remaining`: the **computation arm's static gate table is measured and matched**. Every class and the containment-before-rootness order reproduce there exactly as in rules, so both arms now call one containment predicate and one root gate; the computation arm had gated overlap over its fixed operands only and carried no root gate at all. Both dimensions that round left open are measured at the [repeatable-scope checkpoint](sources/group-and-iteration-probes.md#src-group-count-gates-repeatable-scope): the whole table survives a repeatable declaring group on both arms with `RuleGroup` an ordinary member of the overlap relation, and a **filtered group operand is refused at the filter site**, `MVK_NO_GROUPS_ALLOWED`, against admitted field-filter and unfiltered-group controls in the same run.
- `remaining`: the filtered group operand is **closed for the family** by the reviewed handback at [`SPEC-2026-08-31-09`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-31-09): six carriers refuse, each against its own unfiltered control, and the post-list spelling is a parse error rather than a second refused site, so no spelling attaches a filter to a group operand. Still unmeasured on that shape: a filter naming a **different** group's field than the operand it sits on.
...
- `remaining`: open for the family: the pre-1900 target, a seeded target cell, and a constant under a precondition. Component-omitting rendering is closed — all seven formats, every store measured, one table shared with the stored-input classifier that reads such a cell back.
...
- `remaining`: **DATETIME now has a literal witness and is a writable carrier.** `"05.03.2024T12:30:00"` is admitted into a DATETIME target and stores, re-derived locally rather than taken on report ([checkpoint](sources/cross-layer-routes.md#src-temporal-constant-literal-composition)); the earlier "no witness" reading tested three uncomposed spellings and never tried the concatenation of two shapes the same round had already admitted.
