...
- `remaining`: the fixed operand's movement over a repeatable descendant is **measured and the clause is corrected**. The extent is the subtree's declared *slot capacity*, not its declared field count, at the [nested-capacity checkpoint](sources/group-and-iteration-probes.md#src-filled-field-count-nested-capacity); the two quantities coincide on every flat subtree, which is why the wrong one shipped. Both shapes that checkpoint left untested are now measured at the [deep-capacity checkpoint](sources/group-and-iteration-probes.md#src-filled-field-count-deep-capacity) and both confirm the clause: capacity **compounds** through a second nesting level, and it stops at the operand group rather than multiplying by a repeatable ancestor above it. The list question is answered separately and differently — a group count keeps its operand-list length.
- `remaining`: this project is **narrower than the Kernel** on the ancestor shape and says so. `resolveFixedGroupReference` refuses any fixed group operand whose repeatable scope is nonempty, `repeatableGroupRequiresAddress`, while the Kernel admits it and binds the operand's ancestry from the rule's own iteration, emitting one message per enclosing row with a row-local extent. That gate is shared by the group-list, numeric-computation, numeric-validation, and validation-condition carriers and this round measured one of them, so widening it would claim four carriers on one observation. Closing it properly means measuring the same shape on the other three first.
