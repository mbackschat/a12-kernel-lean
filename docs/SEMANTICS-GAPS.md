...
- Evidence status: maintained a12-dmkits differentials Kernel-lock isolated repeatable/index and value-less-target result-channel seams, typed Number V2 source identity at `7b2e9f7`, and cause-blind computed-target dependency erasure at `351bf9f2`. Multi-step composition, finite repeatable plans, heterogeneous results, destination compatibility, and whole-document transition remain partially or wholly `external evidence pending`.
...
- Current boundary: [§13](IMPLEMENTATION-MAP.md#13--message-interpolation) and [§12](IMPLEMENTATION-MAP.md#12--validation-and-polarity) own the implemented rendering/attachment surface. The smallest checked flat rule-template fragment now validates nonempty printable-ASCII text, `$$`, `$Field$`, and `$Field.value$`, resolves one bare nonrepeatable field in the condition group, requires condition membership, and lowers into the existing post-fire plan without retaining token syntax. SG4 owns computation pointer partition.
- Unresolved source fact: widen beyond the deliberately insufficient ASCII/bare-field fragment only through the actual quoted/bilingual path grammar and diagnostics; audit locale/display invocation, category/index/`BaseYear` lookup, custom output, repeated pointers, `NumericTargetError` formal codes, and rendering orchestration. The field-owned template audit reached a representation blocker: source establishes a distinct fixed-token transformer but not whether its public output decodes `$$`; [`EXP-2026-07-30-01`](A12-DMKITS-SPEC-SYNC-LEDGER.md#exp-2026-07-30-01--field-owned-error-text-dollar-decoding-and-fixed-token-staging) owns the static spelling, dollar-decoding, insertion-opacity, and type-format/requiredness producer matrix.
