...
- `remaining`: bare constants for the remaining kinds — enumeration, temporal, date-range, and Custom — are unmeasured on this route and unrepresentable here. Each needs its own target-check timing measured before a carrier, because the Number one already showed that reading String's timing across the kind boundary would have been wrong.
...
- `remaining`: the **mixed** validation group-count list is authorable and unimplemented. `NumberOfFilledGroups(G1, Rows*)` is admitted by the Kernel, the bare unstarred spelling is refused `MVK_NO_WILDCARD`, and a starred member counts per instantiated filled row, so the extent is the sum of each member's own capacity rather than the operand-list length ([checkpoint](sources/group-and-iteration-probes.md#src-group-count-list-extent)). This arm refuses the shape with `starredGroupCountOperand`; treat that as unimplemented, not as a semantic claim. The all-fixed list's own list-length extent is measured and correct, including a group filled only through a descendant row, so a future capsule must add the starred member without moving the fixed one onto slot capacity.
- `remaining`: that mixed list's **semantics are fully measured**, so the capsule needs no further kernel round. The count is the sum over members — a fixed member contributes its filled indicator, a starred member its in-capacity *instantiated* row count, which counts an instantiated but wholly empty row and is the quantity the sole-star carrier already computes. The extent is the sum of each member's own capacity: one per fixed member, the declared repeatability per starred one, with an unretained repeatability yielding no operand as elsewhere. Both were separated by their own documents rather than inferred; filling every instantiated row would have confounded the row count with the filled count.
- `remaining`: the **shape** of that capsule was worked out and deliberately not started, because it exceeds the Tier 1 touch budget: about nineteen total-match sites across the validation-side elaboration and proof files need a new arm. Its home is `OrderedNumericValidationAtom`, which an ordinary rule reaches through `CheckedValidationCondition.fromOrderedNumeric`, mirroring the computation arm's own wrapper rather than widening the shared scalar atom. It must resolve on the **addressed** route, because a starred member needs a document and outer environment that the flat resolver's context does not carry. The computation arm's `CheckedGroupCountOperand` is the same certificate and should move from `NumericComputation/Core.lean` down to `StarGroup.lean` — no import cycle, `StarGroup` needs one added `SingleGroup` import — but only together with this consumer, since until then the extraction would have a single user.
