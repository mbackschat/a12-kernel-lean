...
- `baseline`: the same carrier now implements the [threshold checkpoint](sources/rule-set-meta-information.md#src-mandatory-information-narrowing): standalone count-root contribution, the three measured direct-seed true-comparison count guards, signed-32-bit threshold narrowing, and authored-literal identity beside the `-1` sentinel. Unsupported count dependency and false-comparison shapes fail closed.
