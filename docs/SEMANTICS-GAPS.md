...
- Open: Number formal-read checking remains unimplemented and must preserve the stored value regime so decimal-valued input receives the strip-and-minimum-scale projection while String-valued input remains verbatim, without replacing exact decimal source identity. Remaining group/addressed expression placements must reuse their existing family owners.
