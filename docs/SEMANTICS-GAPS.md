...
- `baseline`: the same carrier now implements the [threshold checkpoint](sources/rule-set-meta-information.md#src-mandatory-information-narrowing), its [reversed-strict successor](sources/rule-set-meta-information.md#src-mandatory-information-reversed-strict-count), and the [paired false-comparison matrix](sources/rule-set-meta-information.md#src-mandatory-information-false-count-guards): standalone count-root contribution, all four direct-seed comparison spellings, signed-32-bit threshold narrowing, authored-literal identity beside the `-1` sentinel, and accepted false guards with no target contribution. Unsupported count dependency shapes still fail closed.
