...
- `remaining`: addressed Custom and addressed ordinary String separate placement only against a wholly unrelated group. Close it opportunistically when its owner is next touched. The `declaringGroup` residue beside it is closed: `elaborateFieldEntityShape` now certifies the group, so every family building a certificate over that shape carries the fact instead of re-testing it. Its recorded cost was wrong in both halves — the `SingleGroup.lean` guard is public, and the `StarPath.lean` lemma already existed — so only the starred-group arm needed a new local statement.
