...
- `remaining`: the repeatable **constant** target is now implemented for Boolean and Confirm over the shared repeatable-target certificate. The Kernel admits the same shape for String and Number constants too, and this project has no constant-computation family for either kind at all, so those remain unrepresented rather than refused. Result and application correspondence for the repeatable carrier is external evidence pending.
