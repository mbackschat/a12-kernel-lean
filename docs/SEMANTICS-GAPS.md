...
- `remaining`: one group-count shape stays refused after the subtree extent closed ([checkpoint](SOURCES.md#src-nested-descendant-group-count-runtime)). A **repeatable descendant** below a fixed operand is Kernel-admitted statically and refused here, because a scalar context has no instantiated row and the count is unmeasured on both sides. Closing it needs the count itself, not another admission row.
