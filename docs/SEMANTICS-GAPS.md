...
- `remaining`: this project's constant-computation family refuses a **repeatable** target as a local placement decision, reported with no Kernel code. The Kernel admits and computes that shape for String, Number, Boolean, and Confirm alike, so the refusal now contradicts a measured behavior instead of standing on an unknown. Lifting it is a capsule of its own and must enumerate every dispatch arm and admission site first, per [`LF117`](LEAN-FINDINGS.md#lf117--a-semantic-widening-reaches-only-the-dispatch-arm-its-motivating-form-selects); the existing repeatable-target certificate is the carrier to reuse rather than a new gate.
