...
- `remaining`: cross-group fixed-target admission and execution is closed for all four constant families at the [declaring-group gate checkpoint](SOURCES.md#src-computation-declaring-group-gate), which also fixes the containment gate and measures that a repeatable declaring group adds no repetition. What stays open is a repeatable *target* declared cross-group at runtime, non-constant cross-group operations, generated-validation firing, and result/application correspondence. Do not extrapolate the target-scope mechanism to list, construction, repeatable, or generated-validation routes.
- `remaining`: placement is **closed** in both directions and [`computations.md`](implementation/computations.md#cap-first-selected-tables) owns the implemented boundary. A repeatable target must lie at or below the declaring group; a fixed target carries no restriction, since neither a constant, a star aggregate, nor a nonrepeatable direct read derives iteration. Both halves now hold across a **root** boundary as well, so containment is path containment and nothing root-relative. Every family requires a representable declaring group. The [fixed-target star placement checkpoint](SOURCES.md#src-fixed-target-star-placement) owns the measurement including its cross-root rows.
...
- `remaining`: addressed Custom and addressed ordinary String placement is **closed**; this line was stale rather than open. Both owners already hold the sharp three-way separator, read off the cases rather than their docstrings — a wholly unrelated declaring group refused `targetOutsideDeclaringGroup`, the ancestor **admitted**, and a group sharing the target's enclosing prefix without containing it refused. That last row is what an account keyed on a common ancestor would wrongly admit.
- `remaining`: the owners are [Custom](../A12Kernel/Conformance/AddressedCustomFirstFilledComputation.lean) and [ordinary String](../A12Kernel/Conformance/AddressedStringFirstFilledComputation.lean). The `declaringGroup` residue beside it is closed: `elaborateFieldEntityShape` now certifies the group, so every family building a certificate over that shape carries the fact instead of re-testing it. Its recorded cost was wrong in both halves — the `SingleGroup.lean` guard is public, and the `StarPath.lean` lemma already existed — so only the starred-group arm needed a new local statement.
...
- `remaining`: the group-count extent is **closed for repeatable descendants too**, on the route that carries row topology. This entry had deferred it to a repeatable group-runtime batch and warned against a widening of the fixed operand; the stated reason was that the operand lacked the repeatable mechanism the starred form had, and that reason expired when the addressed route's group-count arm acquired it — both arms now sit in one function over one document, so the widening is the batch. The scalar route still refuses, correctly. What stays open on the shape: a repeatable descendant more than one repetition level below the operand, which the gate refuses rather than composing, and whether an all-over-limit row set counts — the clause counts any instantiated row, following the recorded structural rule rather than the starred count's in-capacity domain, which is inherited and not measured on this carrier.
- `remaining`: the starred group-count operand is measured at the [starred group-count checkpoint](SOURCES.md#src-starred-group-count-computation), its **meaning is implemented** in `GroupCountOperandReading`'s fold, and its **static admission is implemented and locked** for the single-starred and mixed forms with the conditional arity gate. What stays open is one projection: the addressed evaluator's in-capacity row count.
- `remaining`: that projection has **landed**, so the operand is closed end to end for the measured shapes: static admission, the fold's meaning, and addressed evaluation over a real document.
- `remaining`: the compute arm's **over-limit** exclusion is measured and no longer inherited, so the operand carries no standing assumption. Five, six, and seven rows in a `max 5` group all count five on both strategies.
- `remaining`: the **fixed** operand's row constituent has no such measurement and the discriminator is now exact rather than vague. Its clause counts any instantiated row of a repeatable descendant, where the starred count excludes over-limit rows, and the separating document is a `max 5` group holding **only** row `7`: the fixed operand reads it as content while an in-capacity account reads it as none. That document is admitted on an all-fixed list and rejected `nonprefixRows` wherever a starred operand forces path resolution, so the two accounts are separable only on the starless route. Capacity `0` would separate them at row `1` instead, but a starred operand refuses it `invalidRepeatability`. Whether either document is authorable in the Kernel is unmeasured; the [repeatable-descendant checkpoint](sources/group-and-iteration-probes.md#src-repeatable-descendant-group-count) carries the dated negative.
- `remaining`: **two starred operands in one list, and a repeated starred group**, are closed at runtime and locked at the fold — cardinalities add per authored position, so a duplicate counts twice and nothing deduplicates it, measured on a12-dmkits' kernel oracle at the [two-star rows](SOURCES.md#src-starred-group-count-computation). Static admission of those spellings is measured too, through the Kernel's own `checkConsistency` on the same estate. Both halves rest on one estate rather than two, since this project's own rows could not be certified to a clean window.
- `remaining`: this line previously read *unmeasured on this carrier and **therefore** refused or unrepresented*, and that inference was false for three of the four shapes it named. All four were re-checked executably on the addressed route with an empty outer environment; what follows replaces the inference with the readings.
- `remaining`: **implemented and answering, external evidence pending** — a starred group owning its own repeatable descendant counts its own rows and is undisturbed by the descendant; a fixed operand naming an **ancestor** of a starred operand's group counts beside it with no overlap gate and no deduplication, the ancestor's presence coming from the very rows the star counts; and a duplicate starred group adds once per authored position in a list that also carries a fixed operand. Each needs a calibration row, not a representation decision.
...
- `remaining`: the **atom shape** no longer selects the operand's meaning, which it did between the widening landing and this repair. An authored list reaches `filledGroupCountMixed` only when some operand carries a star, so the repeatable-descendant widening reached a fixed operand in `NumberOfFilledGroups(Shell, Preferences, Rows*)` and not the identical operand in `NumberOfFilledGroups(Shell, Preferences)`, which stayed on the scalar reader and was refused. `readCheckedNumericComputationAtom_filledGroupCount_eq_mixed` now holds the two shapes equal on an all-fixed list, and `numericComputationEvaluationContext_filledGroupCount_ignoresIteration` restates the guard that had encoded the old delegation: the count reads the document deliberately and still ignores `outer`, `filterRead`, and `starRead`. ([`LF117`](LEAN-FINDINGS.md#lf117--a-semantic-widening-reaches-only-the-dispatch-arm-its-motivating-form-selects))
- `remaining`: the pre-read structural fault pass kept a **second, narrower copy** of the same admission test, so the operand stayed refused end to end after the reader already answered it. Both now decide through `computationOperandAdmitted` and `readGroupCountOperand_fixed_error_iff_notAdmitted` holds them equal. `supportsScalarEvaluation` is the third site and deliberately keeps the narrow query, because the scalar route reads cells and never rows; a case locks that disagreement so the next reader does not complete the sweep.
- `remaining`: the mixed atom had **no arm in that pass at all**, so a left poison's short circuit swallowed an unsupported operand's structural fault entirely where the plain atom reported it. Both arms now share `fixedGroupCountOperandFault?`, held equal by `numericComputationFault_filledGroupCount_eq_mixed`. A starred operand stays outside the gate deliberately: its refusal is a `StarAddressingError` over the document's row topology, which no model-structural pass can answer.
- `route`: **that channel is not the cost, and the reason recorded here for deferring it was wrong** — an instance of [`LF111`](LEAN-FINDINGS.md#lf111--an-obligations-stated-reason-is-an-unverified-claim-too-and-a-wrong-one-parks-work-indefinitely). It said the shape needs a row-topology channel `ScalarComputationContext` lacks, and asked for a second consumer before choosing that channel's shape. Both halves fall to reading the declarations.
- `route`: the topology is already in hand one level up. `NumericComputationEvaluationContext` carries `document`, `outer`, `starRead`, and `filterRead`, and its aggregate projection already evaluates starred sources through them, while the group count used to delegate to `scalar.readFilledGroupCount` and discard all of it. Both group-count atom shapes now read the document through `readGroupCountOperand`. The scalar route genuinely cannot answer and correctly refuses, held by `scalarComputationContext_repeatableAggregate_requiresContext`; the addressed route can, and no longer declines to.
- `route`: the operand representation exists too. `CheckedStarredGroupSource` already has two completed semantic users, the validation assembly's `starredGroup`/`starredGroupPresence` arms and `BooleanValueCount`, which satisfies the reuse rule outright; `StarGroup.lean` already owns the finite declared-capacity lift for structural row counts, which is the exact quantity a starred operand contributes. What is missing is a heterogeneous operand list on the checked atom and the addressed evaluator's own group-count projection.
- `route`: the surface type is the one real gap. `SurfaceGroupReference` is `path | ruleGroup (starred)`, so a starred *path* operand is unrepresentable and the shape is an admitted-subset boundary rather than a wrong static answer. Extending that arm is the capsule's entry point; keep the scalar route refusing, since its refusal is correct rather than incomplete.
...
- `remaining`: the implicit self-validation message's type on a computation carrying a **starred operand** is **measured and closed** at the [message-polarity checkpoint](SOURCES.md#src-starred-operand-message-polarity). It follows the established directional-fillability rule, with declared capacity as the boundary and headroom read per operator against its own quantity. The type is now implemented and replayed against all ten retained documents ([capability](IMPLEMENTATION-MAP.md#cap-computation-self-validation-type)); the message's text, pointer, and referenced set are still not constructed, and belong to SG10.
- `remaining`: what that closure leaves open is narrow and named at the checkpoint's `limit` — a nested or filtered star, a non-Number target, a starred operand inside a precondition, and whether a declared minimum interacts. The over-limit row's effect on the type is **measured**, from the group-count capture's own retained rows rather than a new one.
...
- `remaining`: the type's **direction** is measured and the account is closed — headroom is necessary but not sufficient, and growth must move the quantity toward the seed. What stays open on this carrier is named at the checkpoint's `limit`.
