...
- `remaining`: whole-model checked-plan formal-input collection and its eager `formalErrorsInOperands` projection. Family-local result owners currently accept already-collected residual messages, so an Explain consumer can certify runtime reachability but not reconstruct this separate inventory from Lean authority.
