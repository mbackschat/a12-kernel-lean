...
- `remaining`: widen the checked direct-field formal-input baseline beyond the two addressed DateTime shifts, Number-backed addressed Time construction, addressed `DateFromDateTime`, and ordinary addressed Enumeration direct/literal sources to multi-operation union, group containment, generated index and preliminary findings, rendered messages, and other result families.
