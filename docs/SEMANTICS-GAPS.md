...
- `remaining`: the over-limit exclusion is measured on **two** carriers — a bare constant and a sibling-star `FirstFilledValue` under a repeatable ancestor — and the ancestor case is measured too: an over-limit ancestor row contributes no target, so the excluded row is decided by the whole environment rather than the target level's own coordinate. The conversion, construction, shift, enumeration, and indexed-parallel carriers share the domain as a stated assumption. Still open: three repetition levels, an over-limit row at a middle level, and whether a seeded cell in an excluded row is cleared.
