...
- `remaining`: widen the checked multi-operation direct-field union beyond the successful three-stage Enumeration, Number-to-DateTime, and four-stage `CurrentRepetition` filtered-Enumeration whole-call results to structural failures, additional whole-call families, group containment beyond the fixed checked numeric operation and already resolved filter fields, generated index and preliminary findings, and rendered messages.
