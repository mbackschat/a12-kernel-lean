...
- `remaining`: filtered-star reference operands needed a coordinate witness in three families. The **token** family now has one, off retained bytes ([filtered-star coordinates](sources/message-and-pointer-probes.md#src-message-address-dialects)): the selected target and the `Having`'s own unmarked operands share one address, concrete above the star and `[0]` at it, while the `$` capture resolves at its outer row. Number and Boolean or Confirm still need their own witness or a firing-scope audit.
...
- `remaining`: the address **spelling** question is reframed rather than open as stated: retained artifacts carry **two dialects in one channel**, an instance one that elides index one and a declaration one that leads with a slash, elides nothing, and marks each iterated level `[0]` ([dialect checkpoint](sources/message-and-pointer-probes.md#src-message-address-dialects)). So the overload matrix and the runtime probe need not disagree at all. A **third** surface is settled too: the rendered text of an over-repetition finding carries a bracket-free declaration path and states the offending index and the cap as separate numbers, so a consumer needing the offending row reads `field` rather than parsing the text.
- `remaining`: What stays open is narrowed rather than restated: the **star is excluded** as the selector — a fixed unstarred group operand reaches the slash dialect too, and its address is byte-identical to the starred field's over the same cell, so a consumer cannot recover the operand form from the reference. **Repeatable crossing is excluded too**, from a capture that emits both dialects for fields at the same depth, and so is **expansion**, by two references inside one `Having` that defeat its strict and its slot reading in turn. A `MessagePointer` carrying `fillToFix` must still represent two address kinds rather than one shape with an optional index.
- `remaining`: the dialect selector is **settled and in `spec/`**: an unbound repeatable level is what reaches the slash spelling, the falsifier ran and returned the predicted bare form for a group expanding to nonrepeatable fields only ([selector checkpoint](sources/message-and-pointer-probes.md#src-address-dialect-selector)), and the clause is outbound as [`SPEC-2026-08-31-16`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-31-16). What stays here is the representation: the shared `MessagePointer` retains field identity and partial coordinates but must still distinguish the two address kinds.
...
- `remaining`: the consumers still on the complete cell view are **measured and two are corrected**: the token distinct count and `FirstFilledValue` now read the in-capacity projection on every operand form, locked by the [sweep cases](../A12Kernel/Conformance/CapacityConsumerSweep.lean) and outbound as [`SPEC-2026-08-31-14`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-31-14). Uniqueness is **unfalsifiable on this dimension** — it skips a formally unavailable operand as it skips an empty, so the two accounts agree on every document — and its call sites stay unchanged deliberately. The token value count's starred-field branch is the one item left, and no measurement covers it.
- `remaining`: open on that dimension: a filtered star on any of these carriers, a second repetition level, nesting below the star, the Boolean and temporal uniqueness carriers, and partial validation, which the [sweep checkpoint](sources/group-list-and-capacity-probes.md#src-capacity-consumer-sweep) records as unmeasured rather than settled.
- `remaining`: authoring that model produced a static-legality correction of its own. **`NumberOfDifferentValues` classifies by its first operand**, so its expansion-kind class is not the operator's property and this project had shipped the wrong one; the [first-operand checkpoint](sources/group-list-and-capacity-probes.md#src-distinct-count-first-operand-class) owns the thirty-two rows and [`SPEC-2026-08-31-13`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-31-13) carries it outbound. Still open on that dimension: whether the extrema and `Sum` are genuinely order-free where this operator is not, and every operand kind beyond NUMBER, STRING, DATE, and BOOLEAN as a first operand.
- `remaining`: the three untested retentions on that dimension are **measured and corrected**, outbound as [`SPEC-2026-08-31-11`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-31-11); the [starred-field](sources/group-and-iteration-probes.md#src-starred-field-operand-extent) and [computation-arm](sources/group-and-iteration-probes.md#src-capacity-projection-computation-arm) checkpoints own them and [`cross-clause.md`](implementation/cross-clause.md#cap-number-group-aggregates) owns the coverage. Still open on this dimension: a **Boolean or token starred field** carrier, whose value counts keep the complete formal-cell view and have no measurement; partial validation; and nesting below the star.
...
- `remaining`: `CheckedDocument` **checks** the two structural row rules — no gap in a row sequence, no child without its parent — but retains neither as a certificate, so a proof cannot use them. Both are now locked as executable cases in [`Conformance/CheckedDocument.lean`](../A12Kernel/Conformance/CheckedDocument.lean). The consequence is that the row constituent's capacity question stays a docstring argument instead of a law: density plus a positive repeatability makes the two accounts indistinguishable, which is provable about this project's own types and not measurable. Retaining the certificate is a core-type change with a construction obligation, so take it only when a second consumer needs the invariant in a proof.
...
- `remaining`: this project can express **no row** of that repeatable-declaring-group table, because `resolveFixedGroupReference` refuses every operand with a nonempty repeatable scope and under a repeatable declaring group each inside operand has one. The rows correct nothing here and size a carrier that does not exist yet. The root class within that shape is also unmeasured: reaching it needs a second root group, which `dmtool model new` does not create.
...
- `remaining`: that leg is **not escalated and not changed, and the reason is now a theorem rather than prose**. `ConditionTree.evalVerdict_collapseUnknown` collapses every leaf's `unknown` to `notFired` and shows the tree's outcome collapses the same way, while the collapse fixes every fired verdict — so no depth or mixture of connectives recovers the distinction and no document separates the accounts. Unfalsifiable rather than unmeasured, so not ready as an `EXP-`. Kept open because an Explain consumer reading the leaf verdict does see it, and because the uncited case locking it is the [`LF118`](LEAN-FINDINGS.md) tell.
- `remaining`: the coverage leg is **measured; the route limit that blocked it is retired.** a12-dmkits built the `validatePart` observation this project had recorded as unreachable, and the first use of it found the count and the presence predicates diverging under partial coverage ([checkpoint](sources/group-list-and-capacity-probes.md#src-partial-coverage-group-operands)). [`SPEC-2026-08-31-18`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-31-18) is **accepted with a three-way correction**: the count rule reproduced, and the predicate half is rewritten as three-valued quantifiers over the asymmetric group relevance, `NotAllGroupsFilled` needing a settled unfilled member. Lean already answered that way. What stays open: a repeatable or starred operand, a coverage naming a group rather than a field, and the computation arm.
...
- `remaining`: this project is **narrower than the Kernel** on a fixed group operand that sits under a repeatable ancestor, a shape the [deep-capacity checkpoint](sources/group-list-and-capacity-probes.md#src-filled-field-count-deep-capacity) measures the Kernel evaluating per enclosing row. `resolveFixedGroupReference` refuses any fixed group operand whose repeatable scope is nonempty, `repeatableGroupRequiresAddress`, while the Kernel admits it and binds the operand's ancestry from the rule's own iteration, emitting one message per enclosing row with a row-local extent. That gate is shared by the group-list, numeric-computation, numeric-validation, and validation-condition carriers and this round measured one of them, so widening it would claim four carriers on one observation. Closing it properly means measuring the same shape on the other three first.
- `remaining`: **the two threshold quantifiers' capacity extent is unreached on both estates, and `Document`'s prefix invariant is why.** Separating it needs a group whose over-limit row exists while its prefix does not; the Kernel's own instance factory materializes the prefix ([checkpoint](sources/group-list-and-capacity-probes.md#src-starred-group-quantifier-capacity)) and this route seeds from Document JSON whose rows are a positional array, so no case here can falsify either arm and `nonprefixRows` refuses the topology outright. Lean picked the every-instantiated-row arm; that is an unfalsified choice, not a measurement. Two routes failing is not proof that a third cannot; until one appears [`spec/02`](../spec/02-logic-and-formal-errors.md) states the question as open.
...
- `remaining`: **DATE is measured and implemented**, and [`cap-repeatable-date-constant-computation`](implementation/repeatable-computations.md#cap-repeatable-date-constant-computation) owns its coverage. Its target-check timing is a *third* one: no check at either time. The static gate reads only the literal's spelling — a calendar-valid, zero-padded `DD.MM.YYYY` in double quotes — and the runtime renders that one value into whatever format the target declares, silently discarding components for a target such as `yyyy` ([checkpoint](sources/computation-placement-and-constant-probes.md#src-date-constant-target-formatting), outbound [`SPEC-2026-08-31-02`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-31-02)). The carrier reuses `FullDateTargetFormat`, whose renderer already produces the two measured complete-format strings.
- `remaining`: open for the Date family: the pre-1900 target. Component-omitting rendering is closed — all seven formats, every store measured, one table shared with the stored-input classifier that reads such a cell back.
...
- `remaining`: **DATETIME is written**, and [`cap-repeatable-datetime-constant-computation`](implementation/repeatable-computations.md#cap-repeatable-datetime-constant-computation) owns its coverage. `"05.03.2024T12:30:00"` is admitted into a DATETIME target, re-derived locally rather than taken on report ([checkpoint](sources/computation-placement-and-constant-probes.md#src-temporal-constant-literal-composition)); the earlier "no witness" reading tested three uncomposed spellings and never tried the concatenation of two shapes the same round had already admitted ([`LF127`](LEAN-FINDINGS.md)).
- `remaining`: open on that carrier: **the stored text**. The composition round took static admission only, so the rendering is the declared format's own carried across from the measured Date and Time siblings, and the carrier says so. Its semantic content is that a constant never becomes an `Instant`, so a wall label inside a spring-forward gap stores where a computed instant would have no label at all — the divergence from the computed route is bounded on the other side by a proved agreement wherever the zone resolves.
...
- `baseline`: no Lean owner or `spec/` clause represents the full derivation. The [mandatory-information checkpoint](sources/rule-set-meta-information.md#src-mandatory-information-narrowing) establishes the presence control, standalone count-root contribution, `EvaluatorVisitor` count-guard conjunction, signed-32-bit threshold narrowing, and authored-`-1` sentinel collision.
- `remaining`: inventory and separate the non-count rule families and the composition rules for all three returned sets before choosing a carrier. The observed presence and count shapes do not justify extrapolating a full collector.
- `evidence-needed`: one coherent batch over the remaining recognized rule families, including a repeatable and a `Having` discriminator wherever the source inventory predicts a distinct branch. Each retained case must identify which returned set changes.
