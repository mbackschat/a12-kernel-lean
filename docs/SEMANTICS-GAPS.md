...
- `remaining`: determine cross-group fixed-target admission and execution for operation families beyond the measured Number constant and the current Boolean/Confirm constant specialization. Do not extrapolate the target-scope mechanism to list, construction, repeatable, or generated-validation routes.
