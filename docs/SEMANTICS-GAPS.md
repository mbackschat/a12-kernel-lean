...
- `remaining`: widen [`AddressedRepeatableTarget.lean`](../A12Kernel/Elaboration/AddressedRepeatableTarget.lean) from declaring-group equality to containment. Its blocking measurement is now closed: the [declaring-group gate checkpoint](SOURCES.md#src-computation-declaring-group-gate) has Kernel admission across the full containment range for both a constant and a sibling-star operand, and parent-local correlation identical at the target's own parent, one ancestor up, and the root on both strategies. The change alters a certificate ten families consume plus two exported theorems and four conformance placement rows, so it is a cold-review trigger and takes its own unit; the correlation row covers the String sibling-star representative only.
