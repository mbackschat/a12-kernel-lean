...
- Open: the checked document now preserves decimal-valued versus String-valued Number input, verifies the regime-selected formal read, retains exact signed decimal identity independently, and exposes the selected text at exact addresses. Actual `FieldValueAsString` computation lowering, host wire ingestion, and remaining group/addressed expression placements must reuse that owner rather than reconstruct text from `Rat`.
