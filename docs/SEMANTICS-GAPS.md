...
- Current boundary: one exact one-level finite Number star, one checked correlation shape, checked model-owned general group-star path lowering, general `Document`-to-nested-tree/ordered-`Env` resolution, unfiltered and resolved-validation-`Having` nested Number cell streams with contextual over-repetition classification, all-rows wildcard relevance for that source, caller-supplied RNU rows, and resolved group presence are closed. The resolved validation filter keeps candidate order, separates complete candidate and captured environments, and reads only retained targets. The relevance gate preserves wildcard versus concrete path selectors and returns nonrelevant before target classification; computation-filter poison/dependency and order-aware per-cell relevance remain open. See [§9](IMPLEMENTATION-MAP.md#9--repetition-and-iteration), [reopened-star completeness and addressing](IMPLEMENTATION-MAP.md#reopened-star-structural-completeness-and-addressing), and [resolved Number aggregates](IMPLEMENTATION-MAP.md#resolved-number-aggregates).
- Prerequisite: next attach order-aware per-cell relevance to `FirstFilledValue`, then mixed declarations and general authored filter-condition lowering after SG3 supplies the shared checked tree. Reuse `SingleGroup`, `ReopenedStarDomain`, `ResolvedStarTopology`, the general environment selector, `RelevantEntityPattern`, `ValueListCell`, and the nested Number route; do not introduce a second row tree, cell parser, condition evaluator, or aggregate scan.
