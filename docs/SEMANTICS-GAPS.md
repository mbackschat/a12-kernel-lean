...
- `remaining`: unmeasured on that carrier and therefore unimplemented: two starred members in one list, a starred member whose group owns a further repeatable descendant, an unretained repeatability, and an unavailable member's interaction with the sum. The mixed list's own **overlap** rule is unmeasured too, so overlap stays enforced over the fixed members alone rather than guessed for a mixed pair.
