...
- Open:
  ...
  - source missing-ancestor creation, destination compatibility, whole-document application, and later validation as separate phases; the source-classified-clear versus absent-destination discriminator is pending under [`EXP-2026-07-30-02`](A12-DMKITS-SPEC-SYNC-LEDGER.md#exp-2026-07-30-02--source-classified-clear-against-an-absent-destination);
...
- Unresolved source fact: the capability packet closes supplied order/cycle rejection, flattened first selection, reached-read poison, stripping/overlay, eager operand checking, pointer result partition, application order, validation timing, and locus-dependent repeatable clearing. Pinned application source predicts that a source-classified CLEARED pointer creates a present-empty cell and missing repeatable ancestors when applied to a separate absent destination; the current Lean action transition instead preserves absence, while the maintained peer cases never combine those source and destination states. Reopen only for a new target family or processing context, missing ancestors, rendering, destination compatibility, or another uncovered engine-layer composition.
...
- Evidence status: maintained a12-dmkits differentials Kernel-lock isolated repeatable/index and value-less-target result-channel seams, typed Number V2 source identity at `7b2e9f7`, cause-blind computed-target dependency erasure at `351bf9f2`, and selected-empty same-target termination versus false-guard fall-through at `4ebe84fc`. Both bounded Number/String directions and the finite mixed scalar activation, execution, and family-preserving result projection are source-grounded and internally separated but have no retained composed Kernel observation; finite repeatable plans, destination compatibility, and whole-document transition remain partially or wholly `external evidence pending`.
...
- Open: nonrepeatable `FieldValueAsString` now lowers and executes through the checked document's regime-selected text without reconstructing it from `Rat`. Host wire ingestion and remaining group/addressed expression placements must reuse that owner.
