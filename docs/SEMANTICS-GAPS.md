...
- Current boundary: `ConditionTree` now owns connective shape, empty-row eligibility, reference traversal support, and exact verdict-aware short-circuit evaluation. `FlatCondition` is that tree over flat leaves, while `ValidationCondition` admits flat and resolved numeric-comparison leaves in one tree with shared evaluation, field-reference traversal, and per-leaf relevance. Laws prove that flat embedding preserves every verdict and reference, and cases separate mixed firing, flat-left short-circuit, reached numeric nonrelevance, and cross-family references. Checked whole-rule assembly still accepts only `CheckedFlatCondition`; the mixed tree begins after checked leaf elaboration, generated validation accepts only a nonempty literal fragment, and resolved aggregates are not expression nodes. See [§5](IMPLEMENTATION-MAP.md#5--numbers-and-decimals), [§11](IMPLEMENTATION-MAP.md#11--computations), and [§12](IMPLEMENTATION-MAP.md#12--validation-and-polarity).
- Prerequisite: `CheckedFlatCondition` and `CheckedNumericComparison` now both retain their exact row group; generated flat conditions derive it from the resolved target declaration rather than caller metadata. Next assemble the mixed tree only from certificates that share the same validated model and row group, retain a proof that the resulting core is model-coherent, and route whole-rule reference/error-field checks through the shared traversal. Preserve the at-least-one-field gate and reject target self-reference through that traversal before widening generated operations or adding aggregate leaves.
