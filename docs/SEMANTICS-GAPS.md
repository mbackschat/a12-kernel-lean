...
- `baseline`: the same carrier now implements the [threshold checkpoint](sources/rule-set-meta-information.md#src-mandatory-information-narrowing) and its [reversed-strict successor](sources/rule-set-meta-information.md#src-mandatory-information-reversed-strict-count): standalone count-root contribution, all four direct-seed comparison spellings, signed-32-bit threshold narrowing, and authored-literal identity beside the `-1` sentinel. Unsupported count dependency and false-comparison shapes fail closed.
