...
- `remaining`: the component-dropping renderer now covers all four measured formats — `yyyy`, `yyyy-MM`, `yyyyMM`, `MM`, and `MM-dd` — with the yearless two gated on a declared Base Year and storing no year regardless ([Base Year store checkpoint](sources/cross-layer-routes.md#src-base-year-yearless-store)). What stays open is the compact `MMdd` and `ddMM` spellings [`spec/05`](../spec/05-dates-and-time.md) lists as declarable: the observation route verifies only the four above as fragment formats, so a declaration probe is the next thing needed and neither carrier nor store may be assumed from the dashed siblings.
