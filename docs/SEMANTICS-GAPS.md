...
- `remaining`: repeatable-target placement is **closed** — every gate admits by containment against a validated declaring group, and [`cross-clause.md`](implementation/cross-clause.md#cap-addressed-numeric-placement-and-execution-seam) owns the implemented rule. What stays open below is the fixed-target half.
- `remaining`: placement conformance strength is uneven where it is closed. Addressed Custom and addressed ordinary String refuse only a wholly unrelated group and have no ancestor-admission cell; [`AddressedNumberFirstFilledComputation`](../A12Kernel/Elaboration/AddressedNumberFirstFilledComputation.lean) has no placement cell because its only owner sits at the 800-line decomposition target. Strengthen these opportunistically when those files are next touched, not as their own unit.
- `remaining`: no target-placement gate remains. Every fixed-target family now admits a target the declaring group does not contain, matching the [checkpoint](SOURCES.md#src-fixed-target-star-placement), and each carries a conformance cell for that admission. `ownsDirectDateRangeTarget` lost its group parameter and now asserts model ownership alone.
- `remaining`: the refusal that should still fire is **unimplemented and currently unauthorable**. An unstarred per-row operand of a repeatable declaring group must refuse a fixed target outside it, and no checked elaborator derives iteration from an operand. No witness is known in this estate; the [checkpoint's local-reach row](SOURCES.md#src-fixed-target-star-placement) records the revision and why every admitted fixed-target family — constant, star aggregate, or nonrepeatable direct read — cannot spell the operand that triggers it.
- `remaining`: treat that as a **constraint on the next family, not a to-do**. Any family that admits a per-row operand into a fixed target must derive iteration from that operand and apply at-or-below containment in the same change; adding the check now would be machinery for a shape nothing can author. The measured discriminator is in the [checkpoint](SOURCES.md#src-fixed-target-star-placement): `[Marker]` at a repeatable declaring group refuses while `FirstFilledValue(Rows*/Marker)` at the same group is admitted, so aggregation is the test.
- `remaining`: the DateRange direct list keeps a **separate** narrowing that placement removal did not touch: every source must sit in the declaring group. That is this capsule's own bounded subset, not a Kernel gate — it carries no diagnostic code, and the [checkpoint](SOURCES.md#src-fixed-target-star-placement) admits a cross-group list. Widening it is optional scope, and the admitted bare-name operand spelling cannot express a cross-group source anyway, so it needs an operand-form change first.
- `remaining`: the [fixed-target star placement checkpoint](SOURCES.md#src-fixed-target-star-placement) admits a sibling-star `FirstFilledValue` into a fixed target from its own group, an ancestor, an unrelated sibling, and the starred source's own repeatable group, matching a bare constant at all four, with a repeatable-target control refusing in the same model. Closing the seven is therefore **not** a widening to containment — containment also refuses the measured sibling admission — but removal of the placement gate plus a check on whether the operand derives iteration. Outbound as [`SPEC-2026-08-29-03`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-29-03).
- `remaining`: the second derivation source is unimplemented. The target's own repeatable scope is the implemented source; no checked elaborator derives iteration from an unstarred per-row operand of a repeatable declaring group, which is the source that refuses an otherwise legal fixed target. An unstarred read of a repeatable field from a nonrepeatable declaring group is refused earlier by `MVK_NO_WILDCARD` on a different gate, so removing the placement gate does not open that shape.
...
- `remaining`: retain a `NumericComputationRunFault.targetCheck` witness only if one appears. The obvious candidate is now measured and refused: `SumOfProducts` is the catalog's only scale-increasing operator, and a scale-4 product into a scale-2 target draws `MVK_INVALID_COMPARE_DEC_PLACES` unless suppressed, while suppression routes to `checkWithScaleWarningSuppressed` instead of this arm. The [assignment-scale reach checkpoint](SOURCES.md#src-unsuppressed-assignment-scale-reach) owns the measurement and states what would still defeat it.
