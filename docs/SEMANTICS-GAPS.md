...
- `remaining`: every **repeatable-target** placement gate now admits by containment against the declaring group, matching the [declaring-group gate checkpoint](SOURCES.md#src-computation-declaring-group-gate). Each pairs a declaring-group validity check with an at-or-below containment check, because `[]` is a prefix of every path and would otherwise admit every placement vacuously.
- `remaining`: the checkpoint's misattribution pair is locked once, in [`AddressedFieldValueAsString`](../A12Kernel/Conformance/AddressedFieldValueAsString.lean): an ancestor declaration whose operand is not re-spelled refuses on the source channel as an unresolvable entity, not as a placement refusal. Other families inherit that separation rather than repeating it.
- `remaining`: [`AddressedNumberFirstFilledComputation`](../A12Kernel/Elaboration/AddressedNumberFirstFilledComputation.lean) reaches the widened target gate with no placement cell of its own. Its only conformance owner, [`NumericComputation/Repeatable.lean`](../A12Kernel/Conformance/NumericComputation/Repeatable.lean), sits at the 800-line decomposition target, and the gate site it reaches is already separated twice; add the cell when that file is split rather than crossing the target for a third exerciser of one gate.
- `remaining`: placement conformance strength is uneven and the weak owners are worth strengthening opportunistically, not as their own unit. Enumeration, `FieldValueAsString`, `DateFromDateTime`, `TimeFromDateTime`, and both numeric-leaf gate sites admit an ancestor and refuse a declared group strictly **below** the target. Addressed Boolean refuses a declared group sharing the target's enclosing prefix without containing it, which separates the other direction. Addressed Custom and addressed ordinary String refuse only a wholly unrelated group and have no ancestor-admission cell of their own; they inherit the shared certificate's behavior without separating it locally.
- `remaining`: those gates are the shared [`AddressedRepeatableTarget.lean`](../A12Kernel/Elaboration/AddressedRepeatableTarget.lean) certificate, the [`AddressedEnumerationComputation.lean`](../A12Kernel/Elaboration/AddressedEnumerationComputation.lean) copy, and the four former equality gates in [`AddressedNumericLeaf.lean`](../A12Kernel/Elaboration/AddressedNumericLeaf.lean), [`AddressedFieldValueAsString.lean`](../A12Kernel/Elaboration/AddressedFieldValueAsString.lean), [`AddressedDateFromDateTime.lean`](../A12Kernel/Elaboration/AddressedDateFromDateTime.lean), and [`AddressedTimeFromDateTime.lean`](../A12Kernel/Elaboration/AddressedTimeFromDateTime.lean).
- `remaining`: the **fixed-target** sibling-star families still gate on group equality — [`BooleanFirstFilledComputation.lean`](../A12Kernel/Elaboration/BooleanFirstFilledComputation.lean), [`CustomFirstFilledComputation.lean`](../A12Kernel/Elaboration/CustomFirstFilledComputation.lean), [`StringFirstFilledComputation.lean`](../A12Kernel/Elaboration/StringFirstFilledComputation.lean), [`DateRangeFirstFilledComputation.lean`](../A12Kernel/Elaboration/DateRangeFirstFilledComputation.lean), [`DateRangeConstructionComputation.lean`](../A12Kernel/Elaboration/DateRangeConstructionComputation.lean), [`IndexedDateRangeConstructionComputation.lean`](../A12Kernel/Elaboration/IndexedDateRangeConstructionComputation.lean), and [`TemporalFirstFilledStarComputation.lean`](../A12Kernel/Elaboration/TemporalFirstFilledStarComputation.lean).
- `remaining`: that last one carries five carriers behind a single site: it is the sole placement gate for the completed DateFragment, full-Date, Time, DateTime, and DateRange star computations, so a decision there moves all five at once.
- `remaining`: do not widen those fixed-target gates to containment by analogy. The checkpoint measures a fixed target admitted from a **sibling** group, which containment refuses, and separately measures a fixed target refused only when a repeatable declaring group supplies a per-row operand. Neither row uses a star operand, so the correct gate for a fixed target under a star is unmeasured and both equality and containment are wrong accounts of it.
- `remaining`: the second derivation source is unimplemented. `spec/` states containment under derived iteration, reconciled with a12-dmkits as [`SPEC-2026-08-29-01`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-29-01); the target's own repeatable scope is now the implemented source, but no checked elaborator derives iteration from a per-row operand of a repeatable declaring group, which is the source that refuses an otherwise legal fixed target.
...
- `remaining`: retain a `NumericComputationRunFault.targetCheck` witness if an authored computation makes that arm reachable. No witness is currently known.
...
- `remaining`: non-field parameters `#RootGroup`, `#Zeile`, `#Usb`, `Vordruckzeile`, `Vordruckname`, group index, `index(...)`, and semantic-index suffix, plus their diagnostics.
