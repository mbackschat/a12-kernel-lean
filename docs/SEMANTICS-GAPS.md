...
- `remaining`: every **repeatable-target** placement gate now admits by containment against the declaring group, matching the [declaring-group gate checkpoint](SOURCES.md#src-computation-declaring-group-gate). Each pairs a declaring-group validity check with an at-or-below containment check and carries a conformance cell admitting an ancestor and refusing a declared group strictly below the target.
