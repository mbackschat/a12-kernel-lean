...
- `remaining`: the over-repetition finding **set** is measured — one `zuGrosseZeile` on the row plus one `zuGrosseKontextnummer` per node the document writes beneath it, at any depth ([checkpoint](sources/group-and-iteration-probes.md#src-over-limit-finding-multiplicity)). What stays open is the address **spelling**: the reviewed overload matrix carries a repetition-one index and none on the row-level code, while the runtime probe elides repetition-one and indexes the over-limit row. Whether that is a rendering dialect or a channel difference needs one fixture observed through both routes.
