...
- `baseline`: the [threshold checkpoint](sources/rule-set-meta-information.md#src-mandatory-information-narrowing) separately establishes the standalone count-root contribution, count-guard conjunction, signed-32-bit threshold narrowing, and authored-`-1` sentinel collision. Those distinctions are not yet carried by Lean.
- `remaining`: inventory and measure the still-uncovered recognized rule families, cycles, repetition and index behavior, filter internals, generated rules, and closure shapes beyond the flat acyclic matrix before widening the checked carrier. The measured count-threshold slice still needs a representation that keeps the authored literal separate from its narrowed host value.
...
- `consumer`: Analyze and Explain can now recover authored rule shape and exact derived-set identity for the checked flat fragment. They still need the distinction between no numeric bound and a bound narrowing to the `-1` sentinel before count-threshold rules can join that carrier.
