...
- Current boundary: checked model-owned general group-star paths produce one nested topology and ordered environment stream, including contextual over-repetition and hierarchical omitted tails. The checked Number/`CurrentRepetition` `Having` fragment filters that stream through the common condition tree before target classification. Checked Number `FirstFilledValue` accepts mixed direct, plain-star, and filtered-star slots with ordered lazy relevance. A checked full-validation terminal-repeatable group-star source now feeds the same canonical row count to `NoGroupFilled(G*)`, `AtLeastOneGroupFilled(G*)`, and `NumberOfFilledGroups(G*)`, including created-empty and over-limit rows. Wider filter leaves and authored `Or`, computation-filter poison/dependency, other field kinds and consumers, nonrepeatable terminal groups, mixed starred/plain group operand lists, partial group relevance, caller-supplied RNU rows, and resolved descendant group presence remain separate. See [§9](IMPLEMENTATION-MAP.md#9--repetition-and-iteration), [reopened-star completeness and addressing](IMPLEMENTATION-MAP.md#reopened-star-structural-completeness-and-addressing), [resolved Number `FirstFilledValue`](IMPLEMENTATION-MAP.md#resolved-number-firstfilledvalue), and [checked terminal-repeatable group-star counts](IMPLEMENTATION-MAP.md#checked-terminal-repeatable-group-star-counts).
- Prerequisite: the shared checked condition/expression tree, narrow authored filter lowering, checked mixed Number `FirstFilledValue`, and terminal-repeatable group-star count routes are closed. A later SG2 capsule must reuse `ReopenedStarDomain`, `ResolvedStarTopology`, `CheckedStarPlan`, the general environment selector, `RelevantEntityPattern`, `ValueListCell`, and the shared continuation scan. Before claiming the complete partial-validation boundary, audit whether absent or phantom declared rows require identity beyond the current hierarchical omitted-tail marker.
...
- Current boundary: the binding future architecture is accepted in [`A12-LEAN-COMPUTATION-EXECUTION-PROPOSAL.md`](A12-LEAN-COMPUTATION-EXECUTION-PROPOSAL.md), while resolved first-match tables, selected String operations, checked Number expressions including direct aggregates, target/delta/application, common preconditions, and all-alternative generated validation remain closed narrow components; see [§11](IMPLEMENTATION-MAP.md#11--computations). No checked whole-model plan, general activation, scheduler, rich public result, or whole-document application is implemented.
- Prerequisite: SG1 must supply model-coherent checked document state and SG2 general repeatable addressing; shared guards and expression-valued generated validation are already closed. Before any general SG4 type or scheduler, define the admitted explicit processing context and close the proposal's [bounded source-audit gate](A12-LEAN-COMPUTATION-EXECUTION-PROPOSAL.md#17-bounded-source-audit-required-before-implementation), including formal-operand extraction, schedule construction, cycles, runtime instances, same-target selection, stripping and producer visibility, outcome carriers, result ownership and ordering, application collisions, and generated-validation timing.
