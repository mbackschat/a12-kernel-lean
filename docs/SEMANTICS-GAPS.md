...
- Current boundary: `ConditionTree` owns connective shape, empty-row eligibility, reference traversal support, static all-leaf admission, and exact verdict-aware short-circuit evaluation. `FlatCondition` is that tree over flat leaves, while `ValidationCondition` admits flat and resolved numeric-comparison leaves in one tree with shared evaluation, field-reference traversal, and per-leaf relevance. `CheckedValidationCondition` lifts checked flat/numeric inputs, preserves the exact common row group, rejects mismatches, and carries model/core coherence. `ResolvedRule` and `CheckedResolvedRule` share the metadata/error-field certificate and sole post-verdict message emitter. `NumericOperandScope` preserves ordinary same-group and computation model-wide nonrepeatable admission on the same comparison certificate. One operation-parametric nonempty table now serves literal Number and checked numeric-expression payloads: it shares cardinality, first-match selection, common placement, declaration order, and tolerance while expression mismatches retain every branch in the mixed rule. Cross-group Number/temporal sources, singleton, overlap, tolerance, common false, and target mismatch are separated. Resolved aggregates are not yet expression nodes. See [§5](IMPLEMENTATION-MAP.md#5--numbers-and-decimals), [§11](IMPLEMENTATION-MAP.md#11--computations), and [§12](IMPLEMENTATION-MAP.md#12--validation-and-polarity).
- Prerequisite: inventory `CheckedNumericAggregate` and its resolved operand streams against `AuthoredNumericExpr`, `NumericComparison`, and `ValidationCondition`, then add the smallest aggregate leaf or expression node that preserves existing order, selection/fillability, references, relevance, and phase behavior. Do not add an aggregate-only comparison wrapper or a second fold/evaluator.
