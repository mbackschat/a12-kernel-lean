...
- `remaining`: computed-source Enumeration beyond the exact one-producer cascade and two-producer ordered `FirstFilledValue` join, especially longer chains, more than two sibling producers, and mixed direct/star or filtered computed-source routes.
...
- `remaining`: computed-source Enumeration beyond the exact one-producer cascade and two-producer ordered `FirstFilledValue` join is owned jointly with SG4.
