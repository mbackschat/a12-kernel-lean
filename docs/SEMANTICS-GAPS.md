...
- `remaining`: measure parent-local correlation *runtime* under an ancestor declaration for the exact-address families. Kernel admission is measured for both constant and sibling-star operands at the [declaring-group gate checkpoint](SOURCES.md#src-computation-declaring-group-gate), but every consuming family evaluates correlation against the declaring group, so [`AddressedRepeatableTarget.lean`](../A12Kernel/Elaboration/AddressedRepeatableTarget.lean) deliberately keeps the narrower equality until that runtime is calibrated. Widen the certificate only together with it.
