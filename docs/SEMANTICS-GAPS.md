...
- Unresolved source fact: compose one dynamic shifted-`Now` result into one checked DateTime `AddDays` with explicit per-call `World`, reusing the landed cross-mechanism application without merging dynamic and field-backed carriers. The peer's adjacent day probes still do not calibrate the source-offset/refit mechanism, which remains pending under [`SPEC-2026-07-29-01`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-07-29-01). Reverse calendar-then-elapsed composition, wider formats, legal zones/pre-floor identities, detailed basic-check errors, repeatable placement, broader recursive lowering, and date-like distinct-count identity remain open.
