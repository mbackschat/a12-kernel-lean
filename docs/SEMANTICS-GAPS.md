...
- `remaining`: the `$#…$` group position now carries its measured **static admission** ([§13](IMPLEMENTATION-MAP.md#13--message-interpolation)). Two representation choices inside it await a discriminator: whether the root shorthand names the rule chain's own root or a model-wide root, which the one-root probe model cannot separate and which `FlatModel` does not decide because it enforces no single root; and whether the position accepts the name grammar's segment quoting, which is refused here because no observation admits it.
