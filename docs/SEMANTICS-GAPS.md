...
- `remaining`: two **fixed-target** gates still refuse a target outside the declaring group and are measured narrower than the Kernel: [`DateRangeConstructionComputation.lean`](../A12Kernel/Elaboration/DateRangeConstructionComputation.lean) and [`IndexedDateRangeConstructionComputation.lean`](../A12Kernel/Elaboration/IndexedDateRangeConstructionComputation.lean). Every `FirstFilledValue` family is closed. Those two differ from the closed ones: their target ownership goes through `model.ownsDirectDateRangeTarget declaringGroup target`, so the check is entangled with target resolution rather than a standalone placement test, and their operands are direct endpoint reads rather than star aggregates — measure that shape before assuming it derives no iteration.
