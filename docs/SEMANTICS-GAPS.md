...
- `remaining`: placement is **closed** in both directions and [`computations.md`](implementation/computations.md#cap-first-selected-tables) owns the implemented boundary. A repeatable target must lie at or below the declaring group; a fixed target carries no restriction, since neither a constant, a star aggregate, nor a nonrepeatable direct read derives iteration. Every family requires a representable declaring group. The [fixed-target star placement checkpoint](SOURCES.md#src-fixed-target-star-placement) owns the measurement.
...
- `remaining`: the `$#…$` group position is now **measured** and unimplemented — the [group-parameter checkpoint](SOURCES.md#src-message-group-parameter) fixes its admitted set, its three diagnostic classes, and its absolute-path requirement, outbound as [`SPEC-2026-08-29-04`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-29-04). A Lean capsule for it is the ready next step in this family; it needs no further evidence.
- `remaining`: still unmeasured in that bundle are the group index and the semantic-index suffix in the name position, what any admitted group parameter *renders*, the German condition language, and every non-rule parameter owner sharing the terminal bundle.
