...
- Current boundary: one exact one-level finite unfiltered Number star, one checked correlation shape, checked model-owned general group-star path lowering, general `Document`-to-nested-tree/ordered-`Env` resolution, caller-supplied RNU rows, and resolved group presence are closed; see [§9](IMPLEMENTATION-MAP.md#9--repetition-and-iteration), [reopened-star completeness and addressing](IMPLEMENTATION-MAP.md#reopened-star-structural-completeness-and-addressing), and [resolved Number aggregates](IMPLEMENTATION-MAP.md#resolved-number-aggregates).
- Prerequisite: next route one concrete nested Number cell reader through the checked path and topology stream, then source-audit `Having` discovery/filter order, wildcardable relevance, mixed declarations, and over-limit formal classification. Reuse `SingleGroup`, `ReopenedStarDomain`, existing correlation environments, `ResolvedStarTopology`, and `ValueListCell`; do not introduce a second row tree, cell parser, or aggregate scan.
