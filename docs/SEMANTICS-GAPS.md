...
- Current boundary: resolved first-match tables, selected String operations, checked Number expressions including direct aggregates, target/delta/application, common preconditions, and all-alternative generated validation are closed separately; see [§11](IMPLEMENTATION-MAP.md#11--computations).
- Prerequisite: SG1 supplies document state and SG2 supplies repeatable addressing; shared guards and expression-valued generated validation are already closed. Source-audit scheduling, transitive dependencies, joins, and result-empty provenance before defining transitions.
