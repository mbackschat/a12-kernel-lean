...
- Current boundary: one exact one-level finite Number star, one checked whole-rule correlation shape, checked model-owned general group-star path lowering, general `Document`-to-nested-tree/ordered-`Env` resolution, checked authored and resolved-validation-`Having` nested Number cell streams with contextual over-repetition classification, both all-rows wildcard relevance and order-aware per-cell partial `FirstFilledValue` relevance for one checked source, a multi-slot resolved Number `FirstFilledValue` scan with ordered filter encounter and separate actual-empty/omitted-tail state, caller-supplied RNU rows, and resolved group presence are closed. The authored filter fragment reuses the common condition tree and checked star plan, admits Number/`CurrentRepetition` comparisons and conjunction, requires an ordinary reference to reach a reopened level, checks every ordinary reference against the complete candidate environment and every `$` reference against the captured rule environment, and reads only retained targets. All-rows and per-cell gates share path/index patterns but preserve their distinct operator decisions; wider filter leaves and authored `Or`, computation-filter poison/dependency, checked mixed declarations, and checked multi-operand per-slot relevance remain open. See [§9](IMPLEMENTATION-MAP.md#9--repetition-and-iteration), [reopened-star completeness and addressing](IMPLEMENTATION-MAP.md#reopened-star-structural-completeness-and-addressing), and [resolved Number `FirstFilledValue`](IMPLEMENTATION-MAP.md#resolved-number-firstfilledvalue).
- Prerequisite: the shared checked condition/expression tree and narrow authored filter lowering are closed. Next construct mixed declarations and multi-operand `FirstFilledValue` without introducing a second row tree, cell parser, condition evaluator, or aggregate scan. Reuse `SingleGroup`, `ReopenedStarDomain`, `ResolvedStarTopology`, the general environment selector, `RelevantEntityPattern`, `ValueListCell`, and the nested Number route. Before claiming the complete partial-validation boundary, audit whether absent or phantom declared rows require identity beyond the current hierarchical omitted-tail marker.
