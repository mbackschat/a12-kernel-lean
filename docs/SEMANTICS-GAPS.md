...
- `remaining`: **DATE is measured and ready to implement.** Its target-check timing is a *third* one: no check at either time. The static gate reads only the literal's spelling — a calendar-valid, zero-padded `DD.MM.YYYY` in double quotes — and the runtime renders that one value into whatever format the target declares, silently discarding components for a target such as `yyyy` ([checkpoint](sources/cross-layer-routes.md#src-date-constant-target-formatting), outbound [`SPEC-2026-08-31-02`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-31-02)). The carrier reuses `FullDateTargetFormat`, whose renderer already produces the two measured complete-format strings; the component-omitting target is a stated exclusion because `ofSource?` does not admit it and no full-Date renderer here drops components.
- `remaining`: bare constants for the still-unmeasured kinds — enumeration, date-range, TIME, DATETIME, DATE_FRAGMENT, and Custom — remain unrepresentable here. Each needs its own target-check timing measured before a carrier: Number showed that reading String's timing across the kind boundary would have been wrong, and Date has now produced a timing neither of them has.
