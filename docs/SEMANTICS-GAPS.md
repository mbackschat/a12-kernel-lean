...
- `remaining`: the refusal that should still fire is **unimplemented**, and no witness is known for it in this estate. An unstarred per-row operand of a repeatable declaring group must refuse a fixed target outside it, and no checked elaborator derives iteration from an operand. The [checkpoint's local-reach row](SOURCES.md#src-fixed-target-star-placement) records the revision and the survey behind that: every admitted fixed-target family takes a constant, a star aggregate, or a nonrepeatable direct read.
...
- `remaining`: the DateRange direct list keeps a **separate** narrowing that placement removal did not touch: every source must sit in the declaring group. That is this capsule's own bounded subset, not a Kernel gate — it carries no diagnostic code, and the [checkpoint](SOURCES.md#src-fixed-target-star-placement) admits a cross-group list. Widening it is optional scope. An absolute operand path reaches the narrowing from the admitted surface, so it is a live refusal rather than an unreachable one, and its conformance cell now separates it.
