...
- Current boundary: checked model-owned general group-star paths produce one nested topology and ordered environment stream, including contextual over-repetition and hierarchical omitted tails. The checked Number/`CurrentRepetition` `Having` fragment filters that stream through the common condition tree before target classification. Checked Number `FirstFilledValue` now accepts mixed direct, plain-star, and filtered-star slots; preserves authored order and declaration scale; rejects a sole direct field and repeated direct non-wildcard references before common Number-kind certification; retains repeated wildcard slots; and lazily interleaves each reached slot's topology, filter, relevance, and shared prefix scan. Wider filter leaves and authored `Or`, computation-filter poison/dependency, other field kinds and consumers, caller-supplied RNU rows, and resolved group presence remain separate. See [§9](IMPLEMENTATION-MAP.md#9--repetition-and-iteration), [reopened-star completeness and addressing](IMPLEMENTATION-MAP.md#reopened-star-structural-completeness-and-addressing), and [resolved Number `FirstFilledValue`](IMPLEMENTATION-MAP.md#resolved-number-firstfilledvalue).
- Prerequisite: the shared checked condition/expression tree, narrow authored filter lowering, and checked mixed Number `FirstFilledValue` route are closed. A later SG2 capsule must reuse `ReopenedStarDomain`, `ResolvedStarTopology`, the general environment selector, `RelevantEntityPattern`, `ValueListCell`, and the shared continuation scan. Before claiming the complete partial-validation boundary, audit whether absent or phantom declared rows require identity beyond the current hierarchical omitted-tail marker.
