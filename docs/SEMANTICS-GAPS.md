...
- `route`: the topology is already in hand one level up. `NumericComputationEvaluationContext` carries `document`, `outer`, `starRead`, and `filterRead`, and its aggregate projection already evaluates starred sources through them — then it delegates the group count to `scalar.readFilledGroupCount`, discarding all of it. The scalar route genuinely cannot answer and correctly refuses, held by `scalarComputationContext_repeatableAggregate_requiresContext`; the addressed route can and does not try.
...
- `remaining`: what that closure leaves open is narrow and named at the checkpoint's `limit` — an over-limit row's effect on the type, a nested or filtered star, a non-Number target, a starred operand inside a precondition, and whether a declared minimum interacts.
