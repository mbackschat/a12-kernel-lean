...
- Current boundary: `ConditionTree` owns connective shape, empty-row eligibility, reference traversal support, static all-leaf admission, and exact verdict-aware short-circuit evaluation. `FlatCondition` is that tree over flat leaves, while `ValidationCondition` admits flat and resolved numeric-comparison leaves in one tree with shared evaluation, field-reference traversal, and per-leaf relevance. `CheckedValidationCondition` lifts only checked flat/numeric inputs, preserves the exact common row group, rejects mismatches, and carries model/core coherence. Laws prove flat verdict/reference preservation and checked coherence; cases separate mixed firing, flat-left short-circuit, reached numeric nonrelevance, cross-family references, checked composition, and group mismatch. Checked whole-rule assembly still accepts only `CheckedFlatCondition`; generated validation accepts only a nonempty literal fragment, and resolved aggregates are not expression nodes. See [§5](IMPLEMENTATION-MAP.md#5--numbers-and-decimals), [§11](IMPLEMENTATION-MAP.md#11--computations), and [§12](IMPLEMENTATION-MAP.md#12--validation-and-polarity).
- Prerequisite: next generalize the existing checked/resolved whole-rule boundary to consume `CheckedValidationCondition`, specialize the existing flat route through it, and route error-field membership through the shared traversal without duplicating message rendering or the rule outcome. Then preserve target self-reference through that traversal before widening generated operations or adding aggregate leaves.
