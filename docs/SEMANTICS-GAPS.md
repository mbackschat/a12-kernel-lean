...
- `remaining`: starred computation runtime is **measured and unimplemented** at the [starred group-count checkpoint](SOURCES.md#src-starred-group-count-computation): a starred repeatable operand contributes its instantiated row count, and a mixed list adds that count to a fixed operand's zero-or-one. Select it as a second operand form with its own result domain, reusing the structural row count the starred validation carrier already owns; do not fold it into the fixed operand's presence tally, which cannot express a quantity above one.
