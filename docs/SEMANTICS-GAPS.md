...
- `remaining`: computed-source Enumeration beyond the exact one-producer cascade and two-producer ordered `FirstFilledValue` join. One direct-plus-filtered-star consumer now reads a completed direct Number only inside `Having`; longer chains, more than two sibling producers, Enumeration-valued mixed or filtered computed sources, wider filter producer families, and result/application for that cross-family route remain open.
...
- `remaining`: sibling-parallel and third-level `DateFromDateTime`, materialized topology, later validation, valueless-source reason propagation, and maintained result/application correspondence.
...
- `remaining`: computed-source Enumeration beyond the exact one-producer cascade and two-producer ordered `FirstFilledValue` join is owned jointly with SG4. The exact direct-plus-filtered-star consumer with one computed Number read only by `Having` is closed for execution and Analyze, but its result/application and wider producer or source shapes remain open there.
