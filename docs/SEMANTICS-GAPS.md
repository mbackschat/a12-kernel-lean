...
- `remaining`: six **fixed-target** gates still refuse a target outside the declaring group and are measured narrower than the Kernel: [`BooleanFirstFilledComputation.lean`](../A12Kernel/Elaboration/BooleanFirstFilledComputation.lean), [`CustomFirstFilledComputation.lean`](../A12Kernel/Elaboration/CustomFirstFilledComputation.lean), [`StringFirstFilledComputation.lean`](../A12Kernel/Elaboration/StringFirstFilledComputation.lean), the direct-list route in [`DateRangeFirstFilledComputation.lean`](../A12Kernel/Elaboration/DateRangeFirstFilledComputation.lean), [`DateRangeConstructionComputation.lean`](../A12Kernel/Elaboration/DateRangeConstructionComputation.lean), and [`IndexedDateRangeConstructionComputation.lean`](../A12Kernel/Elaboration/IndexedDateRangeConstructionComputation.lean). `TemporalFirstFilledStarComputation.lean` is closed, which also closed the star route of all five temporal carriers.
- `remaining`: the DateRange direct-list gate is the next one and is measured — a finite list of fixed sources is admitted at all four placements — but it is a larger change than the star route was: its `targetGroup` is a **type index** on `CheckedDateRangeFirstFilledDirectSource`, so removing the gate means renaming that index to the declaring group it actually certifies against, not deleting a field.
