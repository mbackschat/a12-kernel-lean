...
- Unresolved source fact: extend the exact checked result bridge from the first authorable nested shift to a constructed-Date difference operand without dropping legacy-hybrid calendar identity, exact overlap instant, or operand-order precedence. General recursive lowering remains outside until a second completed shape establishes its smallest carrier. The peer's adjacent day probes still do not calibrate the source-offset/refit mechanism, which remains pending under [`SPEC-2026-07-29-01`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-07-29-01). Wider formats, legal zones/pre-floor identities, detailed basic-check errors, repeatable placement, target/destination integration, and date-like distinct-count identity remain open.
