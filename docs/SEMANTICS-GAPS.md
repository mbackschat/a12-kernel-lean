...
- Current boundary: one exact one-level finite unfiltered Number star, one checked correlation shape, checked model-owned general group-star path lowering, general `Document`-to-nested-tree/ordered-`Env` resolution, one unfiltered nested Number cell stream with contextual over-repetition classification, caller-supplied RNU rows, and resolved group presence are closed; see [§9](IMPLEMENTATION-MAP.md#9--repetition-and-iteration), [reopened-star completeness and addressing](IMPLEMENTATION-MAP.md#reopened-star-structural-completeness-and-addressing), and [resolved Number aggregates](IMPLEMENTATION-MAP.md#resolved-number-aggregates).
- Prerequisite: next source-audit and attach `Having` discovery/filter order, then wildcardable relevance and mixed declarations. Reuse `SingleGroup`, `ReopenedStarDomain`, existing correlation environments, `ResolvedStarTopology`, `ValueListCell`, and the nested Number route; do not introduce a second row tree, cell parser, or aggregate scan.
