...
- `remaining`: which **further** group consumers project is the ordinary open question, not a blocker. This estate applies the capacity projection per measured consumer, so a value count, a Boolean or Confirm group operand, and the extrema over a group keep the complete formal-cell view until each has its own row; [`EXP-2026-08-31-02`](A12-DMKITS-SPEC-SYNC-LEDGER.md#exp-2026-08-31-02) carries those observations. `AtLeastOneFieldFilled` and `NoFieldFilled` are measured and agree with the count but have no checked group carrier here to correct.
