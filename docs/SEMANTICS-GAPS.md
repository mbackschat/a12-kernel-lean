...
- `remaining`: the computation's over-repetition channel is modeled as `CheckedDocument.computationOverRepetitionFindings`, a filter of the validation set on group-path comparability, proved a `Sublist`. Open: the clause takes operand group paths as an argument because no checked computation here exposes an operand path list, so nothing derives them; the operand-error text and address spelling; and this project's own measurement of the *upward* half, which rests on the [peer's reproduction](sources/inbound-group-operand-batches.md#src-starred-field-operand-peer-reproduction) rather than on retained bytes here.
