...
- `remaining`: the implicit self-validation message's type on a computation carrying a **starred operand** is **measured and closed** at the [message-polarity checkpoint](SOURCES.md#src-starred-operand-message-polarity). It follows the established directional-fillability rule, with declared capacity as the boundary and headroom read per operator against its own quantity. Nothing is implemented: this project has no implicit computation self-validation message carrier, so the fact is recorded in `spec/` and waits for one.
- `remaining`: what that closure leaves open is narrow and named at the checkpoint's `limit` — an over-limit row's effect on the type, a nested or filtered star, a non-Number target, a starred operand inside a precondition, and whether a declared minimum interacts. Select any of them only with a message carrier in hand.
