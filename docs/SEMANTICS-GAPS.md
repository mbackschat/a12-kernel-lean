...
- Open: model-owned Boolean/Confirm declarations must retain exact configured stored true/false tokens before those count sources can reopen; concrete Boolean/Confirm ingestion and Number minimum-scale checking must retain raw representation separately from parsed values; remaining group/addressed expression placements must reuse their existing family owners.
...
- Current boundary: [§6](IMPLEMENTATION-MAP.md#6--dates-and-time) owns the implemented temporal surface. Signed Berlin `AddDays` and `AddMonths` are closed with operation-specific exact overlap landings; Berlin year shifts and constructed-Date differences remain excluded.
...
- Unresolved source fact: establish Berlin year correction and constructed-Date difference offset policy independently. Wider formats, legal zones/pre-floor identities, detailed basic-check errors, recursive Date expressions, repeatable placement, target/destination integration, and date-like distinct-count identity remain open. Pending peer reconciliation for constructor, precision, amount-source, and operation-specific overlap clauses is tracked in the [outbound ledger](A12-DMKITS-SPEC-SYNC-LEDGER.md).
