...
- `remaining`: extend selected-preliminary execution beyond the [named whole-call baseline](IMPLEMENTATION-MAP.md#cap-selected-computation-preliminary): isolated parallel Number, addressed multi-star Number `FirstFilledValue`, addressed Boolean, ordinary String, Custom, DateFragment, FULL Date, Time, and DateTime `FirstFilledValue`, indexed DateRange construction, addressed direct and multi-operand `FirstFilledValue` Enumeration, addressed `DateFromDateTime`, addressed `TimeFromDateTime`, addressed `Time(...)`, addressed sibling-star DateRange `FirstFilledValue`, addressed DateTime sub-day and calendar-day shifts, the Number-to-DateTime cascade producer source, the three-stage Enumeration cascade source, the Enumeration-to-`FieldValueAsNumber`-to-filtered-Enumeration cascade source, the fixed `CurrentRepetition` Number-to-String-to-Number producer source, and the four-stage filtered-Enumeration final fallback.
