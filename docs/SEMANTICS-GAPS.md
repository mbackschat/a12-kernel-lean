...
- `remaining`: the address **spelling** question is reframed rather than open as stated: retained artifacts carry **two dialects in one channel**, an instance one that elides index one and a declaration one that leads with a slash, elides nothing, and marks each iterated level `[0]` ([dialect checkpoint](sources/temporal-and-message-probes.md#src-message-address-dialects)). So the overload matrix and the runtime probe need not disagree at all. What stays open is which operand shapes select which dialect — a correlation over 22 captures, not a measured cause — and a `MessagePointer` carrying `fillToFix` must represent two address kinds rather than one shape with an optional index.
