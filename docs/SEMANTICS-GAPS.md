...
- Unresolved source fact: source-close the computed `Time` target boundary—admitted target formats/components, generated value shape, target basic checks, empty/poison behavior, and application—before widening the temporal target result family. The peer's adjacent day probes still do not calibrate the source-offset/refit mechanism, which remains pending under [`SPEC-2026-07-29-01`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-07-29-01). Wider formats, legal zones/pre-floor identities, detailed basic-check errors, repeatable placement, broader recursive lowering, and date-like distinct-count identity remain open.
