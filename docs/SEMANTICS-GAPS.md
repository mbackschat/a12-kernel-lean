...
- Unresolved source fact: close checked DateTime `AddDays` without conflating it with elapsed sub-day shifting or the Date-only month/year gate. General recursive lowering remains outside until a concrete third form requires more than the completed result bridges. The peer's adjacent day probes still do not calibrate the source-offset/refit mechanism, which remains pending under [`SPEC-2026-07-29-01`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-07-29-01). Wider formats, legal zones/pre-floor identities, detailed basic-check errors, repeatable placement, target/destination integration, and date-like distinct-count identity remain open.
