...
- `remaining`: **the clearing half is measured and the implementation is knowingly behind it.** A target cell holding a stored value in an over-limit row is `cleared`, so the row leaves the value-producing domain and stays in the clearing one. `computationRowEnvironments` currently drops the row outright, which is right for its value and wrong for its clear: a consumer applying the result would leave a stale value the Kernel removes. The correct shape is a no-value outcome at that row rather than an absent one, which every carrier's outcome type already expresses; the divergence is recorded here rather than left to the next reader of a green build.
