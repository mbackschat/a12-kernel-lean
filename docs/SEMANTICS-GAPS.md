...
- Unresolved source fact: feed one checked DateTime day result into direct-DateTime `DifferenceInDays` in either authored position while preserving exact instant, value-carrying omission provenance, and generated first-formal precedence without adding a temporal AST. The peer's adjacent day probes still do not calibrate the source-offset/refit mechanism, which remains pending under [`SPEC-2026-07-29-01`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-07-29-01). Wider formats, legal zones/pre-floor identities, detailed basic-check errors, repeatable placement, broader recursive lowering, target/destination integration, and date-like distinct-count identity remain open.
