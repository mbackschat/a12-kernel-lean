...
- `remaining`: the remaining kinds are **audited as a batch and no longer need one kernel round each** ([checkpoint](sources/cross-layer-routes.md#src-constant-literal-family-gate), outbound [`SPEC-2026-08-31-03`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-08-31-03)). TIME, ENUM, and DATE_FRAGMENT admit a bare constant and are implementable; DATETIME and DATERANGE have no known witness at that revision and stay unrepresentable for want of one, not by decision.
