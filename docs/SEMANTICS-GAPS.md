...
- Unresolved source fact: admit direct dynamic `AddDays(Now, amount)` under the checked model profile with explicit per-call `World`, exact milliseconds, numeric omission, and cause order before attempting the dynamic reverse composition. The peer's adjacent day probes still do not calibrate the source-offset/refit mechanism, which remains pending under [`SPEC-2026-07-29-01`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-07-29-01). Dynamic calendar-then-elapsed composition, wider formats, legal zones/pre-floor identities, detailed basic-check errors, repeatable placement, broader recursive lowering, and date-like distinct-count identity remain open.
