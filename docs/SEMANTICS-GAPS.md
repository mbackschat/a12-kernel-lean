...
- `remaining`: that projection is small and its inputs exist. `NumericComputationEvaluationContext` already carries `document` and `outer`, and `CheckedStarredGroupSource.inCapacityRowCount` already returns the exact quantity with over-limit rows excluded. What it needs is the arm that reads each operand into a `GroupCountOperandReading` and folds. The scalar route stays refusing, which is correct rather than incomplete.
