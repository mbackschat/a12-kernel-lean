...
- Open:
  ...
  - admit same-target multiplicity by flattening supplied computations into the existing per-target first-selected table; accepted [`SPEC-2026-07-26-03`](A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-07-26-03--same-target-computations-flatten-into-one-first-selected-table) supplies the selected-empty versus false-guard separator and forbids a second selector;
...
- Evidence status: maintained a12-dmkits differentials Kernel-lock isolated repeatable/index and value-less-target result-channel seams, typed Number V2 source identity at `7b2e9f7`, cause-blind computed-target dependency erasure at `351bf9f2`, and selected-empty same-target termination versus false-guard fall-through at `4ebe84fc`. Multi-step composition, finite repeatable plans, heterogeneous results, destination compatibility, and whole-document transition remain partially or wholly `external evidence pending`.
