...
- `remaining`: not yet written: **ENUM** (the only kind whose check a carrier must enforce at elaboration) and **DATE_FRAGMENT** (which needs a component-dropping renderer this project does not have). Neither needs a kernel round; both admission rules are measured at the [family-gate checkpoint](sources/cross-layer-routes.md#src-constant-literal-family-gate).
