...
**`And` is the only connective here because this route refuses the other one, not because the language lacks it.** The A12 filter condition is the ordinary condition production and admits `Or`, parentheses, presence leaves, and String comparisons; this one-group route's own well-formedness predicate requires a conjunctive core and rejects a disjunction at elaboration with a named diagnostic. Read the `Having` grammar above as this operation's admitted subset, and see the [open boundary](#open-boundary) before treating it as the shape of A12 correlation.
...
| Explicit self-exclusion | A conjunction beginning with that repetition inequality never selects the outer row; the reverse spelling follows from the root-swap law below, so the idiom's position is free on this arm and not on the computation arm | `explicitSelfExclusion_drops_outer` |
...
| Conjunct decomposition | A conjunction keeps an environment exactly when both operands keep it, as Booleans; this is the rewrite unit, not the three-valued truth | `correlatedHaving_and_keepsEnvironment` |
| Root operand swap | Swapping a top-level conjunction's two operands leaves the selected list identical, order included; validation arm and pure selector only | `correlatedHaving_selectEnvironments_and_comm` |
| Admission invariance | The same swap leaves environment well-formedness and the reopened-level requirement unchanged, so a legal filter stays legal; proved at both connectives | `correlatedHaving_admission_swap` |
| Truth-to-selection congruence | Two filters agreeing on truth at **every** frame select the same candidates in the same order; this is the general tool for a swap at any depth, since a nested rewrite reduces to agreement at the rewritten node | `correlatedHaving_selectEnvironments_congr` |

A source-to-source consumer needs the last three together, in that order: prove the rewritten condition agrees frame-by-frame, apply the congruence law to get equal selection, and apply admission invariance to know the rewritten model is still accepted. A rewrite that preserves meaning but yields a rejected model is worse than no rewrite, and the first two laws alone do not exclude it.
...
- operand-order invariance is **not** a property of the filter condition; it is a property of the **validation arm**. The kernel's computation arm reads a filter left to right and aborts on the first reached unavailable operand, at both connectives, so the identical swap that the three laws above prove selection-preserving here changes a computed result ([conjunction checkpoint](sources/group-and-iteration-probes.md#src-and-order-computation-arm), [`spec/07` §3](../spec/07-repetition-and-iteration.md)). This operation observes only validation firing membership, so nothing in this suite can detect the difference — which is exactly why it is recorded as a non-law. A consumer that reuses a rewrite proved against this suite on a computation is unsound, and no case here will tell it so.
...
- the filter's `Or`, its presence and String-comparison leaves, and its computation arm are all implemented and measured in the wider Lean theory but are outside **this suite**. The computation arm is the consequential omission: it is order-sensitive where this operation is not, per the non-law above. `Or` also composes differently from `And` under an unavailable operand — a true disjunct keeps its row where a false conjunct drops it — so a consumer generalizing from this suite's conjunctions to disjunctions is extrapolating, not reusing.
