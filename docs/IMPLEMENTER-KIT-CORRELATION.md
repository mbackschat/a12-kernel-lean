...
- operand-order invariance is **not** a property of the filter condition; it is a property of the **validation arm**. The kernel's computation arm reads a filter left to right and aborts on the first reached unavailable operand, at both connectives, so the identical swap that the three laws above prove selection-preserving here changes a computed result ([conjunction checkpoint](sources/having-filter-probes.md#src-and-order-computation-arm), [`spec/07` §3](../spec/07-repetition-and-iteration.md)). This operation observes only validation firing membership, so nothing in this suite can detect the difference — which is exactly why it is recorded as a non-law. A consumer that reuses a rewrite proved against this suite on a computation is unsound, and no case here will tell it so.
