...
| a12-dmkits maintainer or feedback exporter | [`A12-DMKITS-FEEDBACK.md`](A12-DMKITS-FEEDBACK.md) | Read the reconciled dispositions; F6 remains open, while capture-related items were superseded by the maintained boundary |
| a12-dmkits portable-capture implementer | [`A12-DMKITS-CAPTURE-PROPOSAL.md`](A12-DMKITS-CAPTURE-PROPOSAL.md) | The boundary is implemented and repaired; run active Prompt 2 against the exact [`string-direct-cascade-v1` question](../evidence/scenarios/string-direct-cascade-v1/README.md) |
| Semantic-capsule pipeline designer | [`SEMANTIC-CAPSULE-PIPELINE-PROPOSAL.md`](SEMANTIC-CAPSULE-PIPELINE-PROPOSAL.md) | The upstream schema exists; wait for the first official packet before factoring the shared Lean binder and registry |
...
| [`A12-DMKITS-FEEDBACK.md`](A12-DMKITS-FEEDBACK.md) | a12-dmkits maintainers and formalization researchers | Mostly dispositioned cross-project ledger; append only when a new concrete gap is found | Pinned suggestions and gaps discovered here for upstream review, with source links, recommendations, acceptance checks, and reconciled dispositions; never a second owner for a12-dmkits status |
| [`A12-DMKITS-CAPTURE-PROPOSAL.md`](A12-DMKITS-CAPTURE-PROPOSAL.md) | a12-dmkits owner and capture implementers | Adopted design record with active first-client handoff; later operational truth belongs to a12-dmkits docs | Source-maintained portable observation command, packet and qualification boundary, repaired V1 decision, first cascade acceptance scenario, upstream handback, and prompt templates; never a second owner for implementation details |
