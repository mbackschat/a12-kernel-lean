...
| Resuming agent | [`../CLAUDE.md`](../CLAUDE.md) | Resolve the agent profile through [`CODEX-MODEL-SELECTION.md`](CODEX-MODEL-SELECTION.md), then read [`PLAN.md`](PLAN.md) and the relevant gate in [`TESTING.md`](TESTING.md) |
...
| [`CODEX-MODEL-SELECTION.md`](CODEX-MODEL-SELECTION.md) | Agent operators and prompt authors | Current replaceable product mapping | Concrete Codex model/reasoning selection for the stable BEST/REGULAR/SIMPLE profiles, mandatory prompt headers, and enforcement-disclosure rules |
...
| [`PLAN.md`](PLAN.md) | Active maintainers and resuming agents | Volatile checkpoint, never an archive | Verified baseline, active semantic unit, immediate consumer probe, parked boundaries, and minimal resume procedure |
