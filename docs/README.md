...
| Fresh Rust correlation-probe operator | [`RUST-CORRELATION-CONSUMER-PROBE.md`](RUST-CORRELATION-CONSUMER-PROBE.md) | Start the external repository only after the owner gate; the semantic handover remains [`IMPLEMENTER-KIT-CORRELATION.md`](IMPLEMENTER-KIT-CORRELATION.md) |
...
| [`RUST-CORRELATION-CONSUMER-PROBE.md`](RUST-CORRELATION-CONSUMER-PROBE.md) | Owner and external probe operator | Volatile work order until the fresh probe is accepted or cancelled | Exact isolation boundary, allowlist, paste-ready prompt, expected artifacts, acceptance conditions, and phase-2 stop gate for the current Rust correlation experiment |
