...
| a12-dmkits maintainer or feedback exporter | [`A12-DMKITS-FEEDBACK.md`](A12-DMKITS-FEEDBACK.md) | Follow each pinned source link and transfer accepted work into a12-dmkits' own gap or plan surface; never edit the sibling checkout from this project |
| a12-dmkits portable-capture implementer | [`A12-DMKITS-CAPTURE-PROPOSAL.md`](A12-DMKITS-CAPTURE-PROPOSAL.md) | Use only after the current feedback queue is reported complete; reconcile the proposal with that upstream revision, then follow its staged prompts and handback contract |
| Semantic-capsule pipeline designer | [`SEMANTIC-CAPSULE-PIPELINE-PROPOSAL.md`](SEMANTIC-CAPSULE-PIPELINE-PROPOSAL.md) | Stage shared Lean binding, capsule registry, and shipment automation only after the official upstream packet schema is test-driven |
...
| [`ARTIFACTS.md`](ARTIFACTS.md) | Users and reviewers | Stable lifecycle guide | Contents, authority, generation, retention, immutability, and drift policy for `evidence/`, `reference/`, `examples/`, mutation packets/results, and generated-differential profiles/receipts |
...
| [`A12-DMKITS-FEEDBACK.md`](A12-DMKITS-FEEDBACK.md) | a12-dmkits maintainers and formalization researchers | Actionable cross-project queue | Pinned suggestions and gaps discovered here for later upstream review, with source links, recommendations, acceptance checks, and dispositions; never a second owner for a12-dmkits status |
| [`A12-DMKITS-CAPTURE-PROPOSAL.md`](A12-DMKITS-CAPTURE-PROPOSAL.md) | a12-dmkits owner and capture implementers | Active cross-project proposal; freeze with its accepted/rejected upstream revision and route later operational truth to a12-dmkits docs | Source-maintained portable observation command, packet and qualification boundary, first cascade acceptance scenario, upstream handback, and reconciled prompt templates; never upstream implementation status |
| [`SEMANTIC-CAPSULE-PIPELINE-PROPOSAL.md`](SEMANTIC-CAPSULE-PIPELINE-PROPOSAL.md) | a12-kernel-lean architecture and evidence maintainers | Staged proposal; adopted pieces move into their current-state owners and the remaining stages stay visibly proposed | Amortized scenario, shared binding, typed capsule registry, evidence import, shipment generation, and recapture workflow; never current implementation status—implemented facts belong in `ARCHITECTURE.md`, `TESTING.md`, `ARTIFACTS.md`, and `IMPLEMENTATION-MAP.md` |
