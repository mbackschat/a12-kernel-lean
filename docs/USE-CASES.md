...
**Current narrow example:** the internal addressed numeric-operation probe admits only the identity Transform over checked `FieldValueAsNumber`, `RangeAsNumber`, direct Number, `Abs`, Round, and nonempty direct-field `Min`/`Max` operations and proves that it preserves every rich addressed outcome and complete source-relative result view for every checked document. A separate Analyze fingerprint retains target, ordered source fields, repeatable scope, full target policy, conversion projection or interval, wrapper/mode/result scale, and extremum operation/derived scale; executable controls show that stored/category, changed-endpoint, direct/`Abs`, Round-mode, extremum-operation, source-order, cardinality, and cross-family fingerprints differ, but make no semantic-inequivalence claim.
