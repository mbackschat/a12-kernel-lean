...
A later in-context concrete-client probe at revision `40f75e8` exercised the checked authored repeatable-computation boundary rather than repeating the historical cold readback. A temporary Lean client imported the numeric-computation module, elaborated a filtered-star Sum inside arithmetic, derived an extensional dependency set by querying every declaration in the finite checked model, retained the checked `starHaving` payload, and evaluated it through the addressed context. The dependency result included the selected field plus the inner and outer `Having` fields and excluded the target; evaluation produced `14`. Removing `Having` dropped both filter-only dependencies and changed the same input's result to `19`, giving a concrete Transform rejection witness. The probe passes for checked-boundary **Execute**, dependency-oriented **Analyze**, and refusal-oriented **Transform** without a new API. It is not a cold knowledge-transport result, source transformation, general document adapter, scheduler, public result-order claim, or external implementation qualification. Its three caller-supplied read surfaces must still come from one coherent checked document route under [`SG1`](SEMANTICS-GAPS.md#sg1--general-checked-document-construction). No permanent probe source, protocol, runner, harness, or support estate was added.
