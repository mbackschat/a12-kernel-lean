...
| Resolved first-filled, aggregates, Date ranges, and temporal extrema | Execute, Transform, Explain | Consumers recovered prefix sensitivity, filter/missingness provenance, rounding and order constraints, and exact-instant temporal identity. The direct temporal computation profile reconstructs its fixed UTC carriers, clearing/application, and twelve outcomes from canonical clauses plus the calibrated source record. The checked direct DateRange-bound subset additionally exposes both stored policies, exact endpoint identity, fixed-Date comparison in either position, all four numeric components, and non-value behavior. BaseYear, fragments, repeatable addressing, wider comparison and overlap operands, computation targets, research-closed shipment, and public compatibility remain separate. |
