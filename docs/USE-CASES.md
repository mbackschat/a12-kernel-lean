...
| Selected computation preliminary | Compile, Execute | A cold compiler recovered full-call default staging, selected noncomputed mandatory and uniqueness scope, and the distinct direct, dynamic-key, and implicit-index dependency roles across generic, parallel Number, field-key DateRange, and literal-key DateRange artifacts. Independent specialized and generic implementations agreed exactly, root reconciliation matched all four Lean locks, and eight wrong accounts failed. A first Execute consumer then reused one generic preparer across Enumeration, FullDate, and Time results, reproduced four exact whole-call signatures, and killed ten wrong accounts. After three composed integrations, a second fresh Execute consumer carried the same preparation through a direct DateTime amount, a selected producer and cause-blind overlay, and a lazy fallback beneath completed overlays. Lean rejected its initially flattened direct-shift fixture; correcting only that topology yielded five exact signatures, 15 passing natural tests, and ten killed wrong accounts. This qualifies the bounded preparation and named family compositions, not generic scheduling, rendering, wider application, later validation, literal-key execution, Kernel correspondence, protocol, or shipment. |
...
| [Cross-category feasibility laboratory](consumer-probes/README.md) | All ten categories | All ten categories now have a green bounded task with disposable independent consumers and Lean reconciliation. Analyze is green for presence reachability and checked scalar numeric-table cycles, while broader family-neutral graph reconstruction remains amber because Lean has no general graph certificate. Explain became green after SG4 supplied the missing checked direct-field formal-input inventory and exact result projection. Compile now covers both a finite condition and one admitted generated numeric table, including common-before-local selection and complete static input projection. Execute covers that generated table through exact selected-operation and target-check outcomes over 108 finite assignments, then through source-relative public result and separate-destination application over 168 finite combinations, and now transports one selected-preliminary view across both independent result families and three composition seams. Initially green-looking calendar-day and prepared-composition candidates were each rejected by Lean and corrected, demonstrating temporal and topological failure detection. The lab supports the overall semantic-factory approach for bounded tasks, but does not establish general consumers, Kernel correspondence, shipment readiness, or public compatibility. |
