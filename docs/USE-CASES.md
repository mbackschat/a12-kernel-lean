...
The probe therefore passes for resolved-boundary **Execute**, **Explain**, and a small explicitly preconditioned part of **Transform** over its supplied homogeneous profile. It does not establish authored A12 evaluation or rule refactoring: checked path/star/group lowering, actual `Having`/`$`, partial relevance, per-source metadata construction, computation behavior, messages, a source transformation relation, and external kernel evidence remain outside the probe. Subsequent work closes checked model/path/raw construction only for nonempty unfiltered nonrepeatable `Sum`/`MinValue`/`MaxValue` field lists; the historical cold-reader probe was not rerun and its group/star/filter refusal remains valid. No shipment, protocol, runner, external implementation, qualification campaign, or harness was added.
