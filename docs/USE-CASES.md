...
| Resolved first-filled, aggregates, Date ranges, and temporal extrema | Execute, Transform, Explain | Consumers recovered prefix sensitivity, filter/missingness provenance, rounding and order constraints, and exact-instant temporal identity. Safe rewrites require explicit hypotheses; authored parsing, broader addressing, and external calibration remain separate. |
...
**Current narrow example:** the internal addressed numeric-leaf probe admits only the identity Transform over a checked `FieldValueAsNumber` or `RangeAsNumber` leaf and proves that it preserves every rich addressed outcome and complete source-relative result view for every checked document. A separate Analyze fingerprint retains target, source, repeatable scope, full target policy, and projection-or-interval parameters; executable controls show that stored/category, changed-endpoint, and cross-family fingerprints differ, but make no semantic-inequivalence claim.
...
**Current narrow examples:** the internal checked-flat pilot recognizes only the exact root shapes `FieldFilled(f) And FieldNotFilled(f)` and its reverse, returns a proof-bearing witness, and proves that the error condition can never fire. The addressed numeric-leaf probe separately reports one checked computation's expression-operand dependency, prior-target-state classification read, and written target, proves the operand differs from the target, and retains the bounded fingerprint used to detect parameter changes. Neither is a recursive lint, general reachability procedure, command, protocol, or supported shipment. Their precise boundaries are recorded in the [implementation map](IMPLEMENTATION-MAP.md).
