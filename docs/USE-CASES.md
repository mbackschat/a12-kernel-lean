...
**Current narrow example:** the internal addressed numeric-operation probe admits only the identity Transform over checked `FieldValueAsNumber`, `RangeAsNumber`, direct Number, root `Abs`, Round, and nonempty bounded `Min`/`Max` operations over direct or operand-local-`Abs` fields plus at most one immediate literal, and proves that it preserves every rich addressed outcome and complete source-relative result view for every checked document. A separate Analyze fingerprint retains target, ordered source fields, repeatable scope, full target policy, conversion projection or interval, wrapper/mode/result scale, and extremum operation plus exact ordered direct-field/local-`Abs`/literal identity; executable controls show that stored/category, changed-endpoint, direct/root-`Abs`, Round-mode, extremum-operation, source order, cardinality, direct/local-`Abs`, literal position, literal authored scale, and cross-family fingerprints differ, but make no semantic-inequivalence claim.
