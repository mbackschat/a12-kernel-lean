...
| Selected computation preliminary | Compile | A cold compiler recovered full-call default staging, selected noncomputed mandatory and uniqueness scope, and the distinct direct, dynamic-key, and implicit-index dependency roles across generic, parallel Number, field-key DateRange, and literal-key DateRange artifacts. Independent specialized and generic implementations agreed exactly, root reconciliation matched all four Lean locks, and eight wrong accounts failed. This qualifies the bounded preparation and result-channel transport, not scheduling, rendering, application, later validation, literal-key execution, Kernel correspondence, protocol, or shipment. |
