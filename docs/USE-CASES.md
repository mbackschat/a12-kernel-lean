...
| [Cross-category feasibility laboratory](consumer-probes/README.md) | All ten categories | All ten categories were exercised through bounded tasks with disposable independent consumers and Lean reconciliation. Eight category results are green; Analyze is green for presence reachability and amber for computation cycles because Lean has no general graph reconstruction; Explain is amber because the checked DateTime family does not own eager formal-input collection. One initially green-looking Execute candidate was rejected by Lean and corrected after a handover retrieval defect was fixed. The lab supports the overall semantic-factory approach for bounded tasks, but does not establish general consumers, Kernel correspondence, shipment readiness, or public compatibility. |
