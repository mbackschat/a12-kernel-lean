...
| Cross-family computation formal inputs | Analyze, Explain | Cold consumers recovered exact root, enclosing, and leaf placements across Time, FullDate, and String-shaped Enumeration results, then transported the multi-operation union through homogeneous Enumeration and heterogeneous Number-to-DateTime whole-call results. They separated static inventory from runtime reads and typed phase residuals, excluded computed targets and unrelated errors, normalized duplicates, and preserved operation-order-independent extensional findings. Group containment, generated preliminary findings, rendered messages, scheduling, structural failures, shipment, and Kernel correspondence remain outside the probes. |
...
| [Cross-category feasibility laboratory](consumer-probes/README.md) | All ten categories | All ten categories were exercised through bounded tasks with disposable independent consumers and Lean reconciliation. Nine category results are green; Analyze is green for presence reachability and amber for computation cycles because Lean has no general graph reconstruction. Explain became green after SG4 supplied the missing checked direct-field formal-input inventory and exact result projection. One initially green-looking Execute candidate was rejected by Lean and corrected after a handover retrieval defect was fixed. The lab supports the overall semantic-factory approach for bounded tasks, but does not establish general consumers, Kernel correspondence, shipment readiness, or public compatibility. |
