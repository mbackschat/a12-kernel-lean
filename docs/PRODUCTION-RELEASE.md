...
| Current 0.3.0/V2 implementer kits and conformance controls | Check finite current-reference cases under the V2 manifest and suite identities | Current [flat kit](IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md), [correlation kit](IMPLEMENTER-KIT-CORRELATION.md), [`flat-validation-empty-logic-v2.conformance.json`](../reference/flat-validation-empty-logic-v2.conformance.json), [`single-group-correlation-v2.conformance.json`](../reference/single-group-correlation-v2.conformance.json), and [`IMPLEMENTATION-MAP.md`](IMPLEMENTATION-MAP.md) | Development controls only; not a release-readiness declaration |
...
| `referenceSemanticsVersion` | [`Support.lean`](../A12Kernel/Reference/Support.lean) | Version of the executable reference boundary; currently 0.3.0 |
...
The correction from historical 0.2.0/V1 to current 0.3.0/V2 demonstrates the required versioning policy: empty unsigned Number `!= -1` changed from OMISSION to VALUE without changing wire protocol version 1. The retired identity, artifact digests, and exact recovery revisions are preserved only in the [archived experiment record](archived/REFERENCE-SEMANTICS-0.2.0-AND-RUST-EXPERIMENT.md). Protocol 1 requests and responses contain no reference-semantics selector, so a released consumer must inspect the exact executable's `--manifest` output and pin the executable or release digest. A release must not claim that a request alone selects or reveals the semantic account, and a current binary must not be presented as a replay engine for 0.2.0.
