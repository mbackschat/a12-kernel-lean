...
| Capability descriptor, evaluator shipment, and conformance suite | Pin one independently implementable semantic slice, its evidence classifications, exclusions, and cold-consumer contract | [`IMPLEMENTER-GUIDE.md`](IMPLEMENTER-GUIDE.md), [`flat-validation-empty-logic-v1.capability.json`](../reference/flat-validation-empty-logic-v1.capability.json), and shipment-specific artifacts | Flat eight-case development handover is generated and drift-checked; mutation-sensitivity checks passed for one exact source-declared exercise, the observable `Or` half of another, and one additional row-gate variant in a recorded isolated Rust implementation, while generated differentials and six exact source-declared exercises remain pending; correlation associations remain manual and its cold implementation is open |
...
A qualified release may carry only the narrow claim owned by [`PRODUCT-PROPOSAL.md`](PRODUCT-PROPOSAL.md):
