...
No production release has been qualified yet. The Lake package remains `0.1.0`, while the current executable semantics is 0.3.0; neither identifier is evidence of an immutable public release. Their mismatch must be mapped explicitly before publication rather than treated as an accidental equality requirement.
...
| Supported-fragment manifest | Machine-readable fail-closed operation and evidence boundary | [`Support.lean`](../A12Kernel/Reference/Support.lean) and current [`supported-fragment-v2.json`](../reference/supported-fragment-v2.json) | Implemented for the flat and one-group correlation development operations; not a release-readiness declaration |
| Frozen 0.2.0 flat capability descriptor, evaluator shipment, and v1 conformance suite | Pin one independently implementable semantic slice, its evidence classifications, exclusions, and cold-consumer contract | [`IMPLEMENTER-GUIDE.md`](IMPLEMENTER-GUIDE.md), [`flat-validation-empty-logic-v1.capability.json`](../reference/flat-validation-empty-logic-v1.capability.json), [`flat-validation-empty-logic-v1.conformance.json`](../reference/flat-validation-empty-logic-v1.conformance.json), and shipment-specific artifacts | The completed cold-consumer, mutation-qualification, and generated-differential history plus remaining release work are owned by [`IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md`](IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md); this historical shipment is not the current executable account |
| Current 0.3.0 v2 conformance controls | Check finite current-reference cases under the v2 manifest and suite identities | Current [`flat-validation-empty-logic-v2.conformance.json`](../reference/flat-validation-empty-logic-v2.conformance.json), [`single-group-correlation-v2.conformance.json`](../reference/single-group-correlation-v2.conformance.json), and [`IMPLEMENTATION-MAP.md`](IMPLEMENTATION-MAP.md) | Development controls only; no current 0.3.0 capability descriptor or self-contained implementer kit exists, so these suites are not a complete evaluator shipment or release-readiness declaration |
...
| `referenceSemanticsVersion` | [`Lineage.lean`](../A12Kernel/Reference/Lineage.lean), selected as current by [`Support.lean`](../A12Kernel/Reference/Support.lean) | Version of the executable reference boundary; currently 0.3.0, with 0.2.0 frozen as history |
...
The development lineage already demonstrates the required correction policy. Historical reference semantics 0.2.0 remains bound to source revision `9fa50276f5fb70dcd879b0a9712c8d69c0868967`, the principal SHA-256 digests listed in [`reference-semantics-lineage-v1.json`](../reference/reference-semantics-lineage-v1.json), and the selected 152-entry historical evidence/example/principal-artifact inventory in [`reference-semantics-0.2.0.lock.json`](../reference/reference-semantics-0.2.0.lock.json); human handover documents are outside that lock, and the post-revision separating request and replay receipt are bound separately. Current 0.3.0 changes empty unsigned Number `!= -1` from OMISSION to VALUE and uses v2 manifest and suite identities. Protocol 1 requests and responses contain no reference-semantics selector, so a released consumer must inspect the exact executable's `--manifest` output and pin the executable or release digest. A release must not claim that a request alone selects or reveals the semantic account, and a current binary must not be presented as a replay engine for 0.2.0.

The project version currently has a SemVer-shaped value, but no public release-versioning policy, bump matrix, or tag format has been adopted. Before the first release, decide how semantic support expansion, semantic correction, protocol/schema incompatibility, and evidence-only changes affect the project version; define the tag format; and bind the executable to its source/release identity through a `--version` surface or signed release metadata. The support manifest's semantic version identifies the executable account but does not identify a source commit or public release by itself.
