...
> **Status:** proposal, pending adoption, 2026-07-18. This document owns the proposed satisfiability, contradiction-analysis, accepted-document synthesis, and SMT integration architecture. Current sequencing remains in [`PLAN.md`](PLAN.md) and current executable/proof/evidence coverage in [`IMPLEMENTATION-MAP.md`](IMPLEMENTATION-MAP.md). No solver, package, runtime dependency, public command, or support claim is adopted by this proposal. Once adopted and implemented, durable contracts graduate to their owning documents and this proposal is retired or narrowed to unresolved work.
...
| [`ComputationConditionResult`](../A12Kernel/Semantics/ComputationCondition.lean), direct presence, ordered `And`/`Or`, and the operation-neutral first-match selector preserve clean not-true/no-match versus poison; [`evaluateOutcomeWhen`](../A12Kernel/Semantics/StringCascade.lean) separately consumes the common guard before one body | Reuse these result and selection boundaries; add only a correctly shared target/table representation, checked finite contexts, and the remaining condition/operation clauses needed by an adopted query | Supports poison-aware table analysis without pretending that direct presence or first-match selection already supplies condition partitions, implicit validation, or whole-model checking |
...
The exact checked-flat presence pilot is a proof-bearing shape recognizer, not this stage: it searches no domain and neither accepts nor returns a reachability witness. It is evidence that a narrow Analyze consumer can reuse an existing normalized semantic owner without a universal IR.
