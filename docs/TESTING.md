...
The mutation plan is source-maintainer test planning, not retained kernel evidence and not proof that a candidate was mutated. It derives canonical baseline verdicts and every unchanged case identity from the same typed capability descriptor, records the exact seven semantic changes and reviewed expected case deltas, and exhaustively derives all connective-table changes from the live verdict algebra where canonical cases expose only one half of a both-connectives defect. Its result-record requirements call for one mutation at a time, a prediction before editing, the exact patch and raw outputs, complete eight-case results, complete 32-cell algebra results for exercises 5 and 6, missing/additional-difference rejection, restoration after every exercise, and exact-file SHA-256 digests. They are explicitly marked `strictCheckerPending`; until that checker lands, they are requirements rather than a mechanically enforced result schema. Case and algebra results establish suite sensitivity; only the retained patch establishes that the requested internal mechanism was actually changed. The plan is deliberately outside the immutable first Rust cold bundle and does not change its capability, protocol, manifest, or semantic identity.
...
The flat [`capability descriptor`](../reference/flat-validation-empty-logic-v1.capability.json), suite, and [fixtures](../examples/reference-cli/flat-evidence/) are generated through the checked projection-to-protocol bridge before an independent implementation starts. Four flat cases are externally supported for final firing and polarity; the four externally silent cases deliberately source their exact `NotFired` or `Unknown` response from the Lean runtime projection because the observed message boundary cannot distinguish those states. The correlation suite still manually cross-references normalized fixtures to retained case IDs. This difference is part of the integrity claim and must not be blurred.

The selected manifest operation must be unique and its `externalEvidenceBoundary` must carry the same suite ID, `finiteRetainedCasesOnly` claim scope, and runtime/static counts actually present in that suite. The runner's common in-memory integrity self-test validates every canonical case's fixture metadata, evidence classification, and retained ID, then mutates compatibility, manifest, count, evidence, duplicate-member, and closed-object boundaries and requires rejection. The flat suite adds eight checks for the permitted external-support/expected-response-source combinations, yielding 24 guards; correlation retains 16 guards. Run either suite explicitly:
...
The current retained gate reports `42/42`: 36 runtime observations and six static authoring observations. Twelve runtime correlation observations across seven models agree in Groovy-dynamic, static-Java, and the a12-dmkits interpreter. They cover every currently admitted correlation comparison constructor plus the admitted empty and malformed filter-operand observations. The malformed-local witness establishes the observable claims that the malformed row is not kept and healthy equal-key siblings still select one another; it does not establish that a hidden kernel intermediate is Lean's `unknown`. Four additional static observations cover the all-outer rejection, mixed-scale equality rejection, mixed-scale ordering acceptance, and one sibling-repeatable inner-reference rejection. They do not establish a general bilingual authoring parser, a universal operator/diagnostic matrix, general repeatable path elaboration, filtered-result polarity, nested or multi-star binding, or general consumers. The sibling-repeatable observation must not be generalized to claim that every `$` expression or every cross-group reference is invalid. The exact retained cases and limits are indexed in [`EVIDENCE.md`](EVIDENCE.md).
