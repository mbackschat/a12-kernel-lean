...
The process-owned [`ArtifactTreeTest.lean`](../A12Kernel/Process/ArtifactTreeTest.lean) independently locks global portable-path ordering and symlink rejection for the remaining project-local artifact consumers. Ordinary direct-cascade replay no longer decodes or rechecks the retained packet, packet-diff, qualification, or scenario-mutation receipt trees: a12-dmkits owns those transport assurances and certifies one compact semantic bundle. Lean hashes that exact bundle before decoding, pins its producer revision and raw/qualification provenance references, and compares its typed observations with the evaluator. The complete raw trees remain immutable for upstream verification and audit rather than becoming an unverified replacement for the compact interface.
...
[`string-target-validation-projection.json`](../evidence/kernel-30.8.1/string-target-validation-projection.json), [`StringTargetValidationSchema.lean`](../A12Kernel/Evidence/StringTargetValidationSchema.lean), [`StringTargetValidationReplay.lean`](../A12Kernel/Evidence/StringTargetValidationReplay.lean), and [`StringTargetValidationBinding.lean`](../A12Kernel/Evidence/StringTargetValidationBinding.lean) own the nine-case target-validation lane. The input-only projection admits exactly one positive minimum or maximum plus nonempty conservative ASCII source/prior strings and carries no expected table; replay derives the outcome, payloadful delta, and exact three-state target application through the library semantics, while the binder independently compares those results with the retained case artifacts after binding the receipt, four models, nine cases, model constraints, operations, placements, two kernel strategies, and a12-dmkits triangulation. This actively calibrates accepted and errored application over absent and filled priors. The core target checker separately fails closed on produced CR/LF because this capsule does not model target line-break permission; input-ingestion normalization is modeled at an earlier checked-cell boundary. Initially present-empty, no-value, and poison application rows remain outside this lane and must not be inferred from it.
...
Existing retained receipts may describe historical disposable harnesses; their provenance remains immutable history and does not authorize repeating that method. a12-dmkits' [`CONFORMANCE-CORPUS-SPEC.md` §15–§15h](../../a12-rulekit/docs/CONFORMANCE-CORPUS-SPEC.md) owns the frozen raw capture, verification, qualification, mutation, and compact-export procedure. This repository owns only expectation-free scenario inputs, retained portable outputs, and the compact Lean replay described below; do not duplicate the upstream operational procedure here.

In the settled workflow, a12-dmkits performs the complete raw verification and certifies a compact semantic bundle. Retain the complete verified portable packet and packet receipt, packet-local capabilities, out-of-band packet-receipt digest, applicable PROFILE/REPORT/RECEIPT qualification sidecars and their out-of-band receipt digest, and the producer-certified compact bundle. Retain every runner result and declared fidelity gap in the opaque raw audit unit rather than only selected agreeing rows. Exclude kernel binaries, linked types or code, volatile logs, and machine-specific state. Confirm afterward that the sibling's tracked and visible-untracked status is exactly unchanged and remove disposable ignored outputs only after the official artifact has been handed back or deliberately rejected. If the command or required observation channel does not exist, record a specific upstream request and leave the capsule `external evidence pending`; use a temporary proposal only for a coherent capability with its own architecture and acceptance lifecycle. Never patch `RuntimeLaws`, add a temporary capture test, or modify a cloned, archived, or copied a12-dmkits source tree. The live raw command, verification, and compact-export contract belongs to a12-dmkits' [`CONFORMANCE-CORPUS-SPEC.md` §15–§15h](../../a12-rulekit/docs/CONFORMANCE-CORPUS-SPEC.md); the accepted first-client instance is the retained [`string-direct-cascade-v1` unit](../evidence/kernel-30.8.1/captures/string-direct-cascade-v1/README.md); the accepted [`compact pipeline plan`](SEMANTIC-CAPSULE-PIPELINE-PROPOSAL.md) owns future engagement and scaling limits.
