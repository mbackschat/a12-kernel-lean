...
Existing retained receipts may describe historical disposable harnesses; their provenance remains immutable history and does not authorize repeating that method. Every new external capture must use an unchanged, source-maintained a12-dmkits entry point. Before capture, record the sibling's visible status, require an explicit caller-owned ignored output directory, and verify the maintained command cannot rewrite the committed corpus. Collect Groovy-dynamic anchor and Java-static cross-route kernel observations plus interpreter triangulation through the source-owned portable schema; verify its closed inventory, receipt digest, runner and resolved-kernel identities, legality result, predeclared comparison policy, and absence of machine-specific data. Retain the complete verified portable packet/receipt, applicable qualification sidecars and their receipt digests, and the local typed projection; exclude kernel binaries, linked types or code, volatile logs, and machine-specific state. Confirm afterward that the sibling's tracked and visible-untracked status is exactly unchanged and remove permitted ignored outputs when they are no longer needed. If the command or required observation channel does not exist, record a specific upstream proposal and leave the capsule `external evidence pending`; never patch `RuntimeLaws`, add a temporary capture test, or modify a cloned, archived, or copied a12-dmkits source tree. [`A12-DMKITS-CAPTURE-PROPOSAL.md`](A12-DMKITS-CAPTURE-PROPOSAL.md) owns the requested future boundary, and [`SEMANTIC-CAPSULE-PIPELINE-PROPOSAL.md`](SEMANTIC-CAPSULE-PIPELINE-PROPOSAL.md) owns the staged Lean-side amortization.
