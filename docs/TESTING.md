...
Give a12-dmkits feedback as soon as a tool or skill defect makes semantic attribution unsafe, causes avoidable probe cycles, or prevents the smallest source-owned observation. A useful report names the exact a12-dmkits revision, `dmtool` and kernel versions, command family, minimal structured reproducer, expected and actual behavior, semantic-evidence impact, safe workaround if one exists, and concrete acceptance criteria. When the user requests a handoff document, maintain one dated Markdown note outside this repository, such as `A12-DMTOOL-FEEDBACK-YYYY-MM-DD.md`, update it after each probe capsule with new findings or an explicit no-new-defect result, and report its location to the user. Durable workflow belongs here; capsule provenance belongs in [`SOURCES.md`](SOURCES.md); issue history does not belong in either document.
