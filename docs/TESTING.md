...
The current retained gate reports `33/33`: 31 runtime observations and two static diagnostics. Seven correlation observations across four models agree in Groovy-dynamic, static-Java, and the a12-dmkits interpreter. This does not cover correlated filter-operand empty/malformed or unused admitted comparison combinations; those remain `external evidence pending` and should be closed before the repeatable surface broadens.
