...
External capture may write only under paths already ignored by the sibling repository. Before capture, record the sibling's visible status and verify every sibling output path with `git check-ignore`. A disposable harness may run from an ignored adapter workspace under `../a12-rulekit/build/`; when isolation from the live checkout is useful, a digest-pinned disposable source archive may instead be extracted and executed from the system temporary directory while its output path remains an ignored build location recorded by the receipt. Invoke the existing [`CorpusEngines`](../../a12-rulekit/adapter/src/test/kotlin/io/github/mbackschat/a12/dm/adapter/laws/CorpusEngines.kt) or [`CorpusCapture`](../../a12-rulekit/adapter/src/test/kotlin/io/github/mbackschat/a12/dm/adapter/laws/CorpusCapture.kt) patterns and collect Groovy-dynamic, static-Java, and interpreter results without modifying tracked sources. A purpose-built capture receipt must bind the disposable harness digest, normalized command, required environment, source revision, and exact outputs; after the harness is removed, those fields are retained historical assertions, and any normalization digest whose algorithm is not shipped cannot be independently recomputed here. Do not run the live [`CorpusGenerator`](../../a12-rulekit/adapter/src/test/kotlin/io/github/mbackschat/a12/dm/adapter/laws/CorpusGenerator.kt), because its normal job rewrites tracked corpus output. Copy only portable own-domain model, observation, and receipt artifacts into this repository, remove every disposable archive, source, and output, and confirm afterward that the sibling's tracked and visible-untracked status is exactly unchanged. Never leave a temporary source, model, projection, or generated corpus case visible in a sibling worktree.
