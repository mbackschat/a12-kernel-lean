...
Keep a probe small and reproducible. Record the clean a12-dmkits revision plus `dmtool` and kernel versions in [`SOURCES.md`](SOURCES.md), preserve sibling status before and after, and write scratch models only beneath an already ignored project-local or sibling path. Orient with `model describe`, inspect exact declarations with `field read`, consult `operators <id>`, batch known setup edits atomically, and use caller-tagged dry-run checks for the separating matrix. Persist at least one accepted candidate through a structured write, read it back structurally, and finish with `model check`; pair it with the nearest negative control and exact rejection code. This persisted-read-back consistency check is required while `dmtool` remains a comparatively new evidence route, because a successful dry run alone does not establish that the intended structure survived authoring.
