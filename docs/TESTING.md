...
Retain the complete verified portable packet and packet receipt, packet-local capabilities, out-of-band packet-receipt digest, applicable PROFILE/REPORT/RECEIPT qualification sidecars and their out-of-band receipt digest, and the local typed projection. Retain every runner result and declared fidelity gap rather than selected agreeing rows. Exclude kernel binaries, linked types or code, volatile logs, and machine-specific state. Confirm afterward that the sibling's tracked and visible-untracked status is exactly unchanged and remove disposable ignored outputs only after the official artifact has been handed back or deliberately rejected. If the command or required observation channel does not exist, record a specific upstream proposal and leave the capsule `external evidence pending`; never patch `RuntimeLaws`, add a temporary capture test, or modify a cloned, archived, or copied a12-dmkits source tree. [`A12-DMKITS-CAPTURE-PROPOSAL.md`](A12-DMKITS-CAPTURE-PROPOSAL.md) owns the repaired boundary and active Prompt 2; [`SEMANTIC-CAPSULE-PIPELINE-PROPOSAL.md`](SEMANTIC-CAPSULE-PIPELINE-PROPOSAL.md) owns staged Lean-side amortization.
