...
- **Owned-document accuracy:** the selected gap, implementation-map section, plan checkpoint, and every other document owner triggered by the unit contain no sentence the unit made false, no stale exclusion, and no completed narrative in an open-only owner.
...
- **Documentation ownership beyond the routine status owners:** when a document's public responsibility, source route, or resumption contract changes.
