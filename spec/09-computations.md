...
- A direct operation-form `RoundDown`, `RoundUp`, `RoundAccounting`, or `Abs` may consume an admitted `FieldValueAsNumber(...)` or `RangeAsNumber(...)` source. Rounding uses its authored scale and preserves missing-source movement; absolute value preserves source scale, turns symmetric missing conversion zero into grow-only magnitude, and retains the range source's already grow-only nonnegative zero. Computation maps every clean missing result to numeric zero and preserves formal unavailability as poison ([§5](04-numbers-and-decimals.md)).
