...
> **Lean modelling note.** Keep a Number target's exact decimal representation as `{unscaled, scale}` through target checking and typed change comparison: kernel V2 uses scale-sensitive decimal equality, so `7` and `7.00` differ even though their rational amounts agree. Render that representation only at the document boundary, and parse the stored representation on downstream reads; model-configured decimal-separator materialization belongs to that boundary rather than the normalized decimal core. Keep the computed-result formal check a *reduced* variant (no charset/blank baseline) distinct from the input formal check of [§3](02-logic-and-formal-errors.md).
