...
- A direct operation-form `RoundDown`, `RoundUp`, `RoundAccounting`, or `Abs` may consume an admitted `FieldValueAsNumber(...)`, `RangeAsNumber(...)`, numeric field-list aggregate, temporal numeric component, Base-Year-derived component, or Date-only month/year-difference source. Direct numeric `BaseYear` remains rejected because it is the immediate constant child rather than a number-producing operation. Rounding uses its authored scale and preserves missing-source movement; absolute value preserves source scale, runs after the aggregate fold, turns symmetric missing conversion/component/difference or bidirectionally fillable all-empty Sum/extremum zero into grow-only magnitude, and retains already grow-only range or distinct-count zero. Computation maps every clean missing or all-empty result to numeric zero and preserves formal unavailability as poison ([§5](04-numbers-and-decimals.md), [§6](05-dates-and-time.md)).
...
- [ ] **Direct numeric wrappers**: reuse the one source-specific operation-form admission for conversion, range, aggregate, temporal-component, Base-Year-derived component, and Date-difference sources; reject direct numeric `BaseYear`; preserve wrapper scale, clean zero, signed `Abs`, formal poison, and generated validation without a source-specific evaluator.
