...
- A direct operation-form `RoundDown`, `RoundUp`, `RoundAccounting`, or `Abs` may consume an admitted `FieldValueAsNumber(...)`, `RangeAsNumber(...)`, or numeric field-list aggregate source. Rounding uses its authored scale and preserves missing-source movement; absolute value preserves source scale, runs after the aggregate fold, turns symmetric missing conversion or bidirectionally fillable all-empty Sum/extremum zero into grow-only magnitude, and retains already grow-only range or distinct-count zero. Computation maps every clean missing or all-empty result to numeric zero and preserves formal unavailability as poison ([§5](04-numbers-and-decimals.md)).
