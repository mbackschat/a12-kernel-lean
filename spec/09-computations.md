...
Do not put every domain-undefined numeric operation in this clean-empty class. Division by zero belongs to the poison class in §3.2 when the result remains in the numeric family: the invalid numeric result survives legal `Abs`, `Min`, `Max`, and `RoundAccounting` wrappers, marks a Number target invalid, emits `berechnungsWertFehler`, and makes a dependent read poison. A Date-valued consumer projects the same missing numeric amount differently: `AddDays(StartDate, RoundAccounting(div, 2))` produces no Date value, clears a stale Date target, and does not attach `berechnungsWertFehler` to that Date target. These are consuming-operation rules, not contradictory global classifications. The a12-dmkits focused controls confirm the numeric formal-error and dependent-read channels through both kernel routes and retain fresh no-value-output versus stale CLEARED only as delta projections; those delta observations do not imply a clean-empty Number target.
