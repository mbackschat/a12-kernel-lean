...
- A direct operation-form `RoundDown`, `RoundUp`, `RoundAccounting`, or `Abs` may consume an admitted `FieldValueAsNumber(...)` source. Rounding uses its authored scale and preserves missing-source movement; absolute value preserves the conversion's scale and turns symmetric missing zero into grow-only magnitude. Computation maps either clean missing result to numeric zero and preserves formal unavailability as poison ([§5](04-numbers-and-decimals.md)).
