...
- An operation-form `RoundDown`, `RoundUp`, `RoundAccounting`, or `Abs` may be the root or an ordinary operand inside larger arithmetic and consumes one complete admitted plain-arithmetic child, including direct or grouped numeric `BaseYear`, a direct `FieldValueAsNumber(...)`, `RangeAsNumber(...)`, numeric field-list aggregate, temporal numeric component, Base-Year-derived component, or Date-only month/year-difference source. Only immediate or grouped numeric literals are rejected; wrapper-over-wrapper and wrapper-over-extrema forms remain separate. The child retains its own division, power, target-reference, and domain-failure checks, while enclosing division traversal crosses the wrapper and the wrapper structurally separates an outer direct-left power. Rounding uses its authored scale and preserves the child's movement; absolute value preserves child scale, runs after the complete child or aggregate fold, turns symmetric missing conversion/component/difference or bidirectionally fillable all-empty Sum/extremum zero into grow-only magnitude, and retains fixed Base-Year or already grow-only range/distinct-count values. Computation maps every clean missing or all-empty result to numeric zero and preserves formal unavailability as poison ([§5](04-numbers-and-decimals.md), [§6](05-dates-and-time.md)).
...
- [ ] **Numeric wrappers**: admit operation-form rounding/`Abs` at ordinary arithmetic operand positions and delegate each to one complete checked plain-arithmetic child, including direct/grouped numeric `BaseYear`, conversion, range, aggregate, temporal-component, Base-Year-derived component, and Date-difference sources; reject immediate/grouped literal, nested-wrapper, and extrema children; preserve body and enclosing authoring checks, target references, wrapper scale, clean zero, signed `Abs`, formal poison, and generated validation without a source-specific evaluator.
