...
- A root operation-form `RoundDown`, `RoundUp`, `RoundAccounting`, or `Abs` may consume a complete admitted plain-arithmetic expression, including a direct `FieldValueAsNumber(...)`, `RangeAsNumber(...)`, numeric field-list aggregate, temporal numeric component, Base-Year-derived component, or Date-only month/year-difference source. Direct or grouped numeric `BaseYear` and literal children remain rejected as immediate constants. The child retains its own division, power, target-reference, and domain-failure checks. Rounding uses its authored scale and preserves the child's movement; absolute value preserves child scale, runs after the complete child or aggregate fold, turns symmetric missing conversion/component/difference or bidirectionally fillable all-empty Sum/extremum zero into grow-only magnitude, and retains already grow-only range or distinct-count zero. Computation maps every clean missing or all-empty result to numeric zero and preserves formal unavailability as poison ([§5](04-numbers-and-decimals.md), [§6](05-dates-and-time.md)).
...
- [ ] **Root numeric wrappers**: delegate operation-form rounding/`Abs` to one complete checked plain-arithmetic child, including conversion, range, aggregate, temporal-component, Base-Year-derived component, and Date-difference sources; reject immediate literal and numeric `BaseYear` children; preserve body-local authoring and target-reference checks, wrapper scale, clean zero, signed `Abs`, formal poison, and generated validation without a source-specific evaluator.
