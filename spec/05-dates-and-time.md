...
**Overlap admits its operands by year class, and applies no component-set gate.** Every compared operand must include the year or none may; a mix is refused `MVK_DATE_WITH_AND_WITHOUT_YEAR`. The gate reads the whole operand list rather than a distinguished pair, so a list of three or more is refused on any mix, and every declared spelling of one yearless component set behaves alike. A declared Base Year completes each yearless operand and therefore admits every list, including `yyyy-MM-dd` beside `MM`; within one year class the operator admits differing component sets outright, unlike the equality gate above. A completed yearless operand occupies the Base Year alone, so an exact operand naming another year is disjoint from it, while an exact range spanning into the Base Year still meets the completion.
