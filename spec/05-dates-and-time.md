...
Constructed-Date calendar shifts also retain the exact model-zone instant selected by their particular `GregorianCalendar` mutation; equal decoded labels do not imply equal results. Under `Europe/Berlin`, forward `AddDays(Date(30, 9, 1916), 1)` lands on the earlier repeated-midnight instant `1916-09-30T22:00:00Z` (CEST), so it is not equal to freshly constructed `Date(1, 10, 1916)`, which resolves to `1916-09-30T23:00:00Z` (CET). `AddMonths` and `AddYears` use distinct field-mutation paths from `AddDays`: both signed month routes from 1 September/November 1916 and both signed year routes from 1 October 1915/1917 into 1 October 1916 select the later CET instant and therefore equal the fresh target. Month/year shift sign does not select the overlap side, and the `AddDays` no-overshoot selector must not be reused for either operation. A `Calendar.YEAR` landing in a gap likewise normalizes to the post-gap wall label rather than using forward day landing; the February-28 correction is applied around that year mutation.
