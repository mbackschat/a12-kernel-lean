...
- Operand-list `Min`/`Max` and field-list/star `MinValue`/`MaxValue` also admit homogeneous full-component DateTime operands whose declared formats expose the same component set. Selection compares each resolved physical instant, not its rendered wall label: check-relevant valueless operands are skipped, an all-empty selection has no value, any reached non-relevant operand propagates non-relevance, and an empty, omitted-tail, or encountered-filter source retains symmetric missing provenance on a selected value. An exact selected instant therefore remains distinct from an equal-looking stored value on the other side of a model-zone overlap.
