...
Constructed-Date calendar shifts also retain the exact model-zone instant selected by their particular `GregorianCalendar` mutation; equal decoded labels do not imply equal results. Under `Europe/Berlin`, forward `AddDays(Date(30, 9, 1916), 1)` lands on the earlier repeated-midnight instant `1916-09-30T22:00:00Z` (CEST), so it is not equal to freshly constructed `Date(1, 10, 1916)`, which resolves to `1916-09-30T23:00:00Z` (CET). `AddMonths` uses a distinct field-mutation path: both `AddMonths(Date(1, 9, 1916), 1)` and `AddMonths(Date(1, 11, 1916), -1)` select that later CET instant and therefore equal the fresh target. Month-shift sign does not select the overlap side, and the `AddDays` no-overshoot selector must not be reused for this operation.
