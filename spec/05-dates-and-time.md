...
- [ ] Fragment completion: earliest for a value/start, latest for a range end (leap-aware); day-optional `00`; `ValueAsDate` in direct comparison plus the explicit Date addition/difference and `DateTime(...)` Date-operand positions, but not ordinary Date extraction.
