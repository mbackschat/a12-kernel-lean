...
`Date(Day, Month, Year)` builds a date from numeric parts (also: a 2-argument form supplying the year from the Base Year, and a 4-argument century form). For the direct three-Number-field form, each field must declare no fractional digits, admit only nonnegative values by its sign policy or an explicit minimum, and have either the complete stored-length bound for its position (2 for Day/Month, 4 for Year) or the positional maximum: exactly 31 for Day, exactly 12 for Month, or any complete-Year maximum from 1000 through 9999. `maxIntegerDigits` is not the stored-length alternative. `Time(...)` and `DateTime(...)` construct the corresponding values. A constructed date is **invalid** if any referenced part is unfilled, or the parts don't form a real calendar date (`Date(31, 11, 2000)` — November has 30 days; or 30 February).
