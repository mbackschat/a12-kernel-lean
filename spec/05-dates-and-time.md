...
`Date(Day, Month, Year)` builds a date from numeric parts (also: a 2-argument form supplying the year from the Base Year, and a 4-argument `Date(Day, Month, Century, ShortYear)` form). For a direct Number-field component, the field must declare no fractional digits, admit only nonnegative values by its sign policy or an explicit minimum, and have either the complete stored-length bound for its position or the positional maximum. Day accepts width 2 or maximum 31; Month accepts width 2 or maximum 12; complete Year accepts width 4 or any maximum from 1000 through 9999; Century accepts width 2 or any maximum from 10 through 99; Short-Year accepts width 2 or exactly 99. `maxIntegerDigits` is not the stored-length alternative. The four-part form evaluates Day, Month, Century, then Short-Year and forms the year as `Century × 100 + ShortYear`; a fixed Short-Year zero is present, while an unfilled part keeps the construction incomplete. `Time(...)` and `DateTime(...)` construct the corresponding values. A constructed date is **invalid** if any referenced part is unfilled, or the parts don't form a real calendar date (`Date(31, 11, 2000)` — November has 30 days; or 30 February).
