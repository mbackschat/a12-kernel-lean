...
- **A `Calendar.DAY_OF_MONTH` landing assumes the source instant's own offset**, not an offset chosen from the signed amount. The nominal target advances the source wall label by whole days while retaining its clock and millisecond. If the offset actually in force at that candidate differs, the target is re-fitted to the actual offset and that re-fit is kept only when it preserves the target civil date; otherwise the first candidate stands. Adjacent transition probes can resemble a direction rule because their source already carries the offset that rule would choose. Long-range controls separate the accounts: `2023-08-31T02:30 + 213 days` reaches the 2024 spring gap and renders `03:30` CEST, while a backward landing onto the 1916 `23:00–24:00` gap preserves 30 April as `22:30` CET instead of changing the civil date to 1 May.
