...
`Valid(Field, "CustomTypeName")` and `Invalid(Field, "CustomTypeName")` are the other forms. Static model checking accepts any nonempty type name and does not require registry membership. With a registered validator they are complementary for a filled value and neither fires for an empty value. On the raw kernel 30.8.1 rule surface, an unregistered name is degenerate rather than rejected: both predicates fire for a filled field and neither fires for an empty field. The checked Lean profile may require registry membership during elaboration, but that is a deliberate narrowing of the raw kernel boundary, not kernel behavior.
