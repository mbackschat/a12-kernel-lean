...
**A pointer is written in one of two spellings, and one message routinely carries both.** The **instance** spelling is bare and elides a repetition-one coordinate at every level, so it varies with the document: `Order/Lines[2]/Produced`. The **declaration** spelling leads with a slash and elides nothing, writing each level the rule's iteration did not bind as `[0]` while every bound ancestor carries its concrete 1-based row: `/Order[1]/Lines[1]/Entries[0]/Choice`, and `/Probe[1]/Outer[2]/Flat[1]/Rows[0]/RowR` for a group operand under a repeatable ancestor at its second row. A non-repeatable level carries `[1]` in that spelling, so `[0]` marks an unbound iterated level rather than row zero. **Which spelling a pointer takes is decided by whether its resolved path passes a repeatable level the rule's iteration left unbound**, not by whether the operand was starred, expanded from a group, or deeper than the anchor: a fixed unstarred group operand reaches the declaration spelling through a repeatable descendant and is byte-identical there to the starred field's, while a group expanding to non-repeatable fields only stays bare, and two references inside one `Having` split across the two spellings according to nothing but the `$` capture. A consumer therefore cannot recover the authored operand form from a pointer, and must represent two address kinds rather than one shape with an optional index ([checkpoint](../docs/SOURCES.md#src-address-dialect-selector)). The rendered message **text** is a third spelling again, carrying a bracket-free declaration path and stating any index as a separate rendered number.
