...
- **Groups** are internal nodes. Each group is **non-repeatable** (occurs at most once wherever its parent occurs) or **repeatable** (declares an ordered 1-based row-address range `1 … repeatability`; a valid document may instantiate only a prefix `1 … rowCount`, where `rowCount ≤ repeatability`). The model's root is a single group. **`repeatability` is a finite integer of at least two, and it is what makes a group repeatable**: a group declaring none is non-repeatable rather than unbounded, so a wildcard over it is refused `MVK_INVALID_WILDCARD` while its bare name is admitted as an ordinary group operand. A `null` or `0` maximum is refused `MVK_MAX_VALUES_INVALID`. There is therefore no unbounded repeatable group; an implementation may keep an optional maximum as a total function but must not expose the absent case as an authorable model ([checkpoint](../docs/SOURCES.md#src-repeatability-declaration-domain)).
