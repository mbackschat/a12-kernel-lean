...
a12-dmkits' dual-strategy [`AppliedCellStateDiffTest`](../../a12-rulekit/adapter/src/test/kotlin/io/github/mbackschat/a12/dm/adapter/laws/AppliedCellStateDiffTest.kt) (IF126) locks the same-document target-cell cases, where a CLEARED action can exist only for a source-present target. [`SeparateDestinationClearDiffTest`](../../a12-rulekit/adapter/src/test/kotlin/io/github/mbackschat/a12/dm/adapter/laws/SeparateDestinationClearDiffTest.kt) at revision `f6dc22d24cc865c02c7f5b3beec5c0cff0bec140` separately locks retained-result application: CLEARED creates a present-empty target and the missing directly addressed `Lines` row, no source action and ERRORED preserve absence, changed VALUE creates that row and the exact value, and CLEARED empties a filled destination target. Both kernel strategies agree. Complete directly addressed ancestry for deeper paths is the source-fidelity account of the type-neutral V2 placement owner, not an additional runtime observation. The maintained route does not expose repetition count, so predecessor-row padding and created-row order are not claimed here.
