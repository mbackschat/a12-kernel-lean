...
- **Groups** are internal nodes. Each group is **non-repeatable** (occurs at most once wherever its parent occurs) or **repeatable** (declares an ordered 1-based row-address range `1 … repeatability`; a valid document may instantiate only a prefix `1 … rowCount`, where `rowCount ≤ repeatability`). The model's root is a single group. **`repeatability` is a finite integer strictly greater than zero, and it is what makes a group repeatable**: a group declaring none is non-repeatable rather than unbounded, so a wildcard over it is refused `MVK_INVALID_WILDCARD` while its bare name is admitted as an ordinary group operand. A `null`, `0`, or negative maximum is refused `MVK_MAX_VALUES_INVALID` by the consistency gate — the reader accepts all three, so the refusal is not visible at load — and the diagnostic names each affected element separately, with `0` and `null` drawing *"Only values greater than 0 are allowed."* and a negative drawing *"Only values without a sign are allowed."*. **`1` is admitted**, and is a non-repeatable group spelled explicitly. There is therefore no unbounded repeatable group; an implementation may keep an optional maximum as a total function but must not expose the absent case as an authorable model ([checkpoint](../docs/SOURCES.md#src-repeatability-declaration-domain), [inbound domain measurement](../docs/SOURCES.md#src-repeatability-domain-peer-measurement)).
...
Ordinary `referenced` pointers are a **structural projection of the checked condition, not a trace of the values read during evaluation.** Consequently a later `FirstFilledValue` fallback stays referenced even when an earlier operand determined the value. A starred *field* retains wildcard `0` at its reopened group context. A *group* operand of either repetition shape expands recursively to its descendant field pointers rather than yielding a group pointer, and **one depth rule gives both their coordinates**: a repeatable level *above* the operand stays concrete at the firing row, and every repeatable level *at or below* it is wildcarded. For a starred group that boundary is its own star, and every reopened group-context part beneath it is wildcarded — **including a non-repeatable descendant group context**. For an unstarred group the boundary is the operand's own group, which the wildcard gate keeps non-repeatable, so the wildcarding begins at the first repeatable level inside the subtree: `/Shipment/Carrier` reaches `/Shipment[1]/Carrier[1]/Handoffs[0]/Site` beside `/Shipment[1]/Carrier[1]/Name`. The two forms therefore differ in **where** that boundary falls, not in whether one wildcards at all, and the expansion's wildcarded address is a different address of the same firing from the message's own anchor, which stays concrete throughout ([§9](07-repetition-and-iteration.md)). The at-or-below half is measured on both forms, and the concrete half is measured on the **unstarred** one: with a `max 2` repeatable ancestor above a fixed group operand, one document fires the rule twice and each firing's expansion carries that ancestor's own row — `/Probe[1]/Outer[1]/Flat[1]/Rows[0]/RowR` beside `/Probe[1]/Outer[2]/Flat[1]/Rows[0]/RowR` — while the wildcarding still begins at the first repeatable level inside the operand's subtree. For the **starred** form the concrete half is still stated from the rule rather than from a witness. The relation is identical in the single-operand presence, entity-list quantifier, and numeric filled-count positions, and it is what lets an error field inside the operand group satisfy the reference gate indirectly in all three. `Having` field operands and the selected starred value field are included; an unmarked candidate reference is wildcarded, while a `$` reference resolves at its concrete captured outer row; `CurrentRepetition` contributes no field pointer. Membership is set-valued, so no pointer order is specified.

**A message's `referenced` and `fillToFix` entries are partially-known addresses, and one message routinely carries both kinds.** Each is a path plus one coordinate per path part. A part carries its concrete **1-based** row wherever the rule's iteration bound that level, and every non-repeatable part carries `1`; a repeatable level the iteration left **unbound** carries `0` instead, and one address may carry several. So `0` marks an unbound iterated level and is not row zero: an address is byte-identical across documents holding two rows, two rows, and no rows at all. **What decides a level's coordinate is whether the rule's own iteration bound it**, not whether the operand was starred, expanded from a group, or deeper than the anchor: a fixed unstarred group operand reaching through a repeatable descendant produces the same address as a starred field's, a group expanding to non-repeatable fields only leaves nothing unbound, and two references inside a single `Having` differ by nothing but the `$` capture ([checkpoint](../docs/SOURCES.md#src-address-dialect-selector)). An address therefore either names exactly one cell or does not, and a consumer must represent both kinds rather than one shape with an optional index — but it cannot recover the authored operand form from the address, which has to come from the checked condition.

**A12 owns a canonical string only for the exact form** — slashless, repetition one elided, as [§2.1](#21-repetition-contexts-the-iteration-environment) states — and defines neither parser nor renderer for the partially-known one. An observer that prints both kinds therefore chooses its own second form, and two differently-spelled addresses in one artifact may be that choice rather than anything the engine emitted; read the coordinates, not the spelling. The rendered message **text** is a further surface again, carrying a bracket-free declaration path and stating any offending index as a separate rendered number rather than inside the path.
...
- an optional **common precondition**, then one or more **alternatives**. Each alternative contains its operation, an optional precondition, and an optional fixed `toleranceRangeOp`; only the sole alternative may omit its precondition, while every row of a multi-alternative table is guarded ([the complete selection account](09-computations.md#5-where-a-computation-runs--scope-and-the-parallel-join)). The tolerance metadata does not affect first-match selection or operation evaluation; it changes only that alternative's implicit generated mismatch from strict `!=` to the named fixed tolerance-band predicate ([the precise generated-rule account](09-computations.md#6-the-implicit-validation-rule-precisely)). Alternatives are tried top-to-bottom: an absent precondition selects directly, clean false/unknown falls through, a poisoned read aborts, and the **first** selected alternative ends the scan even if its operation later produces no value. No match ⇒ the target is **CLEARED**.
...
- model **include/expansion** (resolving `TypeDefinition`s and included submodels) — assume an already-expanded model. **What makes this exclusion safe is measured, not assumed:** a typedef field is its base kind at every gate tested — a numeric aggregate's kind gate, the distinct count's homogeneity gate, and an ordering comparison — each row matching its plain-base-kind control under that gate's own code, so expansion loses no observable distinction ([checkpoint](../docs/SOURCES.md#src-typedef-resolves-to-base-kind)). A typedef over a config-bearing base kind, where a declared format or domain might not survive resolution, is untested;
