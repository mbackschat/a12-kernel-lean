...
Ordinary `referenced` pointers are a **structural projection of the checked condition, not a trace of the values read during evaluation.** Consequently a later `FirstFilledValue` fallback stays referenced even when an earlier operand determined the value. A starred *field* retains wildcard `0` at its reopened group context. A starred *group* expands recursively to its descendant field pointers rather than yielding a group pointer, and every reopened group-context part beneath the star is wildcarded — **including a non-repeatable descendant group context.** `Having` field operands and the selected starred value field are included; an unmarked candidate reference is wildcarded, while a `$` reference resolves at its concrete captured outer row; `CurrentRepetition` contributes no field pointer. Membership is set-valued, so no pointer order is specified.
