...
- **The operation-form `RoundDown`, `RoundUp`, `RoundAccounting`, and `Abs` wrappers admit a direct `FieldValueAsNumber(...)` child.** The wrapper checker rejects an immediate literal child (including a bracketed literal) and requires a number-like operation; it does not establish a recursive constant-free-subtree rule. Rounding replaces the static scale with the authored places (omitted means 0), preserves the conversion's directional fillability, and maps its selected exact amount through the ordinary rounding operation. `Abs` preserves the conversion's derived scale and transforms fillability by the current sign: symmetric missing zero becomes grow-only magnitude because filling can increase `|0|` but cannot decrease it. Computation produces the corresponding rounded or absolute amount, maps missing to the ordinary numeric zero, and preserves an exact formal cause as poison. These direct-source forms do not imply that every nested wrapper/source composition is admitted.
