...
- **The operation-form `RoundDown`, `RoundUp`, `RoundAccounting`, and `Abs` wrappers admit a direct `FieldValueAsNumber(...)` or `RangeAsNumber(...)` child.** The wrapper checker rejects an immediate literal child (including a bracketed literal) and requires a number-like operation; it does not establish a recursive constant-free-subtree rule. Rounding replaces the static scale with the authored places (omitted means 0), preserves the source's directional fillability, and maps its exact amount through the ordinary rounding operation. `Abs` preserves the source scale and transforms fillability by the current sign: symmetric missing conversion zero becomes grow-only magnitude, while `RangeAsNumber` is already nonnegative and its missing zero remains grow-only. Computation produces the corresponding rounded or absolute amount, maps clean missing through either source to the ordinary numeric zero, and preserves an exact formal cause as poison. These direct-source forms do not imply that every nested wrapper/source composition is admitted.
