...
- **`FieldValueAsNumber(field)` converts one statically numeric String or ordinary closed Enumeration projection into a fixed, non-expandable Number source.** A String source must retain value validation, the exact pattern `[0-9]+`, and a finite maximum length no greater than 15; its result scale is 0. An Enumeration may use its stored values or one named category, and every token in the selected projection must be an ordinary parsable decimal with at most 15 digits after removing a leading minus and decimal point. That decimal grammar admits an optional leading `-`, at most one `.`, at least one digit, and no trailing `.`, while rejecting `+`, exponent notation, whitespace, and comma syntax; its digit predicate follows the host's decimal-digit class rather than being limited to ASCII. The Enumeration result scale is the largest number of fractional digits in the selected token domain. Runtime first applies the selected stored/category projection and then parses that exact token. A missing source yields the real Number `0` with both grow and shrink possibilities in validation; a filled source yields a fixed exact decimal; formal unavailability remains unavailable. Checked computation erases only fillability and sends the same amount or poison through ordinary numeric arithmetic, generated validation, and Number target checking. Table-backed Enumeration, wildcard/repeatable access, and a conversion whose declared domain contains any non-convertible token are rejected during authoring rather than assigned a runtime fallback.
- **The operation-form `RoundDown`, `RoundUp`, `RoundAccounting`, and `Abs` wrappers admit a direct `FieldValueAsNumber(...)`, `RangeAsNumber(...)`, or numeric field-list aggregate child.** The wrapper checker rejects an immediate literal child (including a bracketed literal) and requires a number-like operation; it does not establish a recursive constant-free-subtree rule. Rounding replaces the static scale with the authored places (omitted means 0), preserves the source's directional fillability, and maps its exact amount through the ordinary rounding operation. `Abs` preserves source scale and transforms fillability by the folded result's current sign: symmetric missing conversion zero and bidirectionally fillable all-empty Sum/extremum zero become grow-only magnitude, while the already nonnegative and grow-only missing `RangeAsNumber` or empty distinct-count zero remain grow-only. Aggregate `Abs` runs after the ordered fold, not per operand. Computation produces the corresponding rounded or absolute amount, maps clean missing through scalar sources and all-empty aggregates to the ordinary numeric zero, and preserves an exact formal cause as poison. These direct-source forms do not imply that every nested wrapper/source composition is admitted.
