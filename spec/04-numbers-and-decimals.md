...
- **At most one division per calculation region** without grouping braces: write `{ [G] / [F] } / 2`. Addition/subtraction, power, and `{ … }` establish separate regions for this check. This is a static legality boundary only: braces around a divided factor do not make that factor opaque to the later evaluation-tree rewrite. Powers cannot be nested without braces.
