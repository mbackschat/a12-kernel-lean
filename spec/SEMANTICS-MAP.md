...
- **Upstream semantic provenance.** This set is synchronized through a12-dmkits revision **`cdf79872` (2026-07-17)**. Future synchronizations must update this pin so freshness audits can begin from an exact revision rather than reconstructing an archaeological window.
...
Machine-readable facets exist for only §2 / §5 / §6 / §9 / §12, and a replayable corpus for only §2 / §5 / §6 / §7 / §9 / §11 / §12; elsewhere the ground truth is the canonical prose + findings + the kernel class. Concrete syntax (lexing, keywords, directives) sits outside the taxonomy → [`12-concrete-syntax.md`](12-concrete-syntax.md) here; a12-dmkits locks it under its "Outside the taxonomy" lane.

---

## 10. Lean coverage projection at synchronization

This is the projection from semantic clauses to executable definitions, proofs/counterexamples, and external evidence at the pinned synchronization revision above. It is not the living project registry; consult [`docs/IMPLEMENTATION-MAP.md`](../docs/IMPLEMENTATION-MAP.md) and [`docs/EVIDENCE.md`](../docs/EVIDENCE.md) for current implementation and evidence state. “Implemented” means the named Lean fragment was executable at synchronization; “proved” means an internal consequence of that definition was kernel-checked; neither label claims universal correspondence with the external kernel. The primitive choices below have focused kernel-backed law/differential tests in the read-only a12-dmkits (`a12-rulekit/` checkout). Its portable `empty-polarity` family covers focused empty-Number comparison polarity and empty-String direct/`Length` behavior. Focused portable cases remain absent for Boolean/Confirm empties, malformed branches, simple absolute requiredness, and the legal precision witness; those remain explicit external-adequacy obligations.
...
| §2 empty scalar comparisons and row gate | [`Semantics/FlatValidation.lean`](../A12Kernel/Semantics/FlatValidation.lean), [`Conformance/FlatValidation.lean`](../A12Kernel/Conformance/FlatValidation.lean) | Executable locks for Number-empty as fillable zero, Boolean-empty as not evaluated, Confirm-empty as false, equality/inequality, polarity, and atomic/composed blank-instance row gates | Kernel-backed [`EmptyValueLawsTest`](../../a12-rulekit/adapter/src/test/java/io/github/mbackschat/a12/dm/adapter/laws/EmptyValueLawsTest.java) and [`EmptyRowGateDiffTest`](../../a12-rulekit/adapter/src/test/kotlin/io/github/mbackschat/a12/dm/adapter/laws/EmptyRowGateDiffTest.kt); portable [`empty-polarity`](../../a12-rulekit/corpus/cases/empty-polarity/) covers the admitted empty-Number directional cases and String controls, while Boolean/Confirm replay remains pending | **Implemented, partial:** one resolved non-repeatable field; equality/inequality only; lookup/policy coherence is an admitted boundary until checked elaboration; no paths, ordering, arithmetic, aggregates, or partial-validation override |
...
| §12 polarity and malformed branches | [`Core.lean`](../A12Kernel/Core.lean), [`Semantics/NumericComparison.lean`](../A12Kernel/Semantics/NumericComparison.lean), [`Semantics/FlatValidation.lean`](../A12Kernel/Semantics/FlatValidation.lean), [`Conformance/FlatValidation.lean`](../A12Kernel/Conformance/FlatValidation.lean) | VALUE/OMISSION precedence is proved algebraically; the admitted fixed-right comparisons dispatch sign-aware directional fillability including `notEqual`; VALUE-fired `Or`, fired `And`, not-fired `And`, and OMISSION-fired `Or` malformed-sibling outcomes are executable locks | Kernel-backed [`FormalErrorScopeLawsTest`](../../a12-rulekit/adapter/src/test/java/io/github/mbackschat/a12/dm/adapter/laws/FormalErrorScopeLawsTest.java); portable [`empty-polarity`](../../a12-rulekit/corpus/cases/empty-polarity/) supplies the focused directional Number witnesses, while malformed-branch portable replay remains pending | **Implemented for the admitted flat constructors only:** fixed-right `equal`, `notEqual`, `less`, and `greaterEqual` have directional polarity; operand-pair fillability, aggregates, filters, iteration, and partial relevance remain open |
