...
- **Upstream semantic provenance.** This set is synchronized through a12-dmkits revision **`a8b86ff6` (2026-07-18)** (previously `cdf79872`, 2026-07-17; the 07-18 sync incorporated the model-timezone/DST edge-hour laws and the per-operator DateTime gates into [`01-data-model.md`](01-data-model.md) and [`05-dates-and-time.md`](05-dates-and-time.md)). Future synchronizations must update this pin so freshness audits can begin from an exact revision rather than reconstructing an archaeological window.
...
This read-only semantic map deliberately does not snapshot Lean implementation, proof, conformance, or evidence status: those surfaces evolve independently of a semantic synchronization and a copied table becomes misleading as soon as another capsule lands. Consult [`docs/IMPLEMENTATION-MAP.md`](../docs/IMPLEMENTATION-MAP.md) for clause-level executable and proof coverage, [`docs/EVIDENCE.md`](../docs/EVIDENCE.md) for retained external observations and exact claim boundaries, and [`docs/LEAN-FORMALIZATION.md`](../docs/LEAN-FORMALIZATION.md) for the proof and trust contract. A semantic fact synchronized here is an implementation obligation, not evidence that Lean already implements or mechanically excludes it.
