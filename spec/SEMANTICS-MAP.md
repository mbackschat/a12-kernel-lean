...
| §7 | Strings, patterns & field-type checks | UTF-16 code-unit length; CRLF-normalized reads; legal charset; custom validators; raw-type closure; anchored portable-subset regex; `+` overload | ★★☆☆☆ | [`06-strings-and-enumerations.md`](06-strings-and-enumerations.md) |
...
| **formal error** | a violation of a field's *data-type configuration* (type, pattern, scale, range, charset, leading/trailing blanks, required-and-empty, duplicate index, over-repetition, or registered custom-validator rejection). Puts the cell in the not-check-relevant state and blocks the field from all rules. Ordinary formal errors use fixed, non-authorable messages; a registered custom-validator rejection is the explicit exception and preserves its project code plus optional field-placeholder-aware message. |
...
| §7 | Strings / patterns / field checks | §7 | — | `empty-polarity` | string conversion (`kernel-conversion-java`), `VkString`, `FormalChecker`, `LegalCharTester`, custom field validator |
