...
| §9 | Repetition, iteration & cross-array eval | Rules become per-row queries; ordinary scope comes from compatible condition references, not error placement; negative admission is level-, connective-, operator-, and operand-sensitive across typed direct field operations, including temporal components; joins by index; ordered aggregates; `Having`/`$`; star binding | ★★★★★ | [`07-repetition-and-iteration.md`](07-repetition-and-iteration.md) |
...
- **Rule iteration scope comes from condition *references*, never placement or the error entity.** A rule with no iteration-bearing repeatable condition reference evaluates exactly once; its error entity must be referenced and scope-compatible but does not select a level. Computation target and operand addressing has its separate §11 contract. ([§9](#3-the-taxonomy), [§11](#3-the-taxonomy))
