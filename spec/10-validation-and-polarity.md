...
- **A missing index field with a declared default has phase-specific staging.** Full validation injects the model default before index mandatory/uniqueness and authored-rule evaluation. Partial validation does not inject it: the eligible missing index cell is made unavailable for that call without emitting `mandatoryField` or `uniqueIndex`, so dependent conditions see UNKNOWN and cannot create an error that full validation would not report. This silent suppression is distinct from a generated finding and does not instantiate a phantom row.
