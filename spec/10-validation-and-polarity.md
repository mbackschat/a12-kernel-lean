...
> **Lean modelling note.** Model kernel 30.8.1 partial validation as an ordered three-stage contract: (1) skip an elaborated rule containing any `Having` filter; (2) otherwise emit/evaluate the rule only if its error-field instance belongs to the relevant set after global augmentation; (3) during that evaluation, read an out-of-set reference as `notCheckRelevant`-style **UNKNOWN** and let Kleene logic decide. Derive group relevance separately as `NONE`/`PARTIAL`/`FULL`; do not infer definite absence from one relevant empty child. The starred-aggregate "only when wildcarded" rule and phantom-row evaluation are the other easy-to-miss pieces: the aggregate stays UNKNOWN unless the whole starred level is covered, and a phantom relevant row is still evaluated and may fire through empty-as-zero. Keep relevance as wildcardable cell and group patterns rather than a flat cell list.
