...
- **On this carrier `fillToFix` is all of `referenced` or none of it**, never a proper subset. It tracks the type exactly — empty on every VALUE message, the whole referenced set on every OMISSION one — so it is one decision spelled as a list rather than a per-cell analysis, and the computed target's own pointer is included. Do not read a listed pointer as a claim that this cell is empty: in the measured witness a target types OMISSION on row headroom alone while every referenced cell is filled, and each of those filled pointers is listed. This sharpens the general non-minimality note in [§2](01-data-model.md) into an exact projection for the computation's implicit self-validation message; it is not established for authored rule messages, whose operator-specific projections stay as recorded ([checkpoint](../docs/SOURCES.md#src-starred-operand-message-polarity)).
- The type itself is the ordinary directional rule applied to the computed-versus-stored inequality, with the stored side fixed — the at-capacity documents establish that, since a stored side able to move would have typed them OMISSION. Every measured row has the computed value *below* its seed, so the rows do not separate that directional reading from a coarser one that types OMISSION whenever the quantity can move at all; the directional account is inherited from the rule arm rather than measured here.
