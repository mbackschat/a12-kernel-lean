...
> **Lean modelling note.** Model `validatePart` as `validateFull` with two changes: (1) a rule is emitted only if its error-field instance ∈ relevant set (after auto-adding globals); (2) a reference to a cell ∉ relevant set reads as `notCheckRelevant`-style **UNKNOWN** (Kleene decides). Derive group relevance separately as `NONE`/`PARTIAL`/`FULL`; do not infer definite absence from one relevant empty child. The starred-aggregate "only when wildcarded" rule and the phantom-row evaluation are the other easy-to-miss pieces — the aggregate stays UNKNOWN unless the *whole* starred level is covered, and a phantom (non-existent) relevant row is still evaluated (fires empty-as-0). Keep the relevant set as a set of *wildcardable* cell and group patterns, not a flat cell list, so full group coverage and "the starred level is wildcarded" are expressible.
...
- [ ] `validatePart` = gate rules by relevant error field + read out-of-set refs as UNKNOWN; auto-add globals; derive group `NONE`/`PARTIAL`/`FULL` relevance with positive-presence/negative-absence asymmetry; starred aggregates UNKNOWN unless wildcarded; phantom relevant rows evaluated; uniqueness needs the partner relevant, and RNU requires every composite-key component of every participating row relevant.
