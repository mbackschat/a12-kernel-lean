...
- **A starred aggregate's relevance is per operator, and the operator list is measured rather than inferred from the family.** The all-rows aggregates (`Sum`, `MaxValue`, `MinValue`, `NumberOfDifferentValues`) evaluate only when the starred level is **wildcarded** (or a relevant ancestor covers it) — enumerating every concrete row still leaves them UNKNOWN, because they reach a combiner whose pre-loop survey answers for the whole repetition without reading a cell. Two operators in the same syntactic family do **not** share that gate. `SumOfProducts` reaches neither that combiner nor its survey and runs its own two-list loop, so enumerating every concrete row **evaluates** it. `FirstFilledValue` is **order-aware** (UNKNOWN only when a non-relevant cell precedes the first filled one) and likewise evaluates under concrete rows. Do not extend the wildcard-only gate to an operator on the strength of its resemblance to the four above; each one was measured separately, and the two that differ are exactly the two that own their loops.
