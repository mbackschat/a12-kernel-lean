...
Static negative-condition admission is checked **separately at every ordinary iteration level** selected above. At one level, an `And` is admitted when at least one child both references that level and is itself admitted; an `Or` is admitted only when every child references that level and is admitted. Leaf admission is operator- and operand-sensitive rather than one global “negative operator” flag. For the direct iterating Number-field-versus-immediate-number-literal shape, the kernel treats ordinary `==`, `>=`, and `<=` as unguarded exactly when its host-converted integer form of the literal is zero; strict `<`/`>`, ordinary `!=`, tolerance-not-equal, and a converted nonzero literal are admitted. The host conversion parses the literal as binary64, rounds it to a 64-bit integer, then narrows it to signed 32-bit before the zero test, so this is not equivalent to testing the exact decimal value for mathematical equality with zero. The same classification applies with the literal on either comparison side. A top-level ordinary arithmetic or power expression is instead represented as a composite operation and is not reclassified by the visitor's direct field/list-versus-constant branches: wrapping the same iterating field in such an expression can therefore make this static gate admit a condition whose syntactically direct counterpart it rejects. This paragraph describes static model admission only; runtime still evaluates the original decimal comparison.
