...
The direct plain-field form of `Sum` / `MaxValue` / `MinValue` requires at least two distinct resolved field references. A single group or flattened (`*`) operand is legal through the separate expansion route because it denotes a field scope rather than a singleton direct list. Duplicate operands, including duplicates introduced indirectly through entity expansion, are rejected before aggregate type checks; evaluation never silently deduplicates the authored stream.
