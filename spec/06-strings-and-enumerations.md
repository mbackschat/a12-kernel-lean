...
- In partial validation, a starred entry has two relevance facts. Its instantiated cells are classified individually, but its complete extent is known only when every starred level is covered by a wildcard or relevant ancestor; listing every currently instantiated row concretely does not establish the absent/future rows. An extent-unknown star is consumed like an UNKNOWN on that side: `AtLeastOne` ignores it on either side, `No` is suppressed on either side, and `NotAll` ignores it on the fields side but is suppressed on the values side.
...
- **`NotAll`** first checks whether the fields side has any present cell. With none it does not fire without consuming the values side. Otherwise it consumes the complete values side with the same UNKNOWN/omission classification as `No`, then restarts the fields scan and fires at the first present value outside the set. Empty and UNKNOWN fields-cells are skipped. The fire is **OMISSION** when the outside-witness operand was filtered or the values side has omission potential, otherwise **VALUE**.
...
> **Lean modelling note.** Preserve authored operand boundaries and encounter order rather than flattening each side into one aggregate classification. The three quantifiers have distinct pass structure: values then fields for `AtLeastOne`/`No`, and a fields-presence prepass followed by values and a restarted fields scan for `NotAll`. Return `Verdict`, retain UNKNOWN explicitly, and prove the order separators; the operators are not Boolean duals. The public rule/message projection emits no finding for either `notFired` or `unknown`, so a fields-side `No` match-before-later-UNKNOWN separator is source-grounded and internally executable but is not independently message-observable without a negation consumer.
