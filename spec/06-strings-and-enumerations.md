...
- [ ] Value-list quantifiers classify each cell three-way; a partial starred entry additionally requires wildcard/ancestor coverage to establish its complete extent even when every current row is concrete. `No` poisons on UNKNOWN cell, member, or star extent; `NotAll` only on UNKNOWN member/value-side extent; `Having` escalates a fire to OMISSION. Scalar `Included` is singleton `AtLeastOne`, scalar `NotIncluded` is singleton `NotAll`, and neither fires on an empty or unavailable subject.
