...
For a resolved entity list, let `pₜ` be definitely filled contributions, `pₑ` definitely empty contributions, and `pᵤ` contributions in neither bucket. A checked field is definitely filled when it has a value, definitely empty when cleanly empty, and unavailable when formally invalid or not relevant. An ordinary resolved group uses the product-state classification above; each instantiated row of an admitted starred group increments `pₜ`. The five legal group-list quantifiers consume that common tally without treating unavailable as empty:
...
- [ ] Group-list quantifiers admit checked field and group operands and use one definitely-filled/definitely-empty/unavailable tally; the two wildcard-admitting members may mix or repeat starred groups, each occurrence contributing one filled count per instantiated row. Plain multi-group `NumberOfFilledGroups` remains group-only and is unavailable on any erroneous or not-fully-relevant operand, while the starred single-group form counts rows structurally.
