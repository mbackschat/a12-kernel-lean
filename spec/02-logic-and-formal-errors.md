...
The plain numeric `NumberOfFilledGroups(g1, g2, …)` is stricter than this partial tally: if any operand is erroneous or not fully relevant, the numeric result is unavailable; otherwise it counts operands with admitted content. Keep it separate from `NumberOfFilledGroups(G*)`, which counts in-capacity instantiated rows, including created-but-empty rows, while over-limit rows remain only in the complete structural topology.
...
- **an over-repetition row** — the address-formal check detects a coordinate beyond the group's declared `repeatability` before scalar conversion, supplies the structural finding in place of any would-be scalar finding, and suppresses that row's ordinary checks while retaining its physical row content.
