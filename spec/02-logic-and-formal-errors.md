...
- **Fixed group-list quantifiers admit both field and group operands.** A field operand contributes its checked presence state; a group operand contributes the product state described below. Direct duplicates and group/descendant overlap are rejected. `AllGroupsFilled`, `NotAllGroupsFilled`, and `GroupsNotCollectivelyFilled` require at least two operands and reject root groups and wildcards. `NoGroupFilled` and `AtLeastOneGroupFilled` admit a singleton, including a sole root group; that fixed singleton has the same semantics as the corresponding scalar field/group presence predicate, and a root group is rejected when accompanied by another operand.
...
For an unstarred fixed entity list, let `pₜ` be definitely filled operands, `pₑ` definitely empty operands, and `pᵤ` operands in neither bucket. A checked field is definitely filled when it has a value, definitely empty when cleanly empty, and unavailable when formally invalid or not relevant. A resolved group uses the product-state classification above. The five legal group-list quantifiers consume that common tally without treating unavailable as empty:
...
- [ ] Fixed group-list quantifiers admit checked field and group operands and use one definitely-filled/definitely-empty/unavailable tally; plain multi-group `NumberOfFilledGroups` remains group-only and is unavailable on any erroneous or not-fully-relevant operand, while the starred single-group form counts rows structurally.
