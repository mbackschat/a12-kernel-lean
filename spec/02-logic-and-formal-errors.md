...
**Confirm vs Boolean** differ on the empty case: an unspecified **Confirm** reads `False` in a comparison; an unspecified **Boolean** leaves the comparison *not evaluated* (see [§2](03-empty-and-required.md)). At the storage tier, the effective configured tokens are compared exactly. Under the default profile, a Boolean accepts only lowercase `true` or `false`; any other token is the formal error `feldJaNeinFalsch`. A Confirm accepts only exact lowercase `true` or unspecified; any other nonempty token, including `false`, is `feldJaFalsch`, consistent with "a checkbox whose only value is True". These checks are case-**sensitive**: under the default profile, `TRUE`, `FALSE`, and `True` are invalid stored Boolean tokens, and `TRUE` is invalid for Confirm. Do not import casing from a neighboring layer: a host may case-fold while ingesting a document, and comparison of an already-stored value may case-fold, but neither makes the storage formal check case-insensitive. Condition syntax is different again and uses capitalized `True`/`False` ([§12](12-concrete-syntax.md)).
