...
- **an over-repetition row** — the address-formal check detects a coordinate beyond the group's declared `repeatability` before scalar conversion, supplies the structural finding in place of any would-be scalar finding, and suppresses that row's ordinary checks while retaining its physical row content. In the measured `/Order[1]/Lines[3]/Qty` instance, the complete formal channel contains exactly `zuGrosseZeile@/Order[1]/Lines` and `zuGrosseKontextnummer@/Order[1]/Lines[3]/Qty`, with no Number, Boolean/Confirm, String-length, or other scalar-format finding for that row. **The multiplicity is one row-level code plus one context-number code per node the document writes beneath the row.** Measured on three shapes at kernel 30.8.1, an over-limit row draws exactly one `zuGrosseZeile` on the row address plus one `zuGrosseKontextnummer` for every descendant group key and every present cell key the document instantiates under it, at any depth; the same row written as an empty row object draws the row code alone although descendants are declared beneath it, so the set is read off the written document rather than off the model. Each context-number message names the **over-limit group's** declaration path and the offending index whatever the stamped node's depth, `referenced` holds exactly the stamped address, and `fillToFix` is empty ([checkpoint](../docs/SOURCES.md#src-over-limit-finding-multiplicity)). The two routes spell addresses differently and the reason is unresolved: the overload matrix above carries a repetition-one index but none on the row-level code, while the runtime probe elides repetition-one and indexes the over-limit row. Take the address *set* as measured and its spelling as route-specific.
