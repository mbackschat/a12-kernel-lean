...
- **Group-list quantifiers admit both field and group operands.** A field operand contributes its checked presence state; an ordinary group operand contributes the product state described below. Direct non-wildcard duplicates and strict group/descendant overlap are rejected. Direct duplicate checking skips wildcarded operands, so a repeated starred group remains a repeated authored occurrence. `AllGroupsFilled`, `NotAllGroupsFilled`, and `GroupsNotCollectivelyFilled` require at least two operands and reject root groups and wildcards. **Diagnostic identity is per carrier and does not follow that rule list.** A single operand under those three draws `MVK_PARAMSIZE_INVALID2`, while `NumberOfFilledGroups` draws `MVK_PARAMSIZE_INVALIDGN` for any single unstarred operand and admits a single starred one. There is **no root-group class**: a root operand beside another draws the ancestor-overlap class, because a root is an ancestor of everything. Operand relation splits in two — the same path twice draws `MVK_DUPLICATE_PARAM1`, an ancestor/descendant pair draws `MVK_DUPLICATE_PARAM2` — and both hold across all four carriers. Wildcards split in three: a starred operand where the quantifier forbids one draws `MVK_NO_WILDCARDS_G_ALLOWED`, scalar `GroupFilled(G*)` draws `MVK_NO_WILDCARDS_ALLOWED`, and an unstarred *repeatable* group used as a quantifier operand draws `MVK_NO_WILDCARD`. An unknown group path draws `MVK_INVALID_ENTITY` in both the quantifier and the count carrier. A **sole unstarred repeatable** group operand is admitted exactly when the rule *iterates* that group — the governing variable is the error field's locus, not the rule group, so the same operand list is accepted with the error field inside the repeatable group and draws `MVK_NO_WILDCARD` with it outside, which is also what scalar `GroupFilled` on a repeatable group draws from outside. A repeatable operand beside another always draws `MVK_NO_WILDCARD`. An unguarded sole `NoGroupFilled` on a repeatable group draws `MVK_NEG_CONDITION_IN_ITERATION` instead of any group-list class, and its `GroupFilled`-guarded form is admitted. `NoGroupFilled` and `AtLeastOneGroupFilled` admit a singleton, including a sole root group; a fixed singleton has the same semantics as the corresponding scalar field/group presence predicate, and a root group is rejected when accompanied by another operand.
