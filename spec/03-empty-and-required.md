...
| the **count family** (`NumberOfFilledFields`, `NumberOfValueInFields`, …) | an empty cell is **never counted** (`NumberOfValueInFields(0 In …)` does *not* count an empty number as a match for `0`), but the count stays *growable* — a fired comparison types **OMISSION**; and a **formally invalid** operand makes the whole count UNAVAILABLE rather than being skipped or counted as filled, so no comparison against it fires (measured for `NumberOfFilledFields`, where a filled-beside-invalid pair fired none of three rules comparing the count to `0`, `1`, and `2`). An **all-empty** operand list still counts `0` and compares normally, so emptiness and invalidity are not one state here |
| operand-list **`Min([A],[B])` / `Max(…)`** | an empty **NUMBER** operand is substituted `0` and **competes in the fold** (the `0` can win); an empty **DATE**, **TIME**, or full-component **DATETIME** operand is **skipped** (the temporal-value-bearing side wins) |
...
- **DATE, TIME, and full-component DATETIME `Min`/`Max` fold to *no value*:** a comparison against an all-empty temporal aggregate never fires; a *computation* of it CLEARS its target.
- **`FirstFilledValue` follows its result *kind*:** an all-empty NUMBER selection yields the fillable `0`; BOOLEAN, CUSTOM, STRING, ENUMERATION, DATE, DATE_FRAGMENT, TIME, full-component DATETIME, and DATE_RANGE yield *no value* (NOT-GIVEN), so a computation clears its target.

`FirstFilledValue` scans ordered `entitySpec` operands, not merely a static field list. An operand may be a direct field or wildcard field list, an Enumeration category projection, a literal- or field-keyed semantic-index field, a plain or wildcard group, a literal- or field-keyed semantic-index group, or the correlated form where that owner admits it. Group operands expand recursively to descendant fields in declaration order; category operands expand through their category projection. A semantic-indexed group remains document-selected and cannot be replaced by a static field list. An empty selector or empty/no-match cell continues to the next candidate, a filled value terminates the scan, and reached formal unavailability terminates without being recast as ordinary emptiness. Inside `Having`, a plain captured outer group is admitted but literal- and field-keyed semantic-index selections are rejected with `MVK_SEMANTIC_INDEX_NOT_ALLOWED`.
...
- [ ] All-empty aggregate **identities** per kind (NUMBER→`0`, DATE/TIME/DATETIME min/max→no value; `FirstFilledValue` NUMBER→`0` and every other admitted result kind→no value), with `FirstFilledValue` retaining ordered `entitySpec` expansion rather than a static field list.
