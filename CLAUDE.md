...
lake exe syncFlatHandover --check   # bind the typed flat capability and generated artifacts to retained evidence
...
- [`A12Kernel/EvidenceMain.lean`](A12Kernel/EvidenceMain.lean) — IO-only `lake test` driver for retained runtime and static authoring observations; it is outside the library, conformance, and proof roots. [`A12Kernel/Evidence/FlatProtocolBridge.lean`](A12Kernel/Evidence/FlatProtocolBridge.lean) owns the typed eight-case capability descriptor and the checked projection-to-protocol artifact bridge; [`A12Kernel/FlatHandoverMain.lean`](A12Kernel/FlatHandoverMain.lean) exposes its explicit check/write workflow. [`docs/EVIDENCE.md`](docs/EVIDENCE.md) owns the exact current inventory and claim boundary.
