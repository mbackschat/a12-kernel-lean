...
- After handback, reconcile the request against the actual revision, verify the maintained command and portable artifacts, move durable facts to their owning local documents, and delete the completed handoff. Do not reproduce the upstream implementation locally.
- An unchanged source-owned a12-dmkits command may be run with its output directed only to permitted ignored storage. If the required observation channel or command does not exist, mark the Lean capsule `external evidence pending` and request the upstream improvement; do not fill the gap with a project-authored adapter fork.

The live source-maintained capture contract is owned upstream by a12-dmkits' [`CONFORMANCE-CORPUS-SPEC.md` §15–§15f](../a12-rulekit/docs/CONFORMANCE-CORPUS-SPEC.md); this repository's operating method is in [`docs/TESTING.md`](docs/TESTING.md), and its longer-term amortized workflow is in [`docs/SEMANTIC-CAPSULE-PIPELINE-PROPOSAL.md`](docs/SEMANTIC-CAPSULE-PIPELINE-PROPOSAL.md). [`docs/PLAN.md`](docs/PLAN.md) owns only the current local continuation state.
...
**Differential doctrine:** Kernel differential testing remains the empirical backbone. Each executable Lean capsule must be checked against retained portable observations from the real kernel; proofs establish internal laws; a12-dmkits contributes knowledge, evidence transport, and clean-room triangulation, but its interpreter is never the oracle. Kernel execution happens only through a source-maintained external sibling command, never as a dependency or shipped component of this repository. Codex may run that unchanged command while preserving the sibling's tracked/visible worktree under the rules above, then retain the complete verified portable packet and receipt, packet-local capabilities, request-bound policy and consumed-document identity, implementation/runtime closure, applicable qualification profile/report/receipt sidecars and their out-of-band receipt digests, and this repository's own typed projection. Kernel binaries, linked objects, implementation code, volatile logs, and machine-specific state never cross that boundary. A missing capture capability is an upstream improvement request, not permission to patch a copied harness. A capsule without the observations remains marked `external evidence pending` rather than kernel-correspondence complete. The exact topology and evidence roles are fixed in [`docs/PROJECT-DESIGN.md`](docs/PROJECT-DESIGN.md).
...
- [`A12Kernel/Semantics/`](A12Kernel/Semantics/) — phase observation; shared UTF-16 String length; shared scale-19 numeric truth and directional fillability; the flat Number/Boolean/Confirm core plus direct String equality and String `Length <`/`>=`; staged absolute-required semantics; narrow ordered single-group iteration; captured-outer `$` correlation with filter-only origins, selected presence, a separate outer guard, and per-row firing; and the one-target-instance String computation with separate expression, root-store, positive target-check, payloadful outcome, value-only application, and delta boundaries.
...
- [`A12Kernel/Proofs.lean`](A12Kernel/Proofs.lean) — trusted theorem root importing algebra, information-order, observation, required-staging, directional numeric polarity, the direct-String-versus-`Length` distinction, String computation evaluator/store/target-check/application-value/delta laws and counterexamples, flat and one-star declaration/context coherence, fail-closed repeatable routing, ordered-selection/filter-before-consumer, correlated origin/judgment/selection/self-match, and selected-consumer observation-footprint proofs. Checked-wrapper lemmas expose carried structural certificates; do not describe them as surface-to-core semantic preservation. [`A12Kernel/TrustAudit.lean`](A12Kernel/TrustAudit.lean) and [`A12Kernel/Trust/Environment.lean`](A12Kernel/Trust/Environment.lean) keep the human-readable theorem reports and the authoritative elaborated-declaration audit separate from the trusted roots they inspect.
- [`A12Kernel/Conformance.lean`](A12Kernel/Conformance.lean) — executable locks for the supported fragment, including directional unsigned/signed/fixed Number polarity, direct String equality versus grow-only `Length`, UTF-16 code-unit length, String rejection boundaries, String computation copy/concatenation/root-store/target-check/payloadful-`ERRORED`/value-only-application/delta and poison-order cases, one-group iteration/correlation separating cases, checked one-star lowering and rejection classes, candidate-row validation, `empty=zero` and malformed-local filters, admitted numeric and repetition comparisons, symmetric malformed dropped-versus-kept consumers, and checked non-laws; these are semantic examples, not a substitute for external differential evidence.
- [`A12Kernel/EvidenceMain.lean`](A12Kernel/EvidenceMain.lean) — IO-only `lake test` driver for retained runtime and static authoring observations, including separate operator-sensitive Number/String, clean String-computation, String target-validation, and qualified direct-cascade projections; it is outside the library, conformance, and proof roots. [`A12Kernel/Evidence/StringComputationBinding.lean`](A12Kernel/Evidence/StringComputationBinding.lean) owns strict packet, model, operation, placement, two-kernel-route, and interpreter-triangulation binding for the clean capsule; [`A12Kernel/Evidence/StringTargetValidationBinding.lean`](A12Kernel/Evidence/StringTargetValidationBinding.lean) owns the richer target policy, accepted/error result, payloadful delta, exact external application-state, and triangulation binding; [`A12Kernel/Evidence/StringCascadeBinding.lean`](A12Kernel/Evidence/StringCascadeBinding.lean) owns the retained direct-cascade packet, qualification, consumed-document, runner-fidelity, raw-route, and typed public replay relations. [`A12Kernel/Evidence/FlatProtocolBridge.lean`](A12Kernel/Evidence/FlatProtocolBridge.lean) owns the frozen typed eight-case capability descriptor, its non-writing projection-to-protocol artifact checker, and the source-maintainer mutation qualification plan; [`A12Kernel/FlatHandoverMain.lean`](A12Kernel/FlatHandoverMain.lean) exposes the check and deliberately rejects historical writes. [`A12Kernel/Evidence/OperatorProtocolBridge.lean`](A12Kernel/Evidence/OperatorProtocolBridge.lean) independently binds the current flat suite's directional witness to retained evidence and its normalized request/response. [`docs/EVIDENCE.md`](docs/EVIDENCE.md) owns the exact current evidence inventory and claim boundary; the mutation plan is testing metadata, not evidence. The internal String/Length, String-computation, and direct-cascade capsules are not part of the normalized public protocol or frozen Rust capability.
