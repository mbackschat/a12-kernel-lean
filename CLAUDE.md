...
The language-neutral semantics live in read-only [`spec/`](spec/) — start at [`spec/SEMANTICS-MAP.md`](spec/SEMANTICS-MAP.md) and follow the numbered deep-dives. [`docs/README.md`](docs/README.md) indexes the living project documentation; [`docs/PRODUCT-PROPOSAL.md`](docs/PRODUCT-PROPOSAL.md) owns the general consumer-task taxonomy and product progression, while [`docs/USE-CASES.md`](docs/USE-CASES.md) is its user-facing potential map; [`docs/IMPLEMENTATION-MAP.md`](docs/IMPLEMENTATION-MAP.md) tracks clause-level Lean/proof/evidence status; [`docs/ARTIFACTS.md`](docs/ARTIFACTS.md) explains the contents, authority, generation, and lifecycle of `evidence/`, `reference/`, `examples/`, and `qualification/`; [`docs/IMPLEMENTER-GUIDE.md`](docs/IMPLEMENTER-GUIDE.md) owns the semantic-shipment, research-closure, and independent-consumer contract; [`docs/IMPLEMENTER-KIT-CORRELATION.md`](docs/IMPLEMENTER-KIT-CORRELATION.md) is the concrete one-star `$` evaluator handover; [`docs/IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md`](docs/IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md) is the smaller evidence-bound cold-evaluator handover; [`docs/LEAN-FINDINGS.md`](docs/LEAN-FINDINGS.md) records durable formalization findings; [`docs/LEAN-FORMALIZATION.md`](docs/LEAN-FORMALIZATION.md) owns Lean's role and trust contract; [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) records concrete encoding decisions; [`docs/PROTOCOL.md`](docs/PROTOCOL.md) owns the normalized reference process contract; [`docs/PRODUCTION-RELEASE.md`](docs/PRODUCTION-RELEASE.md) owns production artifact qualification and measured release experiments. [`spec/13-lean-encoding-guide.md`](spec/13-lean-encoding-guide.md) remains consulted staged guidance, not a writable plan.
...
Before resuming an interrupted implementation session, read [`docs/PLAN.md`](docs/PLAN.md); it records the verified boundary, preserved probe material, and exact next-step order.
...
lake exe syncFlatHandover --check   # bind flat evidence artifacts and the post-cold mutation plan to typed source
...
- [`A12Kernel/Semantics/`](A12Kernel/Semantics/) — phase observation, the flat validation core, staged absolute-required semantics, narrow ordered single-group iteration, and captured-outer `$` correlation with filter-only origins, selected presence, a separate outer guard, and per-row firing.
- [`A12Kernel/Elaboration/`](A12Kernel/Elaboration/) — checked lowering from structured, parser-independent surface subsets: normalized flat path resolution and model-derived cell policies, plus one exact group-qualified direct-child star with correlated Number/repetition comparisons, explicit group/scope binding, and operator-specific scale legality.
- [`A12Kernel/Reference/`](A12Kernel/Reference/) — the normalized protocol vocabulary, strict JSON and exact-decimal transport, generated per-operation support metadata, closed decoder, and pure adapters to the existing checked flat and one-group correlation evaluators. These process-boundary modules are outside the library, conformance, and trusted-proof roots.
- [`A12Kernel/Proofs.lean`](A12Kernel/Proofs.lean) — trusted theorem root importing algebra, information-order, observation, required-staging, flat and one-star declaration/context coherence, fail-closed repeatable routing, ordered-selection/filter-before-consumer, correlated origin/judgment/selection/self-match, and selected-consumer observation-footprint proofs. Checked-wrapper lemmas expose carried structural certificates; do not describe them as surface-to-core semantic preservation.
- [`A12Kernel/Conformance.lean`](A12Kernel/Conformance.lean) — executable locks for the supported fragment, including one-group iteration/correlation separating cases, checked one-star lowering and rejection classes, candidate-row validation, `empty=zero` and malformed-local filters, admitted numeric and repetition comparisons, symmetric malformed dropped-versus-kept consumers, and checked non-laws; these are semantic examples, not a substitute for external differential evidence.
- [`A12Kernel/EvidenceMain.lean`](A12Kernel/EvidenceMain.lean) — IO-only `lake test` driver for retained runtime and static authoring observations; it is outside the library, conformance, and proof roots. [`A12Kernel/Evidence/FlatProtocolBridge.lean`](A12Kernel/Evidence/FlatProtocolBridge.lean) owns the typed eight-case capability descriptor, checked projection-to-protocol artifact bridge, and generated source-maintainer mutation qualification plan; [`A12Kernel/FlatHandoverMain.lean`](A12Kernel/FlatHandoverMain.lean) exposes their explicit check/write workflow. [`docs/EVIDENCE.md`](docs/EVIDENCE.md) owns the exact current evidence inventory and claim boundary; the mutation plan is testing metadata, not evidence.
- [`A12Kernel/ReferenceMain.lean`](A12Kernel/ReferenceMain.lean) and [`A12Kernel/ReferenceProcessTestMain.lean`](A12Kernel/ReferenceProcessTestMain.lean) — the one-request CLI and its independent process-level regression gate. [`A12Kernel/CandidateConformanceMain.lean`](A12Kernel/CandidateConformanceMain.lean) runs language-neutral capability suites against independent candidates without adding semantics. [`docs/PROTOCOL.md`](docs/PROTOCOL.md) owns the public contract; [`docs/IMPLEMENTER-KIT-CORRELATION.md`](docs/IMPLEMENTER-KIT-CORRELATION.md) and [`docs/IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md`](docs/IMPLEMENTER-KIT-FLAT-EMPTY-LOGIC.md) own the current candidate handovers.
