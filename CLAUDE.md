...
- [`A12Kernel/Conformance.lean`](A12Kernel/Conformance.lean) — executable locks for the supported fragment, including one-group iteration/correlation separating cases, `empty=zero` and malformed-local filters, admitted numeric and repetition comparisons, symmetric malformed dropped-versus-kept consumers, and checked non-laws; these are semantic examples, not a substitute for external differential evidence.
- [`A12Kernel/EvidenceMain.lean`](A12Kernel/EvidenceMain.lean) — IO-only `lake test` driver for 38 retained observations (36 runtime and two diagnostics), including twelve captured-outer cases across seven models; it is outside the library, conformance, and proof roots.
