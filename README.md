...
The first proof-bearing semantic capsule, the first checked-elaboration slice, and a narrow single-level iteration capsule are implemented. They cover base formal checking and phase observation; typed flat equality/inequality for Number, Boolean, and Confirm; presence predicates; the full-validation row gate; verdict-aware `And`/`Or`; scale-19 `HALF_UP`; the staged absolute/non-repeatable required-field transformation; checked model/path resolution for a deliberately small non-repeatable path subset; model-derived formal checking; and ordered explicit rows for one repeatable group with optional uncorrelated row-local `Having` and a validation-only numeric selected sum. The iteration slice has an exact executable-selector/relation bridge, a filter-before-consumer theorem, separating empty/malformed/current-row non-law cases, and focused external observations. The default build includes executable conformance locks and the trusted proof root. A separate `lake test` driver replays 26 retained kernel 30.8.1 runtime/diagnostic observations through focused typed projections; they corrected bare-name resolution from an assumed ancestor walk to the kernel's declaring-group-or-global rule and exposed one a12-dmkits interpreter disagreement on a malformed `Having` filter. [`docs/EVIDENCE.md`](docs/EVIDENCE.md) states the exact claim boundary. Coverage remains deliberately partial and is recorded clause by clause in [`docs/IMPLEMENTATION-MAP.md`](docs/IMPLEMENTATION-MAP.md); the full path language, `$`, nested/multi-star iteration, general arithmetic/aggregates, computation, and partial validation have not landed.
