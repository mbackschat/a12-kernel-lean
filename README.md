...
| Evidence transport | a12-dmkits provides the maintained kernel-facing capture, qualification, and compact-export boundary. The complete frozen a12-dmkits capture-contract V1 [`string-direct-cascade-v1` raw answer](evidence/kernel-30.8.1/captures/string-direct-cascade-v1/README.md) remains retained unchanged and opaque to ordinary Lean replay. a12-dmkits exporter revision `1b5f463b89adc6cfb81b41121cd6c97855e8cbe3` certifies [`semantic-observations.json`](evidence/kernel-30.8.1/captures/string-direct-cascade-v1/semantic-observations.json), SHA-256 `1d8d253e553eba70fa990975666884833748bed9d9b2b6483f472767a9837c7a`; the bounded `ObservationBundle` reader and typed `StringCascadeProjection` pin, decode, and replay its five cases. The one-time complete-binder comparison passed before the old direct-cascade packet machinery was removed, and `lake test` no longer re-audits the raw packet locally |
...
The evidence approach is intentionally simpler going forward: semantic and theorem work lands first with an honest evidence status; related clauses are calibrated in batches; a12-dmkits certifies the full raw capture; Lean reads only the compact typed observations it needs. Raw packets remain available for audit, but their filesystem, receipt, runner, capability, and qualification mechanics are not reimplemented for each semantic family.
