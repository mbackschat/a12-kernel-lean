...
The internal Date theory also has a small resolved Date-range core: endpoint equality is a valid one-day range, overlap uses closed intervals, inversion never overlaps, and an occurrence-preserving any-pair scan finds duplicate/self-pairs and list-internal pairs. It starts after Date decoding and after empty or unavailable cells have been skipped; authored operand grouping, scalar-versus-list assembly, filters, and VALUE/OMISSION polarity remain a separate consuming layer. [`LF49`](docs/LEAN-FINDINGS.md#lf49--date-range-overlap-is-closed-occurrence-based-and-shape-sensitive) owns the exact distinction.

The internal Date theory now also captures the reason-and-verdict boundary of three-part `Date(day, month, year)` construction: incomplete input and a complete calendar-rejected date are both non-values, but `Invalid` reports them with OMISSION and VALUE polarity respectively. This starts after component checking and a separately supplied calendar decision; exact model-zone/cutover resolution is still open, and the capsule is not part of the public reference process or retained portable evidence. [`LF46`](docs/LEAN-FINDINGS.md#lf46--constructed-date-validity-needs-a-reason-bearing-non-value) explains the distinction and [`IMPLEMENTATION-MAP.md`](docs/IMPLEMENTATION-MAP.md#resolved-three-part-date-construction) owns the exact boundary.
