...
| Evidence transport | a12-dmkits provides the maintained kernel-facing capture and qualification boundary. The complete frozen a12-dmkits capture-contract V1 [`string-direct-cascade-v1` answer](evidence/kernel-30.8.1/captures/string-direct-cascade-v1/README.md) remains retained and currently agrees with the Lean evaluator on all five qualified public observations. The accepted [consolidation plan](docs/SEMANTIC-CAPSULE-PIPELINE-PROPOSAL.md) moves ordinary Lean replay to compact producer-certified semantic bundles and removes packet-specific downstream re-auditing after a measured migration |
