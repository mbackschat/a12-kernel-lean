...
|---|---|
| Lean theory | Executable and proof-bearing fragments cover a signed-or-unknown numeric scale summary with constant expandability and the unsuppressed exact-comparison gate, phase-aware flat validation, direct Number field-to-literal equality/inequality/`<`/`>=` with directional polarity, pure two-operand fixed-threshold tolerance with independent scale-19 normalization and directional gap-closing polarity, pure two-stage decimal rounding with authored places `0..14`, per-node precision-50 `HALF_UP` addition/subtraction/multiplication/division, staged OpenJDK 21 power with kernel-specific reciprocal-first negative exponents, explicit pure arithmetic domain failure for invalid division/power, exact directional fill propagation through `+`, `−`, `×`, and reciprocal-based `÷`, and enclosing arithmetic absorption of domain failures, a first nonrepeatable partial-validation rule gate and out-of-set-reference semantics, exactly-once evaluated-String CRLF normalization shared by comparison, UTF-16 `Length`, and computation reads, the direct-String-versus-`Length` empty distinction, staged requiredness, one-group iteration, one checked captured-outer `$` correlation shape, direct computation-presence conditions with ordered left-to-right `And`/`Or` and operation-neutral first-match alternatives, and narrow one-target String computation capsules separating expression evaluation, root storage, target minimum/maximum checking, payloadful error, exact absent/present-empty/present-value application, and change-only delta reporting |
| Public reference | The current normalized process advertises reference semantics 0.3.0 through [`supported-fragment-v2.json`](reference/supported-fragment-v2.json), exposes a checked flat operation and one named single-group correlation operation, and fails closed outside that boundary; the general numeric-scale, numeric-tolerance, String/Length, and String-computation capsules are not exposed |
| Empirical and formal assurance | Compact retained observations cover flat validation, paths, requiredness, directional Number/String operators, iteration, correlation, clean String computation, and direct String cascade, while the general numeric-scale summary and fixed-threshold tolerance are source-anchored and explicitly external-evidence pending; the theorem root establishes selected evaluator, authoring-gate, store, target-check, delta, algebraic, and structural laws—not universal kernel equivalence |
