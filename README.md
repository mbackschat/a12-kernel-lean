...
The internal resolved-runtime theory now also contains one resolved nonempty literal-key semantic-index value read over a canonical-token key column and Number target. A clean no-match and a matched empty target both read empty, while an unavailable key column is validation-unknown only after no match but poisons computation before any match is read. This makes a clean validation match survive an unrelated bad key even though the same computation read aborts; an invalid target matters only when its row is selected. Path syntax, literal checking, key normalization, presence and fill-quantifier consumers, field-keyed indices, nested repetitions, and portable local evidence remain outside this capsule; [`LF53`](docs/LEAN-FINDINGS.md#lf53--semantic-index-lookup-has-match-first-validation-and-column-first-computation) owns the exact distinction.

The internal Enumeration theory now also compares one already-resolved clean stored token either directly or through one selected positional category. Direct comparison uses the stored token rather than display text; a category maps parallel stored/category vectors by position, and repeated category tokens intentionally make distinct stored values compare alike. Empty input is not evaluated, formal unavailability stays UNKNOWN, and every firing is VALUE. Authoring checks, display-text compatibility, field-to-field comparison, open/dynamic enums, paths, value-list integration, protocol support, and project-local portable evidence remain outside this capsule; [`LF54`](docs/LEAN-FINDINGS.md#lf54--enumeration-categories-are-positional-runtime-projections-not-display-lookups) owns the exact boundary.
