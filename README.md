...
The internal resolved-runtime theory now also contains one resolved nonempty literal-key semantic-index read over a canonical-token key column: a Number value consumer plus kind-independent validation/computation presence projections. A clean no-match and a matched empty target both read empty, so indexed `FieldNotFilled` holds on either; an unavailable key column is validation-unknown only after no match but poisons computation before any match is read. This makes a clean validation match survive an unrelated bad key even though the same computation read aborts; an invalid target matters only when its row is selected. Path syntax, literal checking, key normalization, fill-quantifier consumers, field-keyed indices, nested repetitions, and portable local evidence remain outside this capsule; [`LF53`](docs/LEAN-FINDINGS.md#lf53--semantic-index-lookup-has-match-first-validation-and-column-first-computation) owns the exact distinction.
