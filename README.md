...
- One immutable, model-certified checked document now owns declared topology and address resolution. A shared addressed core resolves checked Number and stored-String operands across independent and nested repeatable groups, preserves hierarchical extent and positional relevance, and keeps structural addressing failures distinct from semantic `UNKNOWN`; typed wrappers retain their own declaration metadata and classifiers.
- Ordered checked Number and stored-String value-list evaluation plus Number aggregates and token distinct count consume that same resolved representation with their distinct stopping and fold rules; there is no parallel consumer-specific stream or topology.
...
- The immediate hard frontier is completing that canonical addressed operand stream across projection-bearing Enumeration and the remaining nested/mixed routes. Whole-document computation scheduling remains gated behind this boundary and its source audit.
