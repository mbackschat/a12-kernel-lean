...
The first proof-bearing semantic capsule, the first checked-elaboration slice, and two narrow single-level iteration capsules are implemented. They cover base formal checking and phase observation; typed flat equality/inequality for Number, Boolean, and Confirm; presence predicates; the full-validation row gate; verdict-aware `And`/`Or`; scale-19 `HALF_UP`; the staged absolute/non-repeatable required-field transformation; checked model/path resolution for a deliberately small non-repeatable path subset; model-derived formal checking; and ordered explicit rows for one repeatable group. Capsule 5a adds optional uncorrelated row-local `Having` and a validation-only numeric selected sum. Capsule 5b adds an explicit captured outer row, inner/outer origins confined to a closed correlated-filter AST, Number equality/inequality/less-than, structural `CurrentRepetition`, naive ordered nested-loop selection, and a guarded any-filled truth observer. Its proofs cover numeric-reference origin stability/locality, evaluator/judgment and selector/relation agreement, selection-level explicit self-exclusion, and selection-level usable self-match; checked non-laws keep direction and implicit self-exclusion honest. A separate `lake test` driver replays 30 retained kernel 30.8.1 observations through focused typed projections; the four `$` cases preserve their actual stored conditions and pin direction `[2,3]`, self-match on every row, excluded-distinct silence, and excluded-duplicate rows `[1,2]`, with both kernel strategies and the a12-dmkits interpreter agreeing. Earlier evidence corrected bare-name resolution and exposed one a12-dmkits interpreter disagreement on a malformed uncorrelated `Having` filter. [`docs/EVIDENCE.md`](docs/EVIDENCE.md) states the exact claim boundary. Coverage remains deliberately partial: repeatable path elaboration and diagnostics, filtered-result polarity, `Document` adaptation, nested/multi-star and cross-group iteration, hash-join refinement, general arithmetic/consumers, computation, and partial validation have not landed.
