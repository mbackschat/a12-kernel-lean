...
Raw packet and sidecar bytes remain immutable evidence and the authoritative audit source. The producing a12-dmkits revisions established their full packet, runner, input, capability, qualification, closure, receipt, recapture, and export verification; revision `45b264b2` later retired that unused implementation from current upstream `main`, with reproduction preserved through detached Git history. Treating the raw unit as opaque during ordinary Lean replay is a deliberate assurance trade for a substantially smaller and scalable semantic gate, not a weakening or reinterpretation of the retained raw evidence.
