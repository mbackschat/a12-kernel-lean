...
/- The checked decimal carrier is sufficient for the kernel host conversion. Binary64 tie-to-even can move an exact value below one half onto one half; Java rounding and signed-32-bit narrowing then determine the static zero test. The below-half rows and the `2^32` pair are separately authorable, so each pins one conversion stage on its own. The long-boundary rows pin the saturation instead: `±(2^63 - 1)` classify *oppositely* despite equal magnitude, and not because of the parse step, which is symmetric here — neither is representable and both parse onto `±2^63`. The split happens at the clamp, because the long range is asymmetric. `+2^63` saturates down to `Long.MAX_VALUE`, whose low 32 bits are all set, while `-2^63` is `Long.MIN_VALUE` exactly, passes through unsaturated, and has them clear. Exact 64-bit integer arithmetic in place of this chain admits the negative literal and agrees on the positive one, so only this row separates it. The third row keeps the pair from reading as "the negative boundary always rejects": one representable step away, the residue is nonzero again. -/
example :
