...
/-- Every DateRange declaration retained by checked singular-overlap admission has either an exact executable policy or one measured direct fragment profile valid for the checked model. -/
