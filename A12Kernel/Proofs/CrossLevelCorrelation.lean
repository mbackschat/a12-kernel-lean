...
/-- Both coordinates of an environment with two distinct named levels resolve uniquely. -/
theorem crossLevelCorrelation_outer_levels_resolve
  ...
  have reverse : context.rows.descendantLevel ≠ context.rows.starLevel :=
    Ne.symm distinct
  simp [CapturedTwoLevelOuterStarContext.outerEnv, Env.uniqueRowAt?, distinct, reverse]
...
theorem crossLevelCorrelation_no_single_outer_row
  ...
  rcases crossLevelCorrelation_outer_levels_resolve context distinctLevels with
