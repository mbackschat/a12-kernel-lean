...
Numeric expressions remain exact rational values. This capsule owns their scale-19 store pre-round and conversion into the exact decimal form used at a Number target. Its rendering preserves that exact identity; it is not the decimal-valued input regime's separate strip-and-minimum-scale formal-read projection, while String-valued input is read verbatim. Target admission, validation, delta projection, scalar ingestion, and document application remain separate.
-/
...
/-- Exact dot-decimal identity text with exactly `scale` fractional digits. Decimal-valued input derives a separate formal-read text by stripping fractional trailing zeros and reapplying the declaration's minimum scale; String-valued input is read verbatim. Model-configured output materialization is a later boundary. -/
