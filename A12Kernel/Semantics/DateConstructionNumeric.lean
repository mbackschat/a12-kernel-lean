...
This capsule implements the resolved `DayFromDate`, `MonthFromDate`, and `YearFromDate` consumer from [`spec/05-dates-and-time.md` §3](../../spec/05-dates-and-time.md#3-constructing-dates-and-checking-validity) for an already-classified three-part `Date(...)` result. A real construction exposes its supplied component. Incomplete and unreal constructions both expose numeric zero, but retain not-given versus present provenance so validation polarity remains observable. Formal unavailability stays UNKNOWN.

The supplied real `DateParts` are authoritative: this projection does not repeat calendar resolution. DateTime composition, date differences, quarter extraction, authored lowering, computation consumers, and target application remain separate boundaries.
-/

namespace A12Kernel

/-- The three direct numeric components admitted by this resolved capsule. -/
