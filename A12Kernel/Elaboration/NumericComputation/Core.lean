...
The measured arithmetic surface retains the target through scale summarization so it reaches the target-scale gate before self-reference: `+`, `-`, or `*` reading the target on exactly one side, against an immediate literal or a different direct Number field. The gate order rather than the authored shape is what that surface encodes. Every other target read keeps its pre-existing immediate local refusal, including when a later operand would otherwise fail resolution, and receives no projected Kernel class. One composition is not yet measured directly: when the other operand is a Number field whose scale differs from the target's, the scale class is projected by composing the measured order with the separately measured scale gate rather than from a row of its own.
...
inductive SurfaceNumericComputationAtom where
  ...
  | booleanValueCount (expected : Bool)
...
reference reached after that gate on the measured arithmetic surface. Other
...
private def FlatModel.resolveNumericComputationExpression
  ...
  expression.mapM fun
    ...
    | .booleanValueCount expected source => do
        let checked ←
          (elaborateBooleanValueCountSource model declaringGroup expected source).mapError
            NumericComputationElabError.booleanValueCount
        pure (.booleanValueCount checked)
