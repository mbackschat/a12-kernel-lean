...
**The compared identity is the operand's exact stored text**, not its decoded value. That is what the *dynamic* kernel runtime reads for this operator, with the generated-Java strategy unread for this comparison, and it is deliberately a different reading from the decoded date `NumberOfDifferentValues` would use over the same entity lists. **That contrast is now measured on both sides**, where an earlier version of this note had the neighbour's half as merely expected because no temporal `NumberOfDifferentValues` overload existed yet. One does — [`TemporalDistinctCount.lean`](TemporalDistinctCount.lean) — and its compared identity is the decoded value at the list's shared declared component set, Kernel-measured with the reversed document as separator ([checkpoint](../../docs/sources/evaluation-and-application-routes.md#src-distinct-count-component-omitting-fold), accepted at [`SPEC-2026-09-06-08`](../../docs/A12-DMKITS-SPEC-SYNC-LEDGER.md#spec-2026-09-06-08)). So two operators over the same entity lists genuinely compare different things: one the stored text, the other the decoded value. A reimplementer sharing one identity between them is wrong in a way no single-operator test can see. The neighbour's own conformance pins it on the one list that separates the two identities — a group carrying a single component set in two spellings, which this operator refuses and that one admits: `2024-03-05` beside `05.03.2024` counts **one** value there, where a stored-text identity would count two. Under the mandatory single format the stored-text and decoded accounts agree *if* one declared format admits one text per value; that assumption is unmeasured and [`SG7`](../../docs/SEMANTICS-GAPS.md) owns its witness class. Reusing the canonical token atom is therefore exact rather than a rendering: the value compared is the stored text itself, so no normal form is invented.
