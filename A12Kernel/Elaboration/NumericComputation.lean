...
/-- Resolve and check one nonrepeatable numeric computation operation in the shared unary-arithmetic or direct-extrema fragment. The default unsuppressed route preserves the exact result-scale gate; the explicit suppression flag bypasses only that gate. Wrapper-over-wrapper/extrema authoring, repeatable evaluation, table integration, target-policy construction, and scheduling remain separate owners. -/
